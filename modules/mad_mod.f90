...
! choose element key
  ...
  case default
    ...
    print *, 'ERROR IN MAKE_MAD_MAP: ELEMENT NOT IMPLEMENTED: ', ele%name
    call err_exit
...
!
  ...
  re(1,3) = (ks*ug/k1) * (-b*S1 + a*Snh1)
  re(1,4) = 4*ug*ks * (-C + Csh)

  re(2,1) = -(ug/2) * (coef1*S2 + coef2*Snh2)
  re(2,2) = re(1,1)
  re(2,3) = ug*ks3 * (C - Csh)
  re(2,4) = ug*ks * (a*S2 + b*Snh2)

  re(3,1) = -re(2,4)
  re(3,2) = -re(1,4)
  re(3,3) = 2*ug * (fm*C + fp*Csh)
  re(3,4) = 2*ug * (r*S2 + q*Snh2)

  re(4,1) = -re(2,3)
  re(4,2) = -re(1,3)
  re(4,3) = (ug/(2*k1)) * (-coef2*S1 + coef1*Snh1)
  re(4,4) = re(3,3)
...
!
  ...
  te(1,1,6) = re(1,1)*df_f + 2*ug*(fp*dC + C*dfp + fm*dCsh + Csh*dfm)
  te(1,2,6) = re(1,2)*df_f + (2*ug/k1) * (dq*S1 + q*dS1 - dr*Snh1 - r*dSnh1)
  te(1,3,6) = re(1,3)*df_f + (ks*ug/k1)*(-db*S1 - b*dS1 + da*Snh1 + a*dSnh1)
  te(1,4,6) = re(1,4)*(df_f - 2) + 4*ks*ug*(-dC + dCsh)

  te(2,1,6) = re(2,1)*(df_f + 1) - &
              (ug/2)*(dcoef1*S2 + coef1*dS2 + dcoef2*Snh2 + coef2*dSnh2)
  te(2,2,6) = te(1,1,6)
  te(2,3,6) = re(2,3)*(df_f - 2) + ks3*ug*(dC - dCsh)
  te(2,4,6) = re(2,4)*(df_f - 1) + ug*ks*(da*S2 + a*dS2 + db*Snh2 + b*dSnh2)

  te(3,1,6) = -te(2,4,6)
  te(3,2,6) = -te(1,4,6)
  te(3,3,6) = re(3,3)*df_f + 2*ug*(fm*dC + C*dfm + fp*dCsh + Csh*dfp)
  te(3,4,6) = re(3,4)*(df_f - 1) + 2*ug*(dr*S2 + r*dS2 + dq*Snh2 + q*dSnh2)

  te(4,1,6) = -te(2,3,6)
  te(4,2,6) = -te(1,3,6)
  te(4,3,6) = re(4,3)*(df_f + 2) + &
               (ug/(2*k1))*(-dcoef2*S1 - coef2*dS1 + dcoef1*Snh1 + coef1*dSnh1)
  te(4,4,6) = te(3,3,6)

  te(1:4,1:4,6) = te(1:4,1:4,6) / 2

  do i = 1, 4
    do j = 1, 4
      te(5,i,j) = -te(2,j,6) * re(1,i) + te(1,j,6) * re(2,i) - &
                     te(4,j,6) * re(3,i) + te(3,j,6) * re(4,i)
    enddo
  enddo
...
!---------------------------------------------------------------------------
!+
! Subroutine track1_mad (start, ele, param, end)
!
      ...
      case (2)
        ...
        if (tt%expn(j) == 2) then
          ...
          do k = j+1, 6
            ...
            print *, 'ERROR: INTERNAL ERROR IN TAYLOR_TO_MAD_MAP!'
