...
analysis part of FPP, we need to say a little bit more about the structures used in PTC. Two such are
the \vn{proble} and \vn{probe_8} which are used for tracking.  The \vn{probe} structure looks like:
\begin{code}
type probe
  real(dp) x(6)
  type(spinor) s(3)
...
corresponding routine will use a \vn{real(dp)}. As an example, consider the subroutine in the
example of \sref{s:real8code}:
\begin{code}
subroutine track(z)
implicit none
type(real_8) :: z(2) 
