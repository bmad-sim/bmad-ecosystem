...
parameter in \sref{s:out.params}. The map file is in a PTC format. For linear maps, an ASCII file
...
enough so that the rampers only need be applied once per beam passage. This is done to minimize
computational time.
...
Used with \vn{ltt%simulation_mode} set to \vn{"BEAM"}. The name of the file that contain the following
columns:
\begin{code}
  Turn                          -- Turn number
  ...
  <Sx>, <Sy>, <Sz>              -- Average spin componants
  Sig_x, Sig_px, Sig_y, 
...
transport maps? Default is False. For testing, can also be used with non-map tracking.
%
...
beam is 100\% polarized. A plot of polarization vs time now gives the damping time.
...
the $(Q_x, Q_y)$ tune plane. The \vn{tune_scan} program can be used to simulate the beam at different in
$(Q_x, Q_y)$ tune plane to see where the beam blows up.
%
\item
...
%
\item
Using SINGLE mode tracking, output a turn-by-turn orbit  (set ltt%particle_output_file and ltt%particle_output_every_n_turns = 1). Then use your favorate plotting program to plot the phase space trajectory. This Will give you a lot of information.
\end{itemize}
