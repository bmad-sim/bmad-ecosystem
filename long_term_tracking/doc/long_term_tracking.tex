...
\author{}
\date{David Sagan \\ December 1, 2022}
...
%
\item["SINGLE"] \Newline
In this mode a single particle is tracked for \vn{ltt%n_turns} turns. The starting coordinates are determined
in the same way as is done with the \vn{"CHECK"} mode.
...
The name of the ASCII file (or files) that are created to hold the particle positions in action-angle
coordinates. Used with \vn{ltt%simulation_mode} set to \vn{"BEAM"}. See \Sref{s:beam.part.dat} 
for more details. If \vn{ltt%action_angle_output_file} is blank, no particle
output file is created. See also the parameter \vn{ltt%beam_binary_output_file} which will produce a
portable binary file. Also see \vn{ltt%phase_space_output_file}.
%
...
\end{code}
In this example, output will happen every 1000 turns. For particle phase space output, use the \vn{particle_output_every_n_turns} parameter.
%
...
is created to hold the particle positions at the end of tracking. This is similar to
\vn{ltt%phase_space_output_file} but here the file will be a binary file that is portable to other
programs and can also be used as input to the \ltt program. The file suffix should be \vn{.h5} or
\vn{.hdf5}. If neither of these suffixes are used the binary file name will have a \vn{.h5} suffix
added. To translate to ASCII, use the \vn{beam_file_translate_format} program. Documentation is in
any Bmad Distribution in the file:
\begin{code}
...
\end{code}
In this example, output will happen every 1000 turns. For ``averages'' files the corresponding
parameter is \vn{averages_output_every_n_turns}.
%
...
\vn{ltt%phase_space_output_file} is blank, no particle output file is created. See also
\vn{ltt%beam_binary_output_file} which will produce a portable binary file. Also see
\vn{ltt%action_angle_output_file}.
%
...
The name of the files are derived from (\sref{s:out.files}):
\begin{code}
...
Each line in the file represents the averages at the end of a turn and a new line is added every
\vn{ltt%averages_output_every_n_turns} number of turns. If \vn{ltt%averages_output_file} is blank,
no averages file is produced. Data is always recorded when the beam is at the \vn{ltt%ele_start}
position independent of where the lattice begins and ends.
