...
\author{}
\date{David Sagan \\ October 13, 2020}
\begin{document}
\maketitle
...
The \ltt program is built atop the the Bmad software toolkit \cite{b:bmad}. The Bmad toolkit is a
...
\vn{ltt%output_every_n_turns} turns.
%
\item["BUNCH"] \Newline
...
When tracking with \vn{ltt%simulation_mode} set to \vn{"SINGLE"} or \vn{"BUNCH"}, the setting of
...
Damping and excitation are cotrolled by two parameters that can be set in the master input file:
\begin{code}
...
precission (radiation damping is not problematic in itself but that is small comfort). To get around
...
maps whose boundries will be at the bend centers. When tracking, the maps will not incorporate any
...
valid when the tracked partices are far from any strong resonances. That is, if you are interested
...
initial particle position will be set to the inital beam center position. The parameters that are used to
...
\begin{code}
  type beam_init_struct
    ...
    real a_norm_emit = 0               ! a-mode normalized emittance (= \(\gamma\,\epsilon\))
    real b_norm_emit = 0               ! b-mode normalized emittance (= \(\gamma\,\epsilon\))
    real a_emit = 0                    ! a-mode emittance (= \(\gamma\,\epsilon\))
    real b_emit = 0                    ! b-mode emittance (= \(\gamma\,\epsilon\))
...
&params
  ...
  ltt%bunch_output_file = "bunch.dat"
...
\begin{description}
\item[ltt\%bunch_output_file] \Newline
Used with \vn{ltt%simulation_mode} set to \vn{"BUNCH"}. The name of the file that is created to hold
the particle positions. This file is created at the end of tracking. This is similar to
...
Used with \vn{ltt%simulation_mode} set to \vn{"BUNCH"}. The name of the ASCII file or files that are
created to hold the particle positions. If \vn{ltt%merge_particle_output_files} is set to False, a
file is created every \vn{ltt%ouput_every_n_turns} turns.  If \vn{ltt%merge_particle_ouput_files} is
set to True, the files are combined into one. See Section~\sref{s:part.out} for more details. If
\vn{ltt%particle_ouput_file} is blank, no particle output file is created. See also
\vn{ltt%bunch_output_file} which will produce a portable binary file.
%
...
If spin tracking is on (\sref{bmad_com%spin_tracking_on = T}), and if all three components of
\vn{beam_init%spin} are zero, the closed orbit spin direction are used.
...
``vertical'' mode.] These emittance can be sepcified by setting \vn{beam_init%a_emit} and/or
...
%------------------------------------------------------------------
\Section{Correcting the Orbit when Radiation Fluctuations are Present}
\label{s:damp.orbit}

When radiation damping is simulated, the closed orbit that is flat when there is no radiation will
show a ``sawtooth" pattern. In an actual ring, this sawtooth pattern will be compensated using
steerings. Thus to simulate the actual ring, compensating steerings should be added to the lattice
file. This section shows how to calculate steering strengths using the \vn{Tao} simulation program.
If you are not familiar with \vn{Tao}, please see the ``Introduction and Tutorial to Bmad and Tao''
which can be accessed via the Bmad we page
\begin{code}
