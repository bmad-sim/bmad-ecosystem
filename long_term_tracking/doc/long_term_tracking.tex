...
Cutoff used to calculate the ``core'' emittance. Default value is 0.5. See Section~\sref{s:core} for
more details. The value of \vn{ltt%core_emit_cutoff} must be greater than zero and less than or
equal to one. Using a \vn{ltt%core_emit_cutoff} near zero may lead to invalid results if the number
of particle to be retained is too small since there will be statistical fluctuations of order
$1/\sqrt{N_c}$ where $N_c$ is the number of particles in the core.
%
...
default value for \vn{ltt%dead_cutoff} is 1.
%
...
core emittance is computed from the remaining 90\%. On a given turn, there are actually three 
separate calculations, one for each normal mode of oscillation. The calculation of the core emittance for
a given normal mode precedes in a number of steps:
\begin{enumerate}
%
\item
...
The reason for going through the winnowing process twice is to remove, to a good degree, any influence
...
The factor $f_n$ in step 6 above is calculated by first calculating the action cutoff $J_C$ so 
that the probability of a particle having an amplitude less than $J_c$ is $C$ (equal to
\vn{ltt%core_emit_cuttoff}). $J_c$ is calculated from the equation
\begin{equation}
...
\begin{equation}
  \epsilon_c(\text{unnorm}) = \frac{1}{C} \, \int_0^{J_c} \! dJ \, \rho(J) \, J
\end{equation}
Combining everything gives the normalization factor
\begin{equation}
  f_n = \frac{\epsilon_c(\text{norm})}{\epsilon_c(\text{unnorm})} = 
  \frac{\epsilon}{\epsilon_c(\text{unnorm})} = 
