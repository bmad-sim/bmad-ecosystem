...
higher order mode cavity resonances, energy ramping, beta squeesing, etc. The output of the program
...
The name of the data file is set by the \vn{ltt%particle_output_file} parameter
...
stochastice radiation effects (but will still have radiation damping) and a stochastic radiation
...
1. This affects the files specified by \vn{ltt%averages_output_file}.
...
\begin{equation}
  ...
  i = $x$, $y$, \text{or}, $z$
\end{equation}
And kurtosis \kappa_4$ is defined by
\begin{equation}
  \kappa_{4i} \equiv \frac{\left< dr_i^4 \right>}{\sigma_i^4} - 3, \quad 
  i = $x$, $y$, \text{or}, $z$
\end{equation}
With this definition, a beam with a Gaussian distribution will have zero kurtosis.
