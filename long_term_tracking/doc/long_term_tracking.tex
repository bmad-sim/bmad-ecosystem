...
the map is given by \vn{ltt%map_order} parameter. A larger map order will mean better accuracy at
the expense of more computation time. Tracking using a map will be extremely quick compared to
element-by-element tracking. However, map tracking can be inaccurate if the map order is too small
or the particle amplitude is too large.
...
\vn{ttl%map_file}. If \vn{ttl%map_file} is blank, a default name will be constructed.
The saved map will be affected by the setting
of \vn{bmad_com%radiation_damping_on}. See \sref{s:track.modes} for more details.

If the \vn{-write} switch is not present, the 1-turn map is read in from \vn{ttl%map_file} if it exists.
\end{description}
...
String switch which sets how particles are tracked. Possible settings are:
\begin{code}
  "standard"    ! Standard
  "ptc"
  "map"
