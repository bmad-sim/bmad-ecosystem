...
enough so that the ramplers only need be applied once per bunch passage. This is done to minimize
computational time.

If \vn{ltt%ramping_on} is set to True, ramping will be done if \vn{ltt%simulation_mode} is set to
"\vn{BUNCH}" or "\vn{SINGLE}". Ramping will be ignored in the other modes.
...
\begin{description}
\item[ltt\%common_master_input_file] \Newline
...
$N$ is set by \vn{ltt%output_every_n_turns}. Columns are specified by setting \vn{ltt%column(<i>)}. The general
syntax is:
\begin{code}
...
default value is 4.
For example, if there are 11 subprocesses, one of these will be designated the ``master'' subprocess
and will just be responsible for collecting the data together. The other 10 subprocesses will run
simulations. If, say, there are 1M particles in the bunch, and if \vn{ltt%mpi_num_runs} has the default
value of 4, then each simulation will use 25K particles (= 1M / (4 * 10)) and each of the 10 subprocesses
will run the simulation about 4 times. The reason why the work is broken up this way is to minimize the
effect that one slow subprocess can have on the simulation time.
%
