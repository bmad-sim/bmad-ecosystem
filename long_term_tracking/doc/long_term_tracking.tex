...
How to do this is covered in the \bmad and \tao Cookbook available from the \bmad web site.
...
Number of turns to average over when computing beam sigma matrix and positions. Default is 1. For
example:
\begin{code}
  ltt%output_every_n_turns = 1000
...
through \vn{(ltt%averaging_window-1)/2}. The number of turns tracked will be extended past
\vn{ltt%n_turns} so that the averaging window used for the point \vn{ltt%n_turns} has the normal
width.
%
\item[ltt\%bunch_binary_output_file] \Newline
Used with \vn{ltt%simulation_mode} set to \vn{"BUNCH"}. This parameter sets the name of the file
...
%
\item[ltt\%one_particle_output_file] \Newline
...
\vn{ltt%particle_output_file}. Also see \vn{ltt%averaging_window}.
%
...
created to hold the particle positions. If \vn{ltt%one_particle_output_file} is set to False, a
...
\vn{ltt%bunch_binary_output_file} which will produce a portable binary file.
%
...
With \vn{ltt%one_particle_output_file} set to \vn{False}, an output data file will be produced
...
If \vn{ltt%particle_output_file} contains a hash character ''\#'', the
...
With \vn{ltt%one_particle_output_file} set to \vn{True}, instead of generating multiple files
over multiple turns, all data is put into one file whose name is given by
\vn{ltt%particle_output_file}. In this case, a hash character in the file name is not treated as
a special character.
