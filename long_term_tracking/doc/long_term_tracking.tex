...
%
\item["BEAM"] \Newline
In this mode a particle beam is tracked for \vn{ltt%n_turns} turns.
%
\item["STAT"] \Newline
...
passage. If set to False (the default), rampers are applied to a given element only once per beam
passage using the time of the center of the bunch passing the center of the element. This is
appropriate for parameters that vary slowly on the time scale of a bunch passage. For example if
ramping magnet strengths with energy ramping which occurs over many turns, calculating once per beam
passage should be a good approximation.
...
\vn{.hdf5}. If neither of these suffixes are used the binary file name will have an \vn{.h5} suffix
