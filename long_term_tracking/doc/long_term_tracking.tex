...
\begin{equation}
  A(J_x, J_y, J_z) \equiv \frac{J_x}{\epsilon_x} + \frac{J_y}{\epsilon_z} + \frac{J_y}{\epsilon_z}
\end{equation}
where $J_{x,y,z}$ are the three normal mode actions of the particle as calculated from Wolski Eq.~(51) and
$\epsilon_{x,y,z}$ are the three emittances as calculated from Wolski Eq.~(30). 
...
core emittances $\epsilon(\unnorm)$ are calculated.
%
\item
...
normalized core emittances and the whole beam emittance will be an indicator for how non-Gaussian
the distribution near the core is.
\end{enumerate}

Normalized core emittances $\epsilon(\norm)$ are calculated from the unnormalized values by multiplying
by a factor $f_n$ such that if the beam shape were Gaussian, the value of the normalized core emittance
will be the same as the whole beam emittance. $f_n$ is calculated by first calculating the 
6D amplitude cutoff $A_c$ so that,
with a Gaussian distribution, the probability of a particle having a 6D amplitude less than $A_c$ is
$\vn{ltt%core_emit_cuttoff} which here will be denoted by $C$. $A_c$ is calculated from the equation
\begin{equation}
  C = \iiint_{A \le A_c} \!\! dJ_x dJ_y dJ_z \, \rho(J_x,J_y,J_z) = 
  \int_0^A_c \! d\tJ_x \int_0^{A_c-\tJ_x/} \! d\tJ_y \int_0^{A_c-\tJ_x-\tJ_y} \! d\tJ_z \, \exp{-(\tJ_x+\tJ_y+\tJ_z} =
  1 - \Left( 1 + A_c + \frac{1}{2} A_c^2 \Right) \exp{-A_c}
  \label{cjjj}
\end{equation}
where $\rho$ is the particle probability density and $\tJ_x = J_x/\epsilon_x$, etc. \Eq{cjjj} is
easily numerically solved for $A_c$ given some value for $C$.  Assuming a Gaussian distribution, The
computed unnormalized core emittance will be
\begin{equation}
  \epsilon_(\unnorm) = \frac{1}{C} \, \iiint_{A \le A_c} \!\! dJ_x dJ_y dJ_z \, \rho(J_x,J_y,J_z) =
\end{equation}
Combining everything gives the normalization factor
\begin{equation}
  f_n = \frac{\epsilon_c(\norm)}{\epsilon_c(\unnorm)} = 
  \frac{\epsilon}{\epsilon_c(\unnorm)} = 
  \frac{C}{1 - (1 + \tilde J_c) \, \exp(-\tilde J_c)}
\end{equation}
where $\tilde J_c \equiv J_c / \epsilon = -\log(1-C)$. The factor $f_n$ will be the same for all
...
\begin{equation}
  ...
  \int_0^{J_c} \! dJ \, \frac{1}{\epsilon} \, \exp(-J/\epsilon) = 
  1 - \exp(-J_c/\epsilon)
\end{equation}
...
\begin{equation}
  \epsilon_c(\unnorm) = \frac{1}{C} \, \int_0^{J_c} \! dJ \, \rho(J) \, J
\end{equation}
Combining everything gives the normalization factor
\begin{equation}
  f_n = \frac{\epsilon_c(\norm)}{\epsilon_c(\unnorm)} = 
