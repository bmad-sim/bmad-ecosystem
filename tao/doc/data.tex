...
Chromaticities. Also see \vn{chrom_ptc}. The calculation uses finite differences with the variation of $pz$ being set by 
...
Chromaticities and the W-function will be calculated even if the geometry of the lattice branch has an open geometry.
In this case, dbeta/dpz and dalpha/dpz at the beginning of the branch can be set in the lattice file by setting
\vn{beginning[dbeta_dpz_a]}, \vn{beginning[dbeta_dpz_b]}, \vn{beginning[dalpha_dpz_a]} and \vn{beginning[dalpha_dpz_b]}.

  %----------------------
  \item[chrom.deta.x, .deta.y] \Newline \hlabel{chrom.deta}
The chromatic dispersion $\partial\eta_{x,y}/\partial p_z$. Unlike the standard
...
The chromatic momentum dispersion derivatives $\partial\eta'_{x,y}/\partial p_z$. Unlike the
standard chromaticities,\vn{chrom.a} and \vn{chrom.b}, these chromaticities are evaluated at
individual elements. The calculation uses finite differences with the variation of $pz$ being set by
\vn{global%delta_e_chrom}.
