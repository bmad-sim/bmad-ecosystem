...
\end{example}
If a secondary lattice file (\sref{s:init.lat}) is used separate the primary lattice file from
the secondary using a comma (and no spaces). Example:
\begin{example}
  tao -lat prim.bmad,second.bmad
\end{example}
...
The optional \vn{<lattice2_file>} specifies a ``secondary'' lattice file that is parsed after
the ``primary'' lattice file specified by \vn{lattice_file}. The secondary lattice
file must only have statements that are valid post lattice expansion.  See the \bmad manual manual
for a discussion of lattice expansion. Note: If a \vn{%slice_lattice} parameter is used with a
