...
Sets parameters for the curve legend (\sref{s:plot.def}). \vn{%curve_legend} is an instance of
a \vn{qp_legend_struct} (\sref{s:qp.legend.str}). Note: When drawing a curve legend for a particular
graph, the \vn{origin} component of \vn{%curve_legend}
is ignored and instead the \vn{graph%curve_legend_origin} is used to position the legend within the graph.
%
