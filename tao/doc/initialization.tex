...
%
\item[-help] \Newline
Print a list of possible startup command line parameters.
%
...
Overrides the \vn{startup_file} (\sref{s:init.begin}) specified in the \tao initialization
file. Note: Multiple startup files can be specified as a comma deliminated list. Note: Either
have no spaces between file names or put the entire list in quotes.
%
...
\item[-version] \Newline
Print \tao version date and stop.
...
\tao can open and execute a command file or files (\sref{s:command.files}) at startup.  The
default name is \vn{tao.startup} but this name can be changed by setting the \vn{startup_file}
component in the \vn{tao_start} namelist. To specify multiple startup files, which will be executed
in order, use a comma deliminated list.
...
spacing between points. Default is -1. Any value less than zero means no comb will be calculated.
