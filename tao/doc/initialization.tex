...
type tao_global_struct:
  ...
  lat_sigma_calc_uses_emit_from = F ! Init sigma mat set equal to beam distribution? 
...
values are:
\begin{example}
  "lat"           ! Default. Calculated from the lattice. For closed geometry branches. 
...
The \vn{"lat"} setting of \vn{global%lat_sigma_calc_uses_emit_from} (which is the default) is for
use with lattice branches that have a closed geometry where the emittances and other parameters can
be computed from the lattice. For branches that are open, \tao will fall back to using the
\vn{beam_init} structure (\sref{s:opti.de.param.struct}).

For everything besides the \vn{"lat"} setting of \vn{global%lat_sigma_calc_uses_emit_from}, 
the calculation of the sigma matrix starts at the lattice
element defined by the \vn{track_start} parameter in the \vn{tao_beam_init} namelist
(\sref{s:beam.init}). With the \vn{"lat"} setting, the initial sigma matrix is taken to be
at the beginning of the lattice branch.
