...
and/or using \tao's \vn{set particle_start} or \vn{change particle_start} commands. With the
\vn{-static} option, \vn{particle_start} is set to the same orbit as currently exists for the closed
orbit. With the \vn{-zero} option, the \vn{particle_start} orbit is set to zero. With the
\vn{-particle_start} option, the \vn{particle_start} orbit is not modified. The default is
\vn{-static} except if the closed lattice is unstable (and there is no well defined orbit) in which
case \vn{-particle_start} is the default.
