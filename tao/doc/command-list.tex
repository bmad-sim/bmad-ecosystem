...
Note: Setting \vn{particle_start} will affect the orbit at the active \vn{fixer} element and the 
default active fixer element is the \vn{beginning} element.
...
parameters from the from the actual to the stored. See the \bmad manual for documentation
on fixer elements. Command format:
\begin{example}
  fixer activate <fixer>
  fixer save <fixer> \{<parameters>\}
  fixer write <fixer> \{<file-name>\}
\end{example}
here \vn{<fixer>} is the name of a fixer element (Note: For the purposes of this discussion, the
\vn{BEGINNING} element is considered a fixer element).
...
The \vn{activate} subcommand activates (turns on) a given fixer element. When a fixer element is
activated, the stored Twiss and orbit parameters will be used to set the actual Twiss and orbit.

The \vn{save} subcommand takes actual parameter values and saves them in the corresponding
stored parameters. Either an individual or a parameter group may be used for \vn{<parameters>}.

Note: To set stored parameters to arbitrary values, use the \vn{set element} (\sref{s:set.element}),
or \vn{change element} (\sref{s:change}) commands. Note: Changing stored values does not affect
the actual values. To load stored values to actual use the \vn{fixer activate} command.

The \vn{write} subcommand write a file of fixer stored values. This file is in Bmad lattice
format. The default file name is \vn{<fixer>.bmad} where \vn{<fixer>} is the name of the fixer
element.
...
When running \tao, to see documentation on any of the subcommands, use the command:
\begin{example}
...
\begin{example}
  help pipe ele:orbit
\end{example}
will show information on the \vn{pipe ele:orbit} subcommand.
...
The \vn{read} command is used to modify the (\bmad) \vn{model} lattice. Format:
\begin{example}
...
The reference species of a lattice branch can be set by setting \vn{ref_species} of the 
\vn{beginning} element of the branch.
...
\begin{example}
  ...
  set ptc_com pancake_symplectic = T ! Symplectic pancake integrator for gen_gradient elements.
\end{example}
...
The \vn{show global} command prints lists of global parameters. Only one of the optional
arguments may be used at one time.

The \vn{-environment} switch shows environment variables defined in any of the \bmad lattice
files. See the \bmad manual for details. If there are multiple universes, only the environment
variables associated with the last universe are displayed.
...
\begin{example}
  ...
  write scibmad ...               ! \sref{s:write.scibmad}
...
The default file name is \vn{lat_\#.pals.yaml} where \vn{\#} is replaced by the universe number. 
...
\subsection{write scibmad}
\label{s:write.scibmad}
...
The default file name is \vn{lat_\#.jl} where \vn{\#} is replaced by the universe number. 
