...
if (graph%floor_plan%orbit_scale /= 0 .and. ele%value(l$) /= 0) then
  if (is_bend) then
    ...
    do j = 0, n
      ...
      f_orb%r(3) = s_here
      f_orb = coords_local_curvilinear_to_floor (f_orb, ele, .false., end_origin = upstream_end$)
  ...
  elseif (ele%key == patch$) then
    ...
    floor1 = coords_local_curvilinear_to_floor (floor, ele, .false., end_origin = downstream_end$)
    call tao_floor_to_screen_coords (graph, floor1, f_orb)
    ...
    do ic = 0, n
      ...
      floor%r(3) = s_here
      floor1 = coords_local_curvilinear_to_floor (floor, ele, .false., end_origin = upstream_end$)
      call tao_floor_to_screen_coords (graph, floor1, f_orb)
