subroutine read_outline (walls, ring, type_warning)
...
do
  ...
  if (z%n_out == -1) then
    print *, 'ERROR IN READ_OUTLINE: OUT ARRAY OVERFLOW FOR: ', z%name
    call err_exit
  endif
  ...
  elseif (units /= 'METRIC') then
    print *, 'ERROR IN READ_OUTLINE: UNKNOWN UNITS: ', units
  ...
  if (name == 'S FLANGE' .and. z%n_out == 0) then
    if (type_warning) print *, 'WARNING: STANDARD FLANGE ("S FLANGE") IGNORED'
    cycle
  endif

  !    if (name == 'D SLD JNT') then
  !      if (type_warning) print *, 'WARNING: FUDGING "? SLD JNT" TO BE "D SLD JNT"'
...
f = 1/0.0254
open (unit = 11, file = 'outside_wall.dat')
write (11, *) &
...
open (unit = 11, file = 'inside_wall.dat')
write (11, *) &
 '   Ix        S(m)       X(m)    Name                   S(in)       X(in) '
