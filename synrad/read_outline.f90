...
subroutine read_outline (outside, inside, ring, type_warning)
  ...
  type (outline_struct) outline(100), outline, outline_in, z
...
!      outline(ixx+1) = outline(ixx)
!      ixx = ixx + 1
!      outline(ixx)%name = '? SLD JNT'
!      outline(ixx)%in%name = '? SLD JNT'
!      outline(ixx)%out%name = '? SLD JNT'
!    endif
...
! form concatenated elements
  ...
  do
    ...
    do j = 1, size(concat%part)
      ...
      found = .false.
      do i = 1, n_outline
        if (concat%part(j)%name == outline(i)%name) then
          found = .true.
          outline_in = outline(i)
          exit
        endif
      enddo
    ...
    outline(n_outline) = outline
    outline(n_outline)%overlay = concat%overlay
...
! match outlines with element names
  ...
  outline_loop: do j = 1, n_vac_parts
    ...
    do i = 1, n_outline
      call str_upcase(name, outline(i)%name)
      if (name == vac_ele(j)%name) then
        vac_ele(j)%ix_outline = i
        if (outline(i)%n_out == 0) cycle outline_loop
        if (j .ne. 1) then
          call check_end (outline(i)%out, 1, 'OUTSIDE BEGINNING')
          call check_end (outline(i)%in, 1, 'INSIDE BEGINNING')
        endif
        if (j .ne. n_vac_parts) then
          call check_end (outline(i)%out, outline(i)%n_out, 'OUTSIDE ENDING')
          call check_end (outline(i)%in, outline(i)%n_in, 'INSIDE ENDING')
        endif
        cycle outline_loop
      endif
    enddo
...
! go through all the vacuum elements and calculate what the wall should be
  ...
  do j = 1, n_vac_parts
    ...
    if (outline(i)%n_out == 0 .or. outline(i)%n_in == 0) cycle
    outline = outline(i)
...
! check that endpoints are correct
  ...
  if (abs(outside%pt(outside%n_pt_tot)%s - ring%ele(ring%n_ele_track)%s) > &
    ...
    type *, '       AND NOT AT RING END OF:', ring%ele(ring%n_ele_track)%s
    call err_exit
  else
