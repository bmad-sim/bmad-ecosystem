...
! read in list of elements
  ...
  if (ios == 0) then
    ...
    open (lun, file = 'u:[cesr.mslog.vacuum.vac_db_new]vac_db_sr.txt',  &
                                                     status = 'old', readonly)
  endif
...
! use a local file if there is one
  ...
  if (ios == 0) then
    ...
    open (unit = lun, file = '[cesr.cesrv]beam_pipe.outline',  &
                                                     status = 'old', readonly)
  endif
...
! write to file
  ...
  f = 1/0.0254
  open (unit = 11, file = 'outside_wall.dat', status = 'new', &
...
!
  ...
  open (unit = 11, file = 'inside_wall.dat', status = 'new', &
                                                 carriagecontrol = 'list')
  write (11, *) &
   '   Ix        S(m)       X(m)    Name                   S(in)       X(in) '
