...
!   hitting wall segments from all elements in the ring
!
! Modules needed:
!   use sr_mod
!
...
! initialize all accumulated power to 0
  ...
  power(1:ring%n_ele_max)%at_wall = 0
  power(1:ring%n_ele_max)%radiated = 0

! loop over all elements

  do ie = 1, ring%n_ele_ring
!    print *, ring%ele_(ie)%name,',',ring%ele_(ie)%type, &
!         ' ele ',ie,' of ',ring%n_ele_ring
    call ele_sr_power (ring, ie, orb, direction, power, inside, outside, gen)
  enddo
