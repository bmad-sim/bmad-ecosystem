...
do n = 1, wall%n_pt_tot
  ...
  if (wptr(n0)%s > wptr(n1)%s) then
    type *, 'ERROR: ', trim(wall_name(wall%side)), ' WALL IX_PT ARRAY NOT ORDERED'
...
if (wptr(0)%s /= 0) then
  type *, 'ERROR: ', wall_name(wall%side), ' WALL DOES NOT START AT S = 0'
  call err_exit
endif

if (wptr(wall%n_pt_tot)%s /= s_lat) then
  type *, 'ERROR: ', wall_name(wall%side), ' WALL DOES NOT GO A FULL TURN'
  call err_exit
endif
