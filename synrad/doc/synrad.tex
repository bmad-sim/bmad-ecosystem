...
%------------------------------------------------------------------
  ...
  \caption{
Photon tracking is in the horizontal $x$-$s$ plane. The wall has a
$-x$ side and a $+x$ side. The wall is defined by a number of vertex
points (shown as circles) with straight lines between vertices. For
the calculation, the straight lines are divided into a number of
roughly equal length short segments as shown for a portion of the
$-x$ side.
  }
  \label{f:wall}
  \end{center}
...
The curvilinear coordinate system used by Synrad corresponds to the
Bmad standard (see the Coordinates chapter in the Bmad
manual\cite{b:bmad}). If there are no vertical bends, the $s$-axis is
the longitudinal axis along the direction of the beam, the $y$-axis is
in the vertical direction, and the $x$-axis is in the horizontal
plane.
...
The $-x$ and outside walls are specified by a number of vertex
...
form is
\begin{example}
  &wall_pt  s = <val> \{x_in = <val>\} \{x_out = <val>} \{phantom = <T/F>\} \{name = <string>\} /
\end{example}
\vn{x_in} defines the $x$ value for the $-x$ wall and \vn{x_out} defines the value for the $+x$ wall.
Example:
\begin{example}
  &wall_pt  s =    0.0000  x_in =  -0.0450  x_out =   0.0450  name = BEAMPIPE_Q00:3  /
  &wall_pt  s =    0.8581  x_in =  -0.0490  x_out =   0.0490  name = WIGGLER_TA:2  /
  &wall_pt  s =    0.8581                   x_out =   0.1223  name = WIGGLER_TA:2 phantom = T /
  ...
\end{example}

Either \vn{x_in}, or \vn{x_out}, or both must be present in each
\vn[wall_pt} namelist. The \vn{name} parameter is used for identifying
individual vertex points.


The wall is constructred by connecting each vertex point to the previous point.
