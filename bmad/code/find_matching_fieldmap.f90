...
!   fm_type       -- integer: Type of fieldmap: cartesian_map$, cylindircal_map$, or gen_gradients$, grid_field$
...
do ib = 0, ele%ix_branch
  do ie = 1, lat%branch(ib)%n_ele_max
    ...
    case (gen_gradients$)
      if (.not. associated(match_ele%gen_gradients)) cycle
      do ix_field = 1, size(match_ele%gen_gradients)
        if (match_ele%gen_gradients(ix_field)%file == file_name) return
      enddo
