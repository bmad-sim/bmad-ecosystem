!+         
! Subroutine transfer_map_calc (lat, orb_map, err_flag, ix1, ix2, ref_orb, 
!                                              ix_branch, one_turn, unit_start, concat_if_possible, spin_map)
!
...
!   orb_map(6)    -- taylor_struct: Initial map (used when unit_start = False)
...
!   unit_start    -- logical, optional: If present and False then orb_map will be
...
type (taylor_struct) :: orb_map(:), a_map(6)
...
if (i1 < i2) then 
  do i = i1+1, i2
    call add_on_to_map (orb_map, branch%ele(i), err_flag, spin_map)
    if (err_flag) return
  enddo
...
! Open lattice with i1 > i2: Track backwards.
  ...
  if (unit_start_this) then
    do i = i2+1, i1
      ...
      if (err_flag) return
    enddo
    call taylor_inverse (orb_map, orb_map)

  else
    call taylor_make_unit (a_map)
    do i = i2+1, i1
      call add_on_to_map (a_map, branch%ele(i), err_flag, spin_map)
      if (err_flag) return
    enddo
    call taylor_inverse (a_map, a_map)
    call concat_taylor (orb_map, a_map, orb_map)
...
subroutine add_on_to_map (orb_map, ele, err_flag, spin_map)
...
if (logic_option(.false., concat_if_possible) .and. associated(ele%taylor(1)%term)) then
  call concat_ele_taylor (orb_map, ele, err_flag, spin_map)
else
  call taylor_propagate1 (orb_map, ele, branch%param, err_flag, orb0, spin_map)
endif
...
do i = 1, 6
  do k = 1, size(orb_map(i)%term)
    if (abs(orb_map(i)%term(k)%coef) < ex_factor(sum(orb_map(i)%term(k)%expn))) cycle
    err_flag = .true.
    !! call out_io (s_error$, r_name, 'TAYLOR ORB_MAP TERM FLOATING OVERFLOW.')
    return
  enddo
enddo
