...
!
! Notes:
!   * If local_ref_frame = True then orb%vec (and bbi coords) will be converted to laboratory coords.
...
!   local_ref_frame -- Logical: If True then input orb is with respect to body coordinates.
...
if (local_ref_frame) then
  ...
  tp%s_lab = s_rel
  orb0 = orb
  call offset_particle (ele, set$, orb0, drift_to_edge = no$, set_hvkicks = .false., s_pos = s_rel, s_out = tp%s_body)
endif
...
if (present(strong_beam)) then
  ...
  if (local_ref_frame) then
    ...
    tp%strong_beam%x_center = tp%orb%vec(1)
    tp%strong_beam%y_center = tp%orb%vec(3)
  endif
endif
