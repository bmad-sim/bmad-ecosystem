...
write (d_unit) lat%a, lat%b, lat%z, lat%param, lat%version, lat%n_ele_track, lat%parser_make_xfer_mats
...
write (d_unit) lat%branch(0)%name, lat%branch(0)%ix_fixer, lat%branch(0)%particle_start
...
do i = 1, ubound(lat%branch, 1)
  ...
  write (d_unit) branch%name, branch%ix_from_branch, branch%ix_to_ele, &
                 branch%ix_from_ele, branch%n_ele_track, branch%n_ele_max, branch%ix_fixer, branch%particle_start
...
if (present(extra)) then
  ...
  write (d_unit) ptc_com%max_fringe_order, ptc_com%exact_model, ptc_com%exact_misalign, &
          ...
          ptc_com%use_orientation_patches, ptc_com%print_info_messages, ptc_com%translate_patch_drift_time, &
          ptc_com%pancake_symplectic, ptc_com%pancake_canonical
else
  write (d_unit) .false.
endif
