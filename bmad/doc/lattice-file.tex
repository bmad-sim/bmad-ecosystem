...
Notice that commands for the \vn{Tao} program are case sensitive. The reason for this difference is
due to one of history. \bmad was developed back in the days where case sensitivity was not important.
...
permitted since this interferes with the notation for finding the $N$\Th element with a given name
...
\begin{example}
  ...
  beginning                        no_superimpose               setenv
  call                             parameter                    slice_lattice
  calc_reference_orbit             parser_debug                 start_branch_at
  combine_consecutive_elements     particle_start               superimpose
  debug_marker                     print                        title
  end_file                         redef                        undefined_vars_evaluate_to_zero 
  expand_lattice                   remove_elements              use
  merge_elements                   return                       use_local_lat_file
                                                                write_digested
\end{example}
...
Gen_gradients map (\sref{s:gen.grad.map}):
\begin{example}
  ename[gen_gradients(N)%field_scale]
  ename[gen_gradients(N)%dz]
  ename[gen_gradients(N)%g_ref]
  ename[gen_gradients(N)%r0(1)]
  ename[gen_gradients(N)%r0(2)]
  ename[gen_gradients(N)%r0(3)]
  ename[gen_gradients(N)%master_parameter]
\end{example}
...
scalar parameters can be defined for the lattice as a whole.  Custom element attributes are useful
...
gold anti-atom nucleus would be designated \vn{antiAu-79}.
...
That is, the value of an expression that uses a redefined constant will depend upon if the expression 
is evaluated before or after the redefinition.
...
%-----------------------------------------------------------------------------
\section{Setenv Statement}
\label{s:setenv}
\index{setenv statement|hyperbf}

The \vn{setenv} statement is used to set an environment variable from within a lattice file.
This is useful to easily switch what directory is searched for \vn{called} (\sref{s:call}) files.
The syntax is:
\begin{example}
  setenv <env_name> = <env_value>
\end{example}
where \vn{<env_name>} is the name of the environment variable and \vn{<env_value>} is the value.
Example:
\begin{example}
  setenv SUB_DIR = sub/version2
  call, file = $SUB_DIR/lat2.bmad
\end{example}
In this example, the name of the called file expands to \vn{sub/version2/lat2.bmad}.

An environment variable \vn{LATTICE_ROOT_DIR} is automatically set to the directory that contains
the top-level lattice file. If multiple lattices are parsed by a program (for example, with multiple
universes in \tao), the value of \vn{LATTICE_ROOT_DIR} is reset each time a top-level lattice file
is parsed. However, environment variables that have defined will be retained even when a new top-level
lattice file is parsed.

Environment variables that are set are local to the execution of the program that calls the lattice
parser and do not persist after the program terminates.
...
\begin{example}
  call, filename = <file-name>   ! or
  call, no_abort_on_open_error, file = <file-name>
\end{example}
If the \vn{no_abort_on_open_error} option is present, a missing file will not abort the parsing.
Examples:
\begin{example}
...
An environment variable \vn{LATTICE_ROOT_DIR} is automatically set to the directory that contains
the head lattice file.

Where a called file is searched for may be modified by using a \vn{use_local_lat_file}
statement. See Section~\sref{s:use.loc} for more details. Additionally, the \vn{setenv}
command (\sref{s:setenv}) may be used to define environment variable on the fly.
...
%--------------------------------------------------------------------------
\section{Undefined_vars_evaluate_to_zero Statement}
\index{undefined_vars_evaluate_to_zero|hyperbf}

Normally, using an undefined variable in an expression is treated as an error. This helps
prevent spelling bugs. This is in contrast to \mad where undefined variables are treated
as zero. Occasionally, it is convenient to follow the \mad convention for lattice files
translated from \mad. To do this, put at the top of the lattice file the command:
\begin{example}
  undefined_vars_evaluate_to_zero 
\end{example}
Note that use of this command is discouraged. The superior fix is just to define the variables
to be zero before any use.
