...
the energy gain of
a particle on the reference orb is what the user expects based upon the set set volateage and phase. See
...
in this case, the tracking algoithms is such that no adjustments are needed.
...
\vn{num_steps} or \vn{ds_step}. The reason why there are two different paramters to choose a step
...
the active region. The active region should have a length less than the length of the element. If
...
apporoximation is used. That is, \vn{K0L} is assumed small compared to one. With this approximation,
