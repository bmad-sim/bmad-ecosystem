...
A \vn{fixer} element is used to set the particle orbit and/or the Twiss parameters at
any point in an open geometry lattice branch. The orbit and/or Twiss parameter can then
be propagated both forward and backwards.
...
A \vn{fixer} element has zero length. For tracking particles, a \vn{fixer} element is like a
\vn{marker}. That is, there is no change in phase space coordinates when a particle is tracked
through a \vn{fixer} element.
...
elements since \vn{beginning_ele} elements essentially act as a fixer element at the beginning
of a branch.
...
Turning off the active fixer will result in the \vn{beginning_ele} being turned on.
