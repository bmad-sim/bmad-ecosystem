...
\begin{example}
  ...
  scatter           = <Logic>   ! Is scattering on? Default is True.
  x1_edge           = <Real>    ! Foil sheet edge in the +x direction. Default: -99 m.
  x2_edge           = <Real>    ! Foil sheet edge in the -x direction. Default:  99 m.
  y1_edge           = <Real>    ! Foil sheet edge in the +y direction. Default: -99 m.
  y2_edge           = <Real>    ! Foil sheet edge in the -y direction. Default:  99 m.
\end{example}
...
By setting the \vn{scatter} parameter to \vn{false} (the default is \vn{true}), the foil will not
scatter the particle (but there will still be an energy loss). That is, the particle direction of
motion through the foil is unvaried. The setting of \vn{scatter} does not affect the energy
loss.\footnote
  {
Note that a constant direction of motion implies that $p_x/(1+p_z)$ and $p_y/(1+p_z)$ are constant
(\Eq{xpa1p}).
  }
...
Example:
\begin{example}
  septum: foil, material_type = "Cu", thickness = 0.127, radiation_length = 12.3, x1_edge = -0.3
\end{example}
