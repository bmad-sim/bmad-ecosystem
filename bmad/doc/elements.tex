...
\begin{tabular}{llll} \toprule
  ...
  Fixer             & \ref{s:fixer}         &  Sad_Mult          & \ref{s:sad.mult}   \\
  Floor_Shift       & \ref{s:floor.ele}     &  Sbend             & \ref{s:bend}       \\
  Foil              & \ref{s:foil}          &  Sextupole         & \ref{s:sex}        \\
  Fork              & \ref{s:fork}          &  Sol_Quad          & \ref{s:sq}         \\
  GKicker           & \ref{s:gkicker}       &  Solenoid          & \ref{s:sol}        \\
  HKicker           & \ref{s:hvkicker}      &  Taylor            & \ref{s:taylor}     \\
  Hybrid            & \ref{s:hybrid}        &  Thick_Multipole   & \ref{s:thick.mult} \\
  Instrument        & \ref{s:monitor}       &  Undulator         & \ref{s:wiggler}    \\
  Kicker            & \ref{s:kicker}        &  VKicker           & \ref{s:hvkicker}   \\ 
...
\begin{tabular}{llll} \toprule
  ...
  GKicker            & \ref{s:gkicker}       &  Sample               & \ref{s:sample}      \\
  Instrument         & \ref{s:monitor}       &                       &                     \\
  \bottomrule
\end{tabular}
}
...
The \vn{species_strong}, \vn{pc_strong}, and \vn{E_tot_strong} give the particle species, momentum
and energy of the strong beam. This is only relevant if the velocity of the strong beam is
not equal to the velocity of the weak beam. Only one of \vn{pc_strong} or \vn{E_tot_strong} needs
to be set as one will be calculated from the other. If both are set, the setting of \vn{pc_strong}
will be used.

The \vn{crossing_time} parameter sets when, relative to the reference time, the center of the strong
...
beam and strong beam centers cross the plane of the \vn{beambeam} element at the same time, the
value of \vn{crossing_time} should be set to the value $-z/c\beta$ at the IP where $z$ is the closed
orbit phase space $z$ and $\beta$ is the closed orbit velocity.

When absolute time tracking (\sref{s:rf.time}) is in use, the \vn{repetition_frequency}
parameter (along with the \vn{crossing_time} parameter) is used to calculate the time that the strong
...
Note: To set Twiss and/or orbit parameters at a point not at the beginning of a branch,
use a \vn{fixer} (\sref{s:fixer}) element.
...
\vn{entrance_end}, then the face angles are measured with respect to the entrance coordinates $(s_1
x_1)$.
...
\vn{angle} is considered the dependent variable so if \vn{l} or \vn{g} is varied, the value of
...
\begin{example}
  ...
  \}
\end{example}
A \vn{prob_pc_r} has one \vn{r_values} component and multiple \vn{row} components. The
...
\begin{example}
  ...
  rf_wavelength               ! RF wavelength. Dependent parameter.
  harmon          = <Real>    ! Harmonic number
  harmon_master   = <Logic>   ! Is harmon or rf_frequency the dependent variable
                              !   with reference energy changes?
  voltage                     ! Cavity voltage. Dependent attribute (\sref{s:depend}).
\end{example}

The Hamiltonian $H_{\text{crab}}$ for a thin crab cavity in energy-time phase space coordinates
(\sref{energy.phase.space}) is\cite{b:crab1}:
\begin{equation}
  H_{\text{crab}} = \frac{-r_q \, e \, V}{c P_0} \, x \, \sin(2 \, \pi \, (f_\txt{rf} \, t + \phi_0))
\end{equation}
where $x$ is the particle position, $t$ is the time, $r_q$ is the charge relative to the reference
particle, $V$ is the ``effective'' cavity voltage, $\phi_0$ is a user settable
phase, and $f_\txt{rf}$ is the RF frequency. The kick is:
\begin{align} 
  \Delta p_x &= - \frac{\partial H_{\text{crab}}}{\partial x} = 
    \frac{r_q \, e \, V}{c \, P_0} \, \sin(2 \, \pi \, (f_\txt{rf} \, t + \phi_0)) \CRNO 
  \Delta E &= -c \, P_0 \, \frac{\partial H_{\text{crab}}}{\partial t} = 
    2 \, \pi \, r_q \, e \, V \, \, f_\txt{rf} \, x \, \cos(2 \, \pi \, (f_\txt{rf} \, t + \phi_0)) 
\end{align} 
Note: The sign of $H_{\text{crab}}$ used by Authors in the literature is not standardized. \bmad
uses the convention such that a particle with the charge of the reference particle and with phase
space $z$ (which has opposite sign to $t$) and $V$ positive will have a positive $\Delta p_x$.
...
\begin{align}
  \int \! B_y &= \frac{-V}{c} \, \sin(2 \, \pi \, (f_\txt{rf} \, t + \phi_0)) \CRNO 
  \int \! E_s &= 2 \, \pi \, \beta \, V \, \, f_\txt{rf} \, x \, \cos(2 \, \pi \, (f_\txt{rf} \, t + \phi_0))
\end{align}
where $\beta = v/c$ is the normalized speed of the particle.

The relationship between \vn{harmon} and $f_\txt{rf}$ is 
\vn{harmon} = $t_0 \cdot f_\txt{rf}$ where $t_0$ is the reference 1-turn transit time.

The \vn{rf_wavelength} is computed to be $c / f_\txt{rf}$. Notice that this is independent of the
velocity of the reference particle. In particular, It will only be true that \vn{harmon} times
\vn{rf_wavelength} will equal the branch length if the reference particle travels at the speed of light.
...
for the diffraction plane. Alternatively, for double digit indices, commas can be used to separate
the indices. For example,
\begin{example}
  a_cryst: crystal, crystal_type = "Si(111)", b_param = -1, ...
  b_cryst: crystal, crystal_type = "Si(2,4,10)", b_param = -1, ...
\end{example}
...
{\em Note! Due to priorities being elsewhere, development of coherent photon tracking is moribund.
As a consequence, tracking with diffraction_plate elements is currently disabled.}
...
\begin{example}
  ...
  delta_E_ref  = <Real>    ! Change in reference energy.
\end{example}
...
The reference energy change from the entrance to the exit end of the element is set by \vn{delta_E_ref}. 
The exception is if this change would make the total energy negative at the exit end. In this case,
the change in reference energy is calculated via:
\begin{example}
  dE_ref = Voltage              ! For DC (rf_frequency = 0) guns.
         = Voltage * cos(phi0)  ! For AC guns.
\end{example}
In the first instance, the change in reference energy is set by \vn{delta_E_ref} independent of the
gun \vn{voltage}. This is generally what is wanted since the reference energy can set the strengths
of elements downstream from the gun and typically having the gun voltage affect downstream element
strengths not desireable. {\em It is thus recommended that, unless there is a good reason not to, to
set \vn{delta_E_ref} appropriately.}
...
\begin{example}
  apex: e_gun, l = 0.23, field_calc = fieldmap, rf_frequency = 187e6, 
                grid_field = call::apex_gun_grid.bmad, delta_E_ref = 1000
\end{example}
...
Note: Before the parameter \vn{delta_E_ref} was introduced as an \vn{e_gun} parameter, the following \vn{e_gun}
parameters were defined:
\begin{example}
  gradient_err, gradient_tot
  voltage_err, voltage_tot
  phi0_err
\end{example}
If the \bmad parser encounters these parameters in a lattice file, an error will be thrown.
...
brackets \vn{\{...\}} to avoid ambiguities. Curly brackets are optional when commas are not
used. Examples:
\begin{example}
...
element both forward and backwards to the ends of the branch. \vn{Fixed} elements are convenient, for
...
Attributes specific to a \vn{fixer} elements are:
\begin{example}
  spin_x_stored, spin_y_stored, spin_z_stored     ! Particle spin

  x_stored, px_stored
  y_stored, py_stored                             ! Particle phase space coordinates
  z_stored, pz_stored

  beta_a_stored, alpha_a_stored, phi_a_stored     ! a-mode Twiss
  dbeta_dpz_a_stored, dalpha_dpz_a_stored

  beta_b_stored, alpha_b_stored, phi_b_stored     ! b-mode Twiss
  dbeta_dpz_b_stored, dalpha_dpz_b_stored

  eta_x_stored, etap_x_stored                     ! Horizontal dispersion
  deta_dpz_x_stored, detap_dpz_x_stored

  eta_y_stored, etap_y_stored                     ! Vertical dispersion
  deta_dpz_y_stored, detap_dpz_y_stored

  mode_flip_stored                                ! Logical: Normal modes flipped? 
  cmat_11_stored, cmat_12_stored                  ! Coupling
  cmat_21_stored, cmat_22_stored
\end{example}

A \vn{fixer} element has zero length. For tracking particles, a \vn{fixer} element acts like a
\vn{marker} element. That is, there is no change in phase space coordinates when a particle is
tracked through a \vn{fixer} element.
...
calculation will temporarily set the \vn{scatter} parameter to false since scattering is a random
...
\begin{example}
  ...
  n_rf_steps      = <Int>     ! Number of steps in kick-drift-kick model.
  ks              = <Real>    ! Normalized solenoid strength.
  bs_field        = <Real>    ! Solenoid field strength.
  longitudinal_mode = <Int>   ! Longitudinal mode. Default is 0. May be 0 or 1.
\end{example}

%% Normalized field with respect to the reference energy at the exit end of the element.
...
\begin{equation}
  \text{dE} = r_q \cdot \text{gradient_tot} \cdot L  \cdot \cos(2 \, \pi \, (\phi_t + \phi_\REF))
  \label{dergl}
\end{equation}
where $r_q$ is the charge of the particle relative to the charge of the reference particle and
...
\begin{example}
  \(\phi_\REF\) = phi0 + phi0_multipass + phi0_err
  \label{lcav.phi}
\end{example}
...
\vn{phi0_multipass} is only to be used with multipass elements to shift the phase of the cavity from pass to
pass. See \sref{c:multipass} for details.

The \vn{phi0_autoscale} and \vn{field_autoscale} parameters are
calculated by \bmad's auto-scale module to adjust the RF amplitude and phase so that during tracking,
the energy gain of
a particle on the reference orb is what the user expects based upon the set set voltage and phase. See
Section~\sref{s:autoscale} for more details. In particular, the gradient and phase are adjusted by
\begin{example}
  gradient (tracking) = gradient_tot * field_autoscale
  \(\phi_\REF\) (tracking) = \(\phi_\REF\) + phi0_autoscale
\end{example}
These autoscale shifts are not used when an element's \vn{tracking_method} is \vn{bmad_standard} since,
in this case, the tracking algorithm is such that no adjustments are needed.
The calculation of autoscale values
can be toggled on/off by setting the \vn{autoscale_phase} and \vn{autoscale_amplitude} logicals.
...
The \vn{n_rf_steps} parameter sets the number of drifts used in the kick-drift-kick model
(\sref{s:lcavity.std}) used when the \vn{tracking_method} is set to \vn{bmad_standard}. When using
a fixed step Runge-Kutta \vn{tracking_method}, the step size is determined by the setting of
\vn{num_steps} or \vn{ds_step}. The reason why there are two different parameters to choose a step
size is due to the fact that, in general, the number of drifts needed for an accurate result
is much smaller than the number Runge-Kutta steps.
...
\begin{example}
  ...
  traveling_wave
\end{example}
The \vn{cavity_type} switch is ignored if a field map is used.  The \vn{traveling_wave} cavity has
an electromagnetic wave traveling (in body coordinates) in the forward direction. The
\vn{standing_wave} cavity has a standing wave which can be decomposed into a forward traveling wave
and a backwards traveling wave.

With the \vn{standing_wave} setting, the transverse trajectory through an \vn{lcavity} is modeled
using equations developed by Rosenzweig and Serafini\cite{b:rosenzweig}. See Section
\sref{s:lcavity.std} for more details.  Note: The transfer matrix for an \vn{lcavity} with finite
\vn{gradient} is never symplectic if the reference energy at either end is different. See
\sref{s:phase.space}. 

With \vn{field_calc} set to \vn{bmad_standard}, the electromagnetic fields in a \vn{lcavity} are
described in Section~(\sref{s:rf.fields}). With \vn{cavity_type} set to \vn{standing_wave}, the
longitudinal mode is set by the \vn{longitudinal_mode} parameter. The possible values are 0 or 1 and
the default setting is 0. The electromagnetic fields are used for tracking when an element's
\vn{tracking_method} is set to \vn{runge_kutta} or any similar tracking method.  If an element's
\vn{cavity_type} parameter is set to \vn{standing_wave}, with \vn{field_calc} set to
\vn{bmad_standard}, the ``active region'' over which there is a finite field is \vn{n_cell}
half-wave pillbox resonators where each pillbox has length $\lambda/2$ (\sref{s:rf.fields}). The
default setting for \vn{n_cell} is -1 which will result in the active region being as large as
possible without exceeding the length of the element.

The dependent parameter \vn{l_active} is set to the length of
the active region. The active region is limited to not exceed the length of the element. If
the length of the element is not equal to the active region, the active region is centered in the
element and the regions to either side are treated as field free.

When an element's \vn{tracking_method} is set to \vn{bmad_standard}, settings for the
parameters \vn{n_cell}, and \vn{longitudinal_mode}, \vn{num_steps}, and \vn{ds_step} are ignored.
...
\vn{Lcavity} elements can model couplers (\sref{s:rf.coupler}) and HOM wakes (\sref{s:wakes}) as well as
multipoles (\ref{s:multip}). Note that multipoles in a \vn{lcavity} do not vary in time.
...
The setting of the \vn{kick0} parameter determines how the zeroth order transfer map vector $\Bf V$ is
...
%
\item[zero] \Newline
The \vn{zero} setting for \vn{kick0} sets $\Bf V$ to zero independent of the element position values.
%
\end{description}
...
Attributes specific to a \vn{mirror} element are:
\begin{example}
  K0L_status      = <Switch> ! not_allowed (default), bends_reference, or straight_reference.
\end{example}
...
The \vn{K0L} parameter presents a compatibility problem since MAD treats a finite \vn{K0L} as a zero
length bend with a corresponding change in reference orbit (\sref{s:ref}). On the other hand,
\vn{PTC} does not do this. An additional consideration is that this can be very confusing to
users. To minimize problems, a finite \vn{K0L} is only allowed if the \vn{K0L_status} switch is set
appropriately. Possible settings of \vn{K0L_status} are:
\begin{example}
  not_allowed           ! Default. K0L not allowed to be set. 
  bends_reference       ! Compatible with MAD.
  straight_reference    ! Compatible with PTC.
\end{example}
The tracking with \vn{K0L_status} set to \vn{bends_reference} follows MAD. Here a paraxial
approximation is used. That is, \vn{K0L} is assumed small compared to one. With this approximation,
a particle with zero $p_z$ will exit the multipole with the same phase space coordinates as it
enters with. Notice that since the reference orbit is shifted, the setting of \vn{K0L} will affect
the placement of elements downstream of the multipole in the global coordinate system (analogous to
the shift in downstream element positions after \vn{sbend} and \vn{rbend} elements).

With \vn{K0L_status} set to \vn{straight_reference}, a finite \vn{K0L} acts as a simple dipole kick
analogous to the \vn{a0} and \vn{b0} dipole kicks when using an \vn{ab_multipole}.
...
\end{example}
where the \vn{E p} pairs are the energy and photon emission probability at that energy. 
...
Due to the way bookkeeping is done for ramper elements, and unlike \vn{group} or \vn{overlay}
...
\begin{example}
  ...
  phi0_multipass  = <Real>    ! Phase variation with multipass elements (rad/2pi) (\sref{c:multipass}).
\end{example}
...
\begin{example}
  ...
  phi0_autoscale  = <Real>    ! Set by Bmad if autoscaling is turned on (rad/2pi). (\sref{c:multipass}).
...
\sref{c:multipass} for more details. \vn{e_charge} is the magnitude of the charge on an electron
...
Section~\sref{s:autoscale} for more details. In particular, the gradient and phase are adjusted by
\begin{example}
  gradient (tracking) = gradient * field_autoscale
  \(\phi_\REF\) (tracking) = \(\phi_\REF\) + phi0_autoscale
\end{example}
These autoscale shifts are not used when an element's \vn{tracking_method} is \vn{bmad_standard} since,
in this case, the tracking algorithm is such that no adjustments are needed.
...
A \vn{sad_mult} element is used to emulate a SAD\cite{b:sad} \vn{MULT} element with the restriction
that \vn{sad_mult} elements cannot be used to simulate RF cavities or bends.
...
\begin{example}
  ...
  fq1, fq2        = <Real>    ! Quadrupole fringe parameters. SAD equivalents: F1, F2.
  fb1, fb2        = <Real>    ! Bend edge fringe parameters. SAD equivalents: FB1, FB2.
...
With \vn{delta_ref_time_user_set} set to \vn{False} (the default), \bmad will calculate the change
in reference time through the element, \vn{delta_ref_time}, by tracking a particle through the wiggler
starting the particle with zero phase-space coordinates (\sref{s:energy}). Sometimes this calculation
does not produce a value for \vn{delta_ref_time} that is appropriate for a simulation. In this case,
\vn{delta_ref_time_user_set} can be set to \vn{True} and \vn{delta_ref_time} can then be set in
the lattice file.
