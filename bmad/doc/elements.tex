...
NOTE! Originally, for compatibility with MAD, a \bmad \vn{Multipole} with a finite \vn{K0L} would
act as a zero length bend with a change in reference orbit (\sref{s:ref}) commensurate with the
value of \vn{K0L}. This was problematical for several reasons. For one, \vn{PTC} does not do this so
...
%issue that radiation effects due to a multipole are ignored. This is done since the amount of
%radiation emitted from a true zero length element would be infinite so there is no good way to
%calculate what the radiation would be. This is generally a reasonable approximation for, say, a zero
%length quadrupole, however ignoring radiation in dipoles can be problematical since dipole radiation
...
and, if K0L is set, an error message will be issued.\footnote
  {
One possibility that was considered was to simply treat \vn{K0L} as a kick without any change in
reference orbit.  This was rejected since it could be confusing to compare MAD and \bmad
computations with lattices that had a finite \vn{K0L}.
  }
In \bmad, an equivalent to a multipole with \vn{K0L} is a \vn{patch} element with finite
\vn{x_pitch} or \vn{y_pitch} along with a \vn{kicker} element with the appropriate kick.
