...
length bend with a corresponding change in reference orbit (\sref{s:ref}) while \vn{PTC} does not do
this. An additional consideration is that this can be very confusing to users. To minimize problems,
a finite \vn{K0L} is only allowed if the \vn{K0L_status} switch is set appropriately. Possibilities
settings are:
\begin{example}
