...
Since there are two possible anti-parallel solutions for $\bfn_0$, \bmad allows the user to specify
which solution to pick if the parameter
\begin{example}
  bmad_com%spin_n0_direction_user_set (\sref{s:bmad.common})
\end{example}
is set \vn{True}. If set \vn{False} (which is the default), the direction is choosen by \bmad using
the convention that chosen spin tune is in the range $[0, \pi]$. From the 1-turn
spin transport quaternian $\bfq$ (\Eqs{qqqqq} and \eq{qt2ui}), the equation to calculate the tune is
\begin{equation}
...
\begin{equation}
  ...
  \label{n0q0}
\end{equation}
where $\sign$ is the sign function. Note that with \Eq{n0q0} of $\bfn_0$ is independent of whether
$\bfq$ or $-\bfq$ is used in the equation.
