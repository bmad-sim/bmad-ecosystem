...
\begin{example}
   ...
   14 -- Dark_Grey\$
   15 -- Light_Grey\$
   16 -- Transparent\$ (not drawn)
\end{example}

In addition, all 138 CSS4 (CSS Color Level 4) named colors are available. These occupy color indices
17 through 154 and can be specified by name (e.g., \vn{"salmon"}, \vn{"darkslategray"},
\vn{"mediumspringgreen"}). The CSS4 specification defines 148 named colors; 10 of these share a name
with an original \quickplot color and are not duplicated in the CSS4 range since they are already
accessible at their original indices: \vn{black}, \vn{blue}, \vn{cyan}, \vn{green}, \vn{magenta},
\vn{orange}, \vn{purple}, \vn{red}, \vn{white}, and \vn{yellow}. Note that CSS4 aliases with
different names but the same RGB value (e.g., \vn{aqua} = \vn{cyan}, \vn{fuchsia} = \vn{magenta})
are included. The CSS4 colors are ordered by visual distinctiveness so that devices with limited
color index support still get the most distinguishable colors at lower indices. To see what 
the CSS4 colors look like, the Wikipedia article on  ``Web colors'' has a handy table of ``Extended colors''
sorted by color types.

Note: The RGB values of \vn{Green}, \vn{Orange}, and \vn{Purple} now match the CSS4 specification.
Users of the previous values can use these CSS4 equivalents:
\begin{example}
  Old Green  (0, 255, 0)   -> "lime"
  Old Orange (255, 127, 0) -> (no exact match; nearest is "darkorange")
  Old Purple (127, 0, 255) -> (no exact match; nearest is "blueviolet")
\end{example}

Integers from 155 onward represent continuous colors. The function \vn{qp_continuous_color} maps a
real value in [0.0, 1.0] to these integers. The colormap uses HLS interpolation with hue going from
240\Deg (blue) to 0\Deg (red), saturation = 0.8, and lightness = 0.6. See
Fig.~\ref{f:plot-continuous-color}. The inverse function \vn{qp_continuous_color_inverse} maps from
integer back to a real in [0, 1].

\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{plot-continuous-color.pdf}
\caption{Continuous colormap used by \vn{qp_continuous_color}. Typical usage: \vn{call qp_routine(..., color = qp_continuous_color(0.25_rp), ...)}}
...
\begin{example}
  type qp_line_struct
    ...
    character(24) color   ! Line color.   Default = "black"
...
\begin{example}
  type qp_symbol_struct
    ...
    character(24)  color       ! Default = "black"
...
\begin{example}
  type qp_axis_struct
    ...
    real(rp) label_offset     ! Offset in inches of the label from the numbering.
                              !  Default = 0.05
    character(24) label_color ! Default = "black"
