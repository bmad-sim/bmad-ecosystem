...
avoid this, an ``intelligent'' bookkeeping system was developed.

The automatic bookkeeping system has been retired and old programs need to be upgraded if
needed. Rule: A program that does not ``directly modify'' element attributes does not have to
modified. Modification of element attributes via \bmad routines (for example, using the
\vn{set_on_off} routine) is ``indirect''. A direct modification is something like the following
...
Historical note: Up through the end of 2025, 
to keep things back compatible with existing programs, the automatic bookkeeping system was set as
the default. The intelligent bookkeeping could be switched on by setting
the parameter:
\begin{example}
  bmad_com%auto_bookkeeper = .false.   ! Default was .true.
\end{example}
Currently, intelligent bookkeeping is assumed and the \vn{auto_bookkeeper} parameter is ignored by
\bmad.
