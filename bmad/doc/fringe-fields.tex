...
only used with \vn{sbend}, \vn{rbend}, and \vn{sad_mult} elements. For \vn{sbend} and \vn{rbend}
...
\begin{equation}
  ...
  {2 \, B_{y0}^2}
\end{equation}
With a similar equation for $F_{H2}$ for the exit end. The conversion between \vn{f1}, \vn{fb1}, and
\vn{fb2} parameters for a SAD \vn{bend} element and a \bmad bend element is
\begin{example}
  fq1 (Bmad) = f1 + fb1 (SAD)
  fq2 (Bmad) = f1 + fb2 (SAD)
\end{example}
Note! To be confusing, the SAD \vn{mult} element also has an \vn{f1} parameter but for the \vn{mult}
element the \vn{f1} parameter is associated with the soft quadrupole fringe (\sref{s:q.soft}).
...
The map is built from three pieces applied in order. For a particle at the entrance edge these are:
a field-free ``wedge'' rotation to the pole face by the edge angle $e_1$, the hard edge fringe kick,
and a dipole-field ``wedge'' rotation back to the body reference frame by $-e_1$. At the exit edge
the order and the signs of the angles are reversed. \bmad\ evaluates all three pieces directly in
\bmad\ phase space coordinates $(x, p_x, y, p_y, z, p_z)$. (Historically these maps were adapted from
PTC and evaluated in PTC coordinates; the equivalent \bmad-coordinate forms are given below.)

Throughout, $\beta_0$ is the reference relativistic beta and the following derived quantities are used:
\begin{equation}
  p_s = \sqrt{(1 + p_z)^2 - p_x^2 - p_y^2}, \qquad
  p_t = \sqrt{(1 + p_z)^2 - p_y^2},
\end{equation}
\begin{equation}
  E_n = \sqrt{(1 + p_z)^2 + \frac{1}{\beta_0^2} - 1} = \frac{E}{P_0 \, c}, \qquad
  \beta = \frac{1 + p_z}{E_n},
\end{equation}
where $p_s$ is the longitudinal momentum, $p_t$ is the momentum projected into the horizontal bend
plane, $E_n$ is the normalized total energy, and $\beta$ is the particle velocity. Since the edge
fields are static and magnetic, the energy $p_z$ (and hence $E_n$ and $\beta$) is invariant through
the edge.

\subsection{Wedge Map}

The wedge transports a particle through a wedge of angle $a$ of a dipole of curvature $b = g_\tot$.
With $p_x'$ the updated horizontal momentum and $p_s' = \sqrt{p_t^2 - p_x'^2}$,
\begin{align}
  p_x' &= p_x \cos a + (p_s - b \, x) \sin a \CRNO
  x'   &= x \cos a
     + \frac{x \, p_x \, \sin 2a + \sin^2\!a \, (2 \, x \, p_s - b \, x^2)}{p_s' + p_s \cos a - p_x \sin a} \CRNO
  L    &= \frac{1}{b} \left[ a + \arcsin \frac{p_x}{p_t} - \arcsin \frac{p_x'}{p_t} \right] \\
  y'   &= y + p_y \, L \CRNO
  z'   &= z - (1 + p_z) \, L \nonumber
\end{align}
with $p_y$ and $p_z$ unchanged. Here $L$ is the arc-length parameter of the map. In the field-free
limit $b \to 0$ (the stage~1 drift to the pole face) the wedge reduces to a rotation of the reference
frame about the vertical axis,
\begin{align}
  x'   &= \frac{x \, p_s}{p_s \cos a - p_x \sin a}, \qquad\qquad
  p_x'  = p_x \cos a + p_s \sin a, \CRNO
  L    &= \frac{x \sin a}{p_s \cos a - p_x \sin a}, \qquad
  y'    = y + p_y \, L, \qquad
  z'    = z - (1 + p_z) \, L.
\end{align}

\subsection{Hard Edge Fringe Kick}

The hard edge fringe kick uses $b = g_\tot$ at the entrance face and $b = -g_\tot$ at the exit face.
Define the slopes $x_p = p_x / p_s$ and $y_p = p_y / p_s$, the field integral product
$K = f_{int} \, h_{gap}$, and the fringe angle
\begin{equation}
  \phi_0 = \arctan \frac{x_p}{1 + y_p^2}
    - 2 \, b \, K \, \bigl[ 1 + x_p^2 \, (2 + y_p^2) \bigr] \, p_s.
\end{equation}
Let $d$ be the matrix and $c_1, c_2$ the coefficients
\begin{equation}
  d = \begin{pmatrix}
    \dfrac{1 + x_p^2}{p_s} & \dfrac{x_p \, y_p}{p_s} & -\dfrac{E_n \, x_p}{p_s^2} \\[10pt]
    \dfrac{x_p \, y_p}{p_s} & \dfrac{1 + y_p^2}{p_s} & -\dfrac{E_n \, y_p}{p_s^2} \\[10pt]
    -x_p & -y_p & \dfrac{E_n}{p_s}
  \end{pmatrix}, \qquad
  c_2 = \frac{b}{\cos^2 \phi_0}, \qquad
  c_1 = \frac{c_2}{1 + \bigl[x_p / (1 + y_p^2)\bigr]^2},
\end{equation}
and let $\Bf{f} = (f_1, f_2, f_3)$ be
\begin{align}
  f_1 &= \frac{c_1}{1 + y_p^2} - 4 \, b \, K \, c_2 \, x_p \, (2 + y_p^2) \, p_s \CRNO
  f_2 &= -\frac{2 \, c_1 \, x_p \, y_p}{(1 + y_p^2)^2} - 4 \, b \, K \, c_2 \, x_p^2 \, y_p \, p_s \\
  f_3 &= -2 \, b \, K \, c_2 \, \bigl[ 1 + x_p^2 \, (2 + y_p^2) \bigr] \nonumber
\end{align}
With the column contractions $q_k = \sum_{i=1}^{3} f_i \, d_{ik}$ for $k = 1, 2, 3$, the kick is
\begin{align}
  y'   &= \frac{2 \, y}{1 + \sqrt{1 - 2 \, q_2 \, y}} \CRNO
  p_y' &= p_y - b \, \tan(\phi_0) \, y' \CRNO
  x'   &= x + \frac{1}{2} \, q_1 \, y'^2 \\
  z'   &= z + \frac{1}{2} \, \beta \, q_3 \, y'^2 \nonumber
\end{align}
with $p_x$ and $p_z$ unchanged. The updated $y'$ is used on the right-hand sides of the $p_y'$, $x'$,
and $z'$ equations. The particle velocity $\beta$ appears in the $z$ kick because the \bmad\
longitudinal coordinate is $z = -\beta \, c \, (t - t_0)$ (\Eq{zbctt}): the fringe changes the
particle's time of flight and the induced change in $z$ carries the factor $-\beta$.
