...
Wakefield effects are divided into short--range (\sref{s:sr.wake.eq} within a bunch) and long--range
(\sref{s:lr.wake.eq} between bunches). Short--range wakes are described in \Sref{s:sr.wake.eq} and
long--range wakes are described in \Sref{s:lr.wake.eq}. The syntax for describing wakes in a lattice
file is given in \Sref{s:wakes}.

Note: The short range wakes uses $z$ (distance) as the dependent variable while the long range wakes
use time as the dependent variable.
