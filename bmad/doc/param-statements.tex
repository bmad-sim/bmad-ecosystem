...
\begin{example}
  ...
  parameter[n_part]                   = <Real>      ! Number of particles in a strong beam-beam bunch.
  parameter[no_end_marker]            = <Logical>   ! Default: False.
  parameter[parser_make_xfer_mats]    = <Logical>   ! Make element transfer maps during lattice parsing?
...
%
  \item[{parameter[parser_make_xfer_mats]}] \Newline
Default is True. If True, element-by-element transfer matrices will be constructed and put in the
digested lattice file (\sref{s:digested}) to save time next time the lattice is parsed. These
matrices are constructed around the zero orbit.  Occasionally, it is not possible to construct such
matrices and this will generate error messages even though the analysis program being used will
otherwise run normally. To suppress such messages, set this parameter to False.
%
  \index{particle}
...
NOTE: THE BEAM STATEMENT IS DEPRECATED. PLEASE DO NOT USE!
