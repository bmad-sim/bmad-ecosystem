...
\begin{example}
  type bmad_common_struct
    ...
    auto_bookkeeper = T                 ! No longer used.
    ...
    gpu_tracking_on = F                 ! GPU-accelerated batch tracking?
...
No longer used. Previously, toggled automatic or intelligent bookkeeping. Now intelligent bookkeeping is assumed. See
...
%
\index{gpu_tracking_on}
\item[\vn{gpu_tracking_on}] \Newline
Toggle for GPU-accelerated batch particle tracking (\sref{s:gpu.tracking}). Default is \vn{False}.
When set to \vn{True}, bunch tracking through eligible elements (\vn{drift}, \vn{quadrupole},
\vn{sbend}, and \vn{lcavity}) is offloaded to an NVIDIA GPU via CUDA kernels. Elements that are not
eligible for GPU tracking are silently tracked on the CPU. The \vn{gpu_tracking_init} routine can be
used to initialize this flag based on hardware availability and the \vn{ACC_ENABLE_GPU_TRACKING}
environment variable. GPU tracking is not compatible with radiation damping, radiation fluctuations,
spin tracking, or backward tracking --- if any of these are active, a bunch will be tracked on the
CPU even if \vn{gpu_tracking_on} is \vn{True}. The \vn{ele_gpu_eligible} function can be used to
query whether a given element supports GPU tracking.
%
\item[\vn{high_energy_space_charge_on}] \Newline
Default is \vn{False}. Toggle to turn on or off the ultra-relativistic space charge effect in
particle tracking (\sref{s:he.space.charge}). Unlike other space charge calcuflations, the high
energy space charge can be done with single particle tracking (that is, tracking a beam of particle
is not necessary). Computationally, this is separate from the lower energy space charge and CSR
calculation (\sref{s:csr}).
%
...
See \sref{s:twiss.off} for details.
%
