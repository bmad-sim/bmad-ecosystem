...
etc.  The call to \Hyperref{r:bmad.parser}{bmad_parser} (line 14) causes the lattice file
\vn{lat.bmad} to be parsed and the lattice information is stored the \vn{lat} variable. Note: To get
a listing of the \vn{lat_struct} components or to find out more about \vn{bmad_parser} use the
...
16 to find the element in the lattice with the name \vn{CLEO_SOL}. Line 17 defines a pointer
variable named \vn{cleo} which is used here as shortcut notation rather than having to write
\vn{eles(1)%ele} when refering to this element. The call to \vn{pointer_to_attribute} in line 18
...
Line 19 changes the \vn{ks} solenoid strength of \vn{cleo}. Since an element attribute has been
changed, the call to \Hyperref{r:set.flags.for.changed.attribute}{set_flags_for_changed_attribute}
in line 20 is needed for \bmad to inform \bmad that this attribute has changed and the call to
\Hyperref{r:lattice.bookkeeper}{lattice_bookkeeper} does the necessary lattice bookkeeping
(\sref{s:lat.book}).

The call to \Hyperref{r:lat.make.mat6}{lat_make_mat6} in line 22 recalculates the linear transfer
matrix for the \vn{CLEO_SOL} element.

In line 27, the program checks if the lattice is circular (\sref{s:param}) and, if so, uses the
...
The program is now ready output some information. Lines 21 through 25 of the program print
...
elements (line 34) makes for a cleaner syntax and reduces typing. The table that is produced is
...
The \Hyperref{r:type.ele}{type_ele} routine on line 44 of the program is used to type out the
\vn{CLEO_SOL}'s attributes and other information as shown on lines 14 through 38 of the output (more
on this later).
...
derived from the \vn{cleo_sol} is put in the \vn{cleo_sol} element as shown in lines 30 through 38
