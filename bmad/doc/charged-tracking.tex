...
\vn{l_rectangle}. This is done to keep the shape of the bend is invarient. Invariance is not
...
end.  $\bfr_2$ is the initial exit point and $\bfr'_2$ is the exit point when one of \vn{rho},
\vn{g}, \vn{b_field}, or \vn{angle} is varied. By construction, the entrance point $\bfr_1$ and the
...
and after (solid line) being tangent to $\bfs_1$ where $\bfs_1$ being the perpendicular to $\bfx_1$.
  ...
  \begin{subfigure}[b]{0.4\textwidth}
    ...
    \caption{
With \vn{fiducial_pt} set to \vn{center}, $\bfr_c$ is the fiducial point at the center. $\bfr_1$ and
$\bfr_2$ are the entrance and exit points before  and $\bfr'_1$ and $\bfr'_2$ are the
the entrance and exit points after variation of one of \vn{rho}, \vn{g}, \vn{b_field} or
\vn{angle}. By construction, the reference curve always goes through $\bfr_c$ and the tangent
of the reference curve at $\bfr_c$ is invariant.}
    \label{f:bend.fid2}
  \end{subfigure}
  \hfill
  \caption{
Geometry with \vn{fiducial_pt} set to (a) \vn{entrance_end} and (b) \vn{center}. $\rho$ 
is the bending radius and $\alpha$ is the bending angle before variation.
  }
  \label{f:bend.fid}
\end{figure}
...
around the point $\bfr_1$ are held fixed and as as a result \vn{e1} does not vary. $\bfr_2$ is the
exit point before variation and $\bfr_3$ is the exit point after.  The position of $\bfr_3$ is
calculated by first calculating the position of $\bfr_1$ in a coordinate system centered at $\bfr_2$
and with axes parallel to the $(\bfs_1, \bfx_1)$ axes of the coordinate system at $\bfr_1$
\begin{equation}
  \bar\bfr_1 = \left( -l_{rectangle}, 1 - \rho \, \cos(\alpha) \right)
\end{equation}
Where the bar denotes that the coordinates are in the $(\bfs_1, \bfx_1)$ system.
The coodinates of $\bfr_1$ are not transformed to the $(\bfe_s, \bfe_x)$ coordinate system
...
\end{equation}
where $\bfR(\theta)$ is the rotation matrix by with a rotation angle of $\theta$.
The angle of 
