...
{\em Routine Type} & {\em Section} \\ \midrule
  ...
  Lattice: Manipulation                       & \ref{r:manipulation}          \\
...
\item[track1_bunch (bunch, ele, err, centroid, direction, bunch_track)] \Newline 
Routine to track a bunch of particles through an element.

\index[routine]{track1_bunch_hom}
\label{r:track1.bunch.hom}
\item[track1_bunch_hom (bunch, ele, direction, bunch_track)] \Newline 
Routine to track a bunch of particles through an element.
...
\label{r:bbi.kick}
\item[bbi_kick (x, y, sigma, nk, dnk)] \Newline 
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} is_time_coords, ele) }] \Newline 
...
\item[\protect\parbox{6in}{
    calc_bunch_params_slice (bunch, bunch_params, plane, slice_center, slice_spread, \\
    \hspace*{1in} err, print_err, is_time_coords, ele) }] \Newline 
Finds all bunch parameters for a slice through the beam distribution.

\index[routine]{calc_bunch_sigma_matrix_etc}
\label{r:calc.bunch.sigma.matrix.etc}
\item[calc_bunch_sigma_matrix_etc (particle, charge, bunch_params, is_time_coords, ele)] \Newline 
...
\item[\protect\parbox{6in}{
    calc_emittances_and_twiss_from_sigma_matrix(sigma_mat, bunch_params, error, \\
    \hspace*{1in} print_err, n_mat) }] \Newline 
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} beam_init_set, print_p0c_shift_warning, conserve_momentum) }] \Newline 
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} beam_init_used, print_p0c_shift_warning, conserve_momentum) }] \Newline 
...
\label{r:track.bunch}
\item[track_bunch (lat, bunch, ele1, ele2, err, centroid, direction, bunch_track)] \Newline 
...
\item[track_bunch_time (bunch, branch, t_end, s_end, dt_step, extra_field)] \Newline 
...
\item[write_beam_file (file_name, beam, new_file, file_format, lat, alive_only)] \Newline 
...
\index[routine]{ele_gpu_eligible}
\label{r:ele.gpu.eligible}
\item[ele_gpu_eligible (ele) result (eligible)] \Newline
Function that returns \vn{True} if an element is eligible for GPU-accelerated tracking
(\sref{s:gpu.tracking}). An element is eligible if its \vn{tracking_method} is \vn{bmad_standard},
it is turned on, and it is one of: \vn{drift}, \vn{quadrupole}, \vn{sbend}, or \vn{lcavity}.

\index[routine]{gpu_tracking_init}
\label{r:gpu.tracking.init}
\item[gpu_tracking_init ()] \Newline
Routine to initialize GPU tracking. Checks for the \vn{ACC_ENABLE_GPU_TRACKING} environment variable
and probes for CUDA hardware. If both conditions are met, sets \vn{bmad_com%gpu_tracking_on} to
\vn{True}. This routine only needs to be called once.

\index[routine]{gpu_tracking_reset}
\label{r:gpu.tracking.reset}
\item[gpu_tracking_reset ()] \Newline
Routine to reset GPU tracking state so that \vn{gpu_tracking_init} can re-probe for hardware on
the next call. Also releases cached GPU device memory and sets \vn{bmad_com%gpu_tracking_on} to
\vn{False}.
...
\item[\protect\parbox{6in}{
    ele_to_fibre_hook (ele, ptc_fibre, param, use_offsets, err_flag) }] \Newline 
...
\item[track1_bunch_hook (bunch, ele, err, centroid, direction, finished, bunch_track)] \Newline 
...
\label{r:track1.custom}
    \item[track1_custom (orbit, ele, param, err_flag, finished, track)] \Newline
...
\item[\protect\parbox{6in}{
    pmd_write_int_to_pseudo_dataset(root_id, dataset_name, descrip_name, unit, value, v_shape, error) }] \Newline 
...
\item[\protect\parbox{6in}{
    pmd_write_real_to_pseudo_dataset (root_id, dataset_name, descrip_name, unit, value, v_shape, error) }] \Newline 
...
\item[pmd_write_units_to_dataset (root_id, dataset_name, descrip_name, unit, error)] \Newline 
...
\item[hdf5_write_beam (file_name, bunches, append, error, lat, alive_only)] \Newline 
...
\label{r:read.a.line}
\item[\protect\parbox{6in}{
    read_a_line (prompt, line_out, trim_prompt, prompt_color, prompt_bold, \\
    \hspace*{1in} history_file) }] \Newline 
...
\item[ran_seed_put (seed, mpi_offset)] \Newline 
...
\label{r:mat.inverse}
\item[mat_inverse (mat, mat_inv, vec0, vec0_inv, ok, print_err)] \Newline
...
\label{r:is.integer}
\item[is_integer (string, int, delims, ix_word)] \Newline 
...
\item[\protect\parbox{6in}{
    location_decode (string, array, ix_min, num, names, exact_case, \\
    \hspace*{1in} can_abbreviate, print_err) }] \Newline 
...
\item[reals_to_string (real_arr, width, n_blank, n_signif, n_decimal) result (str)] \Newline 
...
\item[coord_state_name (coord_state, one_word) result (state_str)] \Newline 
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} save_null_drift, create_jumbo_slave, ix_insert, \\
    \hspace*{1in} mangle_slave_names, wrap) }] \Newline
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} include_upstream_end, include_downstream_end, err_flag, \\
    \hspace*{1in} old_slice, orb_in) }] \Newline 
...
\label{r:create.planar.wiggler.model}
\item[create_planar_wiggler_model (wiggler_in, lat, err_flag, print_err)] \Newline 
...
\item[\protect\parbox{6in}{
  pointer_to_attribute (ele, attrib_name, do_allocation, a_ptr, err_flag, \\
  \hspace*{1in} err_print_flag, ix_attrib, do_unlink) }] \Newline
...
\item[\protect\parbox{6in}{
    pointers_to_attribute (lat, ele_name, attrib_name, do_allocation, ptr_array, 
    \hspace*{1in} err_flag, err_print_flag, eles, ix_attrib, do_unlink) }] \Newline 
...
\item[set_ele_attribute (ele, set_string, err_flag, err_print_flag, set_lords, err_id)] \Newline 
...
\item[\protect\parbox{6in}{
    coords_floor_to_curvilinear (floor_coords, ele0, ele1, status, \\
    \hspace*{1in} w_mat) result (local_coords) }] \Newline 
...
\item[\protect\parbox{6in}{
    coords_floor_to_relative (floor0, global_position, calculate_angles, \\
    \hspace*{1in} is_delta_position) result (local_position) }] \Newline 
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} calculate_angles, relative_to) result (global_position) }] \Newline 
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} relative_to) result(local_position) }] \Newline 
...
\item[orbit_to_local_curvilinear (orbit, ele, z_direction, relative_to) result (local_position)] \Newline 
...
\index[routine]{at_this_ele_end}
\label{r:at.this.ele.end}
\item[at_this_ele_end (now_at, where_at) result (is_at_this_end)] \Newline 
...
\item[\protect\parbox{6in}{
    pointer_to_lord (slave, ix_lord, control, ix_slave_back, lord_type, ix_control, \\
    \hspace*{1in} ix_ic) result (lord_ptr) }] \Newline 
...
\item[\protect\parbox{6in}{
    pointer_to_slave (lord, ix_slave, control, lord_type, ix_lord_back, \\
    \hspace*{1in} ix_control, ix_ic) result (slave_ptr) }] \Newline 
...
\index[routine]{apply_all_rampers}
\label{r:apply.all.rampers}
\item[apply_all_rampers (lat, err_flag)] \Newline 
...
\label{r:set.on.off}
\item[\protect\parbox{6in}{
    set_on_off (key, lat, switch, orb, use_ref_orb, ix_branch, saved_values, 
    \hspace*{1in} attribute, set_val) }] \Newline
Routine to turn on or off a set of elements (quadrupoles,
RF cavities, etc.) in a lat.

\index[routine]{slice_lattice}
\label{r:slice.lattice}
\item[slice_lattice (lat, ele_list, error, do_bookkeeping)] \Newline 
...
\index[routine]{start_branch_at}
\label{r:start.branch.at}
\item[start_branch_at (lat, ele_start, move_end_marker, error)] \Newline 
...
\index[routine]{taper_mag_strengths}
\label{r:taper.mag.strengths}
\item[taper_mag_strengths (lat, ref_lat, except, err_flag)] \Newline 
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} ix_branch, err) }] \Newline 
...
\item[\protect\parbox{6in}{
    match_ele_to_mat6 (ele, start_orb, mat6, vec0, err_flag, \\
...
\item[sol_quad_mat6_calc (ks_in, k1_in, tilt, length, ele, orbit, mat6, make_matrix)] \Newline
...
\item[multipole_kicks (knl, tilt, ele, orbit, pole_type, ref_orb_offset)] \Newline 
...
\item[\protect\parbox{6in}{
    multipole_kick (knl, tilt, n, ref_species, ele_orientation, coord, pole_type, \\
    \hspace*{1in} ref_orb_offset) }] \Newline
Routine to put in the kick due to a multipole. 
...
\item[\protect\parbox{6in}{
    super_mrqmin (y, weight, a, chisq, funcs, storage, alamda, status, \\
    \hspace*{1in} maska, print_err) }] \Newline 
...
\item[\protect\parbox{6in}{
    ele_to_fibre (ele, ptc_fibre, use_offsets, err_flag, integ_order, steps, \\
...
\item[\protect\parbox{6in}{
    ptc_transfer_map_with_spin (branch, t_map, s_map, orb0, err_flag, ix1, ix2, \\
    \hspace*{1in} one_turn, unit_start) }] \Newline 
...
\item[\protect\parbox{6in}{qp_draw_curve_legend (origin, legend, line, symbol, text) }] \Newline
...
\item[make_mat6_symp_lie_ptc (ele, start_orb, end_orb)] \Newline
...
\item[make_mat6_taylor (ele, start_orb, end_orb, err_flag)] \Newline
Routine to make the 6x6 transfer matrix for an element
from a Taylor map.

\index[routine]{make_mat6_tracking}
\label{r:make.mat6.tracking}
\item[make_mat6_tracking (ele, param, start_orb, end_orb, err_flag, spin_only)] \Newline
...
\item[\protect\parbox{6in}{
    type_complex_taylors (complex_taylor, max_order, lines, n_lines, file_id, \\
    \hspace*{1in}out_type, clean) }] \Newline 
...
\item[concat_ele_taylor (taylor1, ele, taylor3, err_flag)] \Newline 
...
\item[\protect\parbox{6in}{
    ele_to_taylor (ele, orb0, taylor_map_includes_offsets, include_damping, \\
    \hspace*{1in} orbital_taylor, spin_taylor) }] \Newline
...
\item[taylor_propagate1 (bmad_taylor, ele, param, err_flag, ref_in)] \Newline
...
\item[\protect\parbox{6in}{
    offset_particle (ele, set, orbit, set_tilt, set_hvkicks, drift_to_edge, s_pos, s_out, \\
    \hspace*{1in} set_spin, mat6, make_matrix, spin_qrot, time) }] \Newline
...
\item[particle_is_moving_forward (orbit, dir) result (is_moving_forward)] \Newline 
...
\item[track1_bunch_csr (bunch, ele, centroid, err, s_start, s_end, bunch_track)] \Newline 
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} ix_branch, track_state, ix_ele_end) }] \Newline
...
\item[\protect\parbox{6in}{
    twiss_and_track_from_s_to_s (branch, orbit_start, s_end, orbit_end, ele_start, \\
    \hspace*{1in} ele_end, err, compute_floor_coords, compute_twiss) }] \Newline
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} track_downstream_end, orbit_start, orbit_end, ele_start, ele_end, \\
    \hspace*{1in} err, compute_floor_coords, compute_twiss, reuse_ele_end) }] \Newline 
...
\item[\protect\parbox{6in}{
    track_a_drift (orb, length, mat6, make_matrix, ele_orientation, \\
    \hspace*{1in} include_ref_motion, time) }] \Newline
...
\label{r:track1.mad}
\item[track1_mad (orbit, ele, param)] \Newline 
...
\item[\protect\parbox{6in}{
  symp_lie_bmad (ele, param, orbit, track, mat6, make_matrix, offset_ele) }] \Newline
Symplectic integration through an element to 0th or 1st order.

\index[routine]{track1_bmad}
\label{r:track1.bmad}
\item[track1_bmad (orbit, ele, param, err_flag, track, mat6, make_matrix)] \Newline
...
\item[track1_linear (orbit, ele, param)] \Newline
...
\item[track1_time_runge_kutta(orbit, ele, param, err_flag, track, t_end, dt_step)] \Newline 
...
\label{r:chrom.calc}
\item[\protect\parbox{6in}{
    chrom_calc (lat, delta_e, chrom_a, chrom_b, err_flag, pz, low_E_lat, high_E_lat, \\
    \hspace*{1in} low_E_orb, high_E_orb, ix_branch, orb0) }] \Newline
...
\label{r:set.z.tune}
\item[set_z_tune (branch, z_tune, ok, print_err)] \Newline
...
\item[twiss_propagate_all (lat, ix_branch, err_flag, ie_start, ie_end)] \Newline
...
\item[normal_mode3_calc (t6, tune, B, HV, above_transition, abz_tunes)] \Newline 
...
\label{r:init.wake}
\item[init_wake (wake, n_sr_long, n_sr_trans, n_sr_z, n_lr_mode, always_allocate)] \Newline 
...
\item[track1_sr_wake (bunch, ele, test_particle)] \Newline 
