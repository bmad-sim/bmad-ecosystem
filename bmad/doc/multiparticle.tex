...
(\sref{s:bmad.common}, \sref{s:beginning}). Currently, the \vn{long_term_tracking} program
implements high energy space charge while \vn{tao} does not (\vn{tao} is not constructed for long
term tracking where these effects would be noticeable).
