...
%-----------------------------------------------------------------
\section{Generalized Gradient Field Modeling}
...
(\sref{s:fieldmap}). One model involves what are called \vn{generalized gradients} (GG). This model
is restricted to modeling DC magnetic or electric fields. In a lattice file, the generalized
gradient field model is specified using the \vn{gen_gradients} attribute as explained in
Sec.~\sref{s:gen.grad.map}.

\bmad uses the curved-curvilinear-coordinate GG formalism of Van der Schueren et
al.\cite{b:gen.grad.curved}, which handles a reference frame of constant curvature $g_{\text{ref}} =
1/\rho$ (and reduces to the straight-coordinate case when $g_{\text{ref}} = 0$). The field is
characterized by three families of midplane derivative functions of the longitudinal coordinate $s$:
the normal (non-skew) functions $b_n(s)$, the skew functions $a_n(s)$, and the solenoidal function
$b_s(s)$, defined by the on-midplane field
\begin{align}
  B_x(x, 0, s) &= \sum_{n \ge 1} a_n(s) \, \frac{x^{n-1}}{(n-1)!} \CRNO
  B_y(x, 0, s) &= \sum_{n \ge 1} b_n(s) \, \frac{x^{n-1}}{(n-1)!} \\
  B_s(0, 0, s) &= b_s(s) \nonumber
\end{align}
where $n$ is the multipole harmonic index ($n = 1$ dipole, $n = 2$ quadrupole, etc.). The full field
$\bfB = (B_x, B_y, B_s)$ and the magnetic vector potential $\bfA = (A_x, A_y, A_s)$ off the midplane
are reconstructed from these functions and their $s$-derivatives $a_n^{[m]}, b_n^{[m]}, b_s^{[m]}$
using a precomputed coefficient table: each output component is a polynomial in $(x, y)$ whose
coefficients are sums of the form $\sum \text{coef} \cdot g_{\text{ref}}^{\,k} \, x^p \, y^q \, G^{[m]}$
(with $G \in \{a_n, b_n, b_s\}$), so the curvature enters through explicit powers of $g_{\text{ref}}$.
The vector potential is constructed so that $\bfB = \nabla \times \bfA$ holds exactly (in the curved
metric of the reference frame). This construction, the coefficient table, and the field-fitting
tools are provided by the \vn{GeneralizedGradients.jl} package\cite{b:gen.grad.jl}; \bmad's
coefficient table (\vn{bmad/modules/gg_coef_table_mod.f90}) is generated from it.

The functions are characterized by specifying each function and its $s$-derivatives at equally spaced
planes $z_i$, up to some maximum derivative order chosen by the user. Interpolation between planes is
done with a two-point Hermite (``non-smoothing spline'') polynomial that matches the value and all
stored derivatives at $z_i$ and $z_{i+1}$. As with the older formalism, the ``locality'' property ---
the field at a point depends only on the two straddling planes --- makes the coefficients easy to fit
and the evaluation fast, at the cost that, because the derivatives are truncated, the reconstructed
field satisfies Maxwell's equations only up to the truncation order.

The obsolete straight-coordinate cylindrical-harmonic formalism (Venturini and Dragt\cite{b:gen.grad}),
used by the removed \vn{gen_grad_map} lattice format, is documented for reference in
Appendix~\sref{s:gen.grad.old}.
