\chapter{Coordinates and Physical Layout}
\label{s:coords.3}
...
Do not confuse this reference orbit (which defines the local coordinate system) with the ``map
reference orbit'' about which the transfer maps are calculated (\sref{s:twiss}). The former is fixed
by the lattice while the latter can be any arbitrary orbit. For the map reference orbit, \bmad uses
the closed orbit for lattice branches with a closed geometry and, for lattice branches with an open
geometry, the map reference orbit is the orbit computed starting at the user defined initial
position at the beginning of the branch (\sref{s:beginning}).
