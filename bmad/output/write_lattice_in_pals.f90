...
integer ix_lord, ix_super, ie1, ib1, indent
...
do ib = 0, ubound(lat%branch, 1)
  ...
  ele_loop: do ie = 0, branch%n_ele_track   !!! Note: Not n_ele_max since superimpose/multipass not handled
    ...
    write (iu, '(3a)') blank(1:indent), trim(ele_name), ':' 
    write (iu, '(3a)') blank(1:indent+2), '- kind: ', trim(pals_ele_type(ele%key))
    ...
    case (match$, taylor$)
      ...
      case (match$)
        ...
        call write_this_taylor(iu, indent, ele, taylor)
        cycle

      case (taylor$)
        call write_this_taylor(iu, indent, ele, ele%taylor)
        cycle
      end select
...
subroutine write_this_taylor(iu, indent0, ele, taylor)

type (ele_struct) ele
type (taylor_struct), target :: taylor(6)
type (taylor_term_struct) term

integer indent0, id, iu, i, j, k, e_max(6)
character(200) line
