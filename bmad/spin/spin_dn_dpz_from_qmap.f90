...
!   dn_dpz_partial2(3,3)  -- real(rp), optional: dn_dpz_partial(i,:) is dn_dpz with only two osccilation 
...
dn_dpz_partial2(1,:) = real(matmul(aa4, n4), rp)

aa4= [aa(1:2,1), aa(5:6,1)];  n4(1:2,:) = n_eigen(1:2,:);  n4(3:4,:) = n_eigen(5:6,:)
dn_dpz_partial2(2,:) = real(matmul(aa4, n4), rp)

aa4 = aa(1:4,1);  n4 = n_eigen(1:4,:)
dn_dpz_partial2(3,:) = real(matmul(aa4, n4), rp)
