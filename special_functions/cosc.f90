...
!
! Routine to calculate (cos(x) - 1) / x^2 and derivatives.
!
! Input:
!   x  -- real(rp):
!   nd -- integer, optional: Derivative order. nd = 0 (default) -> compute (cos(x) - 1) / x^2
!           NOTE: Currently only nd = 0 and nd = 1 are implemented.
!
! Output:
!   y  -- real(rp): nd^th derivative of (cos(x) - 1) / x^2
!-
