...
&params
  ...
  ltt%symplectic_map_tracking         = T
  ltt%set_beambeam_crossing_time      = F
  ltt%space_charge_emit_calc_every_n_turns = -1
  ltt%space_charge_core_cutoff        = 0.8
  ltt%dead_cutoff                     = 0.99
  ltt%b_emittance                     = 1e-9 ! Used with space charge calc.
  ltt%debug                           = F

  ! Bmad_com parameters affecting the tracking.
  ! See the Bmad manual for details.
  bmad_com%spin_tracking_on                = T
  bmad_com%radiation_damping_on            = F
  bmad_com%radiation_fluctuations_on       = F
  bmad_com%high_energy_space_charge_on     = F
  bmad_com%high_energy_space_charge_linear = F
...
%----------------------------------------------------
\subsection{Bmad_com Global Parameter Structure}
\label{s:bmad.com}

\vn{Bmad_com} is a global variable which is a structure holding a number of relavent parameters used
for simulations.  See the \vn{Bmad} manual for details on this structure. \vn{Bmad_com} parameters
can be set either in the lattice file (with name given by the setting of \vn{ltt%lat_file} or in the
master input file for this program. \vn{Bmad_com} Parameters set in the master input file will
override settings in the lattice file.

In the lattice file, \vn{bmad_com} parameters are set using square brackets. For example:
\begin{code}
  bmad_com[high_energy_space_charge_on] = T
\end{code}
In the master input file a percent sign is used. For example:
\begin{code}
  bmad_com%high_energy_space_charge_on = T
\end{code}
...
If False, the maps used for tracking will be a set of truncated Taylor series
polynomials. If True (the default), the tracking maps will be derived from the Taylor map by partially inverting
...
\vn{bmad_com[high_energy_space_charge_on]} to \vn{True} (default is \vn{False}) in the lattice
file. See the Bmad manual documentation for more details. Since the space charge kick is applied
element-by-element, the high energy space charge kick can only applied when \vn{ltt%tracking_method}
is set to \vn{"BMAD"}. The space charge kick can be used in both BEAM and SINGLE mode.
...
To test with a linear space charge kick, set:
\begin{code}
bmad_com%high_energy_space_charge_linear = T
\end{code}

When using BEAM mode, the emittances used in the calculation can be periodically recomputed every $N$ turns
from the tracked beam distribution by setting the input parameters
\begin{code}
ltt%space_charge_emit_calc_every_n_turns    ! Set positive to enable periodic calc. Default is -1
ltt%space_charge_core_cutoff                ! Default is 0.8
\end{code}
Setting \vn{ltt%space_charge_emit_calc_every_n_turns} positive to the desired $N$ will enable the
calculation.  To prevent particles in the tails of the distribution from unduely affecting the
calculation, a core emittance (\sref{s:core}) calculation is used with a cutoff determined by
\vn{ltt%space_charge_core_cutoff}.
