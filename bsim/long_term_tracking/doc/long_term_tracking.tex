...
\author{}
\date{David Sagan \\ February 17, 2024}
...
tracking particles through an extraction line. To turn on extraction tracking, the \vn{ltt%extraction_mode}
must be set to "\vn{ON}". The default value of \vn{ltt%extraction_mode} is blank ("") which disables
extraction tracking.
...
versa for the other particles. Tracking from the septum is the same as with "\vn{BEAM}" simulation mode
tracking from the \vn{ltt%ele_start} point.
