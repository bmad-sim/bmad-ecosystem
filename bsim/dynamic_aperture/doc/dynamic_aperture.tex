...
\author{}
\date{David Sagan \\ March 10, 2026}
...
a common origin point ($\cal O$) which is the closed orbit. The line segments between points is
simply for visualization purposes. The calculation of an aperture curve point along a given ray
involves iteratively tracking particles with different starting $(x, y)$ position values to find the
boundary between stable (green dots) and unstable (red dots) motion.
When the dynamic aperture is plotted (Fig.~\ref{f:da-plot}), the curves are plotted with respect to
($\cal O$). Notice that if the RF is off, $\cal O$ is dependent upon $p_z$. That is, with the RF
off, and with finite dispersion at the starting point, each curve in a DA plot will have a different
$\cal O$.
  }
  \label{f:da-ray}
\end{figure}
...
tracking when the \vn{ltt%tracking_method} parameter is set to "\vn{MAP}" (\sref{s:track.methods}). PTC
...
\begin{figure}[tb]
  ...
  \caption{
Example dynamic aperture plot using \vn{gnuplot}. Since the RF is off here, the $(0,0)$ origin for
each curve is the closed orbit point for the $p_z$ associated with that curve (Fig~\ref{f:da-ray}).
  }
  \label{f:da-plot}
\end{figure}
