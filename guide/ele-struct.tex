...
The \vnes{%a}, \vnes{%b}, and \vnes{%z} components are
...
\index{control_type}
\label{s:control_type}

In a \vn{lat_struct} (\sref{c:lat_struct}), elements in the lattice
...
\index{free_element}\index{super_lord}
\index{group_lord}\index{i_beam_lord}
...
\begin{description}
\item[free_element] 
A \vn{free_element} is just that, An element that is neither
controlls or is controlled. The exception is that attributes of this element
may be controlled by a \vn{group_lord}.

\item[super_lord and super_slave]
\vn{super_lord} and \vn{super_slave} elements
come from superimposing elements on top of other elements (\sref{s:super}).

\item[overlay_lord, overlay_slave]
An \vn{overlay_lord} is an \vn{overlay} element and the elements it
controlls are \vn{overlay_slave}s (\sref{s:overlay}).
...
\item[i_beam_lord]
\index{on_an_i_beam}
An \vn{i_beam_lord} is an \vn{i_beam} element. In this case, the slaves
are marked by setting the \vn{%on_an_i_beam} component in the
\vn{ele_struct}.

\item{multipass_lord, multipass_slave}
...
\vn{A\B1} and \vn{A\B2} would be \vn{multipass_slave}s.
\end{description}
...
\end{example}
The \bmad routines that compute \vn{%mat6} (for example \vn{lat_make_mat6})
...
\vn{%wake%sr_table} stores the tabular form of the short--range wake
(see~\sref{s:wiggler}).\vn{%wake%sr_mode_long}, and \vn{%wake%sr_mode_trans}
store the longitudinal and transverse pseudo--modes. The structure
definition for \vn{%wake%sr_table} is
\begin{example}
...
\end{example}
All \vn{%wake%sr_table(0:)%z} will be negative. Notice that the array lower
bounds is zero since \vn{%wake%sr_table(0)%z} = 0. Wakefield kicks are
applied using \Eqs{delvp} and \eq{pelqxw}.

The \vn{%wake%sr_mode_long} and \vn{%wake%sr_mode_trans} pseudo--mode
