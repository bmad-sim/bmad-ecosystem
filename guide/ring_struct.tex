...
\vne{Overlay} and \vne{Group} elements, and those elements that are
...
an \vne{Overlay} or \vne{Group} then \vn{%control_(:)%ix_attrib} and 
...
\begin{example}
  ...
  enddo
\end{example}
For historical reasons, since a \vne{Group} element only makes changes
...
\begin{example}
    lattice1 = lattice2
\end{example}
the pointers will be handled properly. The result will be that lattice1
will hold the same information as \vn{lattice2} but the pointers in
\vn{lattice1} will point to different locations in physical memory so
that changes to one lattice will not affect the other.
