...
\index{Ring_struct!%n_ele_maxx}
\index{Ele_struct!in ring_struct} 
The definition of the \vn{ring_struct} is shown in
Figure~\ref{f:ring_struct}. The array \vn{%ele_(:)} holds the elements of
the lattice. This array is always allocated with zero as the lower bound.
\vn{%ele_(0)} is a marker element with the name \vn{BEGINNING}.
\vn{%ele_(0)%mat6} is always the unit matrix. \vn{%ele_(0:)} is divided up
...
\end{table}
Note \vn{%n_ele_ring} is an historical artifact and is identical to
\vn{%n_ele_use}
...
\index{Ring_struct!%param!%n_part}
\vn{%param%n_part} is the number of particles in a bunch and is used
by \vne{BeamBeam} element to determine the strength of the beambeam
interaction. \vn{%param%charge} is the charge of a particle or
macroparticle and is used by \vne{Lcavity} elements for wakefield
calculations. The two variables are independent and setting one will not
affect the other. 

\index{Ring_struct!%param!%lost}
\index{Ring_struct!%param!%end_lost_at}
\index{Ring_struct!%param!%aperture_limit_on}
When tracking particles thorugh a lattice the variable
\vn{%param%aperture_limit_on} determines if apertures are checked to
begin with. Even if this is False there is a ``hard'' aperture limit
set by \vn{bmad_com%max_aperture_limit}. The hard aperture limit is
needed so that programs will not bomb due to overflow of floating
point calculations. The default for this hard limit is 1000
meters. \vn{%param%lost} is set by tracking routines to signal if a
particle is lost. \vn{%param%ix_lost} gives the index of the element
at which a particle is lost. and \vn{%param%end_lost_at} gives which
end the particle was lost at. The possible values for
\vn{%param%end_lost_at} are
\begin{example}
  entrance_end$
  exit_end$
\end{example}

\index{Ring_struct!%param!%t1_with_RF}
\index{Ring_struct!%param!%t1_no_RF}
\index{Ring_struct!%param!%stable}
...
Initial allocation of the pointers in a \vn{ring_struct} variable is generally
handled by the \vn{bmad_parser} and \vn{ring_equal_ring} routines.
Once allocated,
local \vn{ring_struct} variables must have the save attribute or the
