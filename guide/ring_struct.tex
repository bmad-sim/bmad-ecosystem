...
\vn{Overlay} and \vn{Group} elements, and those elements that are
...
interaction. \vn{%param%n_part} is also used by \vn{Lcavity} elements for wakefield
calculations. 
...
\end{example}
\vn{element_locator} searches the \vn{ring%ele_(:)} array for an element 
of the correct name. \vn{elements_locator} searches for all elements of a
given key and returns an array of indexes.
The heart of \vn{element_locator} is simplcity itself:
\begin{example}
...
\vn{ring%ele_(i)%value(:)} array. This is done useing the 
...
\vn{set_ele_attribute} checks whether this element is actually free to be varied
and sets the \vn{err_flag} logical accordingly. An element's attribute may not be
freely varied if, for example, the attribute is controled via an \vn{Overlay}.
