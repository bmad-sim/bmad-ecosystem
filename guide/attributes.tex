...
{\em Element}       & {\em Dependent Variables}  & {\em Independent Variables}  \\ \hline
 \vn{BeamBeam}     & \vn{bbi_const}           & 
         \vn{charge}, \vn{sig_x}, \vn{sig_y}, \vn{beam_energy}, \vn{n_part} \\ \hline
 \vn{Elseparator}  & \vn{e_field}, \vn{voltage}   & 
         \vn{hkick}, \vn{vkick}, \vn{gap}, \vn{l}, \vn{beam_energy}    \\ \hline
 \vn{Lcavity}      & \vn{e_loss}, \vn{delta_e}    & 
         \vn{gradient}, \vn{l}                           \\ \hline
 \vn{Rbend}, \vn{Sbend} & \vn{rho}, \vn{angle}, \vn{l_chord} & 
         \vn{g}, \vn{l}                                  \\ \hline
 \vn{Wiggler} (periodic type) & \vn{k1}, \vn{rho}             & 
         \vn{b_max}, \vn{beam_energy}                    \\ \hline
\end{tabular}
}
...
{\em Element}   & {\em Dependent Variable Set}  & 
  ...
  \vn{Lcavity}            & \vn{delta_e}       & \vn{gradient}  \\ \hline
  \vn{Rbend}, \vn{Sbend} & \vn{rho}           & \vn{g}         \\ \hline
  \vn{Rbend}, \vn{Sbend}   & \vn{angle}         & \vn{g}         \\ \hline
  \vn{RFcavity}           & \vn{rf_frequency}  & \vn{harmon}    \\ \hline
\end{tabular}
}
...
\begin{tabular}{|l|l|l|} \hline
  ...
  \vn{Sbend}, \vn{Rbend}  &   \vn{g}      &  \vn{b_field}       \\ \hline
  \vn{Solenoid}           &   \vn{ks}     &  \vn{b_field}       \\ \hline
  \vn{Sol_quad}           &   \vn{ks}     &  \vn{b_field}       \\ \hline
  \vn{Sol_quad}           &   \vn{k1}     &  \vn{b_gradient}    \\ \hline
  \vn{Quadrupole}         &   \vn{k1}     &  \vn{b_gradient}    \\ \hline
  \vn{Sextupole}          &   \vn{k2}     &  \vn{b_gradient}    \\ \hline
  \vn{Octupole}           &   \vn{k3}     &  \vn{b_gradient}    \\ \hline
\end{tabular}
}
...
\index{Patch}
\index{Lcavity}
The \vn{beam_energy} attribute is the referency energy (in eV) for a
particle at the end of an element.
See~\sref{s:phase_space_coords}. This attribute is dependent and thus
it cannot be set for an element directly in the lattice input
file. The value of \vn{beam_energy} for an element is calculated by
\bmad to be the same as the previous element except for \vn{Lcavity}
and \vn{Patch} elements. To set the \vn{beam_energy} at the start of
...
of an \vn{Lcavity}, An \vn{Lcavity} has a \vn{energy_start} attribute
which is just the reference energy at the start of the element.

\index{P0c}
The \vn{beam_energy} attribute give the total energy. The \vn{p0c}
...
\index{Rbend}
\index{Wiggler}
\vn{l} is, for all elements except \vn{Rbend}s, the path length of
the reference particle.  Note that for \vn{Wiggler}s this is not the
...
Amplitude (\vn{KnL}) and a tilt (\vn{Tn}). All other elements that
have multipole attributes specify the multipoles using normal
(\vn{Bn}) and skew (\vn{An}) components. Here \vn{n} ranges from 0
