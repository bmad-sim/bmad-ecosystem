...
{\em Element}       & {\em Dependent Variables}  & {\em Independent Variables}  \\ \hline
 \vne{BeamBeam}     & \vnea{bbi_const}           & 
         \vnea{charge}, \vnea{sig_x}, \vnea{sig_y}, \vnea{beam_energy}, \vnea{n_part} \\ \hline
 \vne{Elseparator}  & \vnea{e_field}, \vnea{voltage}   & 
         \vnea{hkick}, \vnea{vkick}, \vnea{gap}, \vnea{l}, \vnea{beam_energy}    \\ \hline
 \vne{Lcavity}      & \vnea{e_loss}, \vnea{delta_e}    & 
         \vnea{gradient}, \vnea{l}                           \\ \hline
 \vne{Rbend}, \vne{Sbend} & \vnea{rho}, \vnea{angle}, \vnea{l_chord} & 
         \vnea{g}, \vnea{l}                                  \\ \hline
 \vne{Wiggler} (periodic type) & \vnea{k1}, \vnea{rho}             & 
...
{\em Element}   & {\em Dependent Variable Set}  & 
  ...
  \vne{Lcavity}            & \vnea{delta_e}       & \vnea{gradient}  \\ \hline
  \vne{Rbend}, \vne{Sbend} & \vnea{rho}           & \vnea{g}         \\ \hline
  \vn{Rbend}, \vn{Sbend}   & \vnea{angle}         & \vnea{g}         \\ \hline
  \vne{RFcavity}           & \vnea{rf_frequency}  & \vnea{harmon}    \\ \hline
\end{tabular}
}
...
\begin{tabular}{|l|l|l|} \hline
  ...
  \vne{Sbend}, \vne{Rbend} &   \vnea{g}      &  \vnea{b_field}       \\ \hline
  \vne{Solenoid}           &   \vnea{ks}     &  \vnea{b_field}       \\ \hline
  \vne{Quadrupole}         &   \vnea{k1}     &  \vnea{b_gradient}    \\ \hline
  \vne{Sextupole}          &   \vnea{k2}     &  \vnea{b_gradient}    \\ \hline
  \vne{Octupole}           &   \vnea{k3}     &  \vnea{b_gradient}    \\ \hline
\end{tabular}
}
...
\begin{example}
  ...
  roll
\end{example}
The exception here is the \vne{Pitch} element which uses these
...
\vnea{kick}, \vnea{hkick}, and \vnea{vkick} attributes are the integrated
kick of an element in radians. \vn{kick} is only used for \vne{Hkicker}
and \vn{Vkicker} elements. All other elements that can kick use 
\vn{hkick} and \vn{vkick}. The \vn{tilt} attribute will only rotate
a kick for \vne{Hkicker}, \vne{Vkicker}, \vne{Elseparator} and \vne{Kicker}
...
an element as shown in figure~\ref{f:limit}. Notice that an \vne{Ecollimator}
has a elliptical aperture while a \vne{Rcollimator} and all other elements
...
\begin{example}
  ...
  Custom
\end{example}
\vne{Custom} means that the field calculations are done outside of the
\bmad software. A program doing \vne{Custom} field calculations will
...
\vnea{l} is, for all elements except \vne{Rbend}s, the path length 
of the reference particle.
Note that for \vne{Wiggler}s this is not the same as the path length for
