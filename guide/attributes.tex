...
file. The value of \vnea{beam_energy} for an element is calculated by
\bmad to be the same as the previous element except for \vn{Lcavity}
and \vnec{Patch} elements. To set the \vn{beam_energy} at the start of
...
of an \vn{Lcavity}, An \vnec{Lcavity} has a \vnea{energy_start} attribute
which is just the reference energy at the start of the element.

The \vn{beam_energy} attribute give the total energy. The \vnea{p0c}
