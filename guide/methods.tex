...
Two attributes that can affect the way the transfer matrix is
...
The algorithm used for single particle tracking through a given
element is set by that element's \vn{tracking_method} attribute. A
table giving which methods are available with which element classes is
give in Table~\ref{t:track_methods}.
...
slice.  There are several ways to do this integration. The \vni{Boris}
and \vni{Runge_Kutta} methods integrate the equations of motion to
...
This is applicable to \vn{Boris}, \vni{Symp_Lie_Bmad}, and
\vni{Symp_Lie_PTC} integration.

\index{Integrater_order}
\index{Ds_step}
...
that \vni{Symp_Lie_Bmad} always uses an order 2 integrator
independent of the setting of \vn{integrator_order}.

\index{Ds_step}
\index{Rel_tol}
\index{Abs_tol}
\vni{Adaptive_Boris} and \vni{Runge_Kutta} use adaptive step
...
\index{Field_calc}
\vni{Boris}, \vni{Adaptive_Boris}, and \vni{Runge_Kutta} tracking all use
...
\vni{Boris} (Both \vn{Boris} and \vni{Adaptive_Boris}) tracking does not
...
%-----------------------------------------------------------------
\section{Map\_with\_offsets Attribute}
\label{s:mapoff}
\index{Map_with_offsets|textbf}
