...
\label{s:ab_m}
\index{Element class!AB_Multipole|textbf}
...
\section{Accel\_Sol}
\label{s:accel_sol}
\index{Element class!Accel_Sol|textbf}
...
\section{BeamBeam}
\label{s:bbi}
\index{Element class!BeamBeam|textbf}
...
\vn{charge} attribute. The defaut is \vn{charge} = -1 which indicates
...
\label{s:bsq}
\index{Element class!Bend_Sol_Quad|textbf}
...
\label{s:bend}
\index{Element class!Sbend|textbf}
\index{Element class!Rbend|textbf}
...
\label{s:col}
\index{Element class!Ecollimator|textbf}
\index{Element class!Rcollimator|textbf}
...
\section{Custom}
\label{s:custom}
\index{Element class!Custom|textbf}
...
\section{Drift}
\label{s:drift}
\index{Element class!Drift|textbf}
...
\section{Elseparator}
\label{s:elsep}
\index{Element class!Elseparator|textbf}
...
\label{s:hvkicker}
\index{Element class!Hkicker|textbf}
\index{Element class!Vkicker|textbf}
...
\section{Hybrid}
\label{s:hybrid}
\index{Element class!Hybrid|textbf}
...
\section{I\_Beam}
\label{s:i_beam}
\index{Element class!I_Beam|textbf}
...
\label{s:monitor}
\index{Element class!Instrument|textbf}
\index{Element class!Monitor|textbf}
...
\begin{tabular}{|l|l||l|l||l|l|} \hline
  ...
  x_offset = Real       & \ref{s:offset} & y\_limit = Real  & \ref{s:limit}  & integration\_ord = Integer   & \ref{s:integ}  \\ \hline
  y_offset = Real       & \ref{s:offset} & aperture = Real  & \ref{s:limit}  & field\_calc = Switch         & \ref{s:integ}  \\ \hline
  x_pitch  = Real       & \ref{s:offset} & is\_on = Logical & \ref{s:is_on}  & beam\_energy$^*$             & \ref{s:energy} \\ \hline 
  y_pitch  = Real       & \ref{s:offset} &                  &                & p0c$^*$                      & \ref{s:energy} \\ \hline
  \multicolumn{6}{l}{\small $^*$Dependent attribute. See Chapter~\ref{c:attrib}} \\
\end{tabular}
\end{center}
\toffset
...
\vn{Drift}.  The \vn{offset} and \vn{pitch} attributes are not used by
any \bmad routines. If these attributes are used by a program they are
typically used to simulate such things as measurement offsets. The
...
\section{Kicker}
\label{s:kicker}
\index{Element class!Kicker|textbf}
...
\section{Lcavity}
\label{s:lcav}
\index{Element class!Lcavity|textbf}
...
the \vn{sr_wake_file} attribute. The file gives both monopole
...
\vn{END_SETCION} marking the division between sections.
Wakes can be specified via a table of wake versus longitudinal position $z$ 
and/or using a set of ``psudo'' modes (\sref{s:wakefields}). The first section
...
\vn{e_loss} value will affect the reference energy. Since the quantities
like quadrupole k1 strengths and bend strengths are referenced to the 
reference energy, The value of \vn{e_loss} will affect the results even with
...
\section{Marker}
\label{s:mark}
\index{Element class!Marker|textbf}
...
\begin{tabular}{|l|l||l|l||l|l|} \hline
  ...
  x_offset = Real              & \ref{s:energy} &  descrip = String & \ref{s:string} & aperture = Real       & \ref{s:limit}  \\ \hline 
  y_offset = Real              & \ref{s:energy} &  is\_on = Logical & \ref{s:is_on}  & aperture\_at = Switch & \ref{s:limit}  \\ \hline
...
\section{Match}
\label{s:match}
\index{Element class!Match|textbf}
...
\section{Multipole}
\label{s:mult}
\index{Element class!Multipole|textbf}
...
\section{Octupole}
\label{s:oct}
\index{Element class!Octupole|textbf}
...
\section{Patch}
\label{s:patch}
\index{Element class!Patch|textbf}
...
\section{Quadrupole}
\label{s:quad}
\index{Element class!Quadrupole|textbf}
...
\section{RFcavity}
\label{s:rfcav}
\index{Element class!RFcavity|textbf}
...
\section{Sextupole}
\label{s:sex}
\index{Element class!Sextupole|textbf}
...
\section{Solenoid}
\label{s:sol}
\index{Element class!Solenoid|textbf}
...
\section{Sol\_Quad}
\label{s:sq}
\index{Element class!Sol_Quad|textbf}
...
\section{Taylor}
\label{s:tay}
\index{Element class!Taylor|textbf}
...
\section{Wiggler} 
\label{s:wig}
\index{Element class!Wiggler|textbf} 
