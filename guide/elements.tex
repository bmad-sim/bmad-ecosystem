...
\begin{definition}
\item[\vn{l}]  
\vn{l} for a \vn{Rbend} is the chord length and not the arc
length as it is for a \vn{Sbend}. 
After reading in a lattice, \bmad\ will internally convert all
\vn{Rbend}s into \vn{Sbend}s so internally \vn{l} will become the path
...
\item[e1, e2]
\vn{e1} and \vn{e2} are the rotation angles of the entrance and exit pole faces respectively.
An \vn{Sbend} with an \vn{e1} = \vn{e2} = \vn{angle}/2 is equivalent to an 
\vn{Rbend} with \vn{e1} = \vn{e2} = 0.
\item[angle]
\vn{angle} is the total bend angle. A positive \vn{angle} represents a bend towards negative 
$x$ values (see Figure~\ref{f:local_coords}).
\item[k1]
\vn{k1} gives the quadrupole strength.
\item[g, delta\_g, rho]
\vn{rho} is the design bending radius which determines the reference coordinate system 
(see section~\ref{s:ref}).  \vn{g} = 1/\vn{rho} is the curvature function and is proportional
to the design dipole magnetic field. The true field strength is given by \vn{g}~+~\vn{delta_g} 
so changing \vn{delta_g} leaves the design orbit unchanged but varies a particle's orbit.
...
\item[tilt]
\vn{tilt} is the roll angle about the longitudinal axis at the entrence face of the bend.
\vn{tilt} = 0 bends the reference trajectory in the $-x$ direction. 
If the \vn{tilt} attribute is given without any value then the value
$\pi/2$ will be used. This makes for a downward vertical ($-y$) bend.
\end{definition}
