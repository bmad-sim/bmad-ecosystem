...
\begin{tabular}{|l|l||l|l||l|l|} \hline
  ...
  beam\_energy$^*$      & \ref{s:energy}& y\_limit = Real  & \ref{s:limit}  & integration\_ord = Integer   & \ref{s:integ} \\ \hline
  p0c$^*$               & \ref{s:energy}& aperture = Real  & \ref{s:limit}  & field\_calc = Switch         & \ref{s:integ} \\ \hline
  \multicolumn{6}{l}{\small $^*$Dependent attribute. See Chapter~\ref{c:attrib}} \\
\end{tabular}
\end{center}
\toffset
...
\begin{example}
  dE = gradient * L * cos(twopi * (phi - z * rf_frequency / c_light)) - e_loss * charge
\end{example}
...
short--range wakefields. \vn{charge} is set using the \vn{parameter} statement 
(\sref{s:param}) and represents the charge of a bunch. 

The energy change of the reference particle is just the energy change for a 
particle with $z = 0$. Thus
\begin{example}
  dE(reference) = gradient * L * cos(twopi * phi) - e_loss * charge
\end{example}
