...
There are two types of wigglers. Those that that are described using a magnetic field map and those that are described assuming a periodic field by specifying the maximum field and number of poles. For the "map" type wigglers the field is given by a sum of terms: 
\Begineq
...
\begin{example}	
  \vn{b_max} is the maximum magnetic field (in T) on the wiggler centerline. 
  \vn{n_pole} is the number of poles. The period is then L / (2 * N_POLE). 
 ...
 \vn{rho_bend} is the the bending radius at maximum field strength B_MAX. This is calculated by BMAD and is not an input parameter. 
