...
\begin{tabular}{|l|l||l|l||} \hline
  ...
  a$n$, b$n$ multipoles  & \ref{s:multip}  & offsets, pitches, and tilt & \ref{s:offset}  \HH
...
Attributes specific to an \vn{AB_Multipole} element are:
\index{Lrad}
\begin{example}
  lrad = Real     ! Fictitious length for synch radiation calculations.
\end{example}
...
The \vn{x_pitch} and \vn{y_pitch} attributes are not used in tracking.
...
\begin{example}
  sig_x   = Real     ! Hoirzontal strong beam sigma   
...
\begin{tabular}{|l|l||l|l||} \hline
  ...
  kicks                  & \ref{s:kick}    & a$n$, b$n$ multipoles      & \ref{s:multip}  \HH
\end{tabular}
\end{center}
\toffset
...
entrance and exit faces. For compatibility with \mad, if \vn{fint}
...
\begin{tabular}{|l|l||l|l||} \hline
  ...
  Reference energy       & \ref{s:energy}  & tracking \& transfer map   & \ref{c:methods} \HH
  Aperture Limits        & \ref{s:limit}   & length                     & \ref{s:l}       \HH
  Description strings    & \ref{s:string}  &                            &                 \HH 
\end{tabular}
\end{center}
\toffset
...
\begin{example}
  ...
  phi0         = Real   ! Phase (radians/2\(\pi\)) of the reference particle with 
                        !   respect to the RF. phi0 = 0 is on crest.
  dphi0        = Real   ! Used to shift the phase with respect to a multipass lord.
  e_loss       = Real   ! Loss parameter for short range wakefields (V/Coulomb).
  rf_frequency = Real   ! Rf frequency (Hz).
  delta_e               ! Change in energy of an on-crest particle. Dependent attribute (\sref{s:depend}).
...
\begin{tabular}{|l|l||l|l||} \hline
  {\sl Attribute Class}  & \S              & {\sl Attribute Class}      & \S              \HH
  Description strings    & \ref{s:string}  & is\_on                     & \ref{s:is_on}   \HH 
  Reference energy       & \ref{s:energy}  & offsets, pitches, and tilt & \ref{s:offset}  \HH
  Aperture Limits        & \ref{s:limit}   &                            &                 \HH
\end{tabular}
\end{center}
\toffset
...
Attributes specific to a \vn{Wiggler} element are:
\begin{example}
  polarity = Real    
  term(i) = Wig_Term 
\end{example}
...
\begin{example}  
  ...
  rho  
\end{example}
This type of wiggler is considered to have infinitely wide poles so
with \vn{bmad_standard} tracking and transfer matrix calculations the
