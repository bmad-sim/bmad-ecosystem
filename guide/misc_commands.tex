...
The Taylor order (cf.~\ref{taylor}) is the maximum order for a Taylor map.
...
\end{example}
Setting the reference energy using the \vn{energy} attribute here has the same
affect as using the \vn{parameter[beam_energy]} construct. Valid \vn{particle}
switches are
\begin{example}
  positron  ! default
  electron
