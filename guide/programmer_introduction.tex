...
subroutines. In particular, the \ringstruct\ structure holds all the
lattice information: The list of elements, their attributes,
etc. \bmadparser is the routine to parse a particular lattice file and
transfer the information to a \ringstruct\. After it is called the
program uses \sn{twiss\_at\_start} to multiply the transfer matrices
of the individual elements together to form the 1--turn matrix from
the start back to the start. From this \sn{twiss\_at\_start}
calculates the Twiss parameters at the start of the lattice. The next
call to \sn{twiss\_propagate\_all} takes the starting Twiss parameters
...
\vn{ring\%ele\_(0:)} that contains the infomation on each individual
element. \vn{ring\%ele\_(0)} is basically a marker element to denote
the beginning of the array. \vn{ring\%ele\_(i)\%key} is an integer
denoting what type of element (quadrupole, wiggler, etc.) it is.
\vn{ring\%ele\_(i)\%s} is the longitudinal position at the end of an 
element. \vn{ring\%ele\_(i)\%x\%beta} is the $a$--mode (nearly
...
\begin{verbatim}
    ...
    listf twiss_at_%
\end{verbatim}
The second example will match to \sn{twiss\_at\_s} but not
\sn{twiss\_at\_start}.
...
\end{verbatim}
Checking the source code one would find in the module \tn{bmad\_struct}
\begin{verbatim}
