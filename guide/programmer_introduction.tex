...
structures used by the program. The \vn{use bmad} statement at line 3
...
etc. \vn{bmad_parser} (line 12) is the routine which parses a lattice
file and transfers the information to a
\vn{ring_struct} variable.

After \vn{bmad_parser} (line 12) is called the program uses the
routine \vn{twiss_at_start} (line 13) to multiply the transfer
...
next call, to \vn{twiss_propagate_all} (line 14), takes the starting
...
\end{example}
\index{Element attribute!superimpose} The solenoid is 3.51 meters long
and it is superimposed upon the lattice with its center as $s = 0$ by
default. When \vn{bmad_parser} constructs the lattice list of elements
the superposition of \vn{IP_L0} which is a zero--length marker with a
solenoid leaves a zero--length marker. The superposition of the
...
solenoid it came from.  Since the \vn{CLEO_SOL} element only covers
part of the \vn{Q00W} quadrupole \vn{bmad_parser} breaks the
quadrupole into two pieces. One piece that is inside the solenoid is a
\vn{sol_quad} and the piece outside the solenoid is a regular
quadrupole. See Section~\ref{s:super} for more details. Since the
center of the \vn{CLEO_SOL} is a $s = 0$ half of it extends to
...
the \vn{cleo_sol\\3} element is derived from the \vn{cleo_sol} is put
...
with one subroutine call. For example:
  \begin{example}
