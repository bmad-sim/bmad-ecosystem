...
\vn{test.f90} which is in a directory called \vn{<distribution
root>/bmad/simple_program}
...
executable versions of the program. As set in the
\vn{/bmad/simple_program/Makefile} file the executables are to be found in
the directory \vn{../../bin}.  In this case, since there is only one
program file in the directory, and since a simple \vn{gmake} command
was used, the executable files are named after the directory and are
\vn{../bin/simple_program} and \vn{../../bin/simple_program_g}. The
former program is the regular ``production'' executable. The latter
program is a debug version which has information in it about variable
names and such. The debug version can be used with a debugger program
to do such things as view variable values at different points in the
execution of the program etc. You can run the debug version of the
program without a debugger and this is sometimes useful since the
debug version will catch errors such as an array index out of bounds
that the production version would not. The disadvantage of the debug
version is that its execution time is longer.
