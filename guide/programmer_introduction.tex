...
Compliling and linking the program uses the \vn{gmake} command (the
...
named after the diretory and are \vn{../bin/simple_program} and
...
The \cmd{listf} command is like the \cmd{getf} command excecpt that only
...
assumed that you are familer with Fortran90.  The \cs{use bmad}
...
get tracked through (along with elments \vn{Q00E} and \vn{Q00W}) as
