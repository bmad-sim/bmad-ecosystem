...
selected quads in a specified manner. \vn{Groups} are coversed in \sref{s:group}.
\item
...
top of other elements. This is covered in \sref{s:superposition}.
\item
\vn{I_Beam} elements mimic the effect of a support girder. This is
covered in \sref{s:i_beam}.
\item
\vn{Clone} elements are used when the beam recirculates through the same element multiple times. This is covered in \sref{s:clone}.
\end{Itemize}
...
\vn{clone} attribute. A simple example shows how this works.
\begin{example}
  A: lcavity, clone
  my_line: line = (A, A)
  use, my_line
...
\begin{example}
  A\B1, A\B2
\end{example}
Since the two elements are derived from a \vn{clone} element they are
...
\vn{A\B2} is set to $pi$. This is the one attribute that \bmad's
bookkeeping routines will not touch when transfering attribute values
