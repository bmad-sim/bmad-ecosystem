...
example simple we will limit the discussion to one phase space
...
that the 2\Nd order integrator gave the fastest computation time for a
given accuracy.
...
on--axis (\vn{b_max}) and the number of poles (\vn{n_pole}). This type
...
horizontal motion looks like a drift. The vertical motion is moddeled
as a combination focusing quadrupole and focusing octupole
giving a kick/cite{b:corbett}
\Begineq
  \frac{dp_y}{dy} = k_1 \left( y + \frac{2}{3} \, k_z^2 \, y^3 \right)
\Endeq
where
\begin{alignat}{1}
  k_1 &= \frac{-c \, b_{max}}{2 \, \left( P_0 \, (1 + p_z) \right)^2} \CRNO
  k_z &= \frac{2 \, \pi}{\lambda} \\
  \lambda &= \frac{2 \, l}{n_{pole}} \nonumber
\end{alignat}
...
\Begineq
  ...
  \label{pelqxw}
\Endeq
There is a similar equation for $\Delta p_y(i)$. $\WtS$
...
Following Chao\cite{b:chao}, The long--range wakefields are characterized
...
wake components
\begin{alignat}{2}
  a_{\cos} &=  &\frac{c}{2} \, \left( \frac{R}{Q} \right) \,
    e^{-k z_w/ 2 Q} \, \cos (k \, z_w) \, I_m \, \sin(m \theta_w) \CRNO
  a_{\sin} &= -&\frac{c}{2} \, \left( \frac{R}{Q} \right) \,
    e^{-k z_w/ 2 Q} \, \sin (k \, z_w) \, I_m \, \sin(m \theta_w) 
    \label{ac2rq} \\
  b_{\cos} &= -&\frac{c}{2} \, \left( \frac{R}{Q} \right) \,
    e^{-k z_w/ 2 Q} \, \cos (k \, z_w) \, I_m \, \cos(m \theta_w) \CRNO
  b_{\sin} &=  &\frac{c}{2} \, \left( \frac{R}{Q} \right) \,
