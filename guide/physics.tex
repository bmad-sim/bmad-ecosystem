...
the $y = 0$ axis is expanded in a Taylor series (This is not the most
...
component are rotated in the $(x, y)$ plane by an angle $\phi_n$ the
...
To convert a normal component into a skew component the rotation angle
should be
\Begineq
  (n+1) \phi_n = \frac{\pi}{2}
\Endeq
For example, a normal quadrupole rotated by by $45^\circ$ becomes a
skew quadrupole.
...
Note that the $n = 0$ component of an \vnec{AB_Multipole} or \vnec{Multipole}
...
\tt
  ...
  \vnec{Kicker}      & $\sqrt{{\tt Hkick}^2 + {\tt Vkick}^2}$ & 0 \\
  \vnec{Hkicker}     & Kick                                   & 0 \\
  \vnec{Vkicker}     & Kick                                   & 0 \\
  \vnec{Rbend}       & G * L                                  & 0 \\
  \vnec{Sbend}       & G * L                                  & 0 \\
  \vnec{Elseparator} & $\sqrt{{\tt Hkick}^2 + {\tt Vkick}^2}$ & 0 \\
  \vnec{Quadrupole}  & K1 * L                                 & 1 \\
  \vnec{Solenoid}    & KS * L                                 & 1 \\
  \vnec{Sol_Quad}    & K1 * L                                 & 1 \\
  \vnec{Sextupole}   & K2 * L                                 & 2 \\
  \vnec{Octupole}    & K3 * L                                 & 3 \\ \hline
\end{tabular}
...
As discussed in Section~\ref{s:wig}, \bmad \vnec{Wiggler} elements are split into 
...
accelerating cavities (\vn{LCavity}). Wakefield effects are divided
...
\Begineq
  dp_x(i) = \frac{e \, L \, \sum_j |q_j| \, x_j \, \WtS(dz_{ij})}
                 {v \, P_0}
\Endeq
...
\Begineq
  W(z) = \frac{c}{2} \, \left( \frac{R}{Q} \right) \,\,
  e^{k z/ 2 Q} \, \sin (k \, z)
\Endeq
...
not have any explicit dependence on $L$. Also notice that there is an
overall sign difference between how the long--range and short--range
transverse wakes are defined (This is due to the fact that \bmad takes
its short--range wake definition from LIAR and its long--range
definition from Chao).

To make life more interesting differnt people define $R/Q$
