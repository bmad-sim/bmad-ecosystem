...
energies) and sigma matrix as determined by the input parameters:
\begin{align}
...
\Begineq
  ...
  \end{cases}
\Endeq
$\tWl$ is used instead of $\Wl$ to avoid simulation artifacts which
happen when macroparticles are close longitudinally and is due to the
discontinuity of $\Wl$ at $dz = 0$.  $\sigma_{zij}$ is the combined
macroparticle length
\Begineq
