...
Note: Wakefield and Macroparticle simulation has just been implemented
...
that the longitudinal position of the macroparticles of a slice are roughly the
same with no overlap in $z$ between slices. 

If the initial distribution of slices and macroparticles is set--up to 
mirror the Liar program\cite{b:liar} then the slices within a bunch
will be placed 
...
modeled. Quadrupole wakefields can be implemented upon request.
The short--range longitudinal wakefield component for the $i$\Th macroparticle 
...
\Begineq
  ...
  \end{cases}
\Endeq
$\tWl$ is used instead of $\Wl$ to avoid simulation artifacts which happen
when macroparticles 
are close longitudinally and is due to the discontinutiy of $\Wl$ at $dz = 0$. 
$\sigma_{zij}$ is the combined macroparticle length
\Begineq
  \sigma_{zij} = \sigma_{zi} + \sigma_{zj} + \sigma_{z0}
\Endeq
where $\sigma_{zi}$ and $\sigma_{zj}$ are the longitudinal sizes for the $i$\Th and
$j$\Th particles respectively, and $\sigma_{z0} is a small fudge factor needed when
$\sigma_{zi} = \sigma_{zj} = 0$.
