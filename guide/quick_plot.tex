...
\begin{figure}
\centering
\includegraphics[width=5.5in]{plot_example.eps}
...
\vnr{qp_draw_text} call on line 35 draws the title above the two graphs.

The \vnr{qp_read_data} routine on line 29 will skip any ``header''
lines (lines that do not begin with something that looks like a
number). In this instance \vnr{qp_read_data} will read the first, third
forth and fifth data columns and put them into the \vn{x}, \vn{y},
\vn{z}, and \vn{t} arrays.

\vnr{qp_set_page_border}, \vnr{qp_set_box}, and \vnr{qp_set_margin} sets
where the graph is going to be placed (see below for more details).
\vnr{qp_set_box(1, 1, 2, 1)} on line 38 tells \quickplot to put the
first graph in the left box of a 2 box grid. The \vnr{qp_set_margin} on
line 34 sets the margins between the box and the graph axes.

\vnr{qp_calc_and_set_axis} on lines 39 and 40 are used to scale the
...
\vnr{qp_set_line_attrib} are called on lines 44 and 45 to plot the next
...
\vnr{qp_save_state} and \vnr{qp_restore_state} on lines 43 and 47 are
...
attribute stack. \vnr{qp_draw_data} on line 46 is similar to
...
string arguments of \vnr{qp_draw_graph} in lines 41 and 56/57 comes
...
\begin{figure}
  \centering
  \includegraphics{plot_coords.eps}
...
respectively. \vnr{qp_set_axis} can be used to disable mirroring. For example:
\begin{example}
  call qp_set_axis ("Y2", mirror = .false.)  ! y2-axis now independent of y.
\end{example}
\vnr{qp_set_axis} can also be used to set \vn{Y2} axis parameters (axis
minimum, maximum, etc.). 
...
defines a line is called a \vni{qp_line_struct} and is shown in
...
\end{verbatim}
\caption{Definition of the \vni{qp\_line\_struct}.}
\label{f:qp_line_struct}
\end{figure}

The \vni{qp_symbol_struct} defines how symbols are drawn and is shown
...
\end{verbatim}
\caption{Definition of the \vni{qp\_sym\_struct}.}
\label{f:qp_sym_struct}
\end{figure}

The \vni{qp_axis_struct} defines how axes are drawn and is shown
...
\end{verbatim}
\caption{Definition of the \vni{qp\_axis\_struct}.}
