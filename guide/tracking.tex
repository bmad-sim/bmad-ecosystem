...
(See Section~\ref{s:reverse_track}). The routines to track through a
lattice are listed in Section~\ref{r:track}.

\index{Param_struct!lost}
\index{Param_struct!ix_lost}
\index{Tracking!example}
The \vn{track_all} routine serves as a good
...
done in case the number of elements in the lattice has
changed. \vn{orbit(n)} holds the particle's position at the exit end
of the $n$\Th element. The call to \vn{track1} (line~18) tracks through one
element from the exit end of the $n-1$\St\ element to the exit end of
the $n$\Th particle. \vn{lattice%param%lost} is a logical that signals
the calling routine whether a particle has been lost.  This generally
...
\vn{closed_orbit_calc} to do backward tracking.
...
apertures (See Section~\ref{s:limit}) are used to determine if a
