...
\vn{ring%ele_(0)} is essentially a marker element \vn{orbit(0)} is the
...
\begin{example}
  ...
  do i = i1+1, i2
    call concat_taylor (taylor, lat%ele_(i)%taylor, taylor)
  enddo
\end{example}
...
\begin{example}
  ...
  call taylor_make_unit (taylor)  ! Make a unit map
  do i = i1+1, i2
    call call taylor_propagate1 (taylor, lat%ele_(i), lat%param)
  enddo
\end{example}
...
\vn{%ele%num_steps}. The order of the integrator
...
\vn{macro_beam_struct} structure can be done via the
\vn{init_macro_distribution} which implements the LIAR initialization
algorithm (See \sref{s:macro}). Once a beam is initialized,
tracking can be performed using \vn{track_macro_beam}. An example program is
...
The subroutine \vn{calc_macro_bunch_params} will fine the bunch centroid, emittances,
sigmas, twiss parameters and energy-Z correlation.
...
!
  ...
  call calc_macro_bunch_params (beam%bunch(1), lattice%ele_(lattice%n_ele_use), &
                                params)
