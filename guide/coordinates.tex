...
\vne{Patch} elements which can arbitrarily offset the entrance point
...
Notice that, in a \vne{Wiggler}, the reference orbit, which is a
...
matrics are calculated (\sref{s:twiss}). The former is fixed by the
...
The canonical $z$ coordinate is the path length difference between the
particle as it passes a given $s$ position relative relative to the
path length of the reference particle as it passes the same $s$
position. Notice that the time at which the particle and reference
particle pass a given $s$ position are generally not the same. Do not
confuse the canonical $z$ with the particle's longitudinal coordinate
...
At constant momentum, $z$ can be expressed in terms of the time via
\begin{align}
  z(s) &= s - \beta \, c \, t 
  & = - \beta \, c \, \Delta t
\end{align}
where $\beta = v/c$, $t$ is the time at which the particle is at
position $s$, and $\Delta t$ is the time difference between when the
particle passes by a given $s$ position along the reference orbit and
when the reference particle does. Another way of saying this is that,
$z$ will be the longitudinal distance between the particle and the
reference particle when their positions are both measured at a given
time $t$.
