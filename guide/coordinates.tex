...
The canonical $z$ coordinate is the path length difference of the
particle relative to the path length of the reference particle. Do not
confuse this with the longitudinal axis $\Bf z$. By construction the
longitudinal $z$ position of a particle is always zero as shown in
...
With the paraxial approximation and at constant momentum, $z$ can be
expressed in terms of the time via
\begin{align}
  z(s) &= - \beta \, c \, t - s
  & = - \beta \, c \, \Delta t
\end{align}
where $\beta = v/c$ and $\Delta t$ is the time difference between when
the particle passes by a given $s$ position along the reference orbit
and when the reference particle does.
