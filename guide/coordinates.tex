...
See \sref{s:patch}.  If not specified otherwise, the $s = 0$
...
\Endeq
\index{MAD}
\bmad, agian following \mad, computes $\Bf V$ and $\Bf W$ by starting at the first element of the lattice and iteratively using the equations
\Begineq
...
\vn{bend} or a \vn{patch} (that is, for all elements whose reference orbit
through them is a straight line) the corresponding $\Bf R$ and $\Bf S$
are
\index{Rbend}\index{Sbend}\index{Patch}
\Begineq
  \Bf R = 
  \begin{pmatrix}
      0 \\ 0 \\ L
...
where $\rho$ is bend radius (\vn{rho}), $\alpha$ is the bend angle
(\vn{angle}), and $theta_t$ is the tilt angle (\vn{tilt}). Without a
tilt, $\Bf T$ is the unit matrix resulting in $\Bf R = \Bf R_0$ and
$\Bf S = \Bf S_0$.

\index{Patch}\index{Tilt}
For a \vn{patch} element, the $\Bf S$ matrix corresponding to a
non--zero \vn{tilt} is
\Begineq
  \Bf S = 
  \begin{pmatrix}
...
\Endeq
\index{X_pitch}
The $\Bf S$ matrix corresponding to a non-zero \vn{x_pitch} is
\Begineq
  \Bf S = 
  \begin{pmatrix}
...
\Endeq
\index{Y_pitch}
Finally, the $\Bf S$ matrix corresponding to a non-zero \vn{y_pitch} is
\Begineq
  \Bf S = 
  \begin{pmatrix}
    1 & 0             & 0            \\
...
imagining a reference particle traveling along the reference curve who arrives
at position $s$ at time $t_0 = s / \beta c$. $z$ is then given by
\begin{align}
  z(s) &= -\beta \, c \, (t - t_0) \CRNO
    &\equiv - \beta \, c \, \Delta t
\end{align}
$\Delta t$ being the time difference between when the particle passes
by a given $s$ position along the reference orbit and when the
reference particle does. $z$ is then the longitudinal distance between
the particle and the reference particle when the particle is at $s$
with positive $z$ indicating that the particle is ahead of the
reference particle. At constant velocity $z$ can also be interpreted
as the path length difference between the particle as it passes a
given $s$ position relative to the path length of the reference
particle as it passes the same $s$ position. Notice that if a particle
gets an instantaneous longitudinal kick so that $\beta$ is
discontinuous then, from \Eq{zsbct}, the discontinuity in the
reference position will produce a discontinuity in canonical $z$ even
though the particle itself does not move in space.
...
\vn{Bmad_standard} (\sref{c:methods} tracking and transfer matrix calculations
