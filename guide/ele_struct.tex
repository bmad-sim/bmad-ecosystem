...
The routine \vnr{check_attrib_free} will check if an attribute is free to vary.
\begin{example}
  type (ring_struct) lat
  integer ix_ele
  ...
  call element_locator ('Q10W', lat, ix_ele)   ! look for a quadrupole
  call check_attrib_free (lat%ele_(ix_ele), k1$, lat, err_flag, .false.)
  if (err_flag) print *, 'Cannot vary: Q10W k1'
\end{example}
...
\vn{%wake%z_sr2_max} is the maximum $z$ value beyound which the pseudo
