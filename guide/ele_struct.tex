...
%--------------------------------------------------------------------------
\section{Initalization and Element Key}
...
\vn{%value} array use the \vn{attriubte_index} routine.  To go back
from an index in the \vn{%value} array to a name use the
\vn{attribute_name} routine
\begin{example}
  type (ele_struct) ele
  ele%kye = quadrupole$  ! quadrupole element
  print *, 'Index for Quadruple K1:  ', attribute_index(ele, 'K1') ! prints: '4' (= k1\$)
  print *, 'Name for Quadrupole k1\$: ', attrbute_name (ele, k1\$)   ! prints: 'K1' 
\end{example}
