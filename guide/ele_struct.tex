...
\index{Ele_struct!%name}
\index{Ele_struct!%type}
\index{Ele_struct!%alias}
\index{Ele_struct!%descrip}
...
into an \vn{ele_struct}. \bmad has routines for searching for an element by \vn{name}
in a lattice:
\begin{example}
  \vnr{element_locator}   ! locate one element by name
  \vnr{elements_locator}  ! locate an array of elements by name
\end{example}
Other than these routines \bmad ignores these character arrays. 
...
corresponding integer parameter for an element type just add a ``\$''
...
\index{Ele_struct!%sub_key}
\index{Element!Wiggler}
\index{Element!Rbend}
\index{Element!Sbend}
The \vn{%sub_key} component is only used for \vne{Wiggler} and \vne{Rbend} and
\vne{Sbend} elements. For \vn{Wiggler} elements, \vn{%sub_key} is either set to
\begin{example}
  map_type$ or
  periodic_type$
\end{example}
...
The \vn{%x}, \vn{%y}, and \vn{%z} components are
...
\end{example}
See Section~\ref{r:mat} for a complete listing of such routines.
...
\vne{%value(:)} array. For example, the value of the \vn{k1} attribute
...
are stored in the \vne{%a(:)} and \vne{%b(:)} arrays. The full list
of attributes not in \vn{%value(:)} is:
...
\index{Transfer map!Taylor!in ele_struct}
...
Generally most \vn{ele_struct} elements are part of a
\vn{ring_struct} variable so you generally don't have to worry about
allocation/deallocation issues directly. In case you do have a local
\vn{ele_struct} variable within a subroutine then you either have to
