...
outside of a \vn{ring_struct} structure. The \vn{ele_struct}
components of a \vn{ring_stuct} variable will get automatically
initalized.
...
\index{Element!Rbend}
\index{Element!Sbend}
The \vn{%sub_key} is only used for \vne{Wiggler} and \vne{Rbend} and
\vne{Sbend} elements. For \vn{Wiggler} elements is either set to
\begin{example}
  map_type$ or
  periodic_type$
\end{example}
...
scenes when \vn{make_mat6} is called to make transfer matrices. The
general rule is that, to be safe, when elment attributes are changed,
call \vn{attribute_bookkeeper}.
