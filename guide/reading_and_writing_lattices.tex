...
lattice file: \rn{bmad_parser} and \rn{bmad_parser2}.

\rn{bmad_parser} is used to initialize a \tn{ring_struct}
structure from scratch using the information from a lattice
file. Unless told otherwise, after reading in the lattice
\rn{bmad_parser} will compute the 6x6 transfer matrices for each
...
computed). Notice that \rn{bmad_parser} does {\em not} compute any
Twiss parameters.
...
Since parsing can be slow, once \rn{bmad_parser} has transfered the
information from a lattice file into the \tn{ring_struct} it will make
what is called a digested file. A digested file is an image of the
\tn{ring_struct} in binary form. When \rn{bmad_parser} is called it
(actually it is a the subroutine \rn{read_digested_bmad_file} that
...
\rn{bmad_parser} tries to reuse Taylor Maps it finds in the digested
