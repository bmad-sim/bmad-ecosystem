...
used to initialize a \tn{ring_struct} structure from scratch using the
information from an external lattice file. Unless told otherwise, after
reading in the lattice \rn{bmad_parser} will compute the 6x6 transfer
...
to be computed). Notice that \rn{bmad_parser} does {\em
...
called a digested file which is just an image of the \tn{ring_struct} 
in binary form. If
...
parsed originally \vn{bmad_parser} can make sure that the lattice file(s) 
