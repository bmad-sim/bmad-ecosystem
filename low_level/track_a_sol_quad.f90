...
use track1_mod, except_dummy => track_a_sol_quad
...
real(rp) kmat6(6,6), mat2(2,2), rel_p, dz_x(3), dz_y(3), ddz_x(3), ddz_y(3), xmat(6,6), vec0(6)
real(rp) rel_tracking_charge, charge_dir, r_step, step_len, s_off, mass, e_tot, ks, k1, dz4_coef(4,4)
...
do i = 1, n_step
  ...
  if (ele%key == solenoid$) then
    ...
    vec0 = 0
    vec0(6) = orbit%vec(6)
    call sol_quad_mat6_calc (ks, k1, step_len, vec0, xmat, dz4_coef)
    orbit%vec(5) = orbit%vec(5) + low_energy_z_correction (orbit, ele, step_len) + &
                                      sum(orbit%vec(1:4) * matmul(dz4_coef, orbit%vec(1:4))) 
    orbit%vec(1:4) = matmul (xmat(1:4,1:4), orbit%vec(1:4))
  endif
