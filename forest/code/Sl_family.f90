...
CONTAINS
  ...
  SUBROUTINE ADD_to_cavity(EL,NM,F,V) ! EXTENDS THE ADD ROUTINES FROM THE ELEMENT(P) TO THE FIBRE
    IMPLICIT NONE
    TYPE(FIBRE),target, INTENT(INOUT) ::EL
    REAL(DP), INTENT(IN) ::V
    INTEGER, INTENT(IN) ::NM,F
    ...
    CALL ADD_to_cavityr(EL%MAG,NM,F,V)
    CALL ADD_to_cavityp(EL%MAGP,NM,F,V)

  END SUBROUTINE ADD_to_cavity
