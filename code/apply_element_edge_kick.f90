...
!                                                                                apply_sol_fringe, reverse_the_charge)
!
...
!   reverse_the_charge -- logical, optional: If present and True then reverse the particle's charge. This reverses
!                           the applied kick and is useful for going backwards through the edge.
!
! Output:
!   fringe_info   -- fringe_edge_info_struct: Fringe information.
...
real(rp) a_pole_elec(0:n_pole_maxx), b_pole_elec(0:n_pole_maxx), charge
...
charge = charge_of(orb%species)
if (logic_option(.false., reverse_the_charge)) charge = -charge
...
case (solenoid$, sol_quad$, bend_sol_quad$)
  if (logic_option(.true., apply_sol_fringe)) then
    ...
    ks4 = at_sign * charge * hard_ele%value(bs_field$) * c_light / (4 * orb%p0c)
...
case (elseparator$)
  ...
  if (hard_ele%value(l$) /= 0) then
    f = at_sign * charge * (hard_ele%value(p0c$) / hard_ele%value(l$))
