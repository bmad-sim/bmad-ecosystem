...
! If not a real attribute
  ...
  if (index(value, 'ele::') /= 0) then
    ...
    lat_type = model$
    if (index(val, '|model') /= 0) then
      lat_type = model$
    elseif (index(val, '|design') /= 0) then
      lat_type = design$
    elseif (index(val, '|base') /= 0) then
      lat_type = base$
    elseif (index(val, '|') /= 0) then
      call out_io (s_error$, r_name, 'BAD SET VALUE: ' // value)
      return
    endif

    ix = index(val, '|')
    if (ix /= 0) val = val(:ix-1)
    ...
    call lat_ele_locator (val(:ix-1), tao_lat%lat, v_eles, n_eles, err); if (err) return
    ix2 = len_trim(val)
    call pointer_to_attribute (v_eles(1)%ele, val(ix+1:ix2-1), .false., a_ptr, err); if (err) return
    val = all_pointer_to_string (a_ptr, err = err)
    if (err) then
  ...
  do i = 1, size(eles)
    u => s%u(eles(i)%id)
    call set_ele_attribute (eles(i)%ele, trim(attribute) // '=' // trim(val), u%model%lat, err, .false.)
    u%calc%lattice = .true.
  ...
  if (n_set == 0) then
    u => s%u(eles(1)%id)
    call set_ele_attribute (eles(1)%ele, trim(attribute) // '=' // trim(val),  u%model%lat, err)
    u%calc%lattice = .true.
    return
  endif
