...
if (set) then
  ...
  if (v(x_offset_tot$) /= 0 .or. v(y_offset_tot$) /= 0 .or. &
    ...
    coord%vec(1) = coord%vec(1) - v(x_offset_tot$)
    coord%vec(2) = coord%vec(2) - v(x_pitch_tot$)
    coord%vec(3) = coord%vec(3) - v(y_offset_tot$)
    coord%vec(4) = coord%vec(4) - v(y_pitch_tot$) 
  endif

  ! Set: tilt

  call tilt_coords (v(tilt_tot$) + v(tilt_err$), coord%vec, set$)
...
! Unset... 
  ...
  if (v(x_offset_tot$) /= 0 .or. v(y_offset_tot$) /= 0 .or. &
    ...
    call tilt_coords (v(tilt_tot$) + v(tilt_err$), offset, set$)
    ! Now translate to local output coords
