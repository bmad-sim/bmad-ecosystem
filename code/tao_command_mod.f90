...
!------------------------------------------------------------------------------
!+
! Subroutine tao_arg_split (cmd_line, arg_name, n_arg_values, arg, n_arg, err)
!
! This routine splits the command line into arguments.
! Abreviations are acceptable.
! Note: Arguments matched to an arg_name(:) will have the arg_name put in
!   the arg(:) list.
!
! Input: 
!   cmd_line        -- Character(*): The command line.
!   arg_name(:)     -- Character(*): List of recognized arguments.
!   n_arg_values(:) -- Integer: List of number of values for each arg_name(:)
!
! Output:
!   arg(:)          -- Tao_arg_struct: Argument list.
!   n_arg           -- Number of arguments.
!   err             -- Logical: error in splitting words
!
! Anything between single or double quotes is treated as a single word.
!-

subroutine tao_arg_split (cmd_line, arg_names, n_arg_values, arg, n_arg, err)

  implicit none

  type (tao_arg_struct) arg(:)

  integer n_arg, i_word, ix, i, j
  integer n_arg_values(:)

  character(*) cmd_line
  character(100) cmd_word(100), word
  character(*) arg_names(:)
  character(20) :: r_name = 'tao_arg_split'
...
!
  ...
  call tao_cmd_split (cmd_line, size(cmd_word), cmd_word, .false., err)
  if (err) return
  ...
  do 
    ...
    i_word = i_word + 1
    word = cmd_word(i_word)
    if (word == '') exit

    n_arg = n_arg + 1
    arg(n_arg)%name = word

    call match_word (word, arg_names, ix, .true.)
    if (ix < 0) then
      call out_io (s_error$, r_name, 'AMBIGUOUS ARGUMENT: ' // word)
      err = .true.
      return
    endif

    if (ix > 0) then
      arg(n_arg)%name = arg_names(ix)
      do j = 1, n_arg_values(ix)
        i_word = i_word + 1
        arg(n_arg)%value(j) = cmd_word(i_word)
        if (cmd_word(i_word) == '') then
          call out_io (s_error$, r_name, 'SUB-ARGUMENTS MISSING FOR: ' // arg_names(ix))
          err = .true.
          return
        endif
      enddo
