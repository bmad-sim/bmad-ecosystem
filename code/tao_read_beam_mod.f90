...
do i = 1, num_bunch
  do j = 1, num_particle
    ...
    beam%bunch(i)%particle(j)%r%vec = vec
    beam%bunch(i)%particle(j)%r%spin = spin
    beam%bunch(i)%particle(j)%ix_lost = ix_lost
    beam%bunch(i)%particle(j)%charge = charge_bunch / n_particle
  enddo
enddo
