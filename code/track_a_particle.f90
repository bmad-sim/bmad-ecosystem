...
subroutine track_a_particle (closed_orbit, orb0, n_turn, ring, Q_x, Q_y, Q_z)
  ...
  call phase_space_amplitude(ring%ele_(0), coord, amp_x0, amp_y0)
  n_ele = ring%n_ele_use
  type *,' TRACK_A_PARTICLE:'
  type ('(a15,6e10.2)'),'  closed_orbit ', closed_orbit%vec
  type ('(a15,6e10.2)'),'  orb_(0)      ', orb_(0)%vec
  do i = 1, n_turn
    call track_all (ring, orb_)
    if (ring%param%lost)goto 10
      orb_(0) = orb_(n_ele)
      coord%vec = orb_(0)%vec - closed_orbit%vec
      call phase_space_amplitude (ring%ele_(0), coord, amp_x, amp_y)
      x_max = max(x_max, abs(orb_(n_ele)%vec(1)))
      y_max = max(y_max, abs(orb_(n_ele)%vec(3)))
...
!
  ...
  type *, 'Track: end  ', orb_(0)%vec(1), orb_(0)%vec(3),'   ',i,'turns'
  type *, 'Track: max displacement x,y ',x_max, y_max
  type *, 'Track: max amplitude ratio x,y ',amp_x_max/amp_x0, amp_y_max/amp_y0

  type('(3f10.5,2e12.4,2x,2e12.4,i10)'),Q_x,Q_y,Q_z, orb_(0)%vec(1), orb_(0)%vec(3), &
                 amp_x_max/amp_x0, amp_y_max/amp_y0, i
  write(21,'(3f10.5,2e12.4,2x,2e12.4,i10)')Q_x,Q_y,Q_z, orb_(0)%vec(1), orb_(0)%vec(3), &
                 amp_x_max/amp_x0, amp_y_max/amp_y0, i
