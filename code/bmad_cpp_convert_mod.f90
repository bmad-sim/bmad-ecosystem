...
interface 
  subroutine controller_var_to_f (C, Fp) bind(c)
    import c_ptr
    type(c_ptr), value :: C, Fp
  end subroutine
end interface
...
interface 
  subroutine wake_lr_position1_to_f (C, Fp) bind(c)
    import c_ptr
    type(c_ptr), value :: C, Fp
  end subroutine
end interface
...
interface 
  subroutine taylor_field_plane1_to_f (C, Fp) bind(c)
    import c_ptr
    type(c_ptr), value :: C, Fp
  end subroutine
end interface
...
interface 
  subroutine taylor_field_plane_to_f (C, Fp) bind(c)
    import c_ptr
    type(c_ptr), value :: C, Fp
  end subroutine
end interface
...
interface 
  subroutine taylor_field_to_f (C, Fp) bind(c)
    import c_ptr
    type(c_ptr), value :: C, Fp
  end subroutine
end interface
...
interface 
  subroutine ptc_genfield_to_f (C, Fp) bind(c)
    import c_ptr
    type(c_ptr), value :: C, Fp
  end subroutine
end interface
...
!--------------------------------------------------------------------------
!+
! Subroutine controller_var_to_c (Fp, C) bind(c)
!
! Routine to convert a Bmad controller_var_struct to a C++ CPP_controller_var structure
!
! Input:
!   Fp -- type(c_ptr), value :: Input Bmad controller_var_struct structure.
!
! Output:
!   C -- type(c_ptr), value :: Output C++ CPP_controller_var struct.
!-
...
interface
  !! f_side.to_c2_f2_sub_arg
  subroutine controller_var_to_c2 (C, z_name, z_value, z_old_value) bind(c)
    import c_bool, c_double, c_ptr, c_char, c_int, c_double_complex
    !! f_side.to_c2_type :: f_side.to_c2_name
    type(c_ptr), value :: C
...
type(c_ptr), value :: Fp
type(c_ptr), value :: C
type(controller_var_struct), pointer :: F
integer jd, jd1, jd2, jd3, lb1, lb2, lb3
!! f_side.to_c_var
...
!! f_side.to_c2_call
call controller_var_to_c2 (C, trim(F%name) // c_null_char, F%value, F%old_value)

end subroutine controller_var_to_c

!--------------------------------------------------------------------------
!--------------------------------------------------------------------------
!+
! Subroutine controller_var_to_f2 (Fp, ...etc...) bind(c)
!
! Routine used in converting a C++ CPP_controller_var structure to a Bmad controller_var_struct structure.
! This routine is called by controller_var_to_c and is not meant to be called directly.
!
! Input:
!   ...etc... -- Components of the structure. See the controller_var_to_f2 code for more details.
!
! Output:
!   Fp -- type(c_ptr), value :: Bmad controller_var_struct structure.
!-

!! f_side.to_c2_f2_sub_arg
subroutine controller_var_to_f2 (Fp, z_name, z_value, z_old_value) bind(c)
...
end subroutine controller_var_to_f2
...
interface
  ...
  subroutine coord_to_c2 (C, z_vec, z_s, z_t, z_spin, z_field, z_phase, z_charge, z_path_len, &
    ...
    real(c_double) :: z_path_len, z_p0c, z_beta
  end subroutine
end interface
...
interface
  !! f_side.to_c2_f2_sub_arg
  subroutine wake_lr_mode_to_c2 (C, z_freq, z_freq_in, z_r_over_q, z_q, z_angle, z_b_sin, &
...
!! f_side.to_c2_call
call wake_lr_mode_to_c2 (C, F%freq, F%freq_in, F%r_over_q, F%q, F%angle, F%b_sin, F%b_cos, &
...
!! f_side.to_c2_f2_sub_arg
subroutine wake_lr_mode_to_f2 (Fp, z_freq, z_freq_in, z_r_over_q, z_q, z_angle, z_b_sin, &
    z_b_cos, z_a_sin, z_a_cos, z_t_ref, z_m, z_polarized) bind(c)
...
!--------------------------------------------------------------------------
!+
! Subroutine wake_lr_position1_to_c (Fp, C) bind(c)
!
! Routine to convert a Bmad wake_lr_position1_struct to a C++ CPP_wake_lr_position1 structure
!
! Input:
!   Fp -- type(c_ptr), value :: Input Bmad wake_lr_position1_struct structure.
!
! Output:
!   C -- type(c_ptr), value :: Output C++ CPP_wake_lr_position1 struct.
!-
...
interface
  !! f_side.to_c2_f2_sub_arg
  subroutine wake_lr_position1_to_c2 (C, z_vec, z_charge, z_t) bind(c)
    import c_bool, c_double, c_ptr, c_char, c_int, c_double_complex
    !! f_side.to_c2_type :: f_side.to_c2_name
    type(c_ptr), value :: C
    real(c_double) :: z_vec(*), z_charge, z_t
  end subroutine
end interface

type(c_ptr), value :: Fp
type(c_ptr), value :: C
type(wake_lr_position1_struct), pointer :: F
integer jd, jd1, jd2, jd3, lb1, lb2, lb3
!! f_side.to_c_var
...
!! f_side.to_c2_call
call wake_lr_position1_to_c2 (C, fvec2vec(F%vec, 6), F%charge, F%t)

end subroutine wake_lr_position1_to_c

!--------------------------------------------------------------------------
!--------------------------------------------------------------------------
!+
! Subroutine wake_lr_position1_to_f2 (Fp, ...etc...) bind(c)
!
! Routine used in converting a C++ CPP_wake_lr_position1 structure to a Bmad wake_lr_position1_struct structure.
! This routine is called by wake_lr_position1_to_c and is not meant to be called directly.
!
! Input:
!   ...etc... -- Components of the structure. See the wake_lr_position1_to_f2 code for more details.
!
! Output:
!   Fp -- type(c_ptr), value :: Bmad wake_lr_position1_struct structure.
!-

!! f_side.to_c2_f2_sub_arg
subroutine wake_lr_position1_to_f2 (Fp, z_vec, z_charge, z_t) bind(c)
...
end subroutine wake_lr_position1_to_f2
...
if (n1_bunch == 0) then
  ...
  do jd1 = 1, n1_bunch
    call wake_lr_position1_to_f (z_bunch(jd1), c_loc(F%bunch(jd1+1-1)))
  enddo
endif
...
!! f_side.to_c_trans[type, 1, ALLOC]
 n1_lr_mode = 0
if (allocated(F%lr_mode)) then
  n1_lr_mode = size(F%lr_mode); lb1 = lbound(F%lr_mode, 1) - 1
  allocate (z_lr_mode(n1_lr_mode))
  do jd1 = 1, n1_lr_mode
    z_lr_mode(jd1) = c_loc(F%lr_mode(jd1+lb1))
  enddo
endif
!! f_side.to_c_trans[type, 1, ALLOC]
 n1_lr_spline = 0
...
!! f_side.to_f2_trans[type, 1, ALLOC]
if (n1_lr_mode == 0) then
  if (allocated(F%lr_mode)) deallocate(F%lr_mode)
else
  if (allocated(F%lr_mode)) then
    if (n1_lr_mode == 0 .or. any(shape(F%lr_mode) /= [n1_lr_mode])) deallocate(F%lr_mode)
    if (any(lbound(F%lr_mode) /= 1)) deallocate(F%lr_mode)
  endif
  if (.not. allocated(F%lr_mode)) allocate(F%lr_mode(1:n1_lr_mode+1-1))
  do jd1 = 1, n1_lr_mode
    call wake_lr_mode_to_f (z_lr_mode(jd1), c_loc(F%lr_mode(jd1+1-1)))
  enddo
endif
...
!! f_side.to_f2_trans[real, 0, NOT]
F%lr_freq_spread = z_lr_freq_spread
!! f_side.to_f2_trans[logical, 0, NOT]
F%lr_self_wake_on = f_logic(z_lr_self_wake_on)
...
!! f_side.to_c2_call
call cartesian_map_term1_to_c2 (C, F%coef, F%kx, F%ky, F%kz, F%x0, F%y0, F%phi_z, F%type)
...
!--------------------------------------------------------------------------
!+
! Subroutine taylor_field_plane1_to_c (Fp, C) bind(c)
!
! Routine to convert a Bmad taylor_field_plane1_struct to a C++ CPP_taylor_field_plane1 structure
!
! Input:
!   Fp -- type(c_ptr), value :: Input Bmad taylor_field_plane1_struct structure.
!
! Output:
!   C -- type(c_ptr), value :: Output C++ CPP_taylor_field_plane1 struct.
!-
...
type(c_ptr), value :: Fp
type(c_ptr), value :: C
type(taylor_field_plane1_struct), pointer :: F
integer jd, jd1, jd2, jd3, lb1, lb2, lb3
!! f_side.to_c_var
type(c_ptr) :: z_field(3)
...
!! f_side.to_c2_call
call taylor_field_plane1_to_c2 (C, z_field)

end subroutine taylor_field_plane1_to_c

!--------------------------------------------------------------------------
!--------------------------------------------------------------------------
!+
! Subroutine taylor_field_plane1_to_f2 (Fp, ...etc...) bind(c)
!
! Routine used in converting a C++ CPP_taylor_field_plane1 structure to a Bmad taylor_field_plane1_struct structure.
! This routine is called by taylor_field_plane1_to_c and is not meant to be called directly.
!
! Input:
!   ...etc... -- Components of the structure. See the taylor_field_plane1_to_f2 code for more details.
!
! Output:
!   Fp -- type(c_ptr), value :: Bmad taylor_field_plane1_struct structure.
!-

!! f_side.to_c2_f2_sub_arg
subroutine taylor_field_plane1_to_f2 (Fp, z_field) bind(c)
...
do jd1 = 1, size(F%field,1); lb1 = lbound(F%field,1) - 1
  call em_taylor_to_f(z_field(jd1), c_loc(F%field(jd1+lb1)))
enddo

end subroutine taylor_field_plane1_to_f2

!--------------------------------------------------------------------------
!--------------------------------------------------------------------------
!--------------------------------------------------------------------------
!+
! Subroutine taylor_field_plane_to_c (Fp, C) bind(c)
!
! Routine to convert a Bmad taylor_field_plane_struct to a C++ CPP_taylor_field_plane structure
!
! Input:
!   Fp -- type(c_ptr), value :: Input Bmad taylor_field_plane_struct structure.
!
! Output:
!   C -- type(c_ptr), value :: Output C++ CPP_taylor_field_plane struct.
!-
...
type(c_ptr), value :: Fp
type(c_ptr), value :: C
type(taylor_field_plane_struct), pointer :: F
integer jd, jd1, jd2, jd3, lb1, lb2, lb3
!! f_side.to_c_var
type(c_ptr), allocatable :: z_plane(:)
integer(c_int) :: n1_plane
...
!! f_side.to_c_trans[type, 1, ALLOC]
 n1_plane = 0
if (allocated(F%plane)) then
  n1_plane = size(F%plane); lb1 = lbound(F%plane, 1) - 1
  allocate (z_plane(n1_plane))
  do jd1 = 1, n1_plane
    z_plane(jd1) = c_loc(F%plane(jd1+lb1))
  enddo
endif

!! f_side.to_c2_call
call taylor_field_plane_to_c2 (C, trim(F%file) // c_null_char, F%n_link, z_plane, n1_plane)

end subroutine taylor_field_plane_to_c

!--------------------------------------------------------------------------
!--------------------------------------------------------------------------
!+
! Subroutine taylor_field_plane_to_f2 (Fp, ...etc...) bind(c)
!
! Routine used in converting a C++ CPP_taylor_field_plane structure to a Bmad taylor_field_plane_struct structure.
! This routine is called by taylor_field_plane_to_c and is not meant to be called directly.
!
! Input:
!   ...etc... -- Components of the structure. See the taylor_field_plane_to_f2 code for more details.
!
! Output:
!   Fp -- type(c_ptr), value :: Bmad taylor_field_plane_struct structure.
!-

!! f_side.to_c2_f2_sub_arg
subroutine taylor_field_plane_to_f2 (Fp, z_file, z_n_link, z_plane, n1_plane) bind(c)
...
if (n1_plane == 0) then
  ...
  if (allocated(F%plane)) then
    if (n1_plane == 0 .or. any(shape(F%plane) /= [n1_plane])) deallocate(F%plane)
    if (any(lbound(F%plane) /= 1)) deallocate(F%plane)
  endif
  if (.not. allocated(F%plane)) allocate(F%plane(1:n1_plane+1-1))
  do jd1 = 1, n1_plane
    call taylor_field_plane1_to_f (z_plane(jd1), c_loc(F%plane(jd1+1-1)))
  enddo
endif


end subroutine taylor_field_plane_to_f2

!--------------------------------------------------------------------------
!--------------------------------------------------------------------------
!--------------------------------------------------------------------------
!+
! Subroutine taylor_field_to_c (Fp, C) bind(c)
!
! Routine to convert a Bmad taylor_field_struct to a C++ CPP_taylor_field structure
!
! Input:
!   Fp -- type(c_ptr), value :: Input Bmad taylor_field_struct structure.
!
! Output:
!   C -- type(c_ptr), value :: Output C++ CPP_taylor_field struct.
!-
...
type(c_ptr), value :: Fp
type(c_ptr), value :: C
type(taylor_field_struct), pointer :: F
integer jd, jd1, jd2, jd3, lb1, lb2, lb3
!! f_side.to_c_var
integer(c_int) :: n_ptr
...
!--------------------------------------------------------------------------
!+
! Subroutine taylor_field_to_f2 (Fp, ...etc...) bind(c)
!
! Routine used in converting a C++ CPP_taylor_field structure to a Bmad taylor_field_struct structure.
! This routine is called by taylor_field_to_c and is not meant to be called directly.
!
! Input:
!   ...etc... -- Components of the structure. See the taylor_field_to_f2 code for more details.
!
! Output:
!   Fp -- type(c_ptr), value :: Bmad taylor_field_struct structure.
!-
...
if (n_ptr == 0) then
  ...
  if (.not. associated(F%ptr)) allocate(F%ptr)
  call taylor_field_plane_to_f (z_ptr, c_loc(F%ptr))
endif


end subroutine taylor_field_to_f2
...
interface
  !! f_side.to_c2_f2_sub_arg
  subroutine beam_init_to_c2 (C, z_file_name, z_distribution_type, z_spin, z_ellipse, z_kv, &
      z_grid, z_center_jitter, z_emit_jitter, z_sig_z_jitter, z_sig_e_jitter, z_n_particle, &
      ...
      z_center, z_dt_bunch, z_sig_z, z_sig_e, z_bunch_charge, z_n_bunch, z_species, &
      z_init_spin, z_full_6d_coupling_calc, z_use_lattice_center, z_use_t_coords, z_use_z_as_t) &
      bind(c)
    import c_bool, c_double, c_ptr, c_char, c_int, c_double_complex
    !! f_side.to_c2_type :: f_side.to_c2_name
    type(c_ptr), value :: C
    type(c_ptr), value :: z_spin, z_kv
    integer(c_int) :: z_n_particle, z_n_bunch
    logical(c_bool) :: z_renorm_center, z_renorm_sigma, z_init_spin, z_full_6d_coupling_calc, z_use_lattice_center, z_use_t_coords, z_use_z_as_t
    real(c_double) :: z_center_jitter(*), z_emit_jitter(*), z_sig_z_jitter, z_sig_e_jitter, z_random_sigma_cutoff, z_a_norm_emit, z_b_norm_emit
    real(c_double) :: z_a_emit, z_b_emit, z_dpz_dz, z_center(*), z_dt_bunch, z_sig_z, z_sig_e
    real(c_double) :: z_bunch_charge
    character(c_char) :: z_file_name(*), z_random_engine(*), z_random_gauss_converter(*), z_species(*)
...
!! f_side.to_c2_call
call beam_init_to_c2 (C, trim(F%file_name) // c_null_char, z_distribution_type, c_loc(F%spin), &
    z_ellipse, c_loc(F%kv), z_grid, fvec2vec(F%center_jitter, 6), fvec2vec(F%emit_jitter, 2), &
    F%sig_z_jitter, F%sig_e_jitter, F%n_particle, c_logic(F%renorm_center), &
    c_logic(F%renorm_sigma), trim(F%random_engine) // c_null_char, &
    ...
    F%sig_e, F%bunch_charge, F%n_bunch, trim(F%species) // c_null_char, c_logic(F%init_spin), &
    c_logic(F%full_6d_coupling_calc), c_logic(F%use_lattice_center), c_logic(F%use_t_coords), &
...
!! f_side.to_c2_f2_sub_arg
subroutine beam_init_to_f2 (Fp, z_file_name, z_distribution_type, z_spin, z_ellipse, z_kv, &
    z_grid, z_center_jitter, z_emit_jitter, z_sig_z_jitter, z_sig_e_jitter, z_n_particle, &
    z_renorm_center, z_renorm_sigma, z_random_engine, z_random_gauss_converter, &
    z_random_sigma_cutoff, z_a_norm_emit, z_b_norm_emit, z_a_emit, z_b_emit, z_dpz_dz, &
    z_center, z_dt_bunch, z_sig_z, z_sig_e, z_bunch_charge, z_n_bunch, z_species, z_init_spin, &
    z_full_6d_coupling_calc, z_use_lattice_center, z_use_t_coords, z_use_z_as_t) bind(c)
...
!! f_side.to_f2_trans[logical, 0, NOT]
F%use_lattice_center = f_logic(z_use_lattice_center)
!! f_side.to_f2_trans[logical, 0, NOT]
...
interface
  ...
  subroutine lat_param_to_c2 (C, z_n_part, z_total_length, z_unstable_factor, z_t1_with_rf, &
      ...
      z_stable, z_backwards_time_tracking, z_bookkeeping_state, z_beam_init) bind(c)
    import c_bool, c_double, c_ptr, c_char, c_int, c_double_complex
    !! f_side.to_c2_type :: f_side.to_c2_name
    type(c_ptr), value :: C
...
call lat_param_to_c2 (C, F%n_part, F%total_length, F%unstable_factor, mat2vec(F%t1_with_rf, &
    ...
    F%geometry, F%ixx, c_logic(F%stable), c_logic(F%backwards_time_tracking), &
...
interface
  ...
  subroutine bmad_common_to_c2 (C, z_max_aperture_limit, z_d_orb, z_default_ds_step, &
      ...
      z_default_integ_order, z_ptc_max_fringe_order, z_max_num_runge_kutta_step, &
      z_use_hard_edge_drifts, z_sr_wakes_on, z_lr_wakes_on, z_mat6_track_symmetric, &
      z_auto_bookkeeper, z_space_charge_on, z_coherent_synch_rad_on, z_spin_tracking_on, &
    ...
    !! f_side.to_c2_type :: f_side.to_c2_name
    type(c_ptr), value :: C
    integer(c_int) :: z_sad_n_div_max, z_taylor_order, z_default_integ_order, z_ptc_max_fringe_order, z_max_num_runge_kutta_step
...
call bmad_common_to_c2 (C, F%max_aperture_limit, fvec2vec(F%d_orb, 6), F%default_ds_step, &
    ...
    F%ptc_max_fringe_order, F%max_num_runge_kutta_step, c_logic(F%use_hard_edge_drifts), &
...
subroutine bmad_common_to_f2 (Fp, z_max_aperture_limit, z_d_orb, z_default_ds_step, &
    ...
    z_ptc_max_fringe_order, z_max_num_runge_kutta_step, z_use_hard_edge_drifts, z_sr_wakes_on, &
...
!--------------------------------------------------------------------------
!+
! Subroutine ptc_genfield_to_c (Fp, C) bind(c)
!
! Routine to convert a Bmad ptc_genfield_struct to a C++ CPP_ptc_genfield structure
!
! Input:
!   Fp -- type(c_ptr), value :: Input Bmad ptc_genfield_struct structure.
!
! Output:
!   C -- type(c_ptr), value :: Output C++ CPP_ptc_genfield struct.
!-
...
interface
  !! f_side.to_c2_f2_sub_arg
  subroutine ptc_genfield_to_c2 (C, z_vec0) bind(c)
    import c_bool, c_double, c_ptr, c_char, c_int, c_double_complex
    !! f_side.to_c2_type :: f_side.to_c2_name
    type(c_ptr), value :: C
    real(c_double) :: z_vec0(*)
  end subroutine
end interface

type(c_ptr), value :: Fp
type(c_ptr), value :: C
type(ptc_genfield_struct), pointer :: F
integer jd, jd1, jd2, jd3, lb1, lb2, lb3
!! f_side.to_c_var
...
!! f_side.to_c2_call
call ptc_genfield_to_c2 (C, fvec2vec(F%vec0, 6))

end subroutine ptc_genfield_to_c

!--------------------------------------------------------------------------
!--------------------------------------------------------------------------
!+
! Subroutine ptc_genfield_to_f2 (Fp, ...etc...) bind(c)
!
! Routine used in converting a C++ CPP_ptc_genfield structure to a Bmad ptc_genfield_struct structure.
! This routine is called by ptc_genfield_to_c and is not meant to be called directly.
!
! Input:
!   ...etc... -- Components of the structure. See the ptc_genfield_to_f2 code for more details.
!
! Output:
!   Fp -- type(c_ptr), value :: Bmad ptc_genfield_struct structure.
!-

!! f_side.to_c2_f2_sub_arg
subroutine ptc_genfield_to_f2 (Fp, z_vec0) bind(c)
...
end subroutine ptc_genfield_to_f2
...
interface
  ...
  subroutine ele_to_c2 (C, z_name, z_type, z_alias, z_component_name, z_descrip, n_descrip, &
      z_a, z_b, z_z, z_x, z_y, z_bookkeeping_state, z_control_var, n1_control_var, &
      z_cartesian_map, n1_cartesian_map, z_cylindrical_map, n1_cylindrical_map, z_taylor_field, &
      n1_taylor_field, z_grid_field, n1_grid_field, z_floor, z_ptc_genfield, z_mode3, n_mode3, &
      z_photon, n_photon, z_rad_int_cache, n_rad_int_cache, z_space_charge, n_space_charge, &
      z_taylor, z_spin_taylor, z_wake, n_wake, z_wall3d, n1_wall3d, z_map_ref_orb_in, &
      z_map_ref_orb_out, z_time_ref_orb_in, z_time_ref_orb_out, z_value, z_old_value, z_vec0, &
      z_mat6, z_c_mat, z_gamma_c, z_s_start, z_s, z_ref_time, z_r, n1_r, n2_r, n3_r, z_a_pole, &
      n1_a_pole, z_b_pole, n1_b_pole, z_a_pole_elec, n1_a_pole_elec, z_b_pole_elec, &
      n1_b_pole_elec, z_key, z_sub_key, z_ix_ele, z_ix_branch, z_slave_status, z_n_slave, &
      z_n_slave_field, z_ix1_slave, z_lord_status, z_n_lord, z_n_lord_field, z_ic1_lord, &
    ...
    !! f_side.to_c2_type :: f_side.to_c2_name
    type(c_ptr), value :: C
    type(c_ptr), value :: z_a, z_b, z_z, z_x, z_y, z_bookkeeping_state, z_floor
    type(c_ptr), value :: z_ptc_genfield, z_mode3, z_photon, z_rad_int_cache, z_space_charge, z_wake, z_map_ref_orb_in
    type(c_ptr), value :: z_map_ref_orb_out, z_time_ref_orb_in, z_time_ref_orb_out
    integer(c_int) :: z_key, z_sub_key, z_ix_ele, z_ix_branch, z_slave_status, z_n_slave, z_n_slave_field
    integer(c_int) :: z_ix1_slave, z_lord_status, z_n_lord, z_n_lord_field, z_ic1_lord, z_ix_pointer, z_ixx
    ...
    integer(c_int), value :: n_descrip, n1_control_var, n1_cartesian_map, n1_cylindrical_map, n1_taylor_field, n1_grid_field, n_mode3
    integer(c_int), value :: n_photon, n_rad_int_cache, n_space_charge, n_wake, n1_wall3d, n1_r, n2_r
    integer(c_int), value :: n3_r, n1_a_pole, n1_b_pole, n1_a_pole_elec, n1_b_pole_elec
    ...
    real(c_double) :: z_value(*), z_old_value(*), z_vec0(*), z_mat6(*), z_c_mat(*), z_gamma_c, z_s_start
    real(c_double) :: z_s, z_ref_time, z_r(*), z_a_pole(*), z_b_pole(*), z_a_pole_elec(*), z_b_pole_elec(*)
    character(c_char) :: z_name(*), z_type(*), z_alias(*), z_component_name(*), z_descrip(*)
    type(c_ptr) :: z_control_var(*), z_cartesian_map(*), z_cylindrical_map(*), z_taylor_field(*), z_grid_field(*), z_taylor(*), z_spin_taylor(*)
    type(c_ptr) :: z_wall3d(*)
  end subroutine
end interface
...
type(c_ptr), allocatable :: z_control_var(:)
integer(c_int) :: n1_control_var
...
!! f_side.to_c_trans[type, 1, PTR]
 n1_control_var = 0
if (associated(F%control_var)) then
  n1_control_var = size(F%control_var); lb1 = lbound(F%control_var, 1) - 1
  allocate (z_control_var(n1_control_var))
  do jd1 = 1, n1_control_var
    z_control_var(jd1) = c_loc(F%control_var(jd1+lb1))
  enddo
endif
!! f_side.to_c_trans[type, 1, PTR]
 n1_cartesian_map = 0
...
!! f_side.to_c_trans[type, 1, PTR]
 n1_taylor_field = 0
if (associated(F%taylor_field)) then
  n1_taylor_field = size(F%taylor_field); lb1 = lbound(F%taylor_field, 1) - 1
  allocate (z_taylor_field(n1_taylor_field))
  do jd1 = 1, n1_taylor_field
    z_taylor_field(jd1) = c_loc(F%taylor_field(jd1+lb1))
  enddo
endif
!! f_side.to_c_trans[type, 1, PTR]
 n1_grid_field = 0
...
call ele_to_c2 (C, trim(F%name) // c_null_char, trim(F%type) // c_null_char, trim(F%alias) // &
    ...
    n1_control_var, z_cartesian_map, n1_cartesian_map, z_cylindrical_map, n1_cylindrical_map, &
    z_taylor_field, n1_taylor_field, z_grid_field, n1_grid_field, c_loc(F%floor), &
    c_loc(F%ptc_genfield), c_loc(F%mode3), n_mode3, c_loc(F%photon), n_photon, &
    c_loc(F%rad_int_cache), n_rad_int_cache, c_loc(F%space_charge), n_space_charge, z_taylor, &
    z_spin_taylor, c_loc(F%wake), n_wake, z_wall3d, n1_wall3d, c_loc(F%map_ref_orb_in), &
    ...
    fvec2vec(F%vec0, 6), mat2vec(F%mat6, 6*6), mat2vec(F%c_mat, 2*2), F%gamma_c, F%s_start, &
    F%s, F%ref_time, tensor2vec(F%r, n1_r*n2_r*n3_r), n1_r, n2_r, n3_r, fvec2vec(F%a_pole, &
    ...
    F%ix1_slave, F%lord_status, F%n_lord, F%n_lord_field, F%ic1_lord, F%ix_pointer, F%ixx, &
    F%iyy, F%mat6_calc_method, F%tracking_method, F%spin_tracking_method, &
...
subroutine ele_to_f2 (Fp, z_name, z_type, z_alias, z_component_name, z_descrip, n_descrip, z_a, &
    z_b, z_z, z_x, z_y, z_bookkeeping_state, z_control_var, n1_control_var, z_cartesian_map, &
    n1_cartesian_map, z_cylindrical_map, n1_cylindrical_map, z_taylor_field, n1_taylor_field, &
    z_grid_field, n1_grid_field, z_floor, z_ptc_genfield, z_mode3, n_mode3, z_photon, n_photon, &
    z_rad_int_cache, n_rad_int_cache, z_space_charge, n_space_charge, z_taylor, z_spin_taylor, &
    z_wake, n_wake, z_wall3d, n1_wall3d, z_map_ref_orb_in, z_map_ref_orb_out, &
    z_time_ref_orb_in, z_time_ref_orb_out, z_value, z_old_value, z_vec0, z_mat6, z_c_mat, &
    z_gamma_c, z_s_start, z_s, z_ref_time, z_r, n1_r, n2_r, n3_r, z_a_pole, n1_a_pole, &
    z_b_pole, n1_b_pole, z_a_pole_elec, n1_a_pole_elec, z_b_pole_elec, n1_b_pole_elec, z_key, &
    z_sub_key, z_ix_ele, z_ix_branch, z_slave_status, z_n_slave, z_n_slave_field, z_ix1_slave, &
    z_lord_status, z_n_lord, z_n_lord_field, z_ic1_lord, z_ix_pointer, z_ixx, z_iyy, &
...
type(c_ptr), value :: z_a, z_b, z_z, z_x, z_y, z_bookkeeping_state, z_floor
type(c_ptr), value :: z_ptc_genfield, z_mode3, z_photon, z_rad_int_cache, z_space_charge, z_wake, z_map_ref_orb_in
type(c_ptr), value :: z_map_ref_orb_out, z_time_ref_orb_in, z_time_ref_orb_out, z_r, z_a_pole, z_b_pole, z_a_pole_elec
type(c_ptr), value :: z_b_pole_elec
integer(c_int) :: z_key, z_sub_key, z_ix_ele, z_ix_branch, z_slave_status, z_n_slave, z_n_slave_field
integer(c_int) :: z_ix1_slave, z_lord_status, z_n_lord, z_n_lord_field, z_ic1_lord, z_ix_pointer, z_ixx
integer(c_int) :: z_iyy, z_mat6_calc_method, z_tracking_method, z_spin_tracking_method, z_ptc_integration_type, z_field_calc, z_aperture_at
integer(c_int) :: z_aperture_type, z_orientation
...
call bookkeeping_state_to_f(z_bookkeeping_state, c_loc(F%bookkeeping_state))
!! f_side.to_f2_trans[type, 1, PTR]
if (n1_control_var == 0) then
  if (associated(F%control_var)) deallocate(F%control_var)
else
  if (associated(F%control_var)) then
    if (n1_control_var == 0 .or. any(shape(F%control_var) /= [n1_control_var])) deallocate(F%control_var)
    if (any(lbound(F%control_var) /= 1)) deallocate(F%control_var)
  endif
  if (.not. associated(F%control_var)) allocate(F%control_var(1:n1_control_var+1-1))
  do jd1 = 1, n1_control_var
    call controller_var_to_f (z_control_var(jd1), c_loc(F%control_var(jd1+1-1)))
  enddo
endif
...
!! f_side.to_f2_trans[type, 1, PTR]
if (n1_taylor_field == 0) then
  if (associated(F%taylor_field)) deallocate(F%taylor_field)
else
  if (associated(F%taylor_field)) then
    if (n1_taylor_field == 0 .or. any(shape(F%taylor_field) /= [n1_taylor_field])) deallocate(F%taylor_field)
    if (any(lbound(F%taylor_field) /= 1)) deallocate(F%taylor_field)
  endif
  if (.not. associated(F%taylor_field)) allocate(F%taylor_field(1:n1_taylor_field+1-1))
  do jd1 = 1, n1_taylor_field
    call taylor_field_to_f (z_taylor_field(jd1), c_loc(F%taylor_field(jd1+1-1)))
  enddo
endif
...
!! f_side.to_f2_trans[type, 0, NOT]
call ptc_genfield_to_f(z_ptc_genfield, c_loc(F%ptc_genfield))
!! f_side.to_f2_trans[type, 0, PTR]
if (n_mode3 == 0) then
...
interface
  !! f_side.to_c2_f2_sub_arg
  subroutine aperture_scan_to_c2 (C, z_aperture, n1_aperture, z_param, z_ref_orb) bind(c)
    import c_bool, c_double, c_ptr, c_char, c_int, c_double_complex
    !! f_side.to_c2_type :: f_side.to_c2_name
    type(c_ptr), value :: C
...
!! f_side.to_c2_call
call aperture_scan_to_c2 (C, z_aperture, n1_aperture, c_loc(F%param), c_loc(F%ref_orb))
...
!! f_side.to_c2_f2_sub_arg
subroutine aperture_scan_to_f2 (Fp, z_aperture, n1_aperture, z_param, z_ref_orb) bind(c)
