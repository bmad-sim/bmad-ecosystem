...
interface
  ...
  subroutine coord_to_c2 (C, z_vec, z_s, z_t, z_spin, z_e_field_x, z_e_field_y, z_phase_x, &
    ...
    import c_int, c_ptr, c_double_complex, c_double
    type(c_ptr), value :: C
...
interface
  ...
  subroutine ele_to_c2 (C, z_name, z_type, z_alias, z_component_name, z_descrip, z_a, z_b, z_z, &
    ...
    !! f_side.bindc_type :: f_side.bindc_name
    import c_int, c_ptr, c_char, c_double, c_bool
    type(c_ptr), value :: C
...
if (associated(F%descrip)) then
  a_descrip = trim(F%descrip) // c_null_char 
  f_descrip => a_descrip
else
  nullify(f_descrip)
endif
...
call ele_to_c2 (C, trim(F%name) // c_null_char, trim(F%type) // c_null_char, trim(F%alias) // &
    ...
    F%gamma_c, F%s, F%ref_time, c_loc(tensor2vec(F%r, n1_r*n2_r*n3_r)), n1_r, n2_r, n3_r, &
    c_loc(fvec2vec(F%a_pole, n1_a_pole)), n1_a_pole, c_loc(fvec2vec(F%b_pole, n1_b_pole)), &
    n1_b_pole, fvec2vec(F%map_ref_orb_in, 6), fvec2vec(F%map_ref_orb_out, 6), &
    fvec2vec(F%time_ref_orb_in, 6), fvec2vec(F%time_ref_orb_out, 6), F%key, F%sub_key, &
    ...
    c_logic(F%map_with_offsets), c_logic(F%field_master), c_logic(F%is_on), &
    c_logic(F%old_is_on), c_logic(F%logic), c_logic(F%bmad_logic), c_logic(F%on_a_girder), &
...
call c_f_pointer (z_descrip, f_descrip)
if (associated(f_descrip)) then
  if (.not. associated(F%descrip)) allocate(F%descrip)
  call to_f_str(f_descrip, F%descrip)
else
...
interface
  ...
  subroutine lat_to_c2 (C, z_use_name, z_lattice, z_input_file_name, z_title, &
    ...
    !! f_side.bindc_type :: f_side.bindc_name
    import c_int, c_ptr, c_char, c_bool
    type(c_ptr), value :: C
...
call lat_to_c2 (C, trim(F%use_name) // c_null_char, trim(F%lattice) // c_null_char, &
    ...
    F%n_ele_max, F%n_control_max, F%n_ic_max, F%input_taylor_order, c_loc(fvec2vec(F%ic, &
    n1_ic)), n1_ic, c_logic(F%absolute_time_tracking), c_logic(F%rf_auto_scale_phase), &
