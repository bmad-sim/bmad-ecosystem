...
interface
  ...
  subroutine wake_to_c2 (C, z_sr_file, z_lr_file, z_sr_long, z_sr_trans, z_lr_mode, n1_lr_mode, &
      z_wake_amp_scale, z_wake_time_scale, z_z_sr_max, z_lr_freq_spread, z_lr_self_wake_on, &
      z_sr_wake_scale_with_length) bind(c)
    import c_bool, c_double, c_ptr, c_char, c_int, c_double_complex
    !! f_side.to_c2_type :: f_side.to_c2_name
    type(c_ptr), value :: C
    real(c_double) :: z_wake_amp_scale, z_wake_time_scale, z_z_sr_max, z_lr_freq_spread
    type(c_ptr) :: z_lr_mode(*)
    type(c_ptr), value :: z_sr_long, z_sr_trans
    logical(c_bool) :: z_lr_self_wake_on, z_sr_wake_scale_with_length
    integer(c_int), value :: n1_lr_mode
...
call wake_to_c2 (C, trim(F%sr_file) // c_null_char, trim(F%lr_file) // c_null_char, &
    c_loc(F%sr_long), c_loc(F%sr_trans), z_lr_mode, n1_lr_mode, F%wake_amp_scale, &
    F%wake_time_scale, F%z_sr_max, F%lr_freq_spread, c_logic(F%lr_self_wake_on), &
    c_logic(F%sr_wake_scale_with_length))
...
!! f_side.to_f2_trans[real, 0, NOT]
F%wake_amp_scale = z_wake_amp_scale
!! f_side.to_f2_trans[real, 0, NOT]
F%wake_time_scale = z_wake_time_scale
!! f_side.to_f2_trans[real, 0, NOT]
...
!! f_side.to_f2_trans[logical, 0, NOT]
F%sr_wake_scale_with_length = f_logic(z_sr_wake_scale_with_length)
