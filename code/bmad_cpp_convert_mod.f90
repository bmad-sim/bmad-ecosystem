...
interface
  !! f_side.to_c2_f2_sub_arg
  subroutine track_point_to_c2 (C, z_s_body, z_orb, z_field, z_vec0, z_mat6) bind(c)
    import c_bool, c_double, c_ptr, c_char, c_int, c_double_complex
    !! f_side.to_c2_type :: f_side.to_c2_name
    type(c_ptr), value :: C
    real(c_double) :: z_s_body, z_vec0(*), z_mat6(*)
    type(c_ptr), value :: z_orb, z_field
  end subroutine
end interface
...
!! f_side.to_c2_call
call track_point_to_c2 (C, F%s_body, c_loc(F%orb), c_loc(F%field), fvec2vec(F%vec0, 6), &
    mat2vec(F%mat6, 6*6))
...
!! f_side.to_c2_f2_sub_arg
subroutine track_point_to_f2 (Fp, z_s_body, z_orb, z_field, z_vec0, z_mat6) bind(c)
...
interface
  ...
  subroutine bmad_common_to_c2 (C, z_max_aperture_limit, z_d_orb, z_default_ds_step, &
      ...
      z_max_num_runge_kutta_step, z_rf_phase_below_transition_ref, z_use_hard_edge_drifts, &
      z_sr_wakes_on, z_lr_wakes_on, z_mat6_track_symmetric, z_auto_bookkeeper, &
      z_space_charge_on, z_coherent_synch_rad_on, z_spin_tracking_on, &
    ...
    logical(c_bool) :: z_rf_phase_below_transition_ref, z_use_hard_edge_drifts, z_sr_wakes_on, z_lr_wakes_on, z_mat6_track_symmetric, z_auto_bookkeeper, z_space_charge_on
    logical(c_bool) :: z_coherent_synch_rad_on, z_spin_tracking_on, z_spin_sokolov_ternov_flipping_on, z_radiation_damping_on, z_radiation_fluctuations_on, z_conserve_taylor_maps, z_absolute_time_tracking_default
...
call bmad_common_to_c2 (C, F%max_aperture_limit, fvec2vec(F%d_orb, 6), F%default_ds_step, &
    ...
    c_logic(F%rf_phase_below_transition_ref), c_logic(F%use_hard_edge_drifts), &
...
interface
  ...
  subroutine ele_to_c2 (C, z_name, z_type, z_alias, z_component_name, z_descrip, n_descrip, &
      ...
      z_gamma_c, z_spin_quaternion, z_s_start, z_s, z_ref_time, z_a_pole, n1_a_pole, z_b_pole, &
...
call ele_to_c2 (C, trim(F%name) // c_null_char, trim(F%type) // c_null_char, trim(F%alias) // &
    ...
    fvec2vec(F%spin_quaternion, 4), F%s_start, F%s, F%ref_time, fvec2vec(F%a_pole, n1_a_pole), &
...
interface
  !! f_side.to_c2_f2_sub_arg
  subroutine lat_to_c2 (C, z_use_name, z_lattice, z_input_file_name, z_title, z_constant, &
      n1_constant, z_a, z_b, z_z, z_param, z_lord_state, z_ele_init, z_ele, n1_ele, z_branch, &
      n1_branch, z_control, n1_control, z_surface, n1_surface, z_beam_start, z_pre_tracker, &
...
call lat_to_c2 (C, trim(F%use_name) // c_null_char, trim(F%lattice) // c_null_char, &
    trim(F%input_file_name) // c_null_char, trim(F%title) // c_null_char, z_constant, &
    n1_constant, c_loc(F%a), c_loc(F%b), c_loc(F%z), c_loc(F%param), c_loc(F%lord_state), &
    c_loc(F%ele_init), z_ele, n1_ele, z_branch, n1_branch, z_control, n1_control, z_surface, &
    n1_surface, c_loc(F%beam_start), c_loc(F%pre_tracker), fvec2vec(F%custom, n1_custom), &
    n1_custom, F%version, F%n_ele_track, F%n_ele_max, F%n_control_max, F%n_ic_max, &
...
!! f_side.to_c2_f2_sub_arg
subroutine lat_to_f2 (Fp, z_use_name, z_lattice, z_input_file_name, z_title, z_constant, &
    n1_constant, z_a, z_b, z_z, z_param, z_lord_state, z_ele_init, z_ele, n1_ele, z_branch, &
    n1_branch, z_control, n1_control, z_surface, n1_surface, z_beam_start, z_pre_tracker, &
    z_custom, n1_custom, z_version, z_n_ele_track, z_n_ele_max, z_n_control_max, z_n_ic_max, &
    z_input_taylor_order, z_ic, n1_ic, z_photon_type, z_absolute_time_tracking, &
...
interface
  ...
  subroutine bunch_params_to_c2 (C, z_centroid, z_x, z_y, z_z, z_a, z_b, z_c, z_spin, z_sigma, &
    ...
    !! f_side.to_c2_type :: f_side.to_c2_name
    type(c_ptr), value :: C
    real(c_double) :: z_spin(*), z_sigma(*), z_rel_max(*), z_rel_min(*), z_s, z_charge_live
...
call bunch_params_to_c2 (C, c_loc(F%centroid), c_loc(F%x), c_loc(F%y), c_loc(F%z), c_loc(F%a), &
    ...
    fvec2vec(F%rel_min, 6), F%s, F%charge_live, F%n_particle_tot, F%n_particle_live, &
