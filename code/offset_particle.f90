!+
! Subroutine offset_particle (ele, param, set, orbit, 
!                               set_tilt, set_multipoles, set_hvkicks, set_z_offset, ds_pos, set_spin)
!
...
!    dir = ele%orientation * orbit%direction
!
...
if (set) then
  ...
  if (has_orientation_attributes(ele)) then
    ...
    ! z_rel_center is position relative to center with positive 
    if (present(ds_pos)) then
      z_rel_center = ele%orientation * (ds_pos - ele%value(l$)/2)   
    elseif (orbit%direction == 1) then  ! Entering at upstream end
      z_rel_center = -ele%orientation * ele%value(l$) / 2
    else                   ! orbit%direction == -1 -> Entering at downstream end
      z_rel_center = ele%orientation * ele%value(l$) / 2
    endif
    ...
    if (x_off /= 0 .or. y_off /= 0 .or. z_off /= 0 .or. xp /= 0 .or. yp /= 0) then
      ...
      if (ele%key == sbend$ .and. ele%value(g$) /= 0) then
        ...
        orbit%vec(2) = orbit%vec(2) - sign_z_vel * rot(2) * E_rel
        orbit%vec(3) = orbit%vec(3) - off(2)
        orbit%vec(4) = orbit%vec(4) + sign_z_vel * rot(1) * E_rel

        if (set_spn) call rotate_spin (-rot, orbit%spin)

        if (off(3) /= 0 .and. set_z_off) then
          call track_a_drift (orbit, sign_z_vel*off(3))
          orbit%vec(5) = orbit%vec(5) - sign_z_vel*off(3)  ! Correction due to reference particle is also offset.
        endif
      ...
      ! Else not a bend
        ...
        orbit%vec(2) = orbit%vec(2) - sign_z_vel * xp * E_rel
        orbit%vec(3) = orbit%vec(3) - y_off - yp * z_rel_center
        orbit%vec(4) = orbit%vec(4) - sign_z_vel * yp * E_rel
  ...
  if (set_t) then
    ...
    if (ele%key == sbend$ .and. ele%value(roll_tot$) /= 0) then
      ...
      pvec = matmul(m_trans, [orbit%vec(2), orbit%vec(4), sqrt(E_rel**2 - orbit%vec(2)**2 - orbit%vec(4)**2)])
      ...
      orbit%vec(4) = pvec(2)
      orbit%vec(5) = orbit%vec(5) + off(3) * E_rel / pvec(3) 
      orbit%t = orbit%t - off(3) * E_rel / (pvec(3) * c_light * orbit%beta)
    endif
  ...
  if (set_multi) then
    ...
    if (ix_pole_max > -1) then
      do n = 0, n_pole_maxx
        ...
        call ab_multipole_kick (an(n), bn(n), n, param%particle, ele%orientation, orbit, kx, ky, pole_type = electric$, scale = ele%value(l$)/2)
...
! Unset... 
  ...
  if (present(ds_pos)) then
    z_rel_center = ele%orientation * (ds_pos - ele%value(l$)/2)  ! position relative to center.
