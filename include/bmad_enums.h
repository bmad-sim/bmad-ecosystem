...
namespace Bmad {
  const int BMAD_INC_VERSION = 135;
  ...
  const int FINT=13, FINTX=14, HGAP=15, HGAPX=16, H1=17, H2=18;
  const int L=1;
  ...
  const int VOLTAGE=9;
  const int D1_THICKNESS = 10, VOLTAGE_ERR=10, REL_TRACKING_CHARGE = 10;
  const int L_CHORD=10, KS=10, N_SLICE=10, Y_GAIN_CALIB=10, BRAGG_ANGLE=10;
  const int POLARITY=11, CRUNCH_CALIB=11, ALPHA_ANGLE=11, D2_THICKNESS = 11;
  const int E1=11, E_LOSS=11, DKS_DS=11, GAP=11;
  const int DS_PATH_LENGTH=12;
  const int E2=12, X_OFFSET_CALIB=12, V1_UNITCELL=12, PSI_ANGLE=12;
  const int Y_OFFSET_CALIB=13, V_UNITCELL=13, V2_UNITCELL=13;
  const int TRAVELING_WAVE = 13;
  const int PHI0=14, TILT_CALIB=14, F0_RE=14, F0_RE1=14;
  const int PHI0_ERR=15, COEF=15, CURRENT=15, L_POLE=15, PARTICLE = 15;
  const int QUAD_TILT=15, DE_ETA_MEAS=15, F0_IM=15, F0_IM1 = 15, F1 = 15;
  const int GEOMETRY = 16, BEND_TILT=16, MODE=16;
  const int DPHI0=16, N_SAMPLE=16, F0_RE2=16, ORIGIN_ELE_REF_PT=16, F2 = 16;
  const int DPHI0_REF = 17, F0_IM2=17, X_HALF_LENGTH=17, DX_ORIGIN= 17;
  const int LATTICE_TYPE = 17, X_QUAD=17;
  const int DPHI0_MAX=18, REF_POLARIZATION=18, Y_HALF_LENGTH=18, DY_ORIGIN = 18, Y_QUAD=18;
  const int FRINGE_TYPE = 19, FLOOR_SET = 19, PTC_DIR = 19, DZ_ORIGIN = 19, FRINGE_KIND = 19;
  const int FRINGE_AT = 20, DTHETA_ORIGIN = 20, B_PARAM = 20;
  const int L_HARD_EDGE = 21, DPHI_ORIGIN = 21, REF_CAP_GAMMA = 21;
  const int FIELD_SCALE = 22, DPSI_ORIGIN = 22, DARWIN_WIDTH_SIGMA = 22;
  const int ANGLE = 23, N_CELL = 23, X_RAY_LINE_LEN = 23, DARWIN_WIDTH_PI = 23;
  const int X_PITCH = 24;
  const int Y_PITCH = 25;
  const int X_OFFSET = 26;
  const int Y_OFFSET = 27;
  const int Z_OFFSET = 28;
  const int HKICK = 29, D_SPACING = 29, T_OFFSET = 29, X_OFFSET_SOL = 29;
  const int VKICK = 30, L_X = 30, Y_OFFSET_SOL = 30;
  const int BL_HKICK = 31, X_PITCH_SOL = 31, L_Y = 31;
  const int BL_VKICK = 32, Y_PITCH_SOL = 32, L_Z = 32;
  const int BL_KICK = 33, COUPLER_AT = 33, EPS_STEP_SCALE = 34;
  const int B_FIELD = 34, E_FIELD = 34, COUPLER_PHASE = 34;
  const int COUPLER_ANGLE = 35, B_FIELD_ERR = 35;
  const int B1_GRADIENT = 36, E1_GRADIENT = 36, COUPLER_STRENGTH = 36;
  const int B2_GRADIENT = 37, E2_GRADIENT = 37, H_X_NORM = 37;
  const int B3_GRADIENT = 38, E3_GRADIENT = 38, H_Y_NORM = 38;
  const int BS_FIELD = 39, E_TOT_OFFSET = 39, H_Z_NORM = 39, PTC_FIELD_GEOMETRY = 39;
  const int DELTA_REF_TIME = 40;
  const int P0C_START = 41;
  const int E_TOT_START = 42;
  const int P0C = 43;
  const int E_TOT = 44;
  ...
  const int Z_OFFSET_TOT = 49;
  const int TILT_TOT = 50, ROLL_TOT = 50;
  const int POLE_RADIUS = 51, REF_TILT_TOT = 51;
  const int N_REF_PASS = 52;
  const int RADIUS = 53;
  const int REF_TIME_START = 54;
  const int THICKNESS = 55, INTEGRATOR_ORDER = 55;
  const int NUM_STEPS = 56;
  const int DS_STEP = 57;
  const int LORD_PAD1 = 58;
  const int LORD_PAD2 = 59, REF_WAVELENGTH = 59;
  const int SCRATCH = 60;
  ...
  const int A21 = 141, K21L = 141;
  const int B0  = 150, T0  = 150;
  const int B21 = 171, T21 = 171;
  ...
  const int RECTANGULAR = 1, ELLIPTICAL = 2, WALL_APERTURE = 3, SURFACE_APERTURE = 4;
  ...
  const int FULL = 1, NONLIN_ONLY = 2;
