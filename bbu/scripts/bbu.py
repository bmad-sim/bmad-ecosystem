...
if bbu_params.mode == 'current': 
  ...
  exec(open( 'scripts/for_py.txt' ).read())
...
if bbu_params.mode == 'drscan': 
  if ( os.path.exists("scripts/thresh_v_trotb.txt") ): os.remove("scripts/thresh_v_trotb.txt")
  my_file = open('scripts/thresh_v_trotb.txt','a')
