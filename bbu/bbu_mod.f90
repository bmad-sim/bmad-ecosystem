...
! Note: 1 time step = time for the next buch to reach the position of the buch ahead of it.

type bbu_info_struct
  real(rp), allocatable :: time_c(:)  ! Arrival time ot each cavity from the beginning.
  ...
  real(rp)  current                   ! Beam current in amps    ! Was: mA
  real(rp)  b_freq                    ! c_light / bunch-to-bunch spacing (Hz) = 1 / time_step.
  integer   b_time                    ! Simulation number of time steps.
  integer   p_time                    ! Print every p_time time steps.
  real(rp)  n_amp
  real(rp)  p_amp
  real(rp)  n_on
  integer   num_cav_unique                   ! Number of cavities
  integer   hom_num
  integer   n_pass                           ! Number of passes through a single cavity.
  integer   num_cav_track                    ! Number of cavities to track through = n_unique * n_pass
...
integer num_cav_track, h_num, ix_pass, n_pass, num_cav_unique
...
do i = 0, lat%n_ele_track
  ...
  call multipass_chain (i, lat, ix_pass, ix_chain)
  if (size(ix_chain) < 1) then
    print *, 'ERROR IN GET_INFO: LCAVITY ELEMENT NOT IN MULTIPASS REGION: ' // ele%name
    call err_exit
  endif
  if (n_pass > 0) then
    if (size(ix_chain) /= n_pass) then
      print *, 'ERROR IN GET_INFO: CONFUSED MULTIPASS!'
      call err_exit
    endif
  endif
  n_pass = size(ix_chain)
  num_cav_track = num_cav_track + 1
  cavityind(num_cav_track) = i

enddo

! Count the number of HOMs in each cavity

print *, 'Number of passes:', n_pass
num_cav_unique = num_cav_track / n_pass

allocate(bbu%hom(num_cav_unique))

judge = .false.    !  logical -- true if the rf cavity has wake fields
homtotal = 0     !  Total number of HOMs
k = 1

do i = 1, num_cav_unique
...
allocate(erlmat(num_cav_track, m_size, m_size))
allocate(erltime(num_cav_track))

allocate(bbu%mat_c((num_cav_track-1)*m_size*m_size))
allocate(bbu%time_c(num_cav_track))
...
bbu%num_cav_unique = num_cav_unique 
bbu%num_cav_track  = num_cav_track 
bbu%n_pass         = n_pass

write(*,*)'The total number of rf cavities with HOMs is ', bbu%num_cav_unique
...
type (bbu_info_struct) :: bbu
type (coord_struct), allocatable :: coor_t(:)
type (HOM_power) :: Hpower(:)

real(rp) :: cur, q_bunch
real(rp), allocatable :: power_t(:)
real(rp), allocatable :: coor_t_c(:)

integer i, j, ind, n_bunch_in_lat
...
n_bunch_in_lat = bbu%time_c(bbu%num_cav_track) * bbu%b_freq

print *, 'Simulation number of steps:', bbu%b_time
print *, 'Number of bunches in the lattice at any one time: ', n_bunch_in_lat

if (bbu%b_time < n_bunch_in_lat) then
  print *, 'The number of steps for the simulation is not long enough!'
  call err_exit
endif
...
type (HOM_power), dimension(:) :: Hpower
logical :: output
...
call bbu_track(bbu%current, bbu%num_cav_unique, bbu%hom_num, bbu%mat_c, bbu%time_c, bbu%hom, bbu%Q_c, bbu%RoQ_c, bbu%freq_c, bbu%angle_c, bbu%power_c, power_t, coor_t_c, bbu%b_freq, bbu%b_time, bbu%p_time, bbu%n_amp, bbu%p_amp, bbu%n_on, 0)
...
real(rp), allocatable :: coor_t_c(:)
integer :: matrixsize = 4
real(rp), ALLOCATABLE :: Newf_c(:)
real(rp), ALLOCATABLE :: hompower(:)
real(rp)  r, rr, spread
integer   i, j, k
...
! Call the c++ bbu tracking subroutine with option 1
   ...
   call bbu_track(bbu%current, bbu%num_cav_unique, bbu%hom_num, bbu%mat_c, bbu%time_c, bbu%hom, bbu%Q_c, bbu%RoQ_c, Newf_c, bbu%angle_c, hompower, power_t, coor_t_c, bbu%b_freq, bbu%b_time, bbu%p_time, bbu%n_amp, bbu%p_amp, bbu%n_on, 1)
...
allocate(start(bbu%num_cav_unique))
allocate(hompower(size(bbu%power_c)))
...
! Call the c++ bbu tracking subroutine with option 1
  ...
  call bbu_track(bbu%current, bbu%num_cav_unique, bbu%hom_num, bbu%mat_c, bbu%time_c, bbu%hom, bbu%Q_c, bbu%RoQ_c, bbu%freq_c, Newa_c, hompower, power_t, coor_t_c, bbu%b_freq, bbu%b_time, bbu%p_time, bbu%n_amp, bbu%p_amp, bbu%n_on, 1)
...
! Call the c++ bbu tracking subroutine with option 1
  ...
  call bbu_track(bbu%current, bbu%num_cav_unique, bbu%hom_num, bbu%mat_c, bbu%time_c, bbu%hom, NewQ_c, bbu%RoQ_c, bbu%freq_c, bbu%angle_c, hompower, power_t, coor_t_c, bbu%b_freq, bbu%b_time, bbu%p_time, bbu%n_amp, bbu%p_amp, bbu%n_on, 1)
