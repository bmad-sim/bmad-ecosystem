...
\begin{example}
  &params
    ...
    lux_param%intensity_min_det_pixel_cutoff = 1e-6 ! det_pix_out file cutoff

    lux_param%intensity_min_photon1_cutoff = 1e-6  ! photon1_out file cutoff
...
Bin width of histogram in meters. See \sref{s:histo.file} for more details.
  %
...
relative to the maximum pixel intensity. Default is 1e-6
  %
  \item[\vn{lux_param\%intensity_min_photon1_cutoff}] \Newline
Cutoff intensity of a photon below which the photon will not be included in
the photon1_out file (\sref{s:photon1.file}). Note: if \vn{lux_param%scale_initial_field_to_1}
is True (the default), the initial intensity is unity. Default is 1e-6
  %
...
the pixel area of the detector? Default is True.
  %
...
If \vn{photon1_element} is blank (the default), the
...
a ``\#'' character (\sref{s:number}). A blank file name (the default) will prevent a
...
the random number generator. If set to 0 (the default), 
the system clock will be used so that the output results
...
If \vn{reject_dead_to_det_photon1} is set to True (default False), 
do not print to the photon_out file any photons
...
Scale the field so that the initial photon intensity $\sqrt{E_x^2 + E_y^2}$ is unity? 
Default is True. Only used if at least one field
component is non-zero. See \sref{s:photon.descrip} for more details.
  %
  \item[\vn{lux_param\%stop_num_photons}] \Newline
Maximum number of photons to track (\sref{s:det}). Default is zero.
  %
...
The fourth data file will have the same name as \vn{det_pix_out_file} with a ``.histogram''
suffix. This file contains This file will contain a table with columns:
\begin{example}
  histogram_variable
  ... and 14 common columns ..
\end{example}
...
The width of the histogram bins is set by \vn{lux_param%histogram_bin_width}. The value given for a
histogram bin is computed using the formula:
\begin{example}
