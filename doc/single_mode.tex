...
In the following list, commands that start with a slash (\vn{/}) or hyphen (\vn{-})
use two key strokes. For example, the \vn{/v} command is involked by first pressing
the slash (\vn{/}) key followed by the \vn{v} key. Commands that start with \vn{\W}
indicate a control sequence. For example the \vn{\W v} command is involked by 
pressing the \vn{v} key while holding down the \vn{control} key.
