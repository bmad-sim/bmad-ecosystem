...
file is \zzz{xsif_parser}. There are two subroutines in \bmad to read
in a \bmad standard lattice file: \zzz{bmad_parser} and
\zzz{bmad_parser2}. \vn{bmad_parser} is used to initialize a
\vn{lat_struct} structure from scratch using the information from a
...
(\sref{s:lattice.files}) that is created.  Notice that \vn{bmad_parser}
does {\em not} compute any Twiss parameters.

\zzz{bmad_parser2} is typically used after \vn{bmad_parser} if there is
