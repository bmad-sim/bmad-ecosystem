...
\Begineq
  S_i(\Out) = \sum_{j} \sigma_{ij} \, S_i(\In)
\Endeq
where $\Bf S = (s_x, s_y, s_z)$ is the spin and $\sigma_{ij}$ are
...
for given initial orbit coordinates, The $\sigma_{ij}$ matrix must
just represent a rotation about some axis. While use of the $\sigma$
matrix increases the number of Taylor series needed from 9 to 15,
All the series are now only dependent the six orbit coordinates.
