...
\begin{example}
  ...
  \Hyperref{r:make.v.mats}{make_v_mats}(ele, v_mat, v_inv_mat)   ! Form V matrices.
\end{example}
See \sref{r:mat} for a complete listing of such routines.

Since the normal mode and projected dispersions are related, when one is changed 
within a program the appropriate change must be made to the other. To make
...
$\bfV$ matrix apearing in \Eq{tvuv}.
...
routine works by repeated calls to 
\Hyperref{r:twiss.propagate1}{twiss_propagate1} which does
...
\vn{twiss_propagate_all} also computes the twiss parameters for all the
lattice branches.
...
\index{lat_struct}
\index{lat_struct!\%a}\index{lat_struct!\%b}
...
\Hyperref{r:set.z.tune}{set_z_tune} can be
...
There are five routines that must be written to implement a custom 
element they are:
\begin{example}
   \Hyperref{r:radiation.integrals.custom}{radiation_integrals_custom}
   \Hyperref{r:em.field.custom}{em_field_custom}
   \Hyperref{r:field.rk.custom}{field_rk_custom}
