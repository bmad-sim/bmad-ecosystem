...
``\vn{action}'' subcommands which allow the user to control \tao. The other category are the
``\vn{output}'' commands which output information from \tao.
...
Most output is in ``\vn{parameter list form}'' where each line has four fields separated by semicolons:
\begin{example}
