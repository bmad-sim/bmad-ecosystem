...
about a lattice. The components of a \vn{lat_struct} are shown in
Figure~\ref{f:lat.struct}. 

Note: To denote a component of a structure the convention
``\vn{%<component>}'' will be used. For example, ``\vn{%ix_from_ele}''
or ``\vn{%branch%ix_from_ele}'' could be be used to denote the
\vn{ix_from_ele} component of a the \vn{branch} component of the
\vn{lat_struct}. This convention can be ambiguous. For example,
\vn{%n_ele_track} can refer to the component of the \vn{lat_struct}
with that name or the sub-component \vn{%branch%n_ele_track}. Which is
being referred to should be clear in context.
...
The lattice is divided up into a number of ``branches''. If the
lattice does not contain any \vn{branch} or \vn{photon_branch}
elements, there is only one branch and that branch represents the
entire lattice. The total number of branches is $N+11$ where $N$ is
the number of \vn{branch} plus \vn{photon_branch} elements. The branch
information is stored in the \vn{%branch(0:)} array. This array is
always indexed from 0. The \vn{%branch(0)} branch is the ``main''
branch from which other branches may branch. The definition of the
...
\index{key_name}
The \vn{%branch(i)%key} component is set to either
\vn{photon_branch\$} or \vn{branch\$}. The \vn{key_name()} function
will convert the integer key value to a string for printing. The
\vn{%branch(i)%ix_branch} index contains the branch index. That is,
\vn{i}. This can be useful when passing a branch to a subroutine. The
\vn{ix_branch} component.  The \vn{%branch(i)%ix_from_branch} gives
the branch index of the branch that the $i$\Th branch branched off
from. \vn{%branch(i)%ix_from_ele} gives the index in the
...
maintain compatibility with older code, the \vn{%ele(:)} array, which
is used by older code, is pointed to by the \vn{%branch(0)%ele}
array. Similarly, \vn{%branch(0)%n_ele_track},
\vn{%branch(0)%n_ele_max}, and \vn{%branch(0)%param} point to
\vn{%n_ele_track}, \vn{%n_ele_max}, \vn{%param} respectively.

The \vn{%branch%ele(:)} array is always allocated with zero as the lower
...
elements that are tracked through. The
control part holds elements whose \vn{lord_status} (\sref{s:ele.control})
is one of \vn{overlay_lord}, \vn{girder_lord},
\vn{multipass_lord}, \vn{super_lord}, or \vn{Group_lord}.  The bounds of
...
\vn{beam_start} statements (\sref{s:beam.start}) in a \bmad lattice
file. 
...
\end{example}
\vn{element_locator} searches the \vn{%ele(:)} array for an
