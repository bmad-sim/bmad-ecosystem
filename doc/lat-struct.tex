...
\Hyperref{r:attribute.bookkeeper}{attribute_bookkeeper} routine was implemented to take care of the
bookkeeping. To be back compatable with existing programs, the
\vn{attirubte_bookkeeper} routine was included in calculational
routines such as \Hyperref{r:make.mat6}{make_mat6} which is the routine to calculate the
linear transfer map for an element. The addition of lord elements
needed for superposition, multipass, etc., further complicated the
picture since the attribute values of the slave elements had to be
adjusted when the attribute values of the lord elements changed.
Again, ``automatic'' bookkeeping was added to be maintain back
compatability with existing programs. This automatic bookkeeping
system is inefficient since there is no good way to keep track of what
element attributes have been modified which leads to redundant
bookkeeping calculations.

Eventually, as bmad developed and became more complicated, it was
found that in certain instances the bookkeeping load was causing
enormous slowdowns in program execution time. To avoid this, an
``intelligent'' bookkeeping system has been developed. In order to be
back compatable with existing programs, the automatic bookkeeping is
the default. However, all new programs should use the intelligent
bookkeeping system.

To use intelligent bookkeeping, a program has to set the global
\vn{bmad_com%auto_bookkeepper} to false. This should be done once at
the start of the program. Each time an element attribute is set, the
\Hyperref{r:set.flags.for.changed.attribute}{set_flags_for_changed_attribute}
routine must be called. After a set of element attributes have been
modified, \Hyperref{r:lattice.bookkeeper}{lattice_bookkeeper} is
called to do the bookkeeping.
\begin{example}
  type (lat_struct) lat
  ...
  ...
  ...
  call lattice_bookkeeper (lat)         ! Final bookkeeping
\end{example}
