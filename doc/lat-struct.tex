...
information is stored in the \vn{lat%branch(0:)} array. This array is
always indexed from 0. The \vn{lat%branch(0)} branch is the ``main''
...
The \vn{lat%branch%ele(:)} array holds the array of elements in the
...
maintain compatibility with older code, the \vn{lat%branch(0)%ele}
array points to the \vn{lat%ele(:)} array. Space for other
\vn{lat%branch(:)%ele} arrays are allocated from heap storage. 

\index{key_name}
The \vn{lat%branch(:)%key} component is set to either
\vn{photon_branch$} or \vn{branch$}. The \vn{key_name()} function will
convert the integer key value to a string for printing. The
\vn{lat%branch(:)%ix_branch} index gives the index of 
