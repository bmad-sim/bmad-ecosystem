...
\index{not_a_child\$}\index{slice_slave\$}
...
has one or more \vn{super_lord} lords (\sref{s:super}).
  \item[not_a_child\$]\Newline
\vn{Not_a_child} elements are elements that are not \vn{super_slaves},
\vn{multipass_slaves}, nor \vn{slice_slaves}. \vn{Not_a_child} elements can, however, have
attributes that are controlled by \vn{overlay_lords}, \vn{group_lords}, or
\vn{girder_lords}.
...
one lord/slave pair. The \vn{%ix_lord} component gives the index of
...
Except for a \vn{slice_slave}, the \vn{%ic1_lord}, \vn{%n_lord}, and \vn{%n_lord_field}
components of a given slave element, along with the \vn{lat%ic(:)} array, can be used to
find the lords of the slave.  Somewhat simplified, the code for the
\Hyperref{r:pointer.to.lord}{pointer_to_lord} function is:
\begin{example}
