...
The lattice is divided up into the ``root branch'' and, if there are
\vn{branch} or \vn{photon_branch} elements, a number of other
branches (\sref{s:branch}).
...
bound. The \vn{%ele(0)} element of all branches is an \vn{init_ele}
...
for a given \vn{branch} or \vn{photon_branch} element, the index of
the branch that is being branched to and the index of the element that
is being branched to is stored in:
\begin{example}
...
\index{overlay_slave\$}\index{group_slave\$}\index{free\$}\index{slice_slave\$}
