...
about a lattice including any \vn{branch} or \vn{photon_branch} lines
(\sref{s:branch}). The components of a \vn{lat_struct} are shown in
...
The branch information is contained in the \vn{%branch(:)} array. The
\vn{%branch(0:)} array is always indexed from 0 with the $0$ branch
...
dependent variables (\sref{s:depend}) of an element are keep
...
The function \vnr{attribute_free} will
