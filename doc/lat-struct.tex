...
\vn{free\$}.  In the lord section of the lattice, \vn{ele%lord_status}
can be one of:
\begin{description}
  \item[free\$]\Newline
A free lord element does not control anything. This can be sometimes useful
when manipulating lattices but is not generally used.
\item[super_lord\$]\Newline 
A \vn{super_lord} lord element is created when elements are
...
A \vn{girder_lord} is a \vn{girder} element (\sref{s:girder}).  Such
