...
\Hyperref{r:init.lat}{init_lat} is used to initalize the lattice with a single branch. After this
...
\vn{ele%key} = \vn{ramper\$}. Note that a \vn{ramper_lord} will not have any associated slaves.
  %
...
For \vn{ramper} elements, slaves are chosen dynamically at run time. And it is up to the program to 
implement code to use rampers. \bmad provides a helper routine (\Hyperref{r:apply.ramper}{apply_ramper}) to apply a ramper element
to control any given element but \bmad itself never calls \vn{apply_ramper}. If \vn{pointer_to_slave} is
called with a \vn{ramper} lord, a nullified pointer will be returned. Looping over the slave parameters
looks like:
\begin{example}
  type (ele_struct) ramper
  type (control_struct), pointer :: ctl
  ...
  do i = 1, size(ramper%control%ramp)
    ctl => ramper%control%ramp(i)  ! ctl has the slave info
    ...
  enddo
\end{example}
See the \vn{apply_ramper} code for a fuller example.
...
Historically, as the concept of lattice bookkeeping was being developed, to be back compatible with
existing programs, calls to bookkeeping routines were added to calculational routines such as the
tracking routine \Hyperref{r:track1}{track1} and the routine for calculating the linear transfer map
\Hyperref{r:make.mat6}{make_mat6}.  This ``automatic'' bookkeeping system is inefficient since there
is no good way to keep track of what element attributes have been modified which leads to redundant
bookkeeping calculations. Eventually, as \bmad developed and became more complicated, it was found
that the unnecessary bookkeeping load was generally causing a significant slowdown in program
execution time --- even in programs where no element attributes were changed. To avoid this, an
``intelligent'' bookkeeping system was developed. In order to be back compatible with existing
programs, the automatic bookkeeping system is the default. However, given the fact that the
...
false. This is done once at the start of the program. When a set of attributes needs to be modified,
the \Hyperref{r:set.flags.for.changed.attribute}{set_flags_for_changed_attribute} routine must be
called for each element attribute that is set. After all the attributes have been set,
