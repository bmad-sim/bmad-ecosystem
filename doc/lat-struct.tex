...
\Hyperref{r:track1}{track1} and the routine for calculating the linear transfer map
\Hyperref{r:make.mat6}{make_mat6}. The addition of lord elements
...
Again, ``automatic'' bookkeeping was added to maintain back
...
found that the unnecessary bookkeeping load was causing a significant
slowdown in program execution time. Even in programs where no element
attributes were changed. To avoid this, an ``intelligent'' bookkeeping
system was developed. In order to be back compatable with
existing programs, the automatic bookkeeping is the default. However,
all new programs should use the intelligent bookkeeping system.
...
the start of the program. When a set of attributes needs to be
modified, the
