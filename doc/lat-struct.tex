...
generally handled by the \vn{bmad_parser} and \vn{lat_equal_lat}
...
General Note: Since branches are a recent introduction to bmad, at
this time, there are still some routines that have not
...
main branch, with branch index 0, the \vn{%ele(0:)} array is divided up into
...
growth rate per turn of the unstable mode.
...
Nomenclature note:
An element may be referred to by it's
...
\vn{ele%on_a_girder} logical to \vn{True}.
...
\begin{table}[tb]
 ...
 \begin{tabular}{lcccccc} 
  ...
  \hfill
  \subfigure[Possible {ele\%lord_status} and {ele\%slave_status} combinations
    for a given lord/slave pair. ``X'' marks a possible combination. ``1'' indicates
    that the slave will have exactly one lord of the type given in the column. ``Y''
    indicates the combination is not bidirectional.]
  {
  \begin{tabular}{lcccccc} 
    \toprule
...
To find the slaves and lords of a given element, use the
routines \hyperref[r:pointer.to.slave]{pointer_to_slave} and 
...
The following ordering of slaves and lords is observed:
  \begin{description}
  \item[Super_lord slaves:] \Newline
...
will point to the slave at the exit end of the \vn{super_lord}.
  \item[Multipass_lord slaves:] \Newline
...
will point to the slave of the $i$\Th pass.
  \item[Multipass_slave lords:] \Newline
...
from the optional fourth argument of the \vn{pointer_to_lord} and
\vn{pointer_to_slave} functions. Example: The following prints 
...
is given by the \vn{lord%ix1_slave} component of the lord
and the last slave is given by the \vn{lord%ix2_slave} component of the
lord. Example:
\begin{example}
  type (lat_struct), target :: lat
  type (ele_struct), pointer :: lord, slave
...
necessary bookkeeping involved after an attribute change, the
...
The advantage of using \vn{lattice_bookkeeper} is that it is simple to
call and does a complete job. The disadvantage is that, since
...
like \vn{lat_make_mat6} will call bookkeeping routines and this will
add to the execution time of a program. 
...
are:
\begin{example}
  attribute_bookkeeper   ! Bookking of attributes in a given element
  control_bookkeeper
  s_calc
  lat_geomegty
