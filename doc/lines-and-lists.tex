...
The basic lattice branch (\sref{s:branching}) is defined in a lattice
file using what are called \vn{beam lines} and \vn{replacement lists}.
The \vn{beam lines} are divided into two types - lines with and lines without
\vn{replacement arguments}. This essentially corresponds to the \mad
definition of lines and lists. There can be multiple \vn{beam lines}
and \vn{replacement lists} defined in a lattice file and lines and
...
is selected by the \vn{use} statement. The general syntax is
\begin{example}
  use, line1, line2 ...
\end{example}
Only the use of \vn{line1} is manditory. The lines \vn{line1},
\vn{line2}, etc are the lines of the machines to be simulated. For
example, \vn{line1} may correspond to one ring and \vn{line2} may
