...
\vn{Photon_Branch} elements define the source point for X-ray beams.
Example:
\begin{example}
  erl: line = (..., dump, ...)               ! Define the main lattice with a branch 
  use, erl
...
Branches are named after the branch element name. In the above
example, the branch line would be named \vn{DUMP}. Like the main
lattice, \bmad always automatically creates a zeroth element at the
beginning of the branch called \vn{BEGINNING}. The longitudinal \vn{s}
position of an element in a branch is determined by the distance from
