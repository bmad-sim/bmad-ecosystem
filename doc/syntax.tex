...
Expressions may involve arrays of values. If an array is For example:
\begin{example}
  lat::orbit.x[5:8]
...
The list of possible lattice \vn{<parameter>} names is given in \Sref{s:data.types}. The table
...
``\vn{data::orbit.x[10]}''. With the ``\vn{lat::}'' source, the element index, in this case \vn{10},
refers to the 10th lattice element. With the ``\vn{data::}'' source, ``\vn{10}'' refers to the 10\Th
element in the \vn{orbit.x} data array which may or may not correspond to the 10\Th lattice element.
