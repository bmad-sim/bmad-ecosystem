...
A data (\sref{s:data.org}) parameter ``\vn{token}'' is a string that specifies a scaler or an array
...
A variable (\sref{c:var}) parameter ``\vn{token}'' is a string that specifies a scaler or an array
...
time.  A lattice parameter ``\vn{token}'' is a string that specifies a scaler or an array of lattice
...
tracking is turned on.  A beam parameter ``\vn{token}'' is a string that specifies a scaler or an
...
There is some overlap between element parameters and lattice parameters
(\sref{s:lat.token}).  For historical reasons, the \vn{element} parameter syntax roughly
follows a convention developed for \bmad lattice files which is somewhat different from
the convention developed for \tao data. For example, the $a$-mode beta is named
\vn{beta.a} in \tao while \bmad uses the name \vn{beta_a}. See the \bmad manual for more
information on the \bmad lattice file syntax.
