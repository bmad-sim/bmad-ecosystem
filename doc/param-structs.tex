...
The \vn{bmad_common_struct} structure contains a set of global
parameters. There is only one instance
(\sref{c:param.structs}) of this structure and this instance has the
name \vn{bmad_com}. The components of this structure along with the
default values are:
...
cavity whose frequency is not comensurate with the spacing between bunches, absolute time
...
Kapchinsky-Vladimirsky distribution (\sref{s:kv.init})
\begin{example}
...
\vn{lsc_component_on} toggles on or off the longitudinal space charge
kick. 

\vn{tsc_component_on} toggles on or off the transverse space charge
kick. 
