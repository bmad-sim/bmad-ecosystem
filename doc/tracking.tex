...
\begin{example}
  type coord_struct
    ...
    real(rp) p0c        ! Reference momentum. For non-photons: Negative -> going backwards.
    real(rp) beta       ! Velocity / c_light. deltaE/E for photons.
...
The reference momentum in eV is stored in the \vn{p0c} component.  For
charged-particles, \vn{%p0c} may be negative if the particle is
traveling backwards longitudinally. The normalized velocity $v/c$ is
stored in \vn{%beta}.

For photons, the reference momentum \vn{%p0c} is always
positive and \vn{%vec(6)} ($\beta_z$) will be negative if the photon
is going backward. Since the velocity of a photon is always $c$, the 
\vn{%beta} component is used to store the energy deviation from the
reference particle. 
