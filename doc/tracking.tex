...
traveling through. That is, for an element with reversed \vn{orientation} (\vn{ele%orientation} = -1),
a particle with \vn{direction} = 1 will be traveling towards the \vn{entrance} end of the element and
with \vn{direction} = -1 the particle will be traveling towards the \vn{exit} end.
...
The \Hyperref{r:track.many}{track_many} routine can
be used for re. See the \vn{track_many} routine for more details.
...
etc. This can be done by calling \zzz{track1} and supplying the \vn{track} argument. Example:
\begin{example}
