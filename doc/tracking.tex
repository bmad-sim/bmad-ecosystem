...
$n$\Th element. Since \vn{super_lord} (\sref{s:control_type}) elements
have an associated particle position, the upper bound of the
\vn{orbit(:)} array must be at least \vn{lattice%n_ele_max}
(\sref{s:lat_struct}).  The call to \vn{reallocate_coords} does the
...
element \vn{lattice%n_ele_track} (\sref{s:lat_struct}).
...
The order of the integrator (\sref{s:taylor_phys})
...
bunches have generally focused on straight particle tracking (\sref{s:part_track}).
