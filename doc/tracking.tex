...
that was tracked through. If the element tracked through is not associated with a lattice,
\vn{%ix_ele} is set to -1. When initializing a \vn{coord_struct} (see below), \vn{%ix_ele} will be
initialized to \vn{not_set\$}.
...
the \vn{%state} component will be set to one of the other settings. The \vn{lost_neg_x_aperture\$}
setting indicates that the particle was lost at an aperture on the $-x$ side of the element. The
\vn{lost_z_aperture\$} setting is used to indicate that the particle tried to ``turn around''. This
...
The Sokolov-Ternov effect\cite{b:barber} is the self-polarization of charged particle beams due to
