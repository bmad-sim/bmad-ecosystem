...
\begin{example}
  type coord_struct
    ...
    real(rp) s_body     ! Element body longitudinal position.
...
\item{\%s_body} \Newline
The \vn{%s_body} component is used to record the longitudinal position of a particle, in element
body coordinates, as it travels through an element. \vn{%s_body} is basically a ``private'' variable
used by some tracking methods. In general, you should not rely on \vn{%s_body} to be set in any
meaningful way.
