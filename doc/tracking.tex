...
The routine \hyperref{routine!track1} is the routine that tracks through one
element in the lattice. The routine \hyperref{routine!track_all} calls \vn{track1}
in a loop over all elements to track through the entire
lattice. Alternatively the routine \hyperref{routine!track_many} can be used to
...
The \hyperref{track_all} routine serves as a good example of how tracking
...
and/or excitation use the \hyperref{routine!setup_radiation_tracking} routine.
...
this can be overridden using the routine \hyperref{routine!set_taylor_order}. The
routine \hyperref{routine!taylor_coef} can be used to get the coefficient of any
given term.
...
routine \hyperref{routine!ele_to_taylor}. This routine will kill any old Taylor map
before making any new one. To kill a Taylor map (which frees up the
memory it takes up) use the routine \hyperref{routine!kill_taylor}.
...
\hyperref{routine!concat_taylor} can be used
\begin{example}
...
\hyperref{routine!track_many} routine. See the \vn{track_many} routine for more
...
and track through that. The routine \hyperref{routine!lat_reverse} will do
...
\hyperref{routine!init_beam_distribution}. Tracking is then performed similarly to
