...
\begin{example}
  \zzz{twiss_and_track_at_s} (lat, s, ele, orb, orb_at_s, ix_branch, err, use_saved_data)
  \zzz{twiss_and_track_intra_ele}  (ele, param, l_start, l_end, track_entrance,
...
routine \zzz{create_element_slice} can be used to create such slices.
...
\caption[]{Example trajectories in the element coordinate system. Note that the wall and apertures are aligned at $s_{\rm ele} =0$, but offset at $s_{\rm ele} = L.$}
...
%======================TABLE=============================
\begin{table}[tb]
\caption[]{Possible aperture flags set when tracking an individual element. Trajectories are labeled as in Fig.~\ref{fg:tracking-apertures}}
