...
to the forked \vn{branch} is not ``automatic''. That is, since the
...
tracked is simply transfered from the forking element to the forked
...
\begin{example}
  ...
  type (branch_struct) :: to_branch  ! Given forked branch
  ...
  if (ib_from < 0) then
    ...
    call track_a_patch (fork_ele, to_orbit(0))
...
\begin{example}
  type (lat_struct), target :: lat   ! Lattice 
  type (branch_struct), pointer :: to_branch  ! Forked branch
  ...
  call track_a_patch (fork_ele, to_orbit(to_ele%ix_ele))
\end{example}  
...
For static fields tracking backwards is simply the reverse of tracking
...
damping or excitation or RF cavities break this symmetry.
