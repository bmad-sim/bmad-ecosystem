...
misalignments. [Due to space constraints the \vn{phi} and \vn{psi} columns are not shown. They are
...
coordinates of the nominal positions of the elements can be seen by using the \vn{show lat -floor}
command:
\begin{code}
Tao> show lat -floor
...
energy at the exit end of an \vn{lcavity} is set so that a particle entering the cavity with zero
phase space coordinates leaves with zero phase space \vn{pz}.
On the other hand, \vn{rfcavity} elements do not affect the reference energy. Example:
\begin{code}
...
starting one. With the \vn{rfcavity} element, the \vn{pz} coordinate at the end of the element is
now nonzero.
