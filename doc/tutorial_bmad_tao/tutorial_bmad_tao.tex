...
set using a \tao initalization file (\sref{s:init.file}) or by using the ``\vn{set plot_page}''
...
description file(s). Parameters onf this lattice are never varied.
...
supply that powers a chain of magnets can be simulated with a control element.  \vn{Girder} elements
are used for simulating support structures. For example, a \vn{girder} can simulate an I-beam that
supports a number of mangnets or can be used to simulate an optical table.
...
misalignment can be viewd using the \vn{-floor} option with the \vn{show element } command:
\begin{code} 
...
Figure~\ref{f:patch.example}.  By default, \vn{patch} elements are not drawn but the global
coordinates of the nominal positions of the elements can be seen by using the \vn{show lat -fllor}
command:
\begin{code}
Tao> show lat -floor
...
Start \tao as explained in section~\sref{s:tao.start} with the lattice file \vn{orbit.bmad}. The
lattice file specifies a non-zero initial orbit using \vn{beam_start} parameters. The resulting
...
\tao initialization files are used to set inital parameters like the name of the lattice file to be
used, what is to be initally plotted, etc. Initialization files are also used to define \vn{data}
(\sref{s:data}) and \vn{variables} (\sref{s:variables}) that are used for \vn{optimization} (\sref{optimization}).
...
Mode'. The two properties to set are %key_bound and %key_delta. In the input file, adding these
properties as:
\begin{code}
&tao_var
  v1_var%name = 'quad'
...
To be able to do optimization in \tao, you must create appropriate initalization files
(\sref{s:init.file}) that define the \vn{data} (\sref{s:data}) that is used to construct the merit
function and the \vn{variables} (\sref{s:variables}) which will be varied in the optimization
process. Once everything has been setup, the \vn{run} command will start the \vn{optimization}
process.
