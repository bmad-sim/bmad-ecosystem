...
\vn{cylindircal_map} decomposition is discussed in \sref{s:cylind.map}.
...
the \vn{cartesian_map} attribute as exaplined in Sec.~\sref{s:cart.map}.
...
attribute as exaplined in Sec.~\sref{s:cylind.map}.

The \vn{cylindrical} decomposition takes one of two forms depending upon wheather the
fields are time varying or not. The DC decomposition is explained in
Sec.~\sref{s:cylind.dc} while the RF decomposition is exaplained in
...
The DC \vn{cylindrical} parameterization used by \bmad essentially
...
(See \vn{default_integ_order} in \sref{s:bmad.params}).  [Note: Each Taylor field model
