...
\begin{align}
  ...
  \label{egot}
\end{align}
where $G$ is the accelerating gradiant, $k = c / \omega$ is the wave number with $\omega$ beging the
RF frequency.

For standing wave cavities, the RF fields are modeled as $N$ half-wave pillbox cells, each having a
length of $\lambda/2$ where $\lambda = 2 \, \pi / k$ is the wavelength. If the length of the RF
element is not equal to the length of $N$ cells, the active region is centered in the element and
the regions to either side are field free.\footnote{If the length of the cavity is less than $N$
cells the field free region has a negative length. This is equivalent to a drift element with
negative length and does not cause any problems during tracking.}

The field in the standing wave pillbox is modeled either with a $p = 0$ or $p = 1$ longitudinal mode
as detailed in Chapter 3, Section VI of reference \cite{b:lee}. The $p = 1$ longitudinal mode (which
is the default if the longitudinal mode is not set in the element), models the fields with the transverse wall at
infinity. In this case, the fields are
\begin{align}
...
entering with $\phi = 0$ will experience an average gradiant equal to $G$.

For the $p = 0$ longitudinal mode, the fields are modeled using a TM$_{010}$ mode giving a wall
radius at $r_{wall} = 2.405 / k$ and the fields are
\begin{align}
  E_s(r, \phi, s, t)    &= \frac{\pi}{2} \,      G \, J_0(k \, r) \sin(\omega \, t + 2 \, \pi \, \phi) \\
  B_\phi(r, \phi, s, t) &= \frac{\pi}{2 \, c} \, G \, J_1(k \, r) \cos(\omega \, t + 2 \, \pi \, \phi) \nonumber
  \label{egot3}
\end{align}
