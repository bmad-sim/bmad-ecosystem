...
\Endeq
where $r$ is the distance, in the plane of the bend, from the bend center to the
observation point.  In the equations that follow, $r$, and $y$ will be normalized by the
bending radius $\rho$. With this convention, the normalized ``horizontal'' distance $x$
from the reference trajectory is related to $r$ by $r = 1 + x$.
...
The $n$\Th order solutions to Laplace's equation can be found in form
\Begineq
  \phi_{n}^r = \sum_{p = 0}^{\lfloor (n+1)/2 \rfloor} 
             \begin{pmatrix} n + 1 \cr 2 \, p \end{pmatrix} \,
             (-1)^{p+1} \, F_{n+1-2p}(r) \, y^{2p}
  \label{pspn1}
\Endeq
and in the form
\Begineq
  ...
  \label{pspn2}
\Endeq
[Notice that here $n$ is related to $m$ in McMillian's paper by $m = n + 1$.]
where $\lfloor\alpha\rfloor$ means round down to the integer equal to or less than $\alpha$,
and the $F_p(r)$ are related by 
\Begineq
...
For magnetic fields, the $\phi_n^r$ solutions will correspond to skew fields and the
$\phi_n^i$ solutions will correspond to normal fields
\Begineq
  \bfB = \sum_{n = 0}^\infty \left[ \frac{b_n}{n+1} \nabla \phi_n^r + \frac{a_n}{n+1} \nabla \phi_n^i \right]
\Endeq
For electric fields, the reverse is true
\Begineq
  \bfE = \sum_{n = 0}^\infty \left[ \frac{b_{en}}{n+1} \nabla \phi_n^i + \frac{a_{en}}{n+1} \nabla \phi_n^r \right]
\Endeq
...
\begin{align}
  \Phi_n^r &= \phi_n^r + \sum_{k = n+1}^\infty C_{nk} \, \phi_{k1} \CRNO
  \Phi_n^i &= \phi_n^i + \sum_{k = n+1}^\infty D_{nk} \, \phi_{k2}
\end{align}
The $C_{nk}$ and $D_{nk}$ are chosen, order by order, so that $\Phi_n^r$ and $\Phi_n^i$
are horizontally pure (see McMillan). Unlike the vertically pure solutions, a horizontally
pure $\Phi_n$ has an infinite number of terms. 
