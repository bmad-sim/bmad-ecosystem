...
straight line elements, given in analogy to \Eqs{vwlv} and \eq{wws} by
\begin{align}
...
can be calculated by concatination, using \Eqs{vwlv} and \eq{wws}, of
...
element. This is just \Eqs{l00l} with the replacement $L \rightarrow
L/2$. The second transformation is given by \Eqs{swww} above.  The
...
\Eqs{l00l} with the replacement $L \rightarrow -L/2$.
...
to ``\vn{origin}'' coordinates. In this case, the transformation is
\begin{align}
  \bfL &= 
    \begin{pmatrix} 
...
and the bend position (\sref{ss:bend.orient}). The transformation at
...
coordinates at the center of the bend. This is just \Eqs{ltl} with the
replacement $\alpha_b \rightarrow \alpha_b/2$. The second
transformation is given by \Eqs{lcmis}. The last transformaion is from
