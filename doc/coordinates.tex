...
coordinate system for particles as shown in \fig{f:local.coords} and
for orientating lattice elements in space. At a given time $t$, a
particle's position can be described by a point on the reference orbit
a distance $s$ relative to the reference orbit's zero position plus a
transverse offset. This point on the reference orbit is used as the
origin of the local $(x, y, z)$ coordinate system with the $z$--axis
tangent to the reference orbit. The $z$--axis will generally be
pointing in the direction of increasing $s$ but, as discussed in
detail below, will point counter to $s$ for elements that are
reversed. The $x$ and $y$--axes are perpendicular to the reference
orbit. As will be shown later, If the lattice has no vertical bends,
the $y$--axis is in the vertical direction and the $x$--axis is in the
horizontal plane. Notice that by construction, the particle is always
at $z = 0$. The coordinate system so constructed is called the
\vn{``local coordinate system''} or sometimes the \vn{``laboratory
coordinate system''} when there is need to distinguish it from the
\vn{``element coordinate system''} (\sref{s:misalign.track}).
...
\caption[Local reference frame with respect to an element.]{The local
coordinate reference frame at the entrance and exit ends of a lattice
element. A) For straight line elements the two frames are colinear. B)
For bends elements, the two frames are rotated with respectto each
other. With a zero \vn{tilt}, the rotation axis between the entrance
and exit frams is the $y$-axis. The physical element may be displaced
in the local coordinate frame using offsets, tilt, and pitches.}
...
\begin{figure}[tb]
  ...
  \caption[Element longitudinal orientation.]{
Element longitudinal orientation. A) For normal (non-reversed)
elements the $z$-axis is parallel with the $s$-axis. B) For reversed
elements the $z$-axis is antiparalled with the $s$-axis. By
definition, the ``entrance'' and ``exit'' ends of an element are fixed
relative to the element (realtive to the $z$-axis) while the
``upstream'' and ``downstream'' ends are fixed relative to the
$s$-axis.}
  \label{f:stream}
\end{figure}
...
and \vn{floor_shift} elements, are ``straight'' as shown in
\fig{f:ele.coord.frame}A. That is, the reference curve is a straight
line segment. \vn{patch} (\sref{s:patch}), and \vn{floor_shift}
(\sref{s:floor.ele}) elements can arbitrarily orient the entrance
point of an element with respect to the exit point of the previous
element. 

To 

If an element is {\em not} reversed (\sref{s:ele.reverse}), the
upstream and downstream ends of the element correspond to the entrance
and exit ends of the element.  If an element is reversed then entrance
and exit ends are reversed with respect to upstream and downstream.
In a branch, the reference orbit is constructed by arranging the
elements downstream point of one element coincides with the reference
curve at the upstream point of the next element.  The reference orbit
is then the sum of the reference curves. If not specified otherwise
(\sref{s:beginning}), the $s = 0$ position is the entrance point of
the first element.

For a given lattice branch, if there are no \vn{fiducial} elements
\sref{s:fiducial}, The local reference frame is constructed beginning
at the first element in the branch. If there are \vn{fiducial}
elements, the local referece frame is constructed beginning at these
elements.
...
as shown in \fig{f:global.coords}. These threee angles are defined as
follows:
\begin{description}
...
%-----------------------------------------------------------------------------
\subsection{Patch, Floor_Shift and Fiducial Elements}
\label{s:patch.coords}

\index{patch}\index{tilt}\index{floor_shift}
The shift in the reference coordinates from the entrance end to the
exit end of\vn{patch} (\sref{s:patch}) or \vn{floor_shift}
\vn{s:floor.ele} element is given by \Eqs{vwlv} and \eq{wws}. These
equations are also used to calculate the shift in the reference
coordinates between the origin reference coordinates of a
\vn{fiducial} element and the reference coordinates of the
\vn{fiducial} element itself.

The $\Bf L$ vector in the transformation is
\Begineq
  \Bf L = 
  \begin{pmatrix}
    x_{\text{offset}} \,
  ...
  \label{xyz}
\Endeq
The $\Bf S$ matrix used for the coordinate transformation is defined to be
\Begineq
...
\begin{example}
  ...
  \(\Psi\) = tilt
\end{example} 
Note: The form of \Eq{swww} was chosen to be similar to \Eq{wwww}, 
