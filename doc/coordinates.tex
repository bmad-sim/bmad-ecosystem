...
the phase space $z$ coordinate (\sref{s:phase.space}).  The curvature
...
$\theta_g$ is the graze angle of the incoming (entrance) reference coordinates,
...
\vn{mirror} element (\sref{s:mirror}) reflects them. With a crystal
setup for Bragg diffraction and for a mirror, the reference orbit is
modeled as a zero length bend with $\bftilde L = (0, 0, 0)$.  The
total bending angle is
\begin{align}
...
form a plane which is called the element's \vn{bend
plane}. Calculation of the graze angles is given in
Section~\ref{s:crystal.ref}.
...
For a crystal setup for Laue diffraction the entrance coordinates are
offset from the exit coordinates due to the thickness of the crystal
as shown in Fig.~\ref{f:laue.ref}. In this case \vn{graze_angle_in}
and \vn{graze_angle_out} are defined with respect to the surface
normal so that Eq.~\ref{agg} will be true for both Bragg and Laue
diffraction. With Laue diffraction, $\Bf S$ is, like for Bragg
diffraction, calculated from Eqs.~\ref{r00ls} and \ref{lrca1}. The
$\Bf L$ vector is determined by the orientation of the $\Bf H$ vector
and the thickness of the crystal as discussed in
Section~\ref{s:crystal.ref}. Without any \vn{tilt}, the $x$-$z$ plane
of the incoming reference coordinates will be parallel to the $x$-$z$
plane of the outgoing coordinates. However, if the the \vn{psi_angle}
(\sref{s:crystal}) of the crystal is non-zero, these planes will not coincide.
...
reference velocity).  If the particle's velocity is constant, and is
