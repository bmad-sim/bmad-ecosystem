...
tangent to the reference orbit and pointing in the direction of
increasing $s$. The $x$ and $y$--axes are perpendicular to the
reference orbit. As will be sown later, If the lattice has no vertical
...
always at $z = 0$. The coordinate system so constructed is called the
...
the reference orbit of the lattice branches are calculated in order
starting from the first branch (branch \# 0).

For a given lattice branch, assuming there are no \vn{fiducial} elements,
The local reference frame is constructed beginning at the first element in
the branch. 

\index{sbend}\index{rbend}
\index{crystal}\index{mirror}\index{entrance_end}
\index{exit_end}\index{patch}\index{floor_shift}
a lattice branch is comprised of a sequence of elements such
as quadrupoles, bends, RF cavities, etc. Each element has an entrance
