...
%-----------------------------------------------------------------------------
\subsection{Fiducial, Girder, Patch, and Floor_Shift Elements}
\label{s:patch.coords}

\index{patch}\index{tilt}\index{floor_shift}\index{girder}
...
\vn{fiducial} or \vn{girder} element and the reference coordinates of
the element itself.

The $\Bf L$ vector in the transformation for \vn{patch} or \vn{floor_shift}
elements is:
\begin{example}
  \(\bf L\) =  (x_offset, y_offset, z_offset)
\end{example}
For \vn{fiducial} and \vn{girder} elements it is
\begin{example}
  \(\bf L\) = (dx_origin, dy_origin, dz_origin)
\end{example}

The $\Bf S$ matrix used for the coordinate transformation is defined to by
\Begineq
...
with
\begin{example}
       patch \& floor_shift     fiducial \& girder
       -------------------     -----------------
  \(\Theta\)   = x_pitch                dtheta_origin
  \(\Phi\)   = y_pitch                dphi_origin
  \(\Psi\)   = tilt                   dpsi_origin
\end{example} 
Note: The form of \Eq{swww} was chosen to correspond to the form of \Eq{wwww}, 

%-----------------------------------------------------------------------------
\section{Reflection Patch}
...
only of practical concern when doing partial tracking through lattice
elements.
