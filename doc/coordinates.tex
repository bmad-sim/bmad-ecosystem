...
\Begineq
  ...
  \label{ltl}
\Endeq
where $\alpha_b$ is the bend \vn{angle} (\sref{s:bend}),
$\theta_t$ is the \vn{ref_tilt} angle (\sref{s:offset}), and
\Begineq
  \bftilde L = 
  \begin{pmatrix}
  ...
  \label{lra1}
\Endeq
with $\rho$ being the bend radius (\vn{rho}). 
Without a \vn{ref_tilt}, $\bfT$ is the unit matrix
...
%-----------------------------------------------------------------------------
\subsection{Straight Element Misalignment}
\label{s:straight.mis}

The \vn{Element Body} coordinates are the coordinate system attached
to an element. Without any misalignments, where \vn{``misalignments''}
are here defined to be any offset, pitch or tilt (\sref{s:offset}),
...
$E_s$. Since misalignments are defined with respect to the middle of
the element, the transformation between $\Lambda_s$ and $E_s$ is a
three step process:
\Begineq
...
%-----------------------------------------------------------------------------
\subsection{Bend Element Misalignment}
\label{s:bend.mis}
...
For \vn{rbend} and \vn{sbend} elements there is no \vn{tilt}
attribute.  Rather, there is the \vn{roll} attribute and a
\vn{ref_tilt} attribute. The latter affects both the reference orbit
and the bend position (\sref{s:bend.orient}). Furthermore,
\vn{ref_tilt} is calculated with respect to the coordinates at the
beginning of the bend while, like straight elements, \vn{roll},
offsets, and pitches are calculated with respect to the center of the
bend. The different reference frame used for \vn{ref_tilt} versus
everything else means that five transformations are needed to get from
the laboratory frame to the element frame (see
\Eq{llee}). Symbolically:
\Begineq
  \Lambda_s \longrightarrow \Lambda_\text{mid} 
...
The first transformation, $\Lambda_s$ to $\Lambda_\text{mid}$, from
laboratory coordinates at $s$ to coordinates at the center is a
rotation around the center of curvative of the bend and is given by
\Eqs{vwlv} and \eq{wws} with \Eqs{ltl} and \eq{lra1} with the
substitution $\alpha_b (\rightarrow L/2 - s)/\rho$.

The second transformation $\Lambda_\text{mid}$ to $\Omega_\text{mid}$
at the center of the element adds in the misalignments (Note that the
coordinate frame $\Omega_\text{mid}$ is neither a laboratory frame or
an element frame so hence the use of a different symbol $\Omega$). The
...
to the beginning. Again \Eqs{ltl} and \eq{lra1} are used with the 
...
That is, \Eqs{ltl} and \eq{lra1} are used with the substitutions
...
elements, \Eqs{swww} are used to calculate the the shift
...
%-----------------------------------------------------------------------------
\subsection{Backward Tracking and Reversed Elements}

\bmad has the capability of tracking particles backwards in $S$ from the end of the
lattice to the beginning. Tracking in this case is still forward in time. The canonical
momenta $p_x$ and $p_y$ (\Eq{ppp}) with backward tracking, has the same sense as forward
tracking. That is, $p_x$ and $dx/dt$ have the same sign and $p_y$ and $dy/dt$ will have
the same sign. Also, for both forward and backward tracking, a particle with a positive
canonical $z$ coordinate (\Eq{zbctt}) will arrive ahead in time relative to the reference
particle.

As an illustration of backwards tracking, consider a machine with static magnetic fields
and no electric fields. Consider a particle starting at some point $s_1$ and tracked
forward to some point $s_2$. Now, in order to track the particle backwards from $s_2$ to
$s_1$, with the particle following the same transverse orbit backwards as forwards, the initial
$p_x$ and $p_y$ momenta for the backwards track must be reversed in sign from the $p_x$
and $p_y$ of the forward track at $s_2$, and the charge of the particle has to be
reversed. To have the particle have the same ending $z$ value at $s_1$ at the finish of
backwards tracking, the initial $z$ value for backwards tracking must be negative of the
$z$ value at $s_2$ for forward tracking.

The direction of propagation of a particle being tracked backwards through an unreversed
(\sref{s:ele.reverse}) element is opposite the direction of the local $z$-axis. Similarly, the
direction of propagation of a particle being forward tracked is opposite the local
$z$-axis in reversed elements (see \sref{s:ex.patch}). This is important to keep in mind when
