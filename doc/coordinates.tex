...
The $z$ coordinate shown in the figure is not to be confused
with the phase space $z$ discussed in Section~\sref{s:phase.space}.
}
...
coordinate system''} (\sref{s:misalign.track}) which is attached to
...
two frames are rotated with respectto each other. C) For \vn{patch}
...
\begin{figure}[tb]
  ...
  \caption[Element LEGO block concatination.]{
Element LEGO block concatination. A) The reference orbit is
...
For reversed elements the $z$-axis is antiparalled with the
$s$-axis. By definition, the ``entrance'' and ``exit'' ends of an
element are fixed relative to the element (realtive to the $z$-axis)
...
\vn{tilt}, the rotation axis between the entrance and exit frames of a
...
\vn{branch} or \vn{photon_branch} element from which the branch forks
from.
...
is used as the \vn{upsteam} end the \vn{exit} end is used as the
...
\vn{floor_shift} elemeent, does not make physical sense since a
...
If there are \vn{fiducial} elements, the local referece frame is
...
condition is equivalent to the condition on $\bfS$ of
\Begineq
  S(3,3) < 0
  \label{s330}
\Endeq
