...
(that is, position and orientation shifts). Each lattice element has ``\vn{body}'' coordinates which are attached
to the physical element. That is, the electric and magnetic fields of an element are described with respect to \vn{body} coordinates.
If there are no misalignments, the \vn{body} coordinates are aligned with the \vn{local}
coordinates. With misalignments, \vn{body} and \vn{local} coordinates will not coincide.
When discussing \vn{local} vs \vn{body} coordinates, it can be less confusing to use the name
``\vn{laboratory}'' cordinates instead of \vn{local} coordinates.

\begin{figure}[!b]
  \centering
  \includegraphics[width=5.5in]{coordinates.pdf}
...
Each lattice element has \vn{body} coordinates. Generally the \vn{body} coordinate
are the same as the \vn{local} coordinates but if an element is ``misaligned'' (for
illustration purposes, the misalignment shown is much larger than typical),
the two will not coincide.}
  \label{f:coords}
\end{figure}
