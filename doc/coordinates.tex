...
%-----------------------------------------------------------------------------
\subsection{Patch and Floor_Shift Elements}
\label{s:patch.coords}

\index{patch}\index{tilt}\index{floor_shift}
A \vn{patch} (\sref{s:patch}) or \vn{floor_shift} \vn{s:floor.ele}
element shifts the reference orbit.  \Eqs{vwlv} and \eq{wws} are used
to propagate $\Bf W$ and $\Bf V$ through a \vn{patch}. The $\Bf L$
vector for a patch is
\Begineq
  \Bf L = 
  \begin{pmatrix}
    x_{\text{offset}} \,
