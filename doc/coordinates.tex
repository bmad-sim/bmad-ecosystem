...
Fig.~\ref{f:local.coords}. The momenta are defined as
\begin{align}
...
some ``element clock''. \bmad has two modes for calculating this time
called \vn{relative time tracking} and \vn{absolute time
tracking}. 

With \vn{relative time tracking}, which in \bmad is the default, the
element clock is determined by the reference particle.
...
phase space coordinate as discussed in \sref{s:lcav} and
\sref{s:rfcav}. In a storage ring or recirculating linac a particle
