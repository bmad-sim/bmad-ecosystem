...
This is not to be confused with the phase space $z$ coordinate (\sref{s:phase.space}).
The curvature vector $\Bf G$ has a magnitude of $1/\rho$ where $\rho$ is the bending radius.}
...
\begin{figure}
\centering \includegraphics{mirror.pdf} \caption[Reflection by a
mirror or crystal] {Reflection by a mirror or crystal.  The geometry
shown here is appropriate for a tilt angle of $\theta_t = 0$.
$\theta_g$ is the graze angle of the incomming (entrance) reference coordinates,
$\alpha$ is the bend angle of the coordinates.  Point $\cal O$ is the
origin of both the local coordinates just before and just after the
reflection.}  \label{f:mirror}
\end{figure}

%-----------------------------------------------------------------------------
\subsection{Crystal and Mirror Elements}
\label{s:mirror.coords}

\index{crystal}\index{mirror}\index{tilt}
A \vn{crystal} element (\sref{s:mirror}) diffracts photons and a
\vn{mirror} element (\sref{s:mirror}) reflects them.  In both coses,
the reference orbit for such elements are modeled as zero length bends
with $\widetilde \Bf L = (0, 0, 0)$.
The total bending angle for a \vn{crystal} is 
\begin{align}
  \alpha &= \mbox{graze_angle_in} + \mbox{graze_angle_out} &\mbox{! Crystal} \CRNO
  \alpha &= 2 \, \mbox{graze_angle}                        &\mbox{! Mirror}
  \label{agg}
\end{align}
The orientation
of the exit coordinates (the local coordinates after the reflection)
are only affected by the element's tilt and graze angle parameters and
is independent of all other parameters such as the radius of
curvature of the surface, etc. The local $z$-axis of the
entrance coordinates along with the $z$-axis of the exit coordinates
form a plane which is called the elements's \vn{bend plane}.

An example is shown in Fig.~\ref{f:mirror}. This example is
appropriate for a tilt angle of $\theta_t = 0$ (the rotation axis is
here the $y$-axis). Since the element is modeled to be of zero length,
the origin points (marked $\cal O$ in the figure) of the entrance and exit
local coordinates are the same.
...
\begin{align}
  \omega &= \hbar \, c \, P_0 (1 + p_z)
  k &= \frac{2 \, \pi}{\lambda} = \frac{\omega}{c}
\end{align}

In addition to the above, a photon has
four extra parameters $I_x, \phi_x$, and $I_y, \phi_y$ specifying the
intensity and phase of the field along the $x$ and $y$ axes. Since
$I_x$ and $I_y$ are intensities, the electric and magnetic field
components are proportional to the square root of these
quantities. The normalization between field and intensity is
determined by the individual simulation programs and is not part of
the \bmad standard. The electric field oscillates as
\Begineqs
  E_x &\sim \sqrt{I_x} \, e^{i (k \, x - \omega \, t + \phi_x)} \CRNO
  E_y &\sim \sqrt{I_y} \, e^{i (k \, x - \omega \, t + \phi_y)} \CRNO
\Endeqs
and this defines the phases $\phi_x$ and $\phi_y$.
