...
``synchronized'' with elements whose fields are oscillating so it is
the canonical $z$ that will determine what the actual field a particle
will see when traveling through such an element. For example, the
energy change of a particle traveling through an \vn{lcavity} or \vn{rfcavity} will
depend upon $z$ (See \Eq{egl2p} and \Eq{eev2p}).
