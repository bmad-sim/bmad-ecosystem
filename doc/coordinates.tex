...
positive bend angle $\alpha_b$ and for \vn{ref_tilt} angles $\theta_t$
...
(\sref{s:offset}). Without a \vn{ref_tilt}, $\Bf T$ is the unit matrix
...
\begin{align}
  E_x (\Bf r, t) &\sim E_x \, e^{i (k \, (z - z_0) - \omega \, (t - t_0) + \phi_x)} \CRNO
  E_y (\Bf r, t) &\sim E_y \, e^{i (k \, (z - z_0) - \omega \, (t - t_0) + \phi_y)} \CRNO
\end{align}
where $z_0$ and $t_0$ are the photon $z$ position and time
respectively.  The use of $z_0$ and $t_0$ in the above equation is one
of convienience and the photon serves as the origin for the
