...
After lattice expansion (\sref{s:expand}), there are several ways to
specify elements. The general syntax is:
\begin{example}
  \{class::\}{branch>>}element_id{##N}
\end{example}
where \vn{class} is a class name, \vn{branch} is a branch name or
index (\sref{s:branching}), \vn{element_id}

The element index can be used...

After lattice expansion (\sref{s:expand}) a lattice can have multiple
elements of the same name and sometimes it is convenient to be able
single out a particular element.  The general


Sometimes it is convenient to be able to select multiple elements
...
and \vn{``\%''} can be used to can be used (\sref{s:ele.names}). Examples:
\begin{example}
