...
accelerator or storage ring. In \bmad, a lattice defines
...
XSIF\cite{b:xsif}. XSIF (\sref{s:xsif}), developed at SLAC,
has its own documentation and will not be discussed in detail
here. Throughout this manual, unless explicitly stated otherwise,
the lattice format under discussion will always pertain to the \bmad
standard format. 

While \bmad cannotdirectlhy read in \mad files, 
translation between \mad and \bmad lattice files is possible using
the \vn{Universal Accelerator Parser} (\sref{s:aml}).
