...
(1 - 2(\bfn \dotproduct \what\bfs)^2/9)$ can be well approximated by the integrals over the closed
orbit ignoring the finite beam size.  Finally, for most rings, the integral of $g^3 \what\bfb \dotproduct
\partial\bfn/\partial\delta$ is generally small compared to the integral of $g^3 \, \what\bfb
\dotproduct \bfn$ since, in most of the machine, $\what\bfb$ and $\bfn$ will point in the vertical
direction and $\partial\bfn/\partial\delta$ will be perpendicular to the vertical (see \Eq{qq0}).
...
frequency $\kappa$. This is called the ``\vn{resonance}'' coordinate system or the ``\vn{rotating}''
...
\begin{equation}
  ...
  \label{sr0n}
\end{equation}
For the linear approximation to be valid, it must be true that $N \, \epsilon_r \ll 1$. \Eq{sr0n}
...
unbounded. \Eq{eN12n} will still be valid even when there are multiple resonances present since
\Eq{eN12n} must be evaluated on a particular resonance and, it will be seen, the contribution to
\Eq{eN12n} from non-resonant resonances is zero.
...
\begin{equation}
  \xi_{r1} = \frac{\sqrt{2}}{2\pi} \big| \bf{\cal G} \cdot \bfv_k \big| 
  \label{x12pq1}
\end{equation}
and for the difference resonance the strength is
\begin{equation}
  \xi_{r2} = \frac{\sqrt{2}}{2\pi} \big|
