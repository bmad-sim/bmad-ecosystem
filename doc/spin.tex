...
\begin{equation}
  ...
  \label{ejx}
\end{equation}
where $\xi_r$ is the normalized resonance strength. The normalized resonance strength can be related
to the first order spin eigenvectors $\bfn_k$ (\Eq{qnnq}) by comparing \Eq{nsdl} with the equations
...
eigenvector, $\bfv_k$, normalized using \Eq{vsvi0}, represents particle motion with unit action
$J_k$ for that mode. Considering \Eq{ndan}, if only one mode is excited for some particular $k$ and
with some amplitude $\sqrt{J}$, the invariant spin field on the $m$\Th turn is
\begin{equation}
