...
(\sref{s:spin.map}). A quaternion can also be represented in the form:
\begin{equation}
...
The quaternion conjugate $\bfq^*$ is defined
by\footnote
  {
...
with the property
\begin{equation}
  (\bfq_1 \, \bfq_2)^* = \bfq_2^* \, \bfq_1^*
\end{equation}
The quaternion inverse $\bfq^{-1}$ is related to the quaternion conjugate by
\begin{equation}
  \bfq^{-1} = \frac{\bfq^*}{||\bfq||^2}
\end{equation}
where the norm $||\bfq||$ is given by
\begin{equation}
...
In quaternion notation, two rotations $\bfa$ followed by $\bfb$ are combined into one rotation $\bfc$
using the formula
\begin{align}
...
\end{align}
for more information see \cite{b:quat}.
...
\begin{equation}
  ...
  (\arrowbfq \dotproduct \bfv_k) \, \bfn_0 \, \bfq_0^* + 
  \bfq_0 \, \bfn_0 \, (\arrowbfq^* \dotproduct \bfv_k)
  \label{lnqnq}
\end{equation}
...
%-----------------------------------------------------------------   
\section{Single Resonance Analysis}
...
\begin{equation}
  \Phi = Q \, \theta + \Phi_0
\end{equation}
with $\Phi_0$ being the starting phase. \Eq{stos} can be solved by transforming to a coordinate
system rotating about the $z$-axis with a rotational frequency $Q$. This is called the ``\vn{resonance}''
coordinate system or the ``\vn{rotating}'' coordinate system. In this frame, the spin vector
$s_R$ is
\begin{equation}
...
tilted a maximum of $\tan^{-1}(\epsilon_r/\delta)$ away from the $z$-axis. This shows that the
...
\Eq{nsdl} shows that resonance is when $\delta = 0$ or $\nu_0 = Q$.  The use of a sinusoidal
...
At resonance, with $\delta = 0$, \Eq{stos2} shows that if a spin in the resonance coordinate system is initially
aligned along the $\bfn_0$ axis ($\bfs_R = (0, 0, 1)$), After $N$ turns, in the linear approximation, the spin
will be
\begin{equation}
  \bfs_R(N,\bfn_0) = (0, -N \cdot 2 \, \pi \, \epsilon_r, 1)
  \label{sr0n}
\end{equation}
For the linear approximation to be valid, it must be true that $N \, \epsilon_r \ll 1$. \Eq{sr0n} gives a physical
interpretation to the resonance strength $\epsilon_r$. The resonance strength is the angle (modulo $2 \, \pi$) 
that a spin is tipped away from $\bfn_0$ in one turn. For a resonance to be small, the resonance
...
Inversion of \Eq{sr0n} shows how to compute the strength of a linear resonance:
\begin{equation}
  \epsilon_r = \lim_{N \rightarrow \infty} \, \frac{1}{2 \, \pi \, N} \, 
  \left| \bfs_R(N,\bfn_0) - \bfn_0 \right|
  \label{eN12n}
\end{equation}
where the right hand side must be computed on resonance and within the linear approximation.
$\bfs_R(N, \bfn_0)$ is the spin after $N$ turns given an initial spin of $\bfn_0$.
$\bfs_R(N, \bfn_0)$  can be computed via
\begin{equation}
  \bfs_R(N, \bfn_0) = \bfQ_N \, \bfn_0 \, \bfQ_N^*
  \label{sQnQ}
\end{equation}
...
\begin{equation}
  ...
  \label{q12en}
\end{equation}
where $\bfn_0^-$ and $\bfn_0^+$ are defined as
\begin{equation}
  \bfn_0^\pm \equiv \frac{1}{2} \, \big( 1 \pm i \,
  (n_{0,x} \, \cali + n_{0,y} \, \calj + n_{0,z} \, \calk)
  \big)
  \label{n1in}
\end{equation}
For later use, the following identities are useful
\begin{alignat}{2}
   &\bfn_0^+ \bfn_0^+ = \bfn_0^+, \qquad
  &&\bfn_0 = i (\bfn_0^- - \bfn_0^+), \\
   &\bfn_0^- \bfn_0^- = \bfn_0^-, \qquad
  &&\bfn_0^+ \bfn_0^- = \bfn_0^- \bfn_0^+ = \Bf 0
  \nonumber
\end{alignat}

For a particular mode of oscillation, $\bfr_j$ is given by \Eq{x2jv}. 
Without loss of generality, $\phi_0$ in \Eq{x2jv} will be set to zero.
Using this along with the above equations gives
\begin{align}
  \bfZ = \lim_{N \rightarrow \infty} 
    \frac{\sqrt{J}}{2 \pi N} 
    \sum_{j = 0}^{N-1} \Big[
    &\bfn_0^+ (\arrowbfq \cdot \bfv_k)     \bfn_0^- \, e^{ i2\pi j (Q_k + \nu_0)} + 
     \bfn_0^- (\arrowbfq \cdot \BAR\bfv_k) \bfn_0^+ \, e^{-i2\pi j (Q_k + \nu_0)} + {} 
    \nonumber \\[-2ex]
    &\bfn_0^- (\arrowbfq \cdot \bfv_k)     \bfn_0^+ \, e^{ i2\pi j (Q_k - \nu_0)} +
     \bfn_0^+ (\arrowbfq \cdot \BAR\bfv_k) \bfn_0^- \, e^{-i2\pi j (Q_k - \nu_0)} + {}
    \label{zj2pn} \\
    &\bfn_0^- (\arrowbfq \cdot \bfv_k)     \bfn_0^- \, e^{i2\pi j Q_k} + 
     \bfn_0^+ (\arrowbfq \cdot \BAR\bfv_k) \bfn_0^+ \, e^{-i2\pi j Q_k} + \CRNO
    &\bfn_0^+ (\arrowbfq \cdot \bfv_k)     \bfn_0^+ \, e^{i2\pi j Q_k} + 
     \bfn_0^- (\arrowbfq \cdot \BAR\bfv_k) \bfn_0^- \, e^{-i2\pi j Q_k}
   \Big] \nonumber
%    &\hspace{-1in} (\bfn_0^- (\arrowbfq \cdot \bfv_k) + \bfn_0^+ (\arrowbfq \cdot \bfv_k) \bfn_0^-) \, 
%      e^{i2\pi j Q_k} + 
%     (\bfn_0^- (\arrowbfq \cdot \BAR\bfv_k) + \bfn_0^+ (\arrowbfq \cdot \BAR\bfv_k) \bfn_0^-) \, 
%      e^{i2\pi j Q_k} \CRNO
\end{align}
where the bar as in $\BAR\bfv_k$ means complex (not quaternion) conjugate.
the first two terms on the right hand side of \Eq{zj2pn} will be nonzero only
at the sum resonance with $Q_k + \nu_0 = p$ for $p$ an integer. 
The third and fourth terms will be nonzero only at the difference resonance with $Q_k - \nu_0 = p$.
And the last four terms will be nonzero only at
an integer resonance where $Q_k = p$ which can be ignored.
...
\begin{equation}
  ...
  \label{ejx}
\end{equation}
To compute $\xi_r$, the following theorem is needed. 
For any complex quaternion $\bfq$, the quantity $\bfn^+ \bfq \bfn^-$ has
a zero scalar part and therefore can be considered to be a vector. Considering this quantity as a vector, the
magnitude of the real and imaginary parts are equal 
\begin{equation}
  \Big| \re \big( \bfn_+ \, \bfq \, \bfn_- \big) \Big| = 
  \Big| \im \big( \bfn_+ \, \bfq \, \bfn_- \big) \Big|
\end{equation}
This theorem can be be proved by brute force.
Combining the above equations, the strength of a sum resonance is
\begin{equation}
  \xi_r = \frac{\sqrt{2}}{2\pi} \Big|
  \bfn_0^+ ((\arrowbfq -\arrowbfq^*) \cdot \bfv_k) \bfn_0^- \Big| \label{x12pq1}
\end{equation}
and for a difference resonance
\begin{equation}
  \xi_r = \frac{\sqrt{2}}{2\pi} \Big|
  \bfn_0^- ((\arrowbfq -\arrowbfq^*) \cdot \bfv_k) \bfn_0^+ \Big| \label{x12pq2}
\end{equation}
In both equations, the standard complex vector absolute value is used (which is not the same as the
quaternion norm \Eq{1qqq}).

The resonance strength can also be calculated within the \vn{SLIM} formalism (\sref{s:slim})
via \Eq{x1pg}.
...
The strength of linear resonances can be calculated from the $\bfG$ matrix
The corresponding equation to \Eqs{x12pq1} and \eq{x12pq2} is\cite{b:spin.hoff}
\begin{equation}
