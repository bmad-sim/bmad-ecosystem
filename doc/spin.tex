...
The calculation of $P_{dk}$ (\Eq{pdk85}) and $\tao_{dep}^{-1}$ (\Eq{tdep}) involve integrating
...
decoherence process can had by considering what $P_{dk}$ and $\tao_{dep}^{-1}$ would be if only
one oscillation mode was being excited. This is done by calulating $\partial\bfn/\partial\delta$ with
