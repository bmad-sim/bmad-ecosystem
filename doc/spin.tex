...
multiple conventions that are used to define quaternions! The ``\vn{Hamilton}'' convention, which is
widely used in mathematics, physics and other fields, is used here. Use of the ``\vn{JPL}'' (Jet
Propulsion Lab) and ``\vn{STS}'' (``space shuttle convention'') \vn{conventions} are generally
...
%-----------------------------------------------------------------   
\section{Linear Single Resonance Analysis}
...
\begin{equation}
  \nu_s + m_x \, \nu_x + m_y \, \nu_y + m_z \, \nu_z = m_0
\end{equation}
where $m_x$, $m_y$, $m_z$, and $m_0$ are integers. In the linear approximation (\sref{s:dn.calc}),
...
is the action of the orbital oscillations (\sref{s:action.ang})
\begin{equation}
  \epsilon_r = \sqrt{J} \, \xi_r
  \label{ejx}
\end{equation}
where $\xi_r$ is the normalized resonance strength. In this case, the criterion for the resonance
strength to be small (\Eq{r1}) is that $\sqrt{J} \, \xi_r$ is small comapared to one with $J$ being
the action for particle motion at a few beam sigma.

The normalized resonance strength can be related to the first order spin eigenvectors $\bfn_k$
(\Eq{qnnq}) by comparing \Eq{nsdl} with the equations developed in \sref{s:dn.calc}. The eigen
analysis in \sref{s:dn.calc} gives the closed orbit $\bfn_0$ along with the six eigenvectors
...
\Eq{vsvi0}, represents particle motion with unit action $J_k$ for that mode. Considering \Eq{ndan},
...
\begin{equation}
  ...
  \label{nnav}
\end{equation}
where $\phi_0$ is the initial phase (see \Eq{x2jv}). Comparing this to \Eqs{nsdl} and \eq{ejx}, the
constant part of the $\bfn$ vector in \Eq{nsdl} ($\bfn_z$) is associated with $\bfn_0$ in \Eq{nnav}
and the time varying part of $\bfn$ in \Eq{nsdl} ($\bfn_x$ and $\bfn_y$) is associated with $\bfv_k$
in \Eq{nnav}. Taking the ratio of constant to time varying parts in both equations and equating
gives
\begin{equation}
...
$\xi_r$ can be related to the $\bfG$ matrix of the SLIM formalism (\Eq{mm0gd}). The corresponding equation
is\cite{b:spin.hoff}
\begin{equation}
...
eigenvalue which is negative the tune cannot be used here (unlike \Eq{x2dn}).

While \Eqs{x2dn} and \eq{x1pg} can be used to calculate $\xi_r$ within the single resonance model
independent of how large $\delta$ is, in a real machine, where there are many resonances, \Eqs{x2dn}
and \eq{x1pg} are strictly only valid at resonance with $\delta = 0$. In this case \Eq{x2dn} is
singular since, as $\delta$ goes to zero, $\bfn_k$ goes to infinity. In practice, \Eqs{x2dn} and
\eq{x1pg} may be used to calculate $\xi_r$ with reasonable accuracy when $\delta$ is small.
