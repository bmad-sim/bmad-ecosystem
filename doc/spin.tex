...
analysis (\sref{s:spin.res}) biquaternions (quaternions with complex coefficients) will be used. The
...
For any biquaternions $\bfq$, $\bfn^\ppls = I_+(\bfn)$, and $\bfn^\mmin = I_-(\bfn)$, the biquaternion $\bfG =
...
\end{equation}
Furthermore, the real and imaginary parts of $\bfG$ have the following properties
\begin{align}
  & \re(\bfG) \cdot \im(\bfG) = 0 \CRNO
  & \re(\bfG) \cdot \bfn = \im(\bfG) \cdot \bfn = 0 
  \label{GG0} \\
  & |\re(\bfG)| = |\im(\bfG)| \nonumber
\end{align}
These relations are fairly easy to derive. For example, the last one is proved via
\begin{equation}
  4 \, | \re(\bfG) |^2 = (\bfG + \bfG^*) \cdot (\bfG + \bfG^*) =
  \bfn^\ppls \bfq^2 \bfn^\ppls + \bfn^\mmin \bfq^2 \bfn^\mmin =
  -(\bfG - \bfG^*) \cdot (\bfG - \bfG^*) =
  4 \, | \im(\bfG) |^2
\end{equation}
and since the magnitude of both $\re(\bfG)$ and $\im(\bfG)$ must be positive (since $\re$ and $\im$
produce real numbers), it follows that $|\re(\bfG)| = |\im(\bfG)|$.
...
\begin{equation}
  ...
  \bfq_z(\phi) \equiv \cos (\phi/2) +  \sin (\phi/2)) \, \calk
  \label{rqsq}
\end{equation}
...
\begin{equation}
  ...
  \label{q12en}
\end{equation}
where $\bfn_0^\mmin \equiv I_-(\bfn_0)$ and $\bfn_0^\ppls \equiv I_+(\bfn_0)$ are given by \Eq{n1in}.

For a particular mode of oscillation, $\bfr_j$ is given by \Eq{x2jv}.  Without loss of generality,
$\phi_0$ in \Eq{x2jv} will be set to zero.  Using this along with the above equations gives
\begin{align}
  \bfZ = \lim_{N \rightarrow \infty} 
    \frac{\sqrt{J}}{2 \pi N} 
    \sum_{j = 0}^{N-1} \Big[
...
integer resonance where $Q_k = p$ which can be ignored.
...
\begin{equation}
  \xi_r = \frac{\sqrt{2}}{\pi} \big| \bfG \cdot \bfv_k \big| \label{x12pq1}
\end{equation}
and for a difference resonance
\begin{equation}
  \xi_r = \frac{\sqrt{2}}{\pi} \big|
  \bfG \cdot \bfv_k^* \big| \label{x12pq2}
\end{equation}
where $\bfG$ is given by
\begin{equation}
  \bfG = \bfn_0^\ppls \arrowbfq \bfn_0^\mmin
\end{equation}

The resonance strength can also be calculated within the \vn{SLIM} formalism (\sref{s:slim}) via
\Eq{x1pg}. From \Eqs{GG0} it can be seen that $\bfG$ is the quaternion equivalent of the SLIM
$\bfG_{2\cross6}$ matrix. The extra factor of $2\sqrt{2}$ in \Eqs{x12pq1} and \eq{x12pq2} compared 
to \Eq{x1pg} comes from using $overrightarrow\bfG$ which is a complex number instead of using
the equivalent equation involving $\bfG_{2\cross6}$.
