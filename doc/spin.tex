...
\etcetc In a storage ring, the \vn{invariant spin field} $\bfn(\bfr, s)$
...
To compute the $\Bf D$ and $\bfG$ matrices from the spin transport map (\sref{s:spin.map}), the spin
...
this case, the $\Bf D$ matrix is just a unit matrix.  When analyzing an entire ring, the second step
...
$s_2$, the $\Bf D$ matrix is calculated by first transforming $\bfq_0(s_1, s_2)$ to the $(\bfl,
