...
\end{equation}
with $Q$ being the mode orbital tune. The $\sqrt{J}$ factor in \Eq{stos} means that the resonance
...
\Eq{stos} can be solved by transforming to a coordinate system rotating about the $z$-axis with a
...
eigenvector, $\bfv_k$, normalized using \Eq{vsvi0}, represents particle motion with unit action $J_k$
for that mode. Considering \Eq{ndan}, if only one mode is excited for some particular $k$ and with
...
\begin{equation}
  ...
  \label{nnav}
\end{equation}
where $\phi_0$ is the initial phase (see \Eq{x2jv}). Comparing this to \Eq{nsdl}, the constant part
of the $\bfn$ vector in \Eq{nsdl} ($\bfn_z$) is associated with $\bfn_0$ in \Eq{nnav} and the time
varying part of $\bfn$ in \Eq{nsdl} ($\bfn_x$ and $\bfn_y$) is associated with $\bfv_k$ in
\Eq{nnav}. Taking the ratio of constant to time varying parts in both equations and equating gives
\begin{equation}
