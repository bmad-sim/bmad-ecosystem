...
{With the \vn{ref_orbit} attribute set to
\vn{match_at_entrance}, if there is a variation of the reference
energy through a bend from pass to pass, the reference orbit through the 
bend will also vary from pass to pass. With \vn{match_at_entrance}, 
all the reference orbits will be the same at the entrance end of the magnet.}
...
\vn{ref_orbit} and \vn{n_ref_pass}. \vn{n_ref_pass} is an
integer indicating which pass is used to define the reference energy
for the lord element. \vn{ref_orbit} is a switch that can take
the values:
\begin{example}
  single_ref            ! Default
  match_at_entrance
  match_at_exit
\end{example}
A value of \vn{single_ref} (the default) means that the same reference
orbit will be used for all passes. The \vn{match_at_entrance} and
\vn{match_at_exit} values are used when separate reference orbits are
...
Due to the presence of \vn{n_ref_pass}, the reference energy is well
defined for bends in a multipass line. This being the case, the
magnetic field can be computed from knowledge of \vn{g} (or \vn{rho})
and \vn{g_err}. Hence bend elements are the exception to the rule, and
