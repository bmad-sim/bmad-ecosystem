...
lattice has been parsed, the length of the last slave (the slave at
the exit end of the superimposed element) will be varied to keep
things consistent. For example, if element \vn{S} in the first example
of Section~\sref{s:super} is increased from 6~meters to 7~meters, The
...
1~meter. Notice that in this example, Setting the length of \vn{S} to
7~meters in the lattice file will give different results. In this
case, the lattice would look like
\begin{example}
        Element   Key         Length  Total
  1)    Q{\#}1       Quadrupole   6.5      6.5
  2)    Q{\B}S       Sol_quad     3.5     10
  3)    S{\#}1       Solenoid     3.5     13.5
  4)    D{\#}2       Drift        0.5     14
\end{example}
