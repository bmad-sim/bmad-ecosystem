...
independent attribute (\sref{s:depend}). For a bend, there is the
...
\vn{g} determines the layout of the elements downstream from the
bend. To alleviate this problem, \vn{Sbend} and \vn{Rbend} elements
have \vn{e_tot_ref_geometry} and \vn{n_multipass_ref} attributes which
are used to set the reference energy for calculating \vn{g}. If
