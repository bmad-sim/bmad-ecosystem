...
\begin{example}
  ...
  1)    Q{\#}1       Quadrupole   7        7
  2)    Q{\B}S       Sol_quad     3       10
  3)    S{\#}1       Solenoid     3       13
  4)    D{\#}2       Drift        1       14
\end{example}
...
lattice has been parsed, the starting longitudinal position of of the
element will be kept fixed that the ending longitudinal position will
be varied by varying the length of the last slave (the slave at the
exit end of the lord element). For example, if the \vn{S} element in
the first example of Section~\sref{s:super} is increased from 6~meters
to 7~meters, The lattice would look like
\begin{example}
        Element   Key         Length  Total
  1)    Q{\#}1       Quadrupole   7        7
  2)    Q{\B}S       Sol_quad     3       10
  ...
  4)    D{\#}2       Drift        1       15
\end{example}
The difference here is that \vn{S{\#}1} has increased by 1~meter and
the overall length of the lattice has also increased by
1~meter. Notice that in this example varying the if the length of \vn{S}
...
{A) With \vn{ref_orbit} = \vn{match_at_entrance}, the reference orbits of
different pass will be taken
to be the same at the entrance end of the magnet. If
there is a variation of the reference energy through a bend from pass
to pass, the reference orbit through the bend will vary from pass to
pass. B) With \vn{ref_orbit} = \vn{match_global_coords}, the reference orbit}
...
defermined by the reference momentum at the beginning of the lattice
...
\vn{ref_orbit} is changed to \vn{match_at_entrance} and the reference
momentom is set instead of \vn{n_ref_pass}:
\begin{example} 
...
calculated by scaling the radius as clculated from the first pass
...
Since the calculation with \vn{match_global_coordinates} is rather
