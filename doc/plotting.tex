...
funciton, a graph displaying the orbit, and a graph displaying the ``lattice layout'' which shows
...
Underlying all this is \vn{quick_plot} software toolkit (\sref{s:quick.plot}) which was developed
...
\fig{f:plot.page}. Parameters associated with the \vn{plot page} are discussed in \sref{s:plot.page}.
...
numbers which set the width and height. The \vn{plot page} size can also be set when involking \tao
...
sometimes comvenient to start \tao without the plotting window. In this case, the \vn{-noplot} option
...
region within the plot area. Regions may be defined by the user. In addition, for convienience, \tao
...
default \vn{template} plots. Section~\sref{s:template} discussses how to define custom template
...
one of the grid rectagles to be the \vn{box} associated with the \vn{graph}. The is illustrated in
...
Every \vn{plot} has at least one \vn{graph}. How many \vn{graphs} are assocaited with a \vn{plot}
is a matter of convienience and different \vn{graphs} of a \vn{plot} may display different types of
...
%-----------------------------------------------------------------
\subsection{Curve Legend}
\label{s:curve.legend}

The \vn{curve legend} is the legend identifying what curves are associated with what perameters. In
...
will be the longitudinal position $s$-position as in the upper two graphs in \fig{f:plot.typ}. Also
see \Sref{s:draw.ap} for an example where beam apertures are added to the graph.
%
\item[data slice] \Newline
...
Each curve may have an assoicated line that is drawn. The line may be a set of line segments
...
A ``\vn{data}'' graph (\sref{s:plot.data}) is used to draw lattice parameters such as orbits, or
...
\begin{example}
  &tao_template_graph
    ...
    ...
\end{example}
This will plot the difference between the \vn{model} and \vn{design} values. A default value for
\vn{component} may be defined by setting the \vn{graph%component} in the namelist.
If a default is not set. The default is taken to be \vn{model}.
