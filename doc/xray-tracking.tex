...
\sref{s:pos.trans} along with \Eqs{wws2}, and \eq{swww}.
  \item
...
\sref{ss:bragg.track} \sref{ss:coherent.laue} and \sref{ss:incoherent.laue}.
...
has the form given by \Eq{ltl} with the $\bfR_{z}(\theta_t)$ matrix
depending only upon the \vn{ref_tilt} parameter (which is here taken
to be zero). To satisfy \Eq{ltl}, the crystal must be reorientated to
...
satisfy \Eq{ltl}, this vector must have zero $y$ component
\Begineq
...
be the $\bfR_{y}(-\alpha_b)$ matrix given in \Eq{ltl}
\Begineq
