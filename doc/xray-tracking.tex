...
$P_i$, and $r_i$ are arbitrary. That is, \Eq{panda1} will not depend
upon the particular choice of $P_i$ and $r_i$ as long as \Eqs{rpss}
and \eq{p1} are satisfied. This freedom allows simulation to be
...
When traveling through a a vacuum, the photons travel balisticly in
...
into various ``channels''. With coherent photons, the analog to \Eq{rpss} is
\Begineq
  P_i \, r_i = \frac{E_i}{E_0}
  \label{rpss2}
\Endeq
...
\Begineq
  \bfL = \frac{|d\bfkbar_0|^2}{d\bfkbar_0 \cdot \Bf t} \, d\bfkbar_0
...
\Begineq
  ...
  \qquad \text{[forward or Bragg diffracted]}
\Endeq
$\bfL$ is thus
\Begineq
  \bfL = \frac{|d\bfKbar|^2}{d\bfKbar \cdot \Bf t} \, d\bfKbar
\Endeq
...
face of the crystal, if using the \vn{forward_diffracted} or
\vn{undiffracted} reference orbit, the photon is channeled into the
forward going diffraction direction. Otherwiase, for the
\vn{bragg_diffracted} reference orbit, the photon is channeled into
...
through each branch (\Eqs{rpss1} and \eq{rpss2}). For incoherent
...
\Begineq
  r_\alpha = r_\beta = \left[ \frac{|\bfS_\alpha| + |\bfS_\beta|}{\bfS_0} \right]^{1/2}
  \label{rrsss}
\Endeq
where
\Begineq
  \bfS_0 = \frac{1}{2} \, \sqrt{\frac{\mu_0}{\epsilon_0}} \, |\bfE_0^i|^2 
\Endeq
