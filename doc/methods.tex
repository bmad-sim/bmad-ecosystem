...
handle element misalignments or kicks and is generally only used for
testing purposes. Note: Thanks to CERN and Frank Schmidt for
...
In addition to the above parameters, the type of integration in
\vn{PTC} can be set for individual elements using the
...
\end{example}
\vn{matrix_kick}, which is the default, symplectically integrates by
alternating 

The \vn{drift_kick} setting uses 
...
\label{lcavity} Note that for elements like an \vn{lcavtity} where the
...
and downstream, then proforming symplectification, and finally back
