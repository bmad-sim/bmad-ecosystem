...
If the element \vn{static_linear_map} parameter is set to \vn{True} (the default is \vn{False}, this
prevents the linear map which consists of the transfer matrix and the zeroth order part of the map
from being recomputed. For example, if a steering is changed which shifts the reference orbit, the
transfer matrix will, in general, vary but having \vn{static_linear_map} set to \vn{True} will
prevent this variation.
