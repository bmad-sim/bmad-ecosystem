...
size using the setting of \vn{ds_step} for the element being tracked through (\sref{s:integ}).
...
\vn{tracking_method} is set to \vn{boris}, \vn{runge_kutta}, or \vn{time_runge_kutta} the spin will
...
\begin{example}
  ...
  field_calc = <Switch>         ! How the field is calculated (\sef{s:field.calc})
...
\index{runge_kutta}\index{boris}
The \vn{boris}, and \vn{runge_kutta} tracking all use as input the electric and magnetic fields of
...
\begin{example}
  ...
  fieldmap
\end{example}
For \vn{wigglers} and \vn{undulators}, possible values for
\vn{field_calc} are:
\begin{example}
  planer_model     ! This is the default
  helical_model
  custom
  fieldmap
\end{example}
\index{custom}\index{runge_kutta}
Note that with \vn{bmad_standard} tracking, the setting of \vn{field_calc} is ignored except in the
case of \vn{wigglers} and \vn{undulators} where \vn{field_calc} must be set to either
\vn{planer_model} or \vn{helical_model}.
...
\label{s:boris.integ}
\index{boris integration}

\index{boris}
The phase space coordinates used with \vn{boris} tracking are not the standard \bmad
