...
specify a branch. As with element names, a universe previx can be 
given. Example
\begin{example}
...
format. As with element names, a universe previx can be 
given. Example:
\begin{example}
...
\vn{lat_mid::} will evalute the parameter at the middle of the element.
...
When a term has a \vn{ele} or \vn{ele_mid} source, the value of the
term comes from an element parameter such as the element strength.
Element list format (\sref{s:ele.list.format}) is used for the
\vn{<element_id>} so an array of elements can be defined.  The
optional \vn{<component>} indicates what lattice is to be used and is
the same as for terms with a \vn{lat} or \vn{beam} source (see above).
