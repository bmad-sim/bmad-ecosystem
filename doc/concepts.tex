...
When combining components from the same \vn{d1_data}
(\sref{s:data.org}) or \vn{v1_var} (\sref{s:variable.overview}) array
in an expression, the common prefix can be eliminated. For example
\begin{example}
...
``Data-on-the-fly'' can be evaluated using the syntax in \sref{s:data.on.the.fly}.
Additionally, lattice element parameters can be included using the syntax:
\begin{example}
  \{[universe_range]\}ele:\{<class>\}:<name_or_num>[<param_name>]\{<component\}
\end{example}
...
Examples:
\begin{example}
  3@ele:quad:q*[k1]   k1 of all quads in the model lattice of universe 3 with 
                         names beginning with "q".
  ele::3[l]|design   Length of the design lattice element #3 in the viewed universe.
\end{example}
