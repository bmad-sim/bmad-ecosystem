...
(this is similar to the \vn{list} format used to specify lattice elements 
(\sref{s:ele.name.syntax})).
When there is only a single Universe specified, the brackets
\vn{[...]} are optional. When the universe prefix is not present, the
current ``viewed'' (default) universe (\sref{s:view}) is assumed. The
current \vn{viewed} universe can also be specified using the number
\vn{-1}. Additionally, a ``\vn{*}'' can be used as a wild card to 
denote all of the universes. Examples:
\begin{example}
...
Twiss parameters, etc.  Additionally, \vn{variables} can be set up
within \tao that vary these \vn{model} lattice parameters.  A given
...
%-----------------------------------------------------------------
\subsection{Syntax for Specifying Elements}
\label{s:ele.name.syntax}

When a lattice element or elements need to be specified in an expression, 
there are two formats that can be used: \vn{list} format and \vn{class::name} 
format.

\vn{Range} format is a list of element names or indexes separated by blanks, 
commas or colons. The colons represent a range of elements. For example:
\begin{example}
  d23 b34e:567, 45 77
\end{example}
The elements specified In this example are all elements 
...
(\sref{s:variable.overview}), along with lattice parameters can be
referenced in expressions. The general form of a reference is:
\begin{example}
  \{[<universe(s)>]@\}<source>::<ref_type>[\{<ref_point>&<eval_point>]\{|<component>\}
\end{example}
The \vn{<source>} field may be one of:
\begin{example}
  dat         Value is from a \tao datum.
  var         Value is from a \tao variable.
  lat         Value is from the lattice.
  beam        value is from the beam.
\end{example}
For element parameters the form is
\begin{example}
  \{<universe_range>@\}ele::<ele_name_or_num>[<param_name>]\{|<component>\}
\end{example}
Examples:
\begin{example}
  3@lat::orbit.x[34]             Orbit at element 34 in universe 3.
  3@lat::orbit.x[34]|model       Same as above.
  beam::beta.x[28]               Beam data
  lat::n_particle_loss[2&56]     Particle loss between elements 2 and 56.
  3@ele::quad::q*[k1]            k1 of all quads in the model lattice of universe 3 
                                   with names beginning with "q".
  ele::3[l]|design               Length of the design lattice element #3 
...
Note: when combining components from the same \vn{d1_data} or \vn{v1_var}
array in an expression, the common prefix can be eliminated. For
example
\begin{example}
...
\begin{example}
  ...
  <datum_type>           A data type defined in \sref{s:data.types}.
  <ix_ele_start>         Optional \vn{ele_start} index number.
  <ix_ele>               \vn{ele} index number.
...
\vn{orbit.x[22]} and constructing a ``datum-on-the-fly'', for
example, \vn{lat::orbit.x[10]}. In the first example, the ``\vn{[22]}''
refers to the 22nd element in the data array which may or may not
...
The element name can be in \vn{class::name} format (\sref{s:class.name}). 
The optional \vn {<component>} lattice can be
\begin{example}
  model (default)
  base
  design
...
the curve may be specifed using only the graph name
