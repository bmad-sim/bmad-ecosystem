...
(\sref{s:lattice}) plus whatever data (\sref{s:data}) one wishes to
...
description file (\sref{s:init_lat}).
...
lattice description file (\sref{s:init_lat}). In many instances this
...
The term \vn{``datum''} denotes anything that can be calculated. This
...
datums are grouped into what is called a \vn{d1_data}
\vn{structure}. Furthermore, sets of \vn{d1_data} structures are
grouped into what is called a \vn{d2_data} structure.  This is
illustrated in Figure~\ref{f:data_tree}.  For example, a \vn{d2_data}
structure for orbit data would contain two \vn{d1_data} structures ---
one \vn{d1_data} structure for the horizontal orbit data and another
...
data} (\sref{s:data}) command to view information about any data type. 
...
particle tracking. See Sections~\sref{s:globals} and \sref{s:beam_init} for
