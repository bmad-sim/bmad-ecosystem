...
name>} is the element name that can (and generally does) contain the wildcard characters
...
\end{example}
\vn{ele::} will evaluate the parameter at the exit end of the element and 
\vn{ele_mid::} will evalute the parameter at the middle of the element.
...
\vn{element_list}. The optional \vn{<ref_element>} specifies a reference
element for the evaluation. For example, ``\vn{lat::r.56[q0\&qa:qb]}'' is
an array of the $r(5,6)$ matrix element of the transport map between
element \vn{q0} and each element in the range from element \vn{qa} and 
\vn{qb}. The optional \vn{<component>} indicates what lattice 
...
When a term has a \vn{ele} source, the value of the term comes from an
element parameter such as the element strength. 
Element list format (\sref{s:ele.list.format}) is used for the
\vn{<element_id>} so an array of elements can be defined. 
The optional \vn{<component>} indicates what lattice 
is to be used and is the same as for terms with a \vn{lat} or \vn{beam} 
source (see above).
