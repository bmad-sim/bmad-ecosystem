...
elements (\sref{s:ele.name.syntax}).  When there is only a single
...
%-----------------------------------------------------------------
\subsection{Element List Format}
...
Items in a list are separated by a blank character or a comma ",". 
...
character. Thus, for example, ``q*'' matches any element whose name
begins with ``q''. If there are multiple elements in the lattice that
match a given name, all such elements are included. Thus ``d12'' will
match to all elements with the name ``d12''.

An element index is simply the index of the number in the lattice list
of elements.
...
\begin{example}
  <ele1>:<ele2}
\end{example}
where \vn{<ele1>} is the element at the beginning of the range and
...
\vn{<ele1>} and \vn{<ele2>} are part of the range.
...
name>}. Essentially this is an extension of the \vn{element name}
format. Example:
\begin{example}
  "quad::q*"
\end{example}
...
referenced in expressions. The general form of a reference for
anything other than element parameters is:
\begin{example}
...
\end{example}
For element parameters the format is
\begin{example}
...
``\vn{dat::orbit.x[10]''. With the ``\vn{lat::}'' source, the index in
the brackets, \vn{[10]}, refers to the 10th lattice element. With the
``\vn{dat::}'' source, ``\vn{[10]}'' refers to the 10\Th element in
the \vn{orbit.x} data array which may or may not correspond to the
10\Th lattice element.
