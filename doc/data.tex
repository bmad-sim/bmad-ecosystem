...
for more details. 
Note: This type of datum can only be used with \vn{detector}, \vn{instrument}, \vn{monitor} or \vn{marker} elements
...
the global coordinate system. See also \vn{floor.} and \vn{wall.}.
  ...
  %----------------------
  \item[sigma.x, .y, .z, .px, .py, .pz, .Lxy, .$ij$ \hspace{0.2in} $1 \le i,j \le 6$] \Newline
...
\vn{sigma.$ij$} with $1 \le i,j,\ldots \le 6$ are components of the beam sigma matrix.
...
\vn{h00400}, \vn{h22000}, \vn{h00220}, and \vn{h11110}.
  ...
  \vn{normal.h.<monomial>.\{r,i,a\}} data type.
  ...
  \vn{srdt_sxt_n_slices} and \vn{srdt_gen_n_slices} set the number of steps to take through
  ...
  \vn{srdt_use_cache} generates a cache of the cross-products of the linear optics quantities used
  ...
  with the squares of \vn{srdt_sxt_n_slices} and \vn{srdt_gen_n_slices}.  If there is insufficient RAM available, \vn{tao} will
  generate a warning message and revert to a \vn{srdt_use_cache=.false.} state.

  %----------------------
  \item[t.$ijk$,  tt.$ijklm\ldots$ \hspace{0.2in} $1 \le i,j,k,\ldots \le 6$] \Newline
Taylor map components between two points.  The difference between \vn{t.$ijk$} and
\vn{tt.$ijklm\ldots$} is that \vn{t.$ijk$} is restricted to exactly three indices and
\vn{tt.$ijklm\ldots$} is not. \vn{t.$ijk$} is superfluous but is keep for backwards compatibility.
...
\begin{longtable}{llll} 
  ...
  srdt.h<monomial>.\{r,i,a\}          & Normal form driving terms calculated by summation & lat & No \\ \hline
