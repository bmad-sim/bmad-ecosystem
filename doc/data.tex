...
Associated with a datum are three lattice elements called \vn{ele},
\vn{ele_ref}, and \vn{ele_start}. The names of these elements may be
specified by setting the \vn{ele_name} and \vn{ele_ref_name} and
\vn{ele_start_name} components of a datum. Alternatively, these
elements may be specified by setting the \vn{ix_ele} and
\vn{ix_ele_ref} and \vn{ix_ele_start} components of a
datum. \vn{ix_ele} and \vn{ix_ele_ref}, and \vn{ix_ele_start} are the
index of the elements in the array of elements in the lattice. Using
the element index over the element name is necessary when more than
one element in the lattice has the same name.
...
be blank. All other datums must have an associated \vn{ele} element
that is specifed by \vn{ele_name} or \vn{ix_ele}. If a datum has an
associated \vn{ele}, but no associated \vn{ele_start} or \vn{ele_ref},
the \vn{model} value of that datum is the value of the \vn{data_type}
at that element. For example, if a datum has:
\begin{example}
  data_type      = 'orbit.x'
  ele_name       = 'q12'
\end{example}
...
If a datum has an associated \vn{ele_start} element, specified by
either \vn{ele_start_name} or \vn{ix_ele_start}, the datum is
evaluated over a region from the exit end of \vn{ele_start} to the
exit end of \vn{ele}. For example, if a datum has:
\begin{example}
  data_type      = 'beta.a'
  ele_name       = 'q12'
  ele_start_name = 'q45'
...
If a datum has an associated \vn{ele_ref} element, specified by either
\vn{ele_ref_name} or \vn{ix_ele_ref}, the \vn{model} value of the
datum is the value at \vn{ele} (or the value over the range
\vn{ele_start} to \vn{ele} if \vn{ele_start} is specified), minus the
\vn{model} value at \vn{ele_ref}. For example, if a datum has:
\begin{example}
  data_type      = 'beta.a'
  ele_name       = 'q12'
  ele_start_name = 'q45'
...
datums \vn{beta.a[12]} for universe 1 and universe 2. In this example,
the source of both terms in the expression is explicitly given as \vn{dat}.
However, if an explicit source is not specified for a given term,
the \vn{datum%data_source} component in the initialization file 
(\sref{s:init.data} will be used. An expression can also be used as the
\vn{default_data_type} in the \vn{tao_d1_data} structure in the
initialization file. In this case, the 
evaluation point is implicit. For example:
\begin{example}
  default_data_type = "expression: 1@dat::beta.a - 2@dat::beta.a"
\end{example}
And the evaluation point for the datums created will be, just like
other data types, by the specified \vn{datum%ele_name}.

To be valid, 
if an expression has a term with a \vn{dat} source, 
the expression must be evaluated after the \vn{dat} source 
components are evaluated. Data evaluation is done universe by universe
...
that appears earlier in the initialization file. 
In the example above, the expression terms are 
\vn{1@dat::beta.a[12]|model} and \vn{2@dat::beta.a[12]|model}. 
Therefore, this expression
datum should be in universe 2 or higher. Notice that while all datums
must be assigned a universe, in this case, since all the terms explicitly
give a universe number, the value of the datum will
be independent of the universe it is in.

  \index{floor.*}
  \item[floor.*]
This is the global floor position at the exit end of \vn{ele}. See the
\bmad manual for details on the global coordinate system. See also
\vn{rel_floor.*}.

  \index{n_particle_loss}
  \item[n\_particle\_loss] \Newline
If \vn{ele_start} is not specified, \vn{n_particle_loss} gives the number
of particles lost at element \vn{ele}. If \vn{ele_ref} is specified,
\vn{n_particle_loss} gives the cumulative loss between the exit end of
\vn{ele_ref} and the exit end of \vn{ele}. That is, this sum does not
count any losses at \vn{ele_ref} itself.
...
the transfer map from the reference element to eva
This map is only defined for circular lattices. 


  \index{phase.*}
  \item[phase.*]
This is the betatron phase.  If a \vn{d1_data} array has a set of
\vn{phase} datums, and if the \vn{ele_ref} element is {\em not}
...
\vn{model} and \vn{measured} values. If the \vn{ele_ref} element is
specified then there is no arbitrary constant in the evalustion.

  \index{rel_floor.*}
  \item[rel\_floor.*]
This is the global floor position at the exit end of \vn{ele} relative
to the exit end of \vn{ele_ref} in a global coordinate system where the
exit end of \vn{ele_ref} is taken to be at \vn{x = y = z = theta = phi =
0}. See the \bmad manual for details on the global coordinate
system. See also \vn{floor.*}.
...
Calculation of \vn{t.*} and \vn{tt.*} datums involve symplectic integration
...
set by the \vn{ds_step} attribute of that element (see the \bmad
manual for more details). When a smooth curve (\sref{s:template}) is
plotted for \vn{t} and \vn{tt} data types, and the longitudinal
(\vn{"s"}) position is used for the x-axis, the integration step used
in generating the points that define this curve will be decreased if
the the s-distance between points is smaller than the \vn{ds_step}.
In this case, discrepancies between the plot and datum values may be observed.

  \index{unstable_orbit}
  \item[unstable\_orbit] \Newline
The \vn{unstable_orbit} datum is used for linear lattices. For single particle
tracking, the value of an \vn{unstable_orbit} datum is zero if the
tracked particle survives (has not been lost) up to the \vn{ele}
element and, if it has been lost, is set to 
\Begineq
 1 + i_{\mbox{ele}} - i_{\mbox{lost}}
\Endeq
where $i_{\mbox{ele}}$ is the index of \vn{ele} in the lattice and
$i_{\mbox{lost}}$ is the index of the element where the particle was
lost. When tracking beams, the value of \vn{unstable_orbit} is the
averaged value over all particles in the bunch. The default for
\vn{ele}, if \vn{ele_name} nor \vn{ix_ele} is not specified, is to use
the last element in the lattice.

\vn{unstable_orbit} is used in an optimization to avoid
...
is unstable. \vn{unstable_ring} is used in an optimization to avoid
unstable solutions (\sref{s:generalized.design}).
...
\index{unstable_orbit}\index{beta}\index{alpha}\index{eta}\index{eta}
\index{etap}\index{phase}\index{orbit}\index{wire}\index{spin}
\index{cbar}\index{coupling}\index{floor}\index{r}\index{t}\index{tt}
\index{rad_int.5a_e6}\index{rad_int.5b_e6}\index{s_position}\index{e_tot}
...
\begin{tabular}{|l|l|} \hline
  ...
  face1.offset, face2.offset     & Element offset with multipass      \\ \hline
  ...
  rad\_int.5a, rad\_int.5b         & I5 radiation integral              \\ \hline
  rad\_int.5a\_e6, rad\_int.5b\_e6 & Energy normalized I5 radiation integral   \\ \hline
