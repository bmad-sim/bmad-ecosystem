...
\index{Data!Relative}
Associated with a datum are two lattice elements called \vn{ele} and
\vn{ele0}.  The names of these elements may be specified by setting
the \vn{ele0_name} and \vn{ele_name} components of a
datum. Alternatively, these elements may be specified by setting the
\vn{ix_ele} and \vn{ix_ele0} components of a datum. \vn{ix_ele} and
\vn{ix_ele0} are the index of the elements in the array of elements in
...
Datums can be divided up into four classes: \vn{Global} Datums, like
...
only have one associated lattice element given by \vn{ele_name} and
\vn{ele0_name} will be blank.  Datums in the third class are
associated with a section of the lattice. For example, the maximum of
the beta function in a lattice region. In this case the \vn{ele0}
element marks the beginning of the region and \vn{ele} marks the
end. The final class of datums, called \vn{relative} datums, means
that the datum value is set by the difference between the value at the
lattice element at \vn{ele0} and the value at \vn{ele}. The betatron
...
element with index 0 named \vn{BEGINNING}. The datum components
\vn{ix_ele} and \vn{ix_ele0} are the index of \vn{ele} and \vn{ele0}
...
be independent of the universe it is in.
  ...
  index{floor.*}
  \item[floor.*]
This is the global floor position at the exit end of \vn{ele} relative to
the exit end of \vn{ele0}. See the \bmad manual for details on the global
coordinate system. See also \vn{rel_floor.*}.

  \index{n_particle_loss}
  \item[n\_particle\_loss] \Newline
If \vn{ele0} is not specified, \vn{n_particle_loss} gives the number
of particles lost at element \vn{ele}. If \vn{ele0} is specified,
\vn{n_particle_loss} gives the cumulative loss between the exit end of
\vn{ele0} and the exit end of \vn{ele}. That is, this sum does not
count any losses at \vn{ele0} itself.
...
to the exit end of \vn{ele0} in a global coordinate system where the
exit end of \vn{ele0} is taken to be at \vn{x = y = z = theta = phi =
...
\begin{tabular}{|l|l|l|} \hline
  ...
  periodic.tt.$ijklm\ldots$    & \begin{tabular}{l}
                                  Taylor term of the periodic map. \\
  ...
  \end{tabular}                & Relative global (``floor'') position & relative \\ \hline 
...
\begin{example}
  ...
  <ix_ele0>              Optional \vn{ele0} index number.
