...
\begin{equation}
  \text{emit}_{\text{norm}} = \gamma \cdot \text{emit}
\end{equation}
...
\begin{equation}
  \epsilon_{norm} = \gamma \cdot \epsilon
\end{equation}
...
a rotation angle equal to the spin tune.
  ...
  %----------------------
  \item[spin_res.a1, .a2, .b1, .b2, .c1, .c2] \Newline \hlabel{spin.res}
Spin resonance values. \vn{.a1}, \vn{.a2} are the resonance values for the $a$-mode, etc. The \vn{show
spin -ele ...} command will print the values. If not present, the evaluation element for the datum
is the beginning element of the lattice branch.
