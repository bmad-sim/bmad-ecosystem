...
datums \vn{beta.a[12]} for universe 1 and universe 2. To be valid, an
...
\begin{tabular}{|l|l|} \hline
  ...
  rad_int.5a, rad_int.5b         & I5 radiation integral              \\ \hline
  rad_int.5a\_e6, rad_int.5b\_e6 & Energy normalized I5 radiation integral   \\ \hline
...
%------------------------------------------------------------------------
\section{Referencing Datums on the Fly}
\label{s:data.on.the.fly}

An expression like "orbit.x[2]|model" refers to an existing datum
that has been set up in the \tao initialization file.  It is sometimes
useful, however, in \tao commands to be able to refer to \vn{data-on-the-fly}
that have not been so initialized. The syntax for this is:
\begin{example}
  \{[<universe_range>]@\}dat:\{<source>\}:
                           <datum_type>[\{<ix_ele_start>&<ix_ele>]\{|<component>\}
\end{example}
...
example\vn{dat::orbit.x[10]}. In the first example, the ``\vn{[22]}''
