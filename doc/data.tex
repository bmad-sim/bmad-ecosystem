...
Note: Use the \vn{show chromaticity} command to see the series coefficients.
...
Slip factor. Also see \vn{r56_compaction} and \vn{slip_factor_ptc}.
...
gives the order of the Taylor term
\begin{equation}
  \frac{1}{T_0} \, \Delta T(p_z) = \alpha_1 \, p_z + \alpha_2 \, p_z^2 + \alpha_3 \, p_z^3 + \ldots
\end{equation}
Where $T$ is the transit time over one turn and $T_0$ is reference transit time.
$N = 1$, that is \vn{slip_factor_ptc.1}, is the slip factor at $p_z = 0$, etc.
...
Slip factor. Also see \vn{r56_compaction} and \vn{momentum_compaction}.

Phase slip factor Taylor series, which is a function of phase space $p_z$, as calculated from
Etienne Forest's PTC code. [See the \bmad manual for documentation on PTC.] $N$ is an integer which
gives the order of the Taylor term
\begin{equation}
  \frac{1}{T_0} \, \Delta T(p_z) = \alpha_1 \, p_z + \alpha_2 \, p_z^2 + \alpha_3 \, p_z^3 + \ldots
