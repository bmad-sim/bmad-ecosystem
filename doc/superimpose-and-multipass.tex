...
the name so that there will be no confusion. (\vn{\#} has no special meaning other than the fact
that \bmad uses it for mangling names). The next element, \vn{Q{\B}S}, is the part of \vn{Q} that is
inside \vn{S}. \vn{Q{\B}S} is a combination solenoid/quadrupole element as one would
expect. \vn{S{\#}1} is the part of \vn{S} that is outside \vn{Q} but before \vn{M}. This element is
just a solenoid. Next comes \vn{M}, \vn{S{\#}1}, and finally \vn{D\#2} is the rest of the drift
outside \vn{S}.
...
superimposed with a \vn{solenoid} will produce a \vn{sol_quad} element but a \vn{solenoid}
superimposed with a \vn{rfcavity} element will produce an \vn{em_field} element since there is no
...
The superposition offset is the distance from the origin point of the reference element to the
origin point of the element being superimposed.
  }
  \label{f:superimpose}
  \end{figure}
...
Example:
\begin{example}
  superimpose, element = Q1, ref = B12, offset = 1.3, ele_origin = beginning, ref_origin = end
\end{example}
...
\end{example}
\vn{center} is the default setting.  \vn{offset} is the longitudinal offset between the origin
points. The default offset is zero.
...
A superimposed element that extends beyond either end of the lattice will be wrapped around so part
of the element will be at the beginning of the lattice and part of the element will be at the
end. For consistency's sake, this is done even if the \vn{geometry} is set to \vn{open} (for
example, it is sometimes convenient to treat a circular lattice as linear). Example:
\begin{example}
  d: drift, l = 10
...
\begin{example}
  ...
  Z: marker
\end{example}
This will place \vn{S1} upstream and \vn{S2} downstream of \vn{Z}.  If \vn{ref_origin} is not
present or set to \vn{center}, the ordering of the elements will be arbitrary.
...
Notice that, unlike \vn{element reversal}, the rules for how the location of a superimposed element
is calculated is not affected by the use of \vn{line reflection} (\sref{s:lines.wo.arg}). This is
done since line reflection merely changes the order of the elements of the lattice while element
...
(\sref{s:tkm}) will, by necessity, have to be \vn{runge_kutta}, or \vn{time_runge_kutta}
and the \vn{mat6_calc_method} (\sref{s:xfer}) will be set to \vn{tracking}.
...
physical element is done by grouping all lattice elements that have the same ``\vn{signature}''. For
any given element in the lattice, this element has some line it came from. Call this line $L_0$ and
denote by $n_0$ the index where the element under consideration is in the $L_0$ line (the first
element in $L_0$ has index 1, etc.). The $L_0$ line in turn was contained in some other line. Call
this line $L_1$ and denote by $n_1$ the position of $L_0$ in $L_1$. This chain of lines $L_0$,
$L_1$, ..., $L_n$ ends at some point and the last (top) line $L_n$ will be one of the root lines
listed in the \vn{use} statement (\sref{s:use}). For any given element in the lattice, starting with
$L_0$ and proceeding upwards through the chain, let $L_m$ be the first line in the chain that is
marked as \vn{multipass}. If no such line exists for a given element, $L_m$ is taken to be the top
line $L_n$. The \vn{signature} of the element is the list of (line, position) pairs $(L_0, n_0)$,
..., $(L_m, n_m)$ and two elements have the same signature if this list is the same for
both. Elements that have the same signature represent the same physical element and are slaved
...
\vn{g_err} for each slave is adjusted accordingly so that the total normalized field, \vn{g} +
\vn{g_err}, gives the same unnormalized field for all slaves. Note that since the normalized field
...
reference energy. This parameter may be seen in old lattice files and is currently ignored.
