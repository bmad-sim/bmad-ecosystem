...
using the example above, the first element of the lattice, \vn{A\B1}, has the chain of lines $L_0$ =
\vn{m_line}, and $L_1$ = \vn{u_line} and indexes $n_0$ = 1 and $n_1$ = 1. This is identical to the last
element in the lattice (\vn{A\B2}) so these two elements will be grouped together.
...
the slaves makes sense (are the same).
