...
\hyperref{routine!bmad_parser} (line 13) is the routine which parses a lattice file
...
\index{routine!twiss_at_start}
\index{routine!twiss_propagate_all}
After \hyperref{routine!bmad_parser} is called, the program checks if the lattice 
is circular (\sref{s:param}) and, if so, uses the
routine \hyperref{routine!twiss_at_start} (line 15) to multiply the transfer
...
The next call, to \hyperref{routine!twiss_propagate_all}, takes the starting
...
\hyperref{routine!type_ele} is used to type out the element's attributes and other
