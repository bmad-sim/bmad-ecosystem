...
\vn{simple_bmad_program.f90}. \vn{simple_bmad_program.f90} is in the
library area in a directory called
\vn{examples/simple_bmad_program}. Directions on how to compile and
...
call set_flags_for_changed_attribute (lat, cleo, cleo%value(ks$))
...
(\sref{s:lat.ele.change}) is used in line 19 to find the element in
the lattice with the name \vn{CLEO_SOL}. Line 20 defines a pointer
...
element. Line 21 changes the \vn{ks} solenoid strength of this
element. Since an element attribute has been changed, the call to
\Hyperref{r:set.flags.for.changed.attribute}{set_flags_for_changed_attribute} in line 22 is needed for \bmad
to correctly do the necessary lattice bookkeeping
(\sref{s:lat.bookkeeping}). The call to \Hyperref{r:lat.make.mat6}{lat_make_mat6} in line 23
recalculates the linear transfer matrix for the \vn{CLEO_SOL} element.
...
Making \vn{bmad} the working directory, the \vn{gmake -f
M.simple_bmad_program} command will compiling and link the program.
The executables, \vn{simple_bmad_program} and
\vn{simple_bmad_program_g}, are to be found in \vn{../bin/}. Running
the program with the command \vn{../bin/simple_bmad_program} gives the
output as shown in \fig{f:output}.
