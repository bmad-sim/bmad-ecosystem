...
\vnr{bmad_parser} (line 13) is the routine which parses a lattice file
...
After \vnr{bmad_parser} is called, the program checks if the lattice 
is circular (\sref{s:param}) and, if so, uses the
routine \vnr{twiss_at_start} (line 15) to multiply the transfer
...
The next call, to \vnr{twiss_propagate_all}, takes the starting
...
\vnr{lat_ele_locator} (\sref{s:lat.ele.change}) to find the element 
