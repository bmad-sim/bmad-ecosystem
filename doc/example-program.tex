...
\hyperref[r:bmad.parser]{routine!bmad_parser} (line 13) is the routine which parses a lattice file
...
\index{routine!twiss_at_start}
\index{routine!twiss_propagate_all}
After \hyperref[r:bmad.parser]{routine!bmad_parser} is called, the program checks if the lattice 
is circular (\sref{s:param}) and, if so, uses the
routine \hyperref[r:twiss.at.start]{routine!twiss_at_start} (line 15) to multiply the transfer
...
The next call, to \hyperref[r:twiss.propagate.all]{routine!twiss_propagate_all}, takes the starting
...
\hyperref[r:lat.ele.locator]{routine!lat_ele_locator} 
(\sref{s:lat.ele.change}) to find the element 
in the lattice with the name \vn{CLEO_SOL}.
\hyperref[r:type.ele]{routine!type_ele} is used to type out the element's attributes and other
