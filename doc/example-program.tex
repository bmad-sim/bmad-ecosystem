...
After \vnr{bmad_parser} is called, the program check if the lattice 
is circular (\sref{s:param}) and, if so, uses the
routine \vnr{twiss_at_start} (line 14) to multiply the transfer
...
The next call, to \vnr{twiss_propagate_all} (line 15), takes the starting
Twiss parameters and, using the transfer matrices of the individual
elements, calculates the Twiss parameters at all the elements. 
...
which it does on lines 19 through 24 of the program. 
\vn{lat} has within it an array
...
$a$--mode (nearly horizontal mode) beta. The result of this printing is
shown in lines 1 through 12 of the output in Figure~\ref{f:output}. 

The next section of the program, lines 28 through 35, uses the routine
\vnr{lat_ele_locator} (\sref{s:lat.ele.change}) to find the index of an
element in the lattice with the name \vn{CLEO_SOL}.
