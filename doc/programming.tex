...
\begin{description}
  ...
  \item[bmad] \Newline
The \vn{bmad} library contains the routines for relativistic charged particle simulation including
...
message will be generated at the linking stage. The error message might look like:
\begin{example}
...
\end{example}
In this example the subroutine \vn{write_bmad_lattice_file} was called in the program without the
necessary \vn{use} statement. To find the correct \vn{use} statement, use the \vn{getf} or
\vn{listf} command (\sref{s:getf}) to find the file that the subroutine is in. Example:
\begin{example}
...
\end{example}
This shows that the subroutine lives in the file \vn{write_lat_file_mod.f90} the
needed module name is the file name minus the \vn{.f90} suffix.\footnote
  {
Be aware. PTC code does not follow this logic.
  }
So the needed \vn{use} statement in this case is:
\begin{example}
...
\begin{example}
  write_lat_file_mod_mp_write_bmad_lattice_file_
\end{example}
which is to say that the linker knows to look in the module \vn{write_lat_file_mod} for the
subroutine. So here the problem is not a missing use statement but rather the problem is that the
linker cannot find the module itself. This indicates something like a library is missing in the list
of libraries to link to or the order of libraries to link to is wrong. Further documentation on how
the list of libraries to link to is defined is contained on the \bmad web site\cite{b:bmad.web}.
