...
\end{example}
The ``\vn{undefined reference}'' line shows that the subroutine \vn{write_bmad_lattice_file} was
...
\begin{example}
  write_lat_file_mod_mp_write_bmad_lattice_file_
\end{example}
The ``\vn{write_lat_file_mod_mp}'' prefix shows that the linker knows to look in the module
\vn{write_lat_file_mod} for the subroutine. Here the problem is not a missing use statement but
