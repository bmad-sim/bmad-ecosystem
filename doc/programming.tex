...
\end{example}
This shows that the subroutine lives in the file \vn{write_lat_file_mod.f90}. The coding rule that
...
subroutine. Here the problem is not a missing use statement but rather the problem is that the
linker cannot find the module itself. This indicates something like a library is missing in the list
of libraries to link to or the order of libraries to link to is wrong. This kind of problem
generally happens with libraries other than the \bmad library itself. Further documentation on how
