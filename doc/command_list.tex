...
details.
\item[show data]
Shows data information. If \vn{<data_name>} and \vn{<number>} are not
...
shown. \vn{<ele_name>} is a number $n$ then the $n$\Th element in the
...
\item[show var]
Shows variable information. If \vn{<data_name>} and \vn{<number>} are
...
\vskip 0.2in The \vn{use} command unvetoes data or variables and sets
a veto for the rest of the data. See also the \vn{restore} and
\vn{veto} commands.
...
\vskip 0.2in The \vn{veto} command vetoes data or variables. See also
the \vn{restore} and \vn{use} commands.
...
\vskip 0.2in \vn{X-Axis} sets what data is used for the horizontal
...
present then the bounds will be from -\vn{<bound1>} to
\vn{<bound1>}. If neither is present then autoscale will be invoked to
