...
\item[show var]
Shows variable information. If \vn{<var_name>} \vn{<locations>} are
...
Note: The command `\cmd{veto data all 10:20}' will veto all d1_data elements
within the range 10:20 \textit{using the index convention for each d1_data
structure separately}. This may produce curious results if the
indexes for the d1_data structures do not all point to the same lattice
elements. 
