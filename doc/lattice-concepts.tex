...
pair of intersecting storage rings will genarally have two \vn{root}
...
To orient an element in space, \bmad starts with the \vn{global
coordinate} system as outlined in Section~\sref{s:global}. Within the
global coordinate system, \bmad constructs a \vn{local} \vn{reference}
orbit (also called the \vn{laboratory} \vn{coordinate} system) for
each lattice branch (\sref{s:ref}). Finally, the
