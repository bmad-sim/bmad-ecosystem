...
There are two types of controllers. \vn{Function} based controllers and \vn{Spline} based
controllers. \vn{Function} based controllers use mathematical functions to define how slave
attribute values are calculated based upon the values of the controller variables. \vn{Spline} based
controllers use a spline fit to a function defined by a set of points (called ``\vn{knots}'') to
determin the relationship between varialbes and slave attributes. With a spline based controller,
the number of variables is restricted to one.
...
frame as shown in \fig{f:surface}). This coordinate system has the $x$ axis pointing towards the
interior of the element, and the $z$ axis along the plane defined by the entrance and exit reference
orbit. In this coordinate system, the surface curvature is parameterized by the parameter
\vn{spherical_curvature} plus a fourth order polynomial in $z$ and $y$
\Begineq
  {-z} = \left( R_s - \sqrt{R_s^2 - x^2 - y^2} \right) + \sum_{2 \le i+j \le 6} c_{ij} \, x^i \, y^j
  \label{xs2ij4}
\Endeq
where $R_s$ is the radias of the sphere and which is the inverse of the value of
\vn{sperical_curvature}. The polynomial coefficients are set in the lattice file by setting the
...
component and the fields $[\bfE, \bfB]$ vary in time as 
\Begineq
  [\bfE, \bfB] = \Re \Bigl( [\bfE_r + i \, \bfE_i, \bfB_r + i \, \bfB_i]  e^{-i \, \omega \, t} \Bigr)
\Endeq
where $\bfE_r$, $\bfE_i$, $\bfB_r$, and $\bfB_i$ are the real and imaginary components as specified
in the \vn{grid_field} structure.  Example:
\begin{example}
