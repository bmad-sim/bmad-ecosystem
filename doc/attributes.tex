...
calculation.  The \vn{surface} and \vn{wall} settings are used to
define a rectangular aperture. The \vn{surface} setting causes \bmad
to set the four limits (\vn{x1_limit}, etc.) to just cover the surface
grid (\sref{s:surf.grid}) associated with the element. The \vn{wall}
setting causes \bmad to set the four limits to just cover the clear
area of a \vn{diffraction_plate} (\sref{s:wall.diff.plate}).
...
field between points. \vn{map} defines the field using a number of
normal modes. As dissused below, both \vn{grid} and \vn{map} use the 
...
However RF fields can only be associated with \vn{lcavity},
\vn{rfcavity}, and \vn{em_field} elements.

Currently there is the restriction that RF fields must use \vn{map}
and DC fields must use \vn{grid}.
...
\Begineq
  \theta_0 = 
  \begin{cases}
    \text{dphi0_ref} + \dsfrac{f}{f_0} \, (\text{phi_ref} + \text{phi0_err}) & 
    \text{lcavity element} \\
    \text{dphi0_ref} - \dsfrac{f}{f_0} \, \text{phi_ref} & 
...
uses \vn{z_max} to check that the wake simulation is valid. That is,
to be valid, the maximum distance between any two simulated particles
should be less than the magnitude of \vn{z_max}.
...
\index{fringe_type}\index{fringe_at}
...
The \vn{fringe_at} may be set to one of
\begin{example}
  no_end         
  both_ends             ! Default
  entrance_end
...
The \vn{fringe_at} switch is used for vetoing fringe effects at
either of the faces of the element. This is useful in vetoing the
fringe effect in the interior of split elements.

The \vn{fringe_type}, which is used for everything but \vn{sad_mult}
element, may be set to one of the following:
\begin{example}
...
\vn{full_straight}, higher order effects are taken into account.  
...
The \vn{sad_linear}, \vn{sad_nonlin_only} and \vn{sad_full} settings
of \vn{fringe_type} emulate the fringe field tracking used in the SAD
program\cite{b:sad}.  The \vn{sad_linear} setting only uses the linear
part of the fringe, \vn{sad_nonlin_only} ignores the linear part of
