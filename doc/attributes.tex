...
Custom field profiles are used with integration type tracking methods (\sref{s:integ}).
It is important to note that custom field profiles are {\em ignored} by \vn{bmad_standard}
tracking.  Custom field profiles may be defined using \vn{grid} tables or \vn{maps}
(\sref{s:field.tab.map}) and may extend longitudinally beyound the ends of an element
(\sref{s:overlap}).
...
field is specified and when the setting of \vn{field_calc} (\sref{s:integ}) attribute for
both the overlapping and overlapped elements is something other than \vn{bmad_standard}.
