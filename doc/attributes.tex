...
elliptical curvature is more general. In practice, it is sometimes convieneint to be able to specify spherical
...
(meridional) plane, the radius $R_t$ needed for foucusing is
\Begineq
...
\begin{example}
  ...
  wake_amp_scale   = <Real>     ! Factor to scale the wake amplitude. Default = 1.0
  wake_time_scale  = <Real>     ! Factor to scale the time. Default = 1.0
  sr_wake_scale_with_length = <logical>  ! Scale SR wakes with element length. Default = T.
\end{example}

The \vn{lr_freq_spread} attribute is used to randomly spread out the long range mode frequencies
among different cavities. The spread is Gaussian in shape with an RMS of \vn{lr_freq_spread} * $F$
where $F$ is the frequency of a mode. Note: Wake modes that are locked to the fundamental
accelerating mode (\sref{s:lr.wake.file}), are not shifted.

The \vn{lr_self_wake_on} attribute can be used to turn off the longitudinal long-range self-wake
which is the longitudinal kick on the particles of a bunch due to the wake generated by these same
particles. [The transverse self wake is always zero.] The default setting of \vn{lr_self_wake_on} is
...
resistive wall wake are not valid over short time scales. Thus in this case the self wake should be
turned off.

The \vn{wake_amp_scale} is used to scale both the long-range and short-range wakes. This is $A_{amp}$
in \Eqs{wadzk} and \eq{wcrq}. The default is 1.0.

The \vn{wake_time_scale} is used to scale the $z$ factors in the short-range wake equations
(\sref{s:wake.short}) and to scale $t$ in the long-range wake equations (\sref{s:wake.long}):
\begin{example}
  z(used in equations) = wake_time_scale * z(actual)
  t(used in equations) = wake_time_scale * t(actual)
\end{example}
The default is 1.0.

If \vn{sr_wake_scale_with_length} is set to False (the default is True), the length factors in 
\Eqs{delvp} and \eq{pelqxw} are dropped. This is convenient for using zero length elements
with wake simulations.
...
The formulas used to compute the wake field are given in \sref{s:wake.fields}. \bmad has two modes
for tracking particles. One mode tracks individual particles one at a time. The other mode tracks
bunches of particles. Which mode is used for a given program is decided by the program. The wake
field is ignored when tracking individual particles and only used when tracking bunches.
...
The input file name for the short--range wake fields is specified using the \vn{sr_wake_file}
attribute. The file gives both monopole longitudinal and dipole transverse wakes. An example input
file is:
\begin{example}
  ! Pseudo Wake modes:
  ...
  /
\end{example}
Wakes are specified via a set of ``pseudo'' modes (\sref{s:wake.short}). The magnitude of \vn{z_max}
should be set to the maximum $z$ value at which the pseudo mode fit is valid. \bmad will check the
distance between particles does not exceed \vn{z_max}.  If it does, \bmad will report an error.

\index{none}\index{x_axis}\index{y_axis}
The \vn{polarization} parameter is used to specify the wake polarization. Possible settings for this
parameter are:
\begin{example}
  none    ! Default
  x_axis  
  y_axis 
\end{example}
The \vn{polarization} name may be abbreviated.  For example, if the \vn{polarization} is set to
\vn{x_axis}, there is no vertical kick from the pseudo mode.
...
The input file name for the long--range wake fields is specified using the \vn{lr_wake_file}
attribute. The file gives the wake modes by specifying the frequency (in Hz), R/Q (in
$\Omega$/meter$^{2m}$), Q, and m (order number), and optionally the polarization angle (in
radians/2pi) for each cavity mode. The input uses Fortran90 namelist syntax: The data begins with
the string \vn{\&long_range_modes} and ends with a slash \vn{/}. Everything outside this is
ignored. Each mode is labeled \vn{lr(i)} where \vn{i} is the mode index. An example input file is:
\begin{example}
  ...
  /
\end{example}
[Note: The quotation marks are needed with some compilers and not with others.]  A negative frequency
is used to designate wakes that are part of the fundamental accelerating mode. That is, the
frequency of such a mode is set to the value of \vn{rf_frequency} for the lattice element the wake
is associated with. It is an error to define such a mode for elements that do not have a
\vn{rf_frequency} attribute. \bmad needs to know if a wake is part of the fundamental mode due to
timing issues as discussed in \sref{s:rf.time}.

If the polarization angle is set to ``\vn{unpolarized}'' the mode is taken to be unpolarized. [Note:
Technically the unpolarized mode is actually two polarized normal modes. The axes of these two
normal modes can be chosen arbitrary as long as they are at right angles to each other.]
