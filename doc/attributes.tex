...
For reasons of expediency, the arithmetic expressions may not
themselves contain element attributes. However, an expression may
contain symbolic constants defined in the lattice. Example:
\begin{example}
  a_con = atan(0.3)
  ov1: overlay = \{s1[k2]:a_con, ...       ! OK.
  ov2: overlay = \{s2[k2]:0.7*q1[k1], ...  ! BAD, q1[k1] not allowed.
\end{example}
If it is desired to use an element attribute in an expression then
this may be effectively done by defining a variable to take its place.
As an example, consider an overlay that controls the \vn{g_err}
attribute of a bend \vn{bb} such that the value of \vn{g_err} is equal
to the bend strength \vn{g} times the control variable \vn{e_rel}:
\begin{example}
  ov1: overlay = \{bb[g_err]:bb[g]*e_rel\}, var = \{e_rel\}  ! BAD
\end{example}
Since the above construction is not permitted, an equivalent controller
may be defined by adding a second control variable:
\begin{example}
  ov1: overlay = \{bb[g_err]:g*e_rel, bb[g]:g\}, var = \{e_rel, g\}
\end{example}
