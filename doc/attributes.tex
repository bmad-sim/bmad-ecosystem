...
\end{example}
When combined with an element's offsets and tilt (\sref{s:offset}), A program can use
...
\begin{alignat}{1}
  x_{\mbox{meas}} &= N * r_1 + (1 + g_{x\mbox{err}) * \CRNO
          &\left[ (x_{\mbox{actual}} - x_{\mbox{off}}) * \cos (\theta_t + \theta_c) +
                  (y_{\mbox{actual}} - y_{\mbox{off}}) * \sin (\theta_t + \theta_c)  
  y_{\mbox{meas}} &= N * r_2 + (1 + g_{y\mbox{err}) * \CRNO
          &\left[ -(x_{\mbox{actual}} - x_{\mbox{off}}) * \sin (\theta_t - \theta_c) +
                  (y_{\mbox{actual}} - y_{\mbox{off}}) * \cos (\theta_t - \theta_c)  
\end{alignat}
where $N$ is the noise factor, $g_{x\mbox{err})$ and $g_{y\mbox{err})$
are the gain errors, $x_{\mbox{off}}$, and $x_{\mbox{off}}$ are the
element offsets, $\theta_t$ is the tilt angle, $\theta_c$ is the
crunch angle, and $r_1$ and $r_2$ are Gaussian random numbers whose
distribution has unit width.
