...
%-----------------------------------------------------------------
\section{Crystals \& Compounds for X-Rays}
\label{s:cryst.list}

For basic crystalographic and X-ray matter interaction cross-sections,
...
with some structre parameters comming from NIST. The list of available
structures is:
\begin{center}
\begin{tabular}{llll}
...
ALUMINUM_OXIDE                      & MERCURIC_IODIDE                           \\
AMBER                               & METHANE                                   \\
AMMONIA                             & METHANOL                                  \\
ANILINE                             & MIX_D_WAX                                 \\
...
BORON_CARBIDE                       & NYLON_TYPE_11_RILSAN                      \\
BORON_OXIDE                         & OCTANE_LIQUID                             \\
...
Example \vn{diffraction_plate} element. A) The wall of the diffraction
plate is divided into ``clear'' (white) and ``opaque'' (black)
sections. In this example there are two clear sections labeled \#1 and
\#2. B) All sections must be star shaped with respect to the
section's origin. In this example, The section is {\em not} star
shaped since a line drawn from the origin point $o$ to the point $p$
on the boundary intersects the boundary twice in between. In this case
the section can be made star shaped by moving the origin to $o'$.
  }
  \label{f:diff.plate}
\end{figure}

The wall of a \vn{diffraction_plate} (\sref{s:diff.plate}) specifies
where the openings in the plate are.

A \vn{wall} is comprised a a set of sections
(\sref{s:wall.section}). Each section must have its \vn{type}
attribute set to one of:
\begin{example}
  clear
  opaque
\end{example}
A section is called ``clear'' or ``opaque'' depending upon the
setting of its \vn{type} attribute.

A wall is comprised a a set of ``openings''. An opening is a clear
section along with a set of zero or more associated opaque sections.
The opaque sections associated with an opening are the opaque sections
immediatly following the clear section of the opening up to the next
clear section. Thus the number of openings is equal to the number of
clear sections and the first section of the wall must be a clear section.

The default behavior is that a photon will be transmitted if it
``hits'' any opening. A
photon is considered to have hit a given opening if its $(x,y)$
coordinates put in within the clear section of the opening but not
within any opaque section of the opening.

An example diffraction plate geometry
is shown in \fig{f:diff.plate}A.
 In the figure, there are two openings labeled \#1,
and \#2


There are no restructions as to overlap between various sections. 

To enable \bmad to quickly calculate whether a photon has landed on a
clear or opaque section, All sections must be ``star shaped'' with
respect to the section origin. That is, a line drawn from the section
origin to any point on the section boundary must not pass through any
boundary points of the section in between. This is illustrated in
\fig{f:diff.plate}B where the section is not star shaped since a line
drawn from the origin $o$ to the point $p$ on the boundary passes
through two boundary points in between. In this case the section can
trivially be made star shaped by moving the origin to point $o'$. If
it is not possible to make a section star shaped by moving the origin,
the section must be divided into multiple sections.
