...
\caption[Table of dependent variables.]{Table of dependent variables and 
  the independent variables they are calculated from for some selected element types.}
\label{t:dependent}
\end{table}
...
\vn{wall} attribute as detailed below. For X-ray capillay elements
...
non-capillay element will not affect particle tracking.
...
In order for \bmad to quickly track photons through a capillary,
...
To help reduce the complexity of specifying a wall, ``virtual''
...
\begin{example}
  grid = \{ 
    ...
    \ldots \}
\end{example}
Currently the only available type is \vn{rotationally_symmetric_rz}. The format for this type of grid is 
\begin{example}
  grid = \{ 
...
sources: the phase \vn{theta_t0} set for the mode and an overall phase
\vn{phi_ref} given by
\begin{example}
 phi_ref = phi0 + dphi0
\end{example}
...
\Begineq
  \theta_0 = 
  \begin{cases}
    \mbox{theta_t0} + \dsfrac{f}{f_0} \, (\mbox{phi_ref} + \mbox{phi0_err}) & 
    \mbox{lcavity element} \\
    \mbox{theta_t0} - \dsfrac{f}{f_0} \, \mbox{phi_ref} & 
    \mbox{rfcavity element}
  \end{cases}
\Endeq
where $f$ is the mode frequency and $f_0$ is the frequency of the fundamental.

The phase \vn{theta_t0} of the fundamental mode is autoscaled by Bmad
