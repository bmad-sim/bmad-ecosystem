...
%-----------------------------------------------------------------
\section{Element Orientation: Offset, Pitch, Tilt, and Roll Attributes}
\label{s:offset}
...
The following ``\vn{orientational}'' attributes can reorient a physical
element from the reference position:
\begin{example}
  x_offset = <Real>
  y_offset = <Real>
  z_offset = <Real>
  ...
  y_pitch  = <Real>
  tilt     = <Real>    
  tilt_err = <Real>    ! Crystal, mirror, and multilayer_mirror only.  
  roll     = <Real>    ! Sbend and rbend only.
\end{example}
\index{patch}\index{branch}\index{photon_branch}\index{sbend}
Not all of these attributes work for all element types.
...
the element $z$-axis. Transformation formulas between laboratory and
element reference frames is given in \sref{s:patch.coords}.
...
element, can have a curved surface. The attributes for specifying the
surface curvature in the plane of the reference trajectory are:
...
\begin{example}
  ...
  g_trans                = <Real>    ! Transverse 1/radius-of-curvature.
\end{example}
  a2_trans_curve         = <Real>    ! a2 curvature coefficient.
  a3_trans_curve         = <Real>    ! a3 curvature coefficient.
  a4_trans_curve         = <Real>    ! a4 curvature coefficient.
\end{example}

\index{a2_trans_curve}\index{a3_trans_curve}\index{a4_trans_curve}
In the plane transverse to the reference trajectory the attributes
for specifying the surface curvature are
\begin{example}
...
\vn{c2_curve} parameters. The surface curvature is parameterized by
...
The \vn{g_trans} parameter is equal to 1/\vn{R_trans} where
\vn{R_trans} is the radius of curvature of the element surface in the
plane transverse to the bend plane. A positive \vn{g_trans} indicates
that the element is concave from the point of view of the photons.

For the transverse plane the surface curvature is parameterized by the
equation
\Begineq
  -x = a_{2,tot} \, y^2 + a_{3,tot} \, y^3 + a_{4,tot} \, y^4
\Endeq

%%Note: The expansion of the equation
%%\Begibeq
%%  -x = \sqrt{R^2-z^2}
%%\Endeq
%%about a point $(z, x) = R \, (\sin\theta, -\cos\theta)$ is 
%%\Begineq
%%  -x = R \, \cos\theta 
