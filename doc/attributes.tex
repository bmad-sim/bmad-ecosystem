...
\Begineq
  ...
  \label{xynrr}
\Endeq
where $r_1$ and $r_2$ are Gaussian random numbers whose distribution
is centered at zero and has unit width. Additionally
\Begineq
  \begin{pmatrix}
    x \\
    y
...
y)_m$ prepresents the beam position corrected for ``known'' offsets,
...
\begin{alignat}{1}
  K_{a1} &= \frac{-\sqrt{\beta_b}}{\gamma \, \sqrt{\beta_a}} \, \Cbar_{22} \CRNO
  K_{a2} &= \frac{\sqrt{\beta_b}}{\gamma \, \sqrt{\beta_a}} \, \Cbar_{12}
  \label{kbgbc}
\end{alignat}
...
the beam.  Compairing \Eq{xyapk} with \Eq{xykkap} gives the relation
\begin{alignat}{1}
  K_{a1} &= \frac{k_{x1a} \, k_{y1a} + k_{x2a} \, k_{y2a}}{k^2_{x1a} + k^2_{x2a}} \CRNO
  K_{a2} &= \frac{k_{x1a} \, k_{y2a} - k_{x2a} \, k_{y1a}}{k^2_{x1a} + k^2_{x2a}} 
  \label{kkkkkk}
\end{alignat}
This equation is general and can be applied in either the lab or
measurement frame of reference.  \Eq{xynrr} can be used to transform
$(x_i, y_i)_{\mss{lab}}$ in \Eq{xyapk} to the measurement frame of
...
\Begineq
  K_{a1} \rightarrow K_{a1} + r_1 \, \frac{n_f}{N_s \, A_{\mss{osc}}} 
  \comma \qquad
  K_{a2} \rightarrow K_{a2} + r_2 \, \frac{n_f}{N_s \, A_{\mss{osc}}} 
  \label{kkrnn}
\Endeq
Using the above equations, the transformation from the laboratory
coupling to measured coupling is as follows: From a knowledge of the
laboratory $\Cbar$ and Twiss values, the laboratory $K_{a1}$ and
$K_{a2}$ can be calculated via \Eq{kbgbc}. Since the value of $d\phi$
does not affect the final answer, $d\phi$ in \Eq{xykkap} is choisen to
be zero.  Compairing this to \Eq{xyapk} gives
\Begineq
  \begin{pmatrix}
    k_{x1a} \\
    k_{y1a}
  ...
  \end{pmatrix}_{\mss{lab}}
\Endeq
Now \Eq{kkmkk} is used to convert to the measured $k$'s and
\Eq{kkkkkk} then gives the measured $K_{a1}$ and $K_{a2}$. Finally,
Applying \Eq{kkrnn} and then \Eq{kbgbc} gives the measured
...
$K_{b1}$ and $K_{b2}$ are defined by
\Begineq
  \begin{pmatrix}
    x_i \\
    y_i
  ...
  A_{\mss{osc}} \,
  \begin{pmatrix}
    K_{b1} \, \cos \phi_i + K_{b2} \sin \phi_i
    \cos \phi_i \\
  \end{pmatrix}_{\! \mss{lab}}
  \label{xyapk}
\Endeq
Compairing this to David and Rubin\cite{b:coupling} Eq.~55 gives
\begin{alignat}{1}
  K_{b1} &= \frac{\sqrt{\beta_a}}{\gamma \, \sqrt{\beta_b}} \, \Cbar_{11} \CRNO
  K_{b2} &= \frac{-\sqrt{\beta_a}}{\gamma \, \sqrt{\beta_b}} \, \Cbar_{12}
  \label{kbgbc}
\end{alignat}
The $k_{x1b}$, $k_{y1b}$, $k_{x2b}$ and $k_{y2b}$ are defined by using
\Eq{xykkap} with the ``a'' subscript replaced by ``b''. The
relationship between $K$ and $k$ is then
\begin{alignat}{1}
  K_{b1} &= \frac{k_{y1a} \, k_{x1a} + k_{y2a} \, k_{x2a}}{k^2_{y1a} + k^2_{y2a}} \CRNO
  K_{b2} &= \frac{k_{y1a} \, k_{x2a} - k_{y2a} \, k_{x1a}}{k^2_{y1a} + k^2_{y2a}} 
  \label{kkkkkk}
\end{alignat}
...
\Begineq
  ...
  -
  \begin{pmatrix}
    \tan^{-1} \left( \frac{k_{x2a}}{k_{x1a}} \right) \\
    \tan^{-1} \left( \frac{k_{y2b}}{k_{y1b}} \right)
  \end{pmatrix}_{\! \mss{meas}}
\Endeq
