...
Example:
\begin{example}
  ov: overlay = \{q1[k1]:\{0.234, 0.534\}, q2[k1], q3[k1], q4[k1]:\{0.37, 0.12, 0.92\}\}, 
        var = {time}, x_knot = \{0.0, 0.7, 1.0\}
\end{example}K
In this example there are three knot points for each slave parameter. For the slave parameter
\vn{q1[k1]} the knot points are $(0.0, 0.37)$, $(0.7, 0.12)$, and $(1.0, 0.92)$.  If a
...
\vn{interpolate} is set to \vn{spline}. For the cubic spline, extrapolation is only permitted over a
distance outside the x_knot range equal to the difference between the value of the x_knot end point
and its neighbor point. Thus in the above example, exterpolation will be allowed in the intervals
$[-0.7, 0.0]$ and $[1.0, 1.7]$.

an end point and the next nearest point.
