...
the fork, the \vn{fork} elementis considered to have zero length so the exit face
...
\begin{example}
  fringe_type    ! Used for everything with fringes but sad_mult elements
  fringe_kind    ! Used for sad_mult elements only  
...
The \vn{fringe_kind} switch is used for \vn{sad_mult} elements and may be set to 
...
\begin{example}
  ...
  full
\end{example}
\vn{none} means no fringe, the \vn{linear} setting only uses the
linear part of the fringe, \vn{nonlin_only} ignores the linear part of
the fringe, and \vn{full} uses the full fringe.
