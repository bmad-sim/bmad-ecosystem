...
\end{example}
Here the \vn{sk_q20w} overlay controls the \vn{a1} multipole attribute
of element \vn{sex_20w} and the length of \vn{sex_20w} is used as a
scale factor between the overlay's variable \vn{k1} and the controlled
attribute \vn{a1}. The potential problem here is that, to keep the
internal bookkeeping simple, the value of \vn{sex_20w[l]} is evaluated
once during parsing of the lattice file and never reevaluated. This is
in line with the general philosophy of immediate substitution
(\sref{s:arith}). If it is desired to use a variable element attribute
in an expression, this may be effectively done by defining a control
variable to take its place. Thus the above overlay may be recast as:
\begin{example}
...
\index{radius}\index{scale_multipoles}
...
radius'' and is set by the \vn{radius} attribute. The default value of
$r_0$, if the \vn{radius} is not given, is 1.0.  This behavior may be
...
Here \vn{n} ranges from 0 (dipole component) through 21. Example:
\begin{example}
  q1: quadrupole, l = 1.2, b2_elec = 1e6
\end{example}
See \sec{s:elec.field} for how electric multipoles are defined.

Electric multipoles are never scalled by the element's field strength as they are with
magnetic multipoles.
...
\end{example}
The exception is \vn{multipole} and \vn{ab_multipole} elements do not have the
\vn{mulipoles_on} attribute. Rather they can be toggles on/off using the \vn{is_on}
attribute.

%-----------------------------------------------------------------
\section{EM Fields -- Grids and Maps}
\label{s:em.fields}
...
It is important to keep in mind that the fields specified by ``\vn{field
= \{...\}}'' is only used with \vn{runge_kutta} and \vn{boris}
...
\vn{mode_to_autoscale} parameter is explained in
...
\index{em_field}\index{field_factor}
For an \vn{em_field} element, the \vn{master_scale} defaults to the
element parameter
\begin{example}
  field_factor
\end{example}
For all other elements, \vn{master_scale} defaults to \vn{none} which
means that the \vn{master_scale_value} is taken to be unity when
...
Bmad (\sref{s:autoscale}) so that a particle with $z = 0$ will go
...
the fields are given in Section~\sref{s:rf.fields.phys}. 
...
the setting of \vn{ele_anchor_pt}. The grid uses curvilinear coordinates if
\vn{curved_coords} is set to \vn{True}.}
  \label{f:bend.grid}
\end{figure}

For bend elements (\vn{sbend}s, \vn{rbend}s), the grid coordinates, by default, are
Cartesian and {\em not} curved. The orientation of the grid with respect to the bend is
determined by the placement of the anchor point as shown in \fig{f:bend.grid}. In this
...
the bend, the \vn{curved_coords} component of \vn{grid} can be set to \vn{True}. The
setting of \vn{curved_coords} is ignored for non-bend elements.
