...
The following ``\vn{orientational}'' attributes can shift the physical
element in space
\begin{example}
  x_offset = <Real>
  y_offset = <Real>
  ...
  y_pitch  = <Real>
  tilt     = <Real>    
  ref_tilt = <Real>    ! Shifts reference orbit.
...
The \vn{x_pitch} attribute rotates an element such that the exit face
of the element is displaced in the $+x$--direction as shown in
figure~\ref{f:pitch}.  That is, \vn{x_pitch} represents a rotation
around the positive $y$-axis.

Similarly, the \vn{y_pitch} attribute rotates an element about the
negative $x$--axis so that the exit face of the element is displaced
in the $+y$--direction. 
