...
orbit. In this coordinate system, the surface is an ellipsoid plus a fourth order polynomial
in $x$ and $y$:
\Begineq
  {-z} = \frac{1}{g_z} \, \left[ 1 - \sqrt{1 - \sign(g_x) \, (g_x \, x)^2 - \sign(g_y) \, (g_y \, y)^2} \right] + 
  \sum_{2 \le i+j \le 6} c_{ij} \, x^i \, y^j
  \label{xs2ij4}
\Endeq
where the $c_{ij}$ parameterize the fourth order polynomial and $g_x, g_y$, and $g_z$ parameterize
the ellipsoid with $\sign$ being the sign function
\Begineq
