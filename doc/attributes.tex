...
\begin{alignat}{1}
  x_m &= N * r_1 + (1 + g_{x\mss{err}}) * 
          \left[ (x - x_{\mss{off}}) * \cos (\theta_t + \theta_c) +
                  (y - y_{\mss{off}}) * \sin (\theta_t + \theta_c) 
          \right] \\ 
  y_m &= N * r_2 + (1 + g_{y\mss{\scriptsize err}}) * 
          \left[ -(x - x_{\mss{off}}) * \sin (\theta_t - \theta_c) +
                  (y - y_{\mss{off}}) * \cos (\theta_t - \theta_c)  
          \right]
\end{alignat}
where $(x,y)$ is the actual beam position, $(x_m, y_m)$ is the measured position, 
$N$ is the noise factor, $g_{x\mss{err}}$ and $g_{y\mss{err}}$
are the gain errors, $x_{\mss{off}}$, and $x_{\mss{off}}$ are the
