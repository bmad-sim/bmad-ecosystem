...
the first plane and the last plane must be equadistant from the element center.
  \item
...
so there is no good way to cut an element at an arbitrary position.]
  \item
In a bend with \vn{curved_coords} = False, The setting of \vn{ele_anchor_pt} must be
\vn{center}.
  \end{itemize}
...
will then track to the ending plane and finally track backards from the ending plane to
...
\begin{example}
  t1: sbend, k1 = 2, taylor_field = \{
    ...
    dz = 1.2,                r0 = (0, 0, 2.0),
    field_scale = 1.3,       curved_coords = False,
    master_parameter = k1,
    plane(-2) = \{\{x: 0.3| 01\}, \{y: 1.3, 1 2\}, \{z: 0.7, 2 1\}, ...\},
    plane(-1) = \{\{x: 0.6|\}, \{y: 0.4|11\}, \{z: 0.7, 1 0\}, ...\}, 
    ...  \}, field_calc = fieldmap
\end{example}
