...
``actual'' (that is, as calculated from a model lattice). For orbits, the
relationship between measured position $(x, y)_m$ and actual position $(x, y)_a$ is
\Begineq
  \begin{pmatrix}
    x \\
    y
...
position $(x, y)_m$ thus would represents the beam position corrected for
...
The phase and coupling measurement is considered to be the result of
measuring the beam at a detector over $N_s$ turns while the beam oscillates
at a normal mode frequency with some amplitude $A_{\mss{osc}}$. The measured betatron
phase $\phi$ is then
\Begineq
  \begin{pmatrix}
    \phi_a \\
  ...
  \end{pmatrix}_{\! a}
\Endeq
the measured phases are independent of the gain, offset and tilt errors.
In practice this results in a very clean measurement.

The measured coupling is given by
