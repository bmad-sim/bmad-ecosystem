...
coceptionally simple, the rule is imposed that, for an aperture at the
...
the one whose center is closest to the section origin $(x_0, y_0)$.  
...
(\sref{s:limit}. Indeed, unless a program is specifically setup, the
...
The vacuum chamber wall for a particular lattice branch is the sum of
all the chamber walls of the individual elements. The chamber walls of
any two elements may not overlap. The exception is when the
\vn{superimpose} attribute for a wall of an element is set to True. In
this case, any other wall cross-sections from any other elements that
overlap the superimposed wall are discarded.
...
\begin{example}
  ...
  trunk
\end{example}
The geometry of a crotch is shown in \fig{f:crotch}. Two pipes,
called ``leg1'' and ``leg2'', merge into
one pipe called the ``trunk'' pipe. 
The trunk pipe can be either upstream or downstream of the leg pipes.
To describe this situation, five sections are needed: One section 
in each leg pipe which need to have their \vn{type} attribute to 
\vn{leg1} and \vn{leg2}, and three sections in the trunk
