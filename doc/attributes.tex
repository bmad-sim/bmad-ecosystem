...
\vn{phi_ref} for an \vn{lcavity} (\sref{s:lcavity}) differs in sign to
that for an \vn{rfcavity} (\sref{s:rfcavity}). This being the case,
$\theta_0$ is
\Begineq
  \theta_0 = 
  \begin{cases}
    \mbox{theta_t0} + \frac{\omega}{\omega_0} \, (\mbox{phi_ref} + \mbox{phi0_err}) & 
    \mbox{lcavity element}
    \mbox{theta_t0} - \frac{\omega}{\omega_0} \, \mbox{phi_ref} & 
    \mbox{rfcavity element}
  \end{cases}
\Endeq
where $\omega$ is the mode frequency and $\omega_0$ is the frequency of the fundamental.
