...
omitted and will default to zero. To be complete, the number of terms $N$
for the $e$ or $b$ vectors is
\begin{equation}
  N = \frac{L}{dz} + 1
\end{equation}
where $L$ is the length of the RF element. $N$ must be even. The
$n$\Th element in the $e$ or $b$ arrays, with $n$ running from 0 to
$N-1$, is associated with a frequency $\omega_n$
\begin{equation}
  \omega_n = \begin{cases}
...
Recipes\cite{b:nr}.  In practice, the terms at the highest frequencies
(at $n \sim N/2$) may be small. These terms may be dropped from the
$e$ and $b$ arrays. In this case, if there are only $\wt N$ terms
specified, $\wt N$ must be even. The first $\wt N/2$ terms are associated
with $n = 0, \ldots, \wt N/2$ and the last $\wt N/2$ terms are associated
with $n = N - \wt N/2, \ldots, N - 1$.
