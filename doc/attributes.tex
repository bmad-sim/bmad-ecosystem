...
element surface.
  \item[\vn{Position defining elements}]  \Newline
Elements that are used to manipulate the reference orbit or other the 
positions of other elements are
\begin{example}
  branch \& photon_branch    ! \sref{s:branch}
  fiducial                  ! \sref{s:fiducial}
  floor_shift               ! \sref{s:floor.ele}
  girder                    ! \sref{s:girder}
  patch                     ! \sref{s:patch}
\end{example}
...
\vn{rolls} and \vn{tilts}, \vn{ref_tilt} rotates the reference orbit
along with the physical element. A \vn{bend} with a \vn{ref_tilt} of
$\pi/2$ will bend a beam vertically downward (\sref{s:global}). Note
that the \vn{ref_tilt} attribute of \bmad is the same as the MAD
\vn{tilt} attribute.
...
measured with respect to the entrance coordinates as shown in
\fig{f:reflect.orient}.

%-----------------------------------------------------------------
\subsection{Position Defining Elements}
...
\index{patch}\index{branch}\index{photon_branch}\index{sbend}
Not all attributes are applicable to all element types. The following
rules apply:
  \begin{description}
  \item[patch, branch, photon_branch] 
These elements use their orientation attributes to modify the
reference orbit itself.
  \item[sbend, rbend]
The \vn{tilt} attribute for bend elements controls the reference orbit.
The \vn{roll} attribute is to be used rotate the physical element
about the reference orbit (see below).
  \item[crystal, mirror, multilayer_mirror]
Like the bend elements, \vn{tilt} here controls the reference orbit.
The \vn{tilt_err} attribute is to be used to rotate the physical
element about the reference orbit.
  \item[all other elements]
Elements not mentioned above can use the \vn{tilt} attribute to 
rotate the element about the reference orbit.
  \end{description}
...
\vn{girder} elements do not affect the reference orbit so \vn{tilt}
and \vn{tilt_tot} will be the same for any element whose \vn{tilt}
attribute affects the reference orbit.
...
\begin{example}
  ...
  crystal
\end{example}
For these elements, the \vn{surface} setting is the default.  A
setting of \vn{surface} means that the aperture limits are calculated
in the frame of reference of the surface of the element. To keep
things conceptually simple, the \vn{offset_moves_aperture} setting is
ignored for these elements. If a \vn{surface} aperture is used, the
aperture calculation implicitly assumes that
\vn{offset_moves_aperture} is set to True. For \vn{entrance_end} or
\vn{exit_end} apertures, the aperture calculation implicitly assumes
that \vn{offset_moves_aperture} is set to False.
...
Various X-Ray elements, namely \vn{crystal}, \vn{mirror}, and
\vn{multilayer_mirror}, can have a curved surface.
...
\Begineq
  {-x} = \sum_{2 \le i+j \le 4} c_{ij} \, z^i \, y^j
  \label{xs2ij4}
\Endeq
...
\begin{example}
  ...
  curvature_z<i>_y<j>  = <Real>   
\end{example}
where \vn{<i>} and \vn{<j>} are integers in the range 0 through 4. Example:
\begin{example}
  c2: crystal, curvature_z2_y0 = 37, ...
\end{example}
in this example, \vn{curvature_z2_y0} corresponds to the $c_{20}$ term
in \Eq{xs2ij4}. 

If the \vn{d_detec} or \vn{d_source} attributes are
set, they set the $c_{20}$ and $c_{40}$ coefficients so that the
element will focus light coming from a detector a distance
\vn{d_source} away onto a detector a distance \vn{d_detec} from the
element as indicated in \fig{f:surface}. Explicitly, these coefficients are
calculated via the equations
\begin{align}
  c_{20} &= \mbox{curvature_z2_y0} + \frac{1}{2 \, R_c} \CRNO
  c_{40} &= \mbox{curvature_z4_y0} + \frac{1}{4 \, R_c^3} 
\end{align}
where 
\Begineq
  \frac{2}{R_c} = \frac{\theta_{g,in}}{d_s} + \frac{\theta_{g,out}}{d_d}
\Endeq
with
\begin{align}
  d_s &= \begin{cases} 
  ...
  \end{cases}
\end{align}
and $\theta_{g,in}$ and $\theta_{g,our}$ are the entrance and exit
graze angles.
