...
The coupling measurement is considered to be the result of
...
Compairing \Eq{xyapk} with \Eq{xykkpp} gives the relation
\begin{alignat}{1}
  K_{a1} &= \frac{k_{x1a} \, k_{y1a} + k_{x2a} \, k_{y2a}}{k^2_{x1a} + k^2_{x2a}} \CRNO
  K_{a2} &= \frac{k_{x1a} \, k_{y2a} - k_{x2a} \, k_{y1a}}{k^2_{x1a} + k^2_{x2a}} 
  \label{kkkkkk}
\end{alignat}
This equation is general and can be applied in either the lab or measurement frame 
of reference.
\Eq{xynrr} can be used to transform $(x_i, y_i)_{\mss{lab}}$ in \Eq{xyapk}
...
via \Eq{kbgbc}. Since the value of $d\phi$ does not affect the final answer, $d\phi$ in 
\Eq{xykkpp} is choisen to be zero. 
Compairing this to \Eq{xyapk} gives
\Begineq
  \begin{pmatrix}
    k_{x1a} \\
    k_{y1a}
  ...
  \end{pmatrix}_{\mss{lab}}
\Endeq
Now \Eq{kkmkk} is used to convert to the measured $k$'s and \Eq{kkkkkk} then gives the measured 
$K_{a1}$ and $K_{a2}$. Finally, Applying \Eq{kkrnn} and then \Eq{kbgbc} 
