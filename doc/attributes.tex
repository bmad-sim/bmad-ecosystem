...
A \vn{wall} is comprised a a set of \vn{sections} as discussed in
\sref{s:wall.section}.


A wall is comprised a a set of ``clears''. An opening is a clear
section along with a set of zero or more associated mask sections.
The mask sections associated with an opening are the mask sections
...
A \vn{wall} is comprised a a set of sections (\sref{s:wall.section}).
Each section must have its \vn{type} attribute set to one of:
\begin{example}
  clear
  mask
\end{example}
A section is called ``clear'' or ``mask'' depending upon the
...
within any mask section of the opening.

An example diffraction plate geometry is shown in \fig{f:diff.plate}A.
In the figure, there are two openings labeled \#1, and \#2
...
clear or mask section, All sections must be ``star shaped'' with
