...
called the ``\vn{forked} \vn{branch}''. 

The only difference between \vn{fork} and \vn{photon_fork} is that the
default particle type for the forked branch forked from a \vn{fork}
element is the same particle type as the base branch. The default
particle type for the forked branch from a \vn{photon_fork} element is
...
\begin{example}
  direction    = <+/- 1>      ! Direction of branch being forked to.
...
\vn{direction} attribute of the \vn{fork} element indicates whether the
...
The offset, pitch, and tilt attributes of a forking element can be
...
\begin{example}
  ...
  pb: photon_fork, x_offset = 0.01, to_line = x_line
  x_line: line = (X1, X2, ...)           ! Defines forked branch
  use, from_line
\end{example}
...
Forking elements have zero length and within a lattice branch, a
forking element acts like a \vn{marker}. Using the above example, a
...
attributes of a forking element only affect particles that are
transferred to the \vn{to_line} when arriving at the forking element.
...
The ``-'' indicates that there is no equivalent setting in SAD.

The \vn{eps_step_scale} is equivalent to the SAD \vn{eps} attribute. The default
\vn{eps_step_scale} value is 1 which is the same default as SAD.
