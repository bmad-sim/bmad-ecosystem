...
\begin{tabular}{llll} \toprule
  ...
  AC_Kicker         & \ref{s:ac.kick}     &  Multipole         & \ref{s:mult}     \\
  BeamBeam          & \ref{s:bbi}         &  Null_Ele          & \ref{s:null.ele} \\
  Beginning_Ele     & \ref{s:begin.ele}   &  Octupole          & \ref{s:oct}      \\
...
\item Scale the field using the funciton $A(\delta t)$. If \vn{ref_time_offset} is set to \vn{True} (the 
default), $\delta t = t - t_0 - t_{ref}$ where $t$ is the time and $t_0$ is set by the \vn{t_offset}
attribute, and $t_{ref}$ is the reference time at the entrance end of the element. If
\vn{ref_time_offset} is set to \vn{False}, $\delta t = t - t_0$. Note: The time $t$ is always the
absolute time
\end{enumerate}
The time is calculated, like the RF cavities, using either relative or absolute time
(\sref{s:rf.time}).  Note that if a particle is tracked for multiple turns in a closed lattice with
relative time tracking, the field of an \vn{ac_kicker} element is independent of which turn it is and
only depends upon the phase space $z$ coordinate.
...
to calculated before the position of the \vn{fiducial} element is
...
\end{example}
This lists the variables that control the slave attributes.
...
\index{var}
Attributes specific to a \vn{Overlay} element are:
\begin{example}
  var = \{<var1>, <var2>, ...\}   ! List of variables.
\end{example}
This lists the variables that control the slave attributes.
...
A straight line element like a \vn{drift} or a \vn{quadrupole} has the exit face
...
parameters (\sref{s:depend}) and these parameters will be computed. Here the \vn{patch} is called
...
geometry of the \vn{patch} will vary as well. See Section~\sref{s:ex.erl} for an example.
...
can be used to set the reference energy/momentum at the exit end. It is is an error
...
Example:
\begin{example}
  wig2: wiggler, l = 1.6, b_max = 2.1, n_pole = 8
\end{example}
...
maximum field on the centerline, and $k$ is given in terms of the pole length (\vn{l_pole}) by
\Begineq
...
(\sref{c:methods}), the vertical focusing is assumed small so averaged over a period the horizontal
...
With \vn{field_calc} set to \vn{helical_model}, and with \vn{bmad_standard} tracking, the vertical
transport is the same as with the \vn{planar_model} and the horizontal transport is the same as the
vertical.
...
choice of phase is such that, with an even number of poles, a particle that enters the wiggler
on-axis will leave the wiggler on-axis provided there is an even number of poles. Notice that with
