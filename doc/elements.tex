...
Additionally, the following conversion is done on all \vn{Rbend}s
\begin{example}
...
The \vn{e_tot_ref_geometry} or \vn{n_multipass_ref} attributes are
only used when a bend is part of a \vn{multipass} line and is used to
set the reference geometry of the bend. See section~\sref{s:multipass}
for more details.
...
\begin{example}
  ...
  etap_a0  = <Real>,  etap_b0  = <Real>   ! Beginning eta' 
  etap_a1  = <Real>,  etap_b1  = <Real>   ! Ending eta'
...
\vn{patch}. Hence, the \vn{x} coordinate of a particle going through a
patch with a positive \vn{x_offset} will be decreased.
...
\sref{s:wiggler.phys} for more details.
...
\end{example}
As explained in \sref{s:wiggler.phys} this type of wiggler is
