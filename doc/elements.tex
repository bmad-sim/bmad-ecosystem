...
\item Scale the field using the function $A(\delta t)$. If \vn{ref_time_offset} is set to \vn{True} (the 
default), $\delta t = t - t_0 - t_{ref}$ where $t$ is the time, $t_0$ is the value of the
\vn{t_offset} attribute, and $t_{ref}$ is the reference time at the entrance end of the element. If
\vn{ref_time_offset} is set to \vn{False}, $\delta t = t - t_0$. Note: Unlike RF cavity elements,
the time $t$ here is always the absolute time (\sref{s:rf.time}) and not the relative time.
\end{enumerate}

There are two ways to specify the dimensionless time variation $A(\delta t)$ of the field. One way is to
specify points on the $A(\delta t)$ curve using the \vn{amp_vs_time}
attribute. Example:
\begin{example}
...
\end{example}
For times before of the range specified by \vn{amp_vs_time}, the amplitude is taken to be the
amplitude of the first point of the range. Similarly, for times after the the range specified by
\vn{amp_vs_time}, the amplitude is taken to be the amplitude of the last point of the range.
...
phases \vn{phi} with the \vn{frequencies} attribute are \vn{radians/2pi}.
...
\end{example}
\vn{parameter[n_part]} is the nominal number of particles of the strong beam. \vn{parameter[n_part]}
is a global setting (\sref{s:param}) and is used for all \vn{beambeam} elements.  To vary the number
of particles in an individual \vn{beambeam} element, the \vn{charge} attribute is used.  The default
is \vn{charge} = -1 which indicates that the strong beam has the opposite charge of the weak beam.
...
the strong beam. Note that in \mad the attributes used to offset the strong beam are called \vn{xma}
and \vn{yma}. \vn{x_pitch} and \vn{y_pitch} gives the beam--beam interaction a crossing angle. This
is the full crossing angle, not the half-angle. The \vn{tilt} attribute can be used to tilt the
strong beam.
...
strong beam can have a finite \vn{sig_z}, the length of the \vn{beambeam} element is zero. This is
achieved by drifting a particle at the beginning and at the end of tracking so that the longitudinal
starting point and ending points are at the \vn{beambeam} element. For example, with \vn{n_slice} =
2 the calculation would proceed as follows:
\begin{example}
  0. Start with the particle longitudinally at the \vn{beambeam} element.
  1. Offsets, pitches and tilt are applied to convert from laboratory to element body coordinates (\sref{s:lab.body.transform}.
  ...
  7. Offsets, pitches and tilt are applied to convert from element body to laboratory coordinates.
\end{example}
...
parameters at $s_0$ are set by \vn{beta_a}, \vn{beta_b}, \vn{alpha_a}, and \vn{alpha_b}.  If
\vn{beta_a} is zero (the default), the $a$-mode Twiss parameters as calculated from the lattice is
used. Similarly, if \vn{beta_b} is zero (the default), the $b$-mode Twiss parameters as calculated
from the lattice is used.
...
An \vn{beginning_ele} element, named \vn{BEGINNING}, is placed at the beginning of every branch
...
automatic and it is not permitted for a lattice file to define any other \vn{beginning_ele}
elements.
...
{Coordinate systems for (a) \vn{rbend} and (b) \vn{sbend} elements.  For the bends drawn as viewed
from ``above'' (viewed from positive $y$), \vn{g}, \vn{angle}, \vn{rho}, \vn{e1} and \vn{e2} are all
positive.}
  \label{f:bend}
\end{figure}
...
\vn{e1} and \vn{e2} for an \vn{rbend} gives a rectangular magnet (\fig{f:bend.rbend}). Zero \vn{e1}
and \vn{e2} for an \vn{sbend} gives a wedge shaped magnet (\fig{f:bend.sbend}).  An \vn{sbend} with
...
\sref{s:ref}). \vn{g} = 1/\vn{rho} is the curvature function and is proportional to the design
dipole magnetic field. The true field strength is given by \vn{g}~+~\vn{g_err} so changing
\vn{g_err} leaves the design orbit unchanged but varies a particle's orbit. See \Sref{s:ex.chicane}
for an example.
  \index{h1}\index{h2}
  \item[h1, h2] \Newline
...
it is for an \vn{sbend}.  However, after reading in a lattice, \bmad will internally convert all
\vn{rbend}s into \vn{sbend}s, additionally, the \vn{l_chord} attribute will be set to the input
\vn{l}, and \vn{l} will be set to the true path length (see above). Alternatively for an \vn{rbend},
instead of setting \vn{l}, the \vn{l_arc} attribute can be set to the true arc length.
  \index{ref_tilt}
  \item[ref_tilt] \Newline
The \vn{ref_tilt} attribute rotates a bend about the longitudinal axis at the entrance face of the
bend. \vn{ref_tilt} = 0 bends the bends the element in the $-y$ direction. See \fig{f:tilt.bend}. It
is important to understand that \vn{ref_tilt}, unlike the \vn{tilt} attribute of other elements,
bends both the reference orbit along with the physical element. Note that the MAD \vn{tilt}
attribute for bends is equivalent to the \bmad \vn{ref_tilt}. Bends in \bmad do not have a \vn{tilt}
attribute.
  \end{description}
...
When a crystal is bent (\sref{s:s.curve}), the $\bfH$ vector is assumed follow the surface
...
A \vn{converter} element represents a target onto which particles are slammed in order to generate
...
\begin{example}
  ...
  E_out_min       = <Real>      ! Minimum outgoing particle energy.
  E_out_max       = <Real>      ! Maximum outgoing particle energy.
  angle_out_max   = <Real>      ! Maximum outgoing angle.
  species_out     = <SpeciesID> ! Output species.
  E_tot           = <Real>      ! Output reference energy.
  P0c             = <Real>      ! Output reference momentum
  beta_a          = <Real>      ! Output a-mode beta.
  beta_b          = <Real>      ! Output b-mode beta.
  alpha_a         = <Real>      ! Output a-mode alpha.
  alpha_b         = <Real>      ! Output b-mode alpha
  eta_x           = <Real>      ! Output horizontal dispersion.
  eta_y           = <Real>      ! Output vertical dispersion
  etap_x          = <Real>      ! Output horizontal dispersion derivative.
  etap_y          = <Real>      ! Output vertical dispersion derivative.
\end{example}

The species of the outgoing particles is specified by the \vn{species_out} parameter
(\sref{s:species.name}).

The reference energy (\sref{s:ref.energy}) can be set by setting the reference total energy, \vn{E_tot}
or the reference momentum \vn{p0c$). One of these must be set but not both.

A converter element must be the first element in a lattice branch (\sref{s:branch.def}) excepting
possible \vn{marker} elements. A \vn{fork} (\sref{s:fork}) element may be used to connect the
elements that come before the converter to the converter. Example:
\begin{example}
  parameter[particle] = electron
  parameter[geometry] = open
  to_cvter: fork, to_line = post_linac
  cvter, species_out = positron, p0c = 3e6
  pre_linac: line = (..., to_cvter)
  post_linac: line = (cvter, ...)
  post_linac[particle] = positron
  use, pre_linac
\end{example}
The line before the converter, \vn{pre_linac}, ends in a \vn{fork} element called \vn{to_cvter}
which connects to the line named \vn{post_linac} containing the converter. The reference particle
for the \vn{post_linac} line is set to \vn{positron} in contrast to the \vn{pre_linac} line where
\vn{electron}s are the reference particles. It is not manditory for the reference particle of the
branch containing the \vn{converter} to be the same as the \vn{species_out} parameter of the
converter itself though typically this will be what is wanted.  In the above example, the reference
momentum of the \vn{converter} is set to 3~MeV and this reference momentum will used in the rest of
the \vn{post_linac} line up to any possible \vn{lcavity} elements. The reference energy for any
possible marker elements before the \vn{converter} in the \vn{post_linac} line will be automatically
set to the reference energy in the \vn{pre_linac} line at the \vn{fork} element.

The \vn{distribution} parameter specifies the distribtion of outgoing particles for a given
converter thickness. Multiple \vn{distribution} instances with differing thicknesses may be present
in an element. The actual thickness of the converter will be taken to be the element's, length
\vn{L} parameter. During tracking, the outgoing distribution will be computed by interpolating
between the two distributions that bracket the actual thickness. The exception is when there is only
one\vn{distribution} present. In this case, the element length is ignored that the supplied
distribution will be used independent of the \vn{thickness} set in the distribution. If there are
multiple \vn{distributition}s, specifying an element length that is outside the range of thicknesses
will generate an error.

There is a program called \vn{converter_element_modeling} which caculates converter output
distributions. This program outputs text of the correct format which then can be used when
constructing converter elements. The \vn{converter_element_modeling} program is based upon the
\vn{Geant} simulation toolkit for the simulation of the passage of particles through matter. The
\vn{converter_element_modeling} program is distributed with \bmad. Specifically, it is in the
directory
\begin{example}
  \$ACC_ROOT_DIR/util_programs/converter_element_modeling
\end{example}
[See your local \bmad Geru if you don't know how to find this directory.] There is documentation for
running the program in this directory.

When tracking, for every particle impinging on a target, one outgoing particle will be generated.
All outgoing particles will be assigned a ``\vn{weight}'' that represent how many outgoing particles
a single incoming particle will generate. For example, if an incoming particle with a particular
energy would generagte 0.42 particles on average, the outgoing particle will have a weight of 0.42.

To make simulations more efficient, the \vn{E_out_min}, \vn{E_out_max}, and \vn{angle_out_max}
parameters can be set to restrict the energy and angle range of outgoing particles. If the outgoing
particles are restricted in energy or angle, the weight of the outgoing particles will be
approriately adjusted such that the weighted distribution of outgoing particles within the energy
and/or angle restricted range is independent of the whether or not there is are restrictions.
A value of zero (the default) for any one of these parameters means that that parameter is not used.

A converter element does not have a well defined transfer matrix. 
The Twiss and dispersion parameters
...
\end{center}
\toffset
See \sref{s:list.crab.cavity} for a full list of element attributes.
...
The \vn{to_element} attribute for a forking element is used to designate the element of the target
...
\begin{example}
  ...
  marker
\end{example}
When the \vn{to_element} is not specified, the default is to connect to the beginning of the target
branch if \vn{direction} is 1 and to connect to the end of the target branch if \vn{direction} is
...
The reference energy of a target branch line, needs to be set using line parameter statements
(\sref{s:beginning}). The default reference particle type of a branch line will be a \vn{photon} is
using a \vn{photon_fork} or will be the same type of particle as the base branch if a \vn{fork}
element is used.
...
(\sref{s:use}) are searched for fork elements that branch to new target branches. If found, the
...
girder but may appear in the girder slave list. If a \vn{drift} does appear in the slave list, only
\vn{marker} elements, but not \vn{drift} elements will be ignored when determining if elements are
consecutive. Note: If a drift-like element is desired to be supported by a \vn{girder}, use a
\vn{pipe} element instead. \vn{Marker} elements present in a girder support region, but not
mentioned in the girder slave list, are simply ignored.
...
There are two types of \vn{group} elements: \vn{Expression} based and \vn{spline} based.
...
The general syntax for a \vn{spline} based  \vn{group} element is
\begin{example}
...
Example of a \vn{spline} based \vn{group} element:
\begin{example}
...
a spline interpolation based upon the knot points specified (\sref{s:go.syntax}).
...
If the arithmetical expression used for an \vn{group} contains an element attribute,
...
\begin{example}
  ...
  rf_frequency    = <Real>    ! RF frequency (Hz).
  voltage                     ! Cavity voltage. Dependent attribute (\sref{s:depend}).
  l_hard_edge     = <Real>    ! Active region length. Dependent attribute (\sref{s:depend}).
  n_cell          = <Int>     ! Number of cavity cells. Default is 1.
  longitudinal_mode = <Int>   ! Longitudinal mode. Default is 1. May be 0 or 1.
\end{example}

The dependent variable \vn{voltage} attribute can be used in place of \vn{gradient} as discussed in
\sref{s:depend}.  \vn{voltage} is a dependent attribute and is defined to be
\begin{example}
  voltage = gradient * L
\end{example}
...
\sref{s:rf.time}.  The phase $\phi_{\text{ref}}$ is
\begin{example}
...
(\sref{s:bmad.com}), the \vn{e_loss} attribute can be used to modify the gradient in order to
...
particles in a bunch. \vn{e_charge} is the charge on an electron (Table~\ref{t:constants}). Notice
that the \vn{e_loss} term is independent of the sign of the charge of the particle.
...
When an element's \vn{tracking_method} is set to \vn{runge_kutta} (\sref{s:tkm}), the ``active
region'' over which there is a finite field is \vn{n_cell} half-wave pillbox resonators where each
pillbox has length $\lambda/2$ (\sref{s:rf.fields}). The default setting for \vn{n_cell} is 1. The
dependent parameter \vn{l_hard_edge} is set to the length of the active region. The active region
should have a length less than the length of the element. If the length of the element is not equal
to the active region, the active region is centered in the element and the regions to either side
are treated as field free.

When an element's \vn{tracking_method} is set to \vn{runge_kutta}, the fields used with
\vn{field_calc} set to \vn{bmad_standard} is described in Section~(\sref{s:rf.fields}). With
\vn{cavity_type} set to \vn{standing_wave}, the longitudinal mode is set by the
\vn{longitudinal_mode} parameter. The possible values are 0 or 1 and the default setting is 1. 

Note: When an element's \vn{tracking_method} is set to \vn{bmad_standard}, settings for \vn{n_cell},
and \vn{longitudinal_mode} are ignored.
...
parameters. Similarly, \vn{c11_mat0}, etc. are the beginning components of the
$\bfC$ coupling matrix (\sref{s:coupling}).
...
\vn{delta_time} is zero, the transfer map through the element will be the $\Bf M$ matrix as discussed
...
There are two types of \vn{overlay} elements: \vn{Expression} based and \vn{spline} based.
...
The general syntax for a \vn{spline} based  \vn{overlay} element is
\begin{example}
...
If the arithmetical expression used for an \vn{overlay} contains an element attribute,
care must be taken if that element attribute is changed. This is discussed in
\sref{s:arith} and \sref{s:go.syntax}.
...
{A) A \vn{patch} element can align its exit face arbitrarily with respect to its entrance face. B)
The reference length of a \vn{patch} element is the longitudinal distance from the entrance origin
...
with respect to one another as shown in \fig{f:patch}A. The length \vn{l} of a \vn{patch} is a
dependent (\sref{s:depend}) parameter and is the longitudinal ($z$ component) distance from the
entrance origin to the exit origin using the exit end reference coordinates as shown in
\fig{f:patch}B. See \sref{s:patch.prob} for a further discussion on the coordinate system of a
\vn{patch}. Notice that while the length of the patch is defined with respect to the exit
...
\vn{patch}'s attributes and is independent of everything else.  Example:
\begin{example}
...
reference coordinates of the next element (or any element downstream of the \vn{patch} as long as
the elements in between have zero length) before the reference coordinates of the \vn{patch} are
computed. A \vn{flexible} \vn{patch} will have the its offsets, pitches, and tilt as dependent
...
The coordinates of the lattice element downstream of a \vn{flexible} \vn{patch} is can be computed
...
\vn{E_center}, and \vn{sig_E}, the second mode is characterized by the parameters \vn{E2_center} and
\vn{sig_E2}. The probability of emitting a photon in the second mode is given by \vn{E2_probability}.
...
\begin{example2}
  ...
  uniform
\end{example2}
The \vn{gaussian} setting gives Gaussian distributions for the two modes with width
set by \vn{sig_E} and \vn{sig_E2}. The \vn{uniform} setting gives a flat
...
Energy width of the two modes in eV. See \vn{energy_distribution} for more
details.
...
\sref{s:multipass}. \vn{e_charge} is the charge on an electron (Table~\ref{t:constants}). Notice
...
map is only used for \vn{runge_kutta}, \vn{fixed_step_runge_kutta}, and \vn{symp_lie_bmad}
tracking (\sref{s:tkm}).  Only the fundamental mode has an analytical formula for the symplectic
tracking. In the future, the other modes could be used with \vn{symp_lie_bmad} tracking using a
...
``Soft edge'' end fields may be simulated by using Runge-Kutta tracking and setting the \vn{field_calc}
parameter of the element to \vn{soft_edge}. Example:
\begin{example}
...
\vn{Sx}, \vn{Sy}, \vn{Sz}). The syntax for the spin part uses the first or second forms as
...
\index{k1_pseudo}\index{g_max}\index{l_period}
...
\begin{example}
  ...
  n_period   = <Real>  ! The number of periods (L / L_period). A settable dependent attribute (\sref{s:depend}).
  ...
  k1_pseudo            ! Pseudo K1 value. A Dependent attribute (\sref{s:depend}).
  ...
  x_ray_line_len = <Real> ! Deprecated.
\end{example}
...
\index{x_ray_line_len}
\vn{x_ray_line_len} is the length of an associated x-ray synchrotron light line measured from the
exit end of the element. This is used for machine geometry calculations and is irrelevant for
lattice computations. \vn{x_ray_line_len} is deprecated. Use a \vn{photon_fork} (\sref{s:fork})
with an attached lattice branch instead.
...
For the \vn{planar_model}, wigglers use a simplified model where the wiggler has infinitely wide
...
maximum field on the centerline, and $k$ is given in terms of the period length (\vn{l_period}) by
\begin{equation}
...
(\sref{c:methods}), the vertical focusing is assumed small so averaged over many periods the horizontal
...
\end{alignat}
with \vn{k1_pseudo} (a dependent element attribute) being the linear focusing constant. Notice that the focusing
is only in the vertical plane. The subscript \vn{pseudo} is used to remind the user that this is not the same
as the \vn{k1} attribute of a quadrupole since the focusing is only in one plane.
