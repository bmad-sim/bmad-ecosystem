...
\begin{example}
  ...
  rho                = <Real>     ! Design bend radius. A settable dependent param (\sref{s:depend}).
...
the lattice is constructed. \vn{Overlay} elements where {\em none} of its slave elements are part of
the finished lattice are ignored and will not be part of the finished lattice. It is not permitted
to have \vn{overlay} elements that have some slave elements that are part of the finished lattice
...
\end{example}
Where \vn{L} is the length of the \vn{patch} as shown in \fig{f:patch} and \vn{beta_ref} is the
...
it is recommended that a \vn{patch} element be split into two consecutive patches if the \vn{patch}
has finite \vn{l} and \vn{E_tot_offset} values.
...
\vn{Ramper} elements are a type of \vn{control} element (\sref{s:lord.slave}) used to make
variations in the attributes of other elements while a program is running. The \vn{ramper} element
is similar to an \vn{overlay} element except that \vn{ramper} elements are designed to efficiently
control large sets of elements. \vn{Ramper} elements where implemented to solve the problem of
simulating machine ramping where the strength of many elements in a machine are varied continuously
as a function of time. The drawback of \vn{ramper} elements is that they can be only be used with
programs that that are designed to handle them. In particular, the \vn{long_term_tracking} program
that is part of the \bmad Distribution. \etcetc...
...
\end{center}
\toffset
See \sref{s:list.ramper} for a full list of element attributes along with a their units.

There are two types of \vn{ramper} elements: \vn{Expression} based and \vn{spline} based.
The general syntax for a \vn{expression} based \vn{ramper} element is
\begin{example}
...
The general syntax for a \vn{spline} based  \vn{ramper} element is
\begin{example}
...
An \vn{ramper} element is used to control the attributes of other elements. 
