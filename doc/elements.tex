...
\begin{tabular}{|l|l||l|l|} \hline
  ...
  Drift              & \ref{s:drift}         &  Photon_Fork        & \ref{s:fork}       \HH
  Ecollimator        & \ref{s:col}           &  Photon_Init        & \ref{s:photon.init}\HH
  Fiducial           & \ref{s:fiducial}      &  Pipe               & \ref{s:monitor}    \HH
  Floor_Shift        & \ref{s:floor.ele}     &  Rcollimator        & \ref{s:col}        \HH
...
For \vn{a$n$} and \vn{b$n$}, $n$ is in the range 0 through 21.
...
\begin{example}
  autoscale_phase     = <Logical>  ! Automatic phase scaling (\sref{c:autoscale}).
  autoscale_amplitude = <Logical>  ! Automatic amplitude scaling (\sref{c:autoscale}).
...
\end{example}
where \vn{<coef>} is a real number and \vn{<out>}, \vn{<e1>},
\vn{<n1>}, etc. are integers. Notice that the first form uses
a comma ``,'' to separate the \vn{<coef>} from \vn{<e1>} while
the second form uses a vertical bar ``|'' to separate \vn{<coef>}
from \vn{<n1>}.
...
A \vn{\{<Wig_Term>\}} has the form (see \Eqs{cm1} through \eq{cm3}):
\Begineq
  \{ C, k_x, k_y, k_z, \phi_x, \phi_y, \phi_z \}
\Endeq
The \vn{polarity} value is used to scale the magnetic field. By
default, \vn{polarity} has a value of 1.0.  Example:
\begin{example}
  wig1: wiggler, l = 1.6,
        term(1) = \{0.03, 3.00, 4.00, 5.00, 0, 0, 0.63\},
        term(2) = ...
  ...
...
\vn{Note:} When the wiggler model was first developed, the definition of the field
was somewhat different. The old definition assumed horizontal and vertical symmetry
and had a different normalization:
The old style first form was
\begin{alignat}{5}
  B_x &= -&\wt C \, &\dfrac{k_x}{k_y} & \sin(k_x x) \sinh(k_y y) \cos(\kzz) &\CRNEG
  B_y &=  &\wt C \, &                 & \cos(k_x x) \cosh(k_y y) \cos(\kzz) &\qquad \mbox{! Old style} \CRNEG
  B_s &= -&\wt C \, &\dfrac{k_z}{k_y} & \cos(k_x x) \sinh(k_y y) \sin(\kzz) &\label{f1} \\
  & \makebox[1pt][l]{with $k_y^2 = k_x^2 + k_z^2$ .} &&&&  \nonumber
\end{alignat}
The old style second form was
\begin{alignat}{5}
  B_x &=  &\wt C \, &\dfrac{k_x}{k_y} & \sinh(k_x x) \sinh(k_y y) \cos(\kzz) &\CRNEG
  B_y &=  &\wt C \, &                 & \cosh(k_x x) \cosh(k_y y) \cos(\kzz) &\qquad \mbox{! Old style} \CRNEG
  B_s &= -&\wt C \, &\dfrac{k_z}{k_y} & \cosh(k_x x) \sinh(k_y y) \sin(\kzz) &\label{f2} \\
  & \makebox[1pt][l]{with $k_y^2 = k_z^2 - k_x^2$ ,} &&&&  \nonumber
\end{alignat}
The old style third form was
\begin{alignat}{5}
  B_x &=  &\wt C \, &\dfrac{k_x}{k_y} & \sinh(k_x x) \sin(k_y y) \cos(\kzz) &\CRNEG
  B_y &=  &\wt C \, &                 & \cosh(k_x x) \cos(k_y y) \cos(\kzz) &\qquad \mbox{! Old style} \CRNEG
  B_s &= -&\wt C \, &\dfrac{k_z}{k_y} & \cosh(k_x x) \sin(k_y y) \sin(\kzz) &\label{f3} \\
  & \makebox[1pt][l]{with $k_y^2 = k_x^2 - k_z^2$ .} &&&& \nonumber
\end{alignat}
The correspondence between $\wt C$ and $C$ is given by comparing
\Eqs{f1}, \eq{f2}, and \eq{f3} with \Eqs{cm1}, \eq{cm2}, \eq{cm3}. In
...
\begin{example}
  wig1: wiggler, l = 1.6, \&
        ...
        term(2) = \{0.03, 3.00, 4.00, 5.00, 0, 0, 0.63\}, \&  ! New style
        term(3) = ...
\end{example}
Note: For backwards compatibility in terms of tracking results, \bmad,
when doing symplectic tracking, will use a different vector potential
with fields defined in the old style (\sref{s:wiggler.std}). If the
field is zero at the ends of the wiggler, this distinction does not
matter. However, if the field is not zero at the ends, the ``edge
kick'' inherent in the symplectic tracking will be different.
