...
calculate the position of this element before the the position of the \vn{fiducial} element is
...
the setting of \vn{direction}. To shift the the global position of a forked line, \vn{patch}
...
\vn{<e1>} corresponding to $e_1$, \vn{<e2>} corresponding to $e_2$, etc.  For example, a term in a
Taylor map that was
\Begineq
...
would be written using the third form as:
\begin{example}
  tt4336 = 2.73
\end{example}

The spin (\sref{s:spin.dyn}) part of the transport map (\sref{s:spin.map}) is described using
quaternions (\sref{s:quat}). The syntax for the spin part uses the first or second forms as
described above. The four components of a quaternion are labeled (\vn{S1}, \vn{Sx}, \vn{Sy},
\vn{Sz}).  An individual Taylor term is of the form:
\Begineq
...
``\vn{map type}'' wigglers and are discussed in \sref{s:wiggler.map}. Wigglers that are described
assuming a periodic field are called ``\vn{periodic type}'' wigglers and are described in
...
\vn{Map type} wigglers are modeled using a field map as described in section~\vn{s:fieldmap}.
\vn{Map type} wigglers have their \vn{field_calc} parameter set to \vn{fieldmap} and \vn{field_calc}
will, for historic purposes default to \vn{fieldmap} if there is a field map present.

\index{polarity}
\index{term (for a wiggler)}
Attributes specific to \vn{map type} \vn{wigglers} element are:
\begin{example}
...
The \vn{b_max} attribute for a \vn{map type} \vn{wiggler} is set by \bmad to be the maximum field
on-axis computed for \vn{polarity} = 1.
...
Attributes specific to \vn{periodic type} wigglers the attributes are: 
...
\end{example}
Notice that here \vn{b_max} is an independent parameter to be set in the lattice file and not
a dependent parameter which is calculated by \bmad.
...
\begin{alignat}{1}
  ...
  \label{bbkykz}
\end{alignat}
where the input parameter $\vn{b_max}$ is the maximum field on the centerline and $k$ is
...
This type of With \vn{bmad_standard} tracking and transfer matrix
calculations the vertical focusing is assumed small so averaged over a period the horizontal motion
looks like a drift and the vertical motion is modeled as a combination focusing quadrupole and
focusing octupole giving a kick\cite{b:corbett}
\Begineq
  \frac{dp_y}{dz} = k1 \left( y + \frac{2}{3} \, k_z^2 \, y^3 \right)
\Endeq
where
\begin{alignat}{2}
  g_max &= \frac{e \, B_{\max}}{P_0 \, (1 + p_z)} \\
  k1 &= \frac{-1}{2} \, g_max^2 
\end{alignat}
with $k1_\text{pseudo}$ being the linear focusing constant. Notice that the focusing is only in
the vertical plane.
...
With \vn{bmad_standard} tracking (\sref{s:tkm}), particle transport used a simplified model where
the motion is averaged over many periods. For the \vn{planar_model}, the horizontal motion looks
like a drift. For the

It is assumed that there is an integer number
...
\index{x_ray_line_len}
All wigglers have the following attributes:
\begin{example}
...
When the wiggler model was first developed, the only \vn{map type} available was a Cartesian map
(\sref{s:fieldmap}).  The syntax for specifying a this Cartesian map was different from what it is
currently. The old syntax for a Cartiesian map term was:
\Begineq
