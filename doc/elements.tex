...
\begin{tabular}{|l|l||l|l|} \hline
  ...
  Drift             & \ref{s:drift}       &  Pipe              & \ref{s:monitor}  \HH
  E_Gun             & \ref{s:e.gun}       &  Quadrupole        & \ref{s:quad}     \HH
  Ecollimator       & \ref{s:col}         &  Rbend             & \ref{s:bend}     \HH
  ElSeparator       & \ref{s:elsep}       &  Rcollimator       & \ref{s:col}      \HH
  EM_Field          & \ref{s:em.field}    &  RFcavity          & \ref{s:rfcav}    \HH
...
\begin{tabular}{|l|l||l|l|} \hline
  ...
  Instrument         & \ref{s:monitor}       &  Photon_Branch     & \ref{s:branch}     \HH
...
The advantage to using \vn{em_field} elements is that they
...
center of \vn{ib} is at $s = 7$~meters (\vn{ib} starts at $s = 0$ which is
the beginning of \vn{q1} and ends at $s = 14$ which is the end of
\vn{q2}). Like other elements, pitch is calculated from the center of
a \vn{girder} element (see Sec.~\ref{s:offset}). The center of
\vn{q2} is at $s = 12$ so the distance between the center of \vn{ib}
...
\begin{example}
  dE = gradient_tot * L * cos(twopi * phi)
\end{example}
\index{multipass}
...
\end{example}
and the phase \vn{phi} is
\begin{example}
  phi = phi_particle + phi0 + dphi0 + phi0_err + dphi0_ref
\end{example}
\begin{example}
...
\vn{phi_particle} is proportional to the absolute time the particle reaches 
...
\begin{example}
  ...
  phi_ref = phi0 + dphi0
\end{example}
Where z is the particles' $z$ phase space coordinate (\sref{s:phase.space}).
