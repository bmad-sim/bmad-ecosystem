...
\begin{example}
  ...
  exact_multipoles   = <Logical>  ! Curved coordinate correction? Default False.
...
The \vn{exact_multipoles} logical (default \vn{False}) determines if the multipole fields,
both magnetic and electric, and including \vn{k1} and\vn{k2}, are corrected for the finite
curvature of the reference orbit in a bend. ``exact'' here means that the multipole fields
are corrected using a multipole expansion up to 21\St order. That is, the difference
between what is implemented here and a true exact calculation should be insignificant
under realistic conditions.

The implementation of the multipole field is imported from PTC (\sref{c:ptc}) with the kind help
of \'Etienne Forest. To turn on the calculation with PTC based tracking (\sref{c:methods}), set
\begin{example}
