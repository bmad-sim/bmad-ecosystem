...
\index{marker}
An \vn{e_gun} element can only be used in a ``linear'' (non-recirculating) lattice branch
Only one \vn{e_gun} can be present in a lattice branch and, if it is present, it must be (except for possibly
\vn{marker} elements) the first element in the branch. 
...
reaches the cavity. See section~\sref{srf.time} for a discussion on
