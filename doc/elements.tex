...
\begin{example}
  ...
  interpolation = <switch>                                   ! linear or spline (default).
\end{example}
...
\begin{example}
  ...
  wall_radius     = <Real>    ! Wall radius. Dependent attribute (\sref{s:depend}).
\end{example}
...
\vn{longitudinal_mode} parameter. The possible values are 0 or 1 and the default setting is 1. If
the mode is set to 0, the \vn{wall_radius} dependent parameter is calculated using \Eq{rk}. If the
mode is 1, the wall radius is infinite and in this case \vn{wall_radius} is set to zero. For the
mode set to 0, the number of cells, \vn{n_cell}, is restricted to one since, in this case, if there
were to be more than one cell, alternating cells would alternately accelerate and decelerate.
