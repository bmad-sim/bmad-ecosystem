...
\begin{tabular}{|l|l||l|l|} \hline
  ...
  Bend_Sol_Quad     & \ref{s:bsq}         &  Null_Ele          & \ref{s:null.ele} \HH
  Branch            & \ref{s:branch}      &  Octupole          & \ref{s:oct}      \HH
  ...
  Floor_Position    & \ref{s:floor.ele}   &  RFcavity          & \ref{s:rfcav}    \HH
...
\begin{tabular}{|l|l||l|l|} \hline
  {\it Element}      & {\it Section}         & {\it Element}      & {\it Section}      \HH
  Branch             & \ref{s:branch}        &  Match             & \ref{s:match}      \HH
  Capillary          & \ref{s:capillary}     &  Monitor           & \ref{s:monitor}    \HH 
  Crystal            & \ref{s:crystal}       &  Mirror            & \ref{s:mirror}     \HH
  Custom             & \ref{s:custom}        &  Multilayer_Mirror & \ref{s:multilayer} \HH
  Drift              & \ref{s:drift}         &  Patch             & \ref{s:patch}      \HH
  Floor_Position     & \ref{s:floor.ele}     &  Photon_Branch     & \ref{s:branch}     \HH 
  Instrument         & \ref{s:monitor}       &  Pipe              & \ref{s:monitor}    \HH
...
\begin{tabular}{|l|l||l|l|} \hline
  {\it Element}  & {\it Section}     & {\it Element}  & {\it Section}    \HH
  Group          & \ref{s:group}     &   Init_Ele     & \ref{s:init.ele} \HH
  Girder         & \ref{s:girder}    &   Overlay      & \ref{s:overlay}  \HH
\end{tabular}
}
\caption{Table of element classes not directly used for tracking.}
...
\begin{tabular}{|l|l||l|l|} \hline
  {\sl Attribute Class}      & \s                & {\sl Attribute Class}      & \s              \HH
  a$n$, b$n$ multipoles      & \ref{s:multip}    & Is_on                      & \ref{s:is.on}   \HH
  Aperture Limits            & \ref{s:limit}     & Length                     & \ref{s:l}       \HH
  Chamber wall               & \ref{s:wall}      & Offsets, pitches, and tilt & \ref{s:offset}  \HH
  Description strings        & \ref{s:string}    & Reference energy           & \ref{s:energy}  \HH 
  Field table or map         & \ref{s:em.fields} & Symplectify                & \ref{s:symp}    \HH
...
This formula holds for both positive and negative angles.
  \index{exact_fringe}
  \item[exact_fringe]
When \vn{e1} or \vn{e2} are non-zero, \vn{bmad_standard} tracking, by default, uses a second order
map to handle the edge focusing. In some cases, for instance in a chicane, this is
not good enough. With \vn{exact_fringe} to True, the wedge shape of the edge is properly 
taken into account. The hard edge model is still used here. \vn{exact_fringe} = True will
be somewhat slower.
  \index{angle}
  \item[angle] \Newline
...
\begin{example}
  ...
  E_tot_start  = <Real>     ! Starting reference energy for a branch (eV).
  p0c_start    = <Real>     ! Starting reference momentum for a branch (eV).
  lattice_type = <Switch>   ! Define linear or circular branch
  particle     = <type>     ! Eg: positron, photon, etc.
\end{example}
...
using the transformation (cf.~Figure~\ref{f:global.coords})
\begin{equation}
...
The \vn{E_tot_start} and \vn{p0c_start} attributes set the starting
reference energy of the branch. This is analogous to the
\vn{parameter[E_tot]} and \vn{parameter[p0c]} parameters used to set
the root branch (\sref{s:param}). This is useful, for example, for a
photon branch that is branching from a storage ring where the photon
energy is not simply related to the particle energy. If
\vn{E_tot_start} and \vn{p0c_start} are both not set, the starting
reference energy of the root branch is taken to be the same as the reference
energy at the branch element in the that is being branched from (the ``from''
branch). Not setting both \vn{E_tot_start} and \vn{p0c_start} is only
allowed if the mass of the particle of the ``from'' branch is the same
as the mass of the particle in the ``to'' branch.

The \vn{lattice_type} attribute sets whether the branch is linear or
circular and is analogous to the \vn{parameter[lattice_type]} parameter. 
The default is \vn{linear_lattice}. Notice a difference here: The default for
the \vn{parameter[lattice_type]} parameter is determined by whether there is
an \vn{lcavity} element in the root branch. 

The \vn{particle} attribute sets the type of particle used for calculations in the branch.
This is analogous to the \vn{parameter[particle]} attribute for the root branch. 

Example showing an injection line branching to a ring branching to an x-ray line:
\begin{example}
  parameter[root_branch_name] = inj_line
  inj: line = (..., ring, ...)               ! Define the injection line
  use, inj                                   ! Injection line is the root
  ring: branch, to = ring_layout, 
          lattice_type = circular_lattice    ! Branch to the ring
  ring_layout: line = (..., x_ray, ...)      ! Define the ring
  x_ray: photon_branch, E_tot_start = 1e3, to = x_layout ! 1 KeV reference photon.
  x_layout: line = (...)                     ! Define the x-ray line
\end{example}
...
momentum, the reference momentum and energy associated with an \vn{e_gun} element
is calculated as outlined in Section~\sref{s:energy}.
...
%-----------------------------------------------------------------
\section{Floor_Position}
\label{s:floor.ele}
\index{floor_position|hyperbf}

A \vn{floor_position} element sets the global floor coordinates.

General \vn{floor_position} element attributes are:
\begin{center}
\tt
\begin{tabular}{|l|l||l|l|} \hline
  {\sl Attribute Class}  & \s              & {\sl Attribute Class}      & \s              \HH
...
\index{x_position}\index{y_position}
\index{z_position}\index{theta_position}
\index{phi_position}\index{psi_position}
Attributes specific to a \vn{floor_position} elements are:
\begin{example}
  x_position      = <Real>
  y_position      = <Real>
  z_position      = <Real>
...
The \vn{floor_position} element sets the global position
(\sref{s:global}) of the reference coordinates at the exit end of the
element.  Here, the transfer map through the \vn{floor_position} will
be the unit map. That is, the phase space coordinates of a particle
will not change when tracking through such an element.

Setting the floor position at an element will affect the floor
position of all elements following the element but will not affect the
preceeding element. The \vn{floor_position} element can be used, for
example, to restore the correct geometry when a section of the lattice
is represented by, say, a \vn{taylor} type element.

Example:
\begin{example}
  floor_set: floor_position, x_position = 3.2
\end{example}
...
\begin{tabular}{|l|l||l|l|} \hline
  ...
  RF Fields              & \ref{s:em.fields}  & Chamber wall               & \ref{s:wall}     \HH
\end{tabular}
\end{center}
\toffset
...
\begin{example}
  ...
  n_cell       = <Integer> ! Number of cavity cells.
\end{example}
...
\vn{boris} and \vn{symp_lie_bmad} tracking
...
layers of differencing material. The materials used is given by
...
decreased. The \vn{t_offset} offsets the particle time
See \sref{s:patch.coords} for formulas for the reference
...
\begin{example}
  ...
  include_fringe = <Logical> ! Model quadrupole fringe fields? Default: False.
 \end{example}
...
The \vn{include_fringe} switch toggles whether the fringe fields are
taken into account\cite{b:quad.fringe}. Note: Fringe fields may not be
accounted for in all tracking methods.
...
\begin{tabular}{|l|l||l|l|} \hline
  ...
  Reference energy       & \ref{s:energy}     & Tracking \& transfer map   & \ref{c:methods}   \HH
  Aperture Limits        & \ref{s:limit}      & Length                     & \ref{s:l}         \HH
  Hkick \& Vkick         & \ref{s:kick}       & RF Fields                  & \ref{s:em.fields} \HH
  RF Couplers            & \ref{s:rf.coupler} & RF Wakes                   & \ref{s:rf.wakes}  \HH
  Chamber Wall           & \ref{s:wall}       &                            &                   \HH
\end{tabular}
\end{center}
\toffset
