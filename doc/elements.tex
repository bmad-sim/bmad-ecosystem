...
the case where PTC is doing exact model tracking but the \vn{exact_multipoles} switch is set to
\vn{off}, PTC will still be treating the multipoles as \vn{horizontally_pure} even though
\bmad tracking will be treating them as straight line multipoles.
...
The \vn{geo} logical is used for translation between \bmad and SAD lattice files. It is
ignored in all \bmad computations. \vn{geo} marks which edge of a solenoid where, in SAD,
the orientation of the solenoid is determined by the orientation parameters in the SAD
\vn{SOL} element at the edge.
...
\begin{example}
  ...
  geo             = <Logical> ! Used for translation between \bmad and SAD.
\end{example}
\index{x_offset}
