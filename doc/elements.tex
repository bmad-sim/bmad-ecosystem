...
\begin{example}
  ...
  is_mosaic          = <Logical>
  mosaic_thickness   = <Real>
  mosaic_angle_rms_in_plane  = <angle>
...
\end{example}
That is, \vn{sad_mult} elemements cannot be used to simulate RF cavities or bends (but a \vn{sad_mult}
...
parameter of the elemnt to \vn{soft_edge}. Example:
\begin{example}
...
\vn{Periodic type} wiggers are modeled assuming the field is periodic longitudinally. \vn{Periodic
...
depending upon the geometry. As a result, using the pole length was depracated and instead the
...
in the vertical and horizontal planes is the same as with the transport in the verticalplane with
...
map was different from what it is currently. The old syntax for a Cartiesian map term was:
\Begineq
