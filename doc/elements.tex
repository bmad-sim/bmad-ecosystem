...
Examples:
\begin{example}
  ramp_pc: ramper = \{*[e_tot]:\{4e+08, 4.00532e+08, 4.01982e+08, ...\}\},
                var = \{time\}, x_knot = \{0, 0.001, 0.002, ...\}
  amp = 1e9;  omega = 0.167;  t0 = 0.053
  ramp_rf: ramper = \{rfcavity::*[voltage]:amp*sin(omega *(t + t0)),
        rfcavity::*[phi0]:0.00158*t^2 \}, var = \{time\}
\end{example}
Here \vn{ramp_pc} is used to ramp the reference energy \vn{e_tot}. The \vn{*[e_tot]} construct
means that the ramper will be applied to the \vn{e_tot} attribute of all elements. \vn{Ramp_pc}
uses a spline fit to interpolate between the knot points specified in the element definition.

The \vn{ramp_rf} ramper in the above example varias the \vn{voltage} and \vn{phase} (\vn{phi0}) of
all \vn{rfcavity} elements (specified by \vn{rfcavity::*}). Here mathematical expressions are used
