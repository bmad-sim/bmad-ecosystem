...
There are three types of wigglers. Those that that are described using a magnetic field map
(``\vn{map type}''), those that are described assuming a periodic field (``\vn{periodic type}''),
and those that use a custom field. \vn{periodic type} wigglers are have their \vn{field_calc}
attribute \vn{s:field.calc} set to one of:
\begin{example}
...
The \vn{b_max} attribute for a \vn{map type} \vn{wiggler} is the maximum field computed for
\vn{polarity} = 1.
...
%-----------------------------------------------------
\subsection{Periodic Type Wiggler Element Tracking}
...
%-----------------------------------------------------
\subsection{Old Syntax for Wiggler Cartesian Map}
\label{s:old.wiggler}

When the wiggler model was first developed, the only \vn{map type} available was a Cartesian map (\sref{s:fieldmap}).
The syntax for specifying a this Cartesian map
was different from what it is currently. The old syntax for a Cartiesian map term was:
\Begineq
