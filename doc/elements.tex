...
\sref{s:wiggler.periodic}. Wigglers that use a custom field have their. \vn{periodic type} wigglers are have
their \vn{field_calc} attribute \sref{s:field.calc} set to one of:
\begin{example}
  planer_model    ! Default
  helical_model
\end{example}
while \vn{map type} wigglers have their \vn{field_calc} attribute set to \vn{fieldmap} and custom type
wigglers have their \vn{field_calc} attribute set to \vn{custom}.

%-----------------------------------------------------
\subsection{Map Type Wigglers}
\label{s:wiggler.map}

The \vn{map type} wigglers are modeled using a field map as described in section~\vn{s:fieldmap}.
...
\end{example}
In this example the field is reversed.

The \vn{b_max} attribute for a \vn{map type} \vn{wiggler} set by \bmad to be the maximum field
on-axis computed for \vn{polarity} = 1.

Note: There is no \vn{bmad_standard} tracking for a \vn{map_type} \vn{wiggler}. \vn{symp_lie_bmad}
type tracking is discussed in \sref{s:symp.track}
...
Example:
\begin{example}
  wig2: wiggler, l = 1.6, b_max = 2.1, n_pole = 7
\end{example}
