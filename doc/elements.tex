...
elements must be used. In fact, the \vn{direction} parameter is mearly an indicator to a program on
...
\vn{exp1}, \vn{exp2}, etc. are the arithmetial expressions, that are functions of \vn{var1},
...
a spline interpolation based upont the knot points specified (\sref{s:go.syntax}).
...
The default value of \vn{ds_slice} is 0.01 meter.
  ...
  \item[\vn{E_center}, \vn{E_center2}] \Newline
...
See Section~\sref{s:rowland} for an example lattice that can be used to simulate
a Rowland circle spectrometer using a \vn{photon_init} element.
...
\vn{taylor} element. The phase spece transport is completely determined by the Taylor map and is
