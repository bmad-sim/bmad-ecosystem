...
cross-sectional slices.  B) Each cross-section is made up of a number of vertices.
The segments between the vertices can be either a line segment, the arc of a circle, 
...
constructing an arc between two given points. The one chosen (a) for 
...
of an ellipse. It is manditory that a cross-section be convex. That
...
Each individual cross-section begins with the word \vn{cross}. The
...
arc used to construct a circular segment of a cross-section is the
``convex'' arc. That is, the arc that maintains the convexivity of the
cross-section as a whole as shown in Fig.~\ref{f:cap.arc}. The same
logic applies to the construction of an ellipse segment.
...
$(0,0)$ must be in the interior of all cross-sections and all cross
...
constructed. The solution chosen will be the shorter one. With this
...
shown in Fig.~\ref{f:cap.arc}. Also note that it is perfectly fine for the
total capillary volume to not be convex.
