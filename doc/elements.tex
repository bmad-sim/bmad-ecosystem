...
\vn{A}, $\calO_C$ is the reference frame at the downsream end of
...
elements that are supported are ariented with respect to one another.
...
\vn{origin_ele_ref_pt} which constructus the girder's
...
cooincident with the entrance end frame of an element named
...
the first element in the list of supoorted elements. In this example
...
Once the \vn{origin} referece frame is established, the reference
frame of the girder can be offset
from the \vn{origin} fram using the parameters
\begin{example}
  dx_origin    dtheta_origin
  dy_origin    dphi_origin
  dz_origin    dpsi_origin
\end{example} 
The orientation of the \vn{girder}'s referece frame from the \vn{origin}
...
\vn{origin} fram is the same as the girder's reference frame.
...
When a \vn{girder} ``supports'' an element, then that elements
orientation attributes (\vn{x_offset}, etc. \sref{s:offset}) give the
orientation of the element with respect to the \vn{girder}. The
orientation with respect to the local reference coordinates is given
by \vn{x_offset_tot}, which are computed from the orientation
attributes of the element and the \vn{girder}.
...
\begin{example}
  ...
  d: drift, l = 8
  ib: girder = \{q1, q2\}, x_pitch = 0.002, x_offset = 0.03
  this_line: line = (q1, d, q2)
  use, this_line
\end{example}
\index{overlay}
In this example, \vn{ib} supports elements \vn{q1} and \vn{q2}. The
``center'' of \vn{ib} is at $s = 7$~meters (\vn{ib} starts at $s = 0$
which is the beginning of \vn{q1} and ends at $s = 14$ which is the
end of \vn{q2}). Like other elements, the orientation of a girder is
calculated with respect to its center (more on this below). The center
of \vn{q2} is at $s = 12$ so the distance between the center of
\vn{ib} and \vn{q2} is $ds = 5$. The \vn{x_pitch} of \vn{ib} produces
an offset at the center of \vn{q2} of $0.01 = 0.002 * 5$. This, added
to the offsets of \vn{ib} and \vn{q2}, give the total offset of
\vn{q2} to be $0.06 = 0.01 + 0.03 + 0.02$. (A small angle
approximation is used here, in actuallity, small angles are not
assumed.) The total \vn{x_pitch} of \vn{q2} is $0.022 = 0.02 + 0.001$.
...
The energy kick felt by a particle, assuming no phase slipage, is 
\begin{example}
...
\mad. The integrated energy kick felt by a particle, assuming no phase slipage, is 
\begin{example}
