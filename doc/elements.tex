...
\begin{example}
  ...
  E_tot           = <Real>      ! Output reference energy (eV).
  P0c             = <Real>      ! Output reference momentum (eV).
  beta_a_out      = <Real>      ! Output a-mode beta.
  beta_b_out      = <Real>      ! Output b-mode beta.
  alpha_a_out     = <Real>      ! Output a-mode alpha.
  alpha_b_out     = <Real>      ! Output b-mode alpha
  eta_x_out       = <Real>      ! Output horizontal dispersion.
  eta_y_out       = <Real>      ! Output vertical dispersion
  etap_x_out      = <Real>      ! Output horizontal dispersion derivative.
  etap_y_out      = <Real>      ! Output vertical dispersion derivative.
\end{example}
...
momentum would generagte 0.42 particles on average, the outgoing particle will have a weight of 0.42.

In a tracking simulation, a single outgoing particle is generated for each incoming particle.
To make simulations more efficient, the \vn{pc_out_min}, \vn{pc_out_max}, and \vn{angle_out_max}
parameters can be set to restrict the momentum and angle range of outgoing particles. If the outgoing
particles are restricted in momentum or angle, the weight of the outgoing particles will be
approriately adjusted such that the weighted distribution of outgoing particles within the momentum
