...
\begin{example}
  ...
  field_master   = <Logic> ! Is ks or bs_field value the master (\sref{s:field.master})?
\end{example}
...
\begin{example}
  ...
  field_master       = <Logic>  ! See \ref{s:field.master}.
\end{example}
...
or \vn{hgapx} using the value of \vn{fint} or \vn{hgap} will only be done before lattice expansion
(\sref{s:expand}).
...
\begin{example}
  ...
  field_master = <Logic>  ! See \sref{s:field.master}.
\end{example}
...
set by the exit end coordinate system (see \fig{f:patch}). In this case, the length of the patch is
calculated as the perpendicular distance between the origin of the patch's entrance coordinate
system and the exit face of the patch as shown in \fig{f:patch}B. If \vn{ref_coords} is set to
\vn{entrance_end}, The reference coordinates are set by the entrance end coordinate system. In this
case the length is calculated as the perpendicular distance between the entrance face and the origin
of the exit coordinate system which is equal to the value of
\vn{z_offset}. Example:
\begin{example}
...
changes through the patch via \Eq{zbctt}. To provide flexibility, the \vn{t_offset} attribute can be
