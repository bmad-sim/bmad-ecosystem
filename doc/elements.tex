...
When using \vn{runge_kutta} tracking (\sref{s:tkm}) through a field map (\sref{s:fieldmap}), the
\vn{bmad_standard} field is \vn{n_cell} half-wave resonators (\sref{s:rf.fields}). The longitudinal
mode is set by the \vn{longitudinal_mode} parameter. The default setting for \vn{longitudinal_mode}
is 1 the possible values are 0 or 1. If the mode is set to 0, the \vn{wall_radius} dependent
parameter is calculated using \Eq{rk}. If the mode is 1, the wall radius is treated as infinite.
