...
\index{e1}\index{e2}\index{f1}\index{fint}\index{fintx}\index{l_arc}
...
This formula holds for both positive and negative angles.
  \index{f1}
  \item[f1] \Newline
\vn{f1} is the strength of the SAD ``linear'' fringe field. This
...
element \vn{C}, and $\calO_G$ is the default \vn{origin} reference
frame of the girder. $r_{CA}$ is the vector from $\calO_A$ to
$\calO_C$. The length \vn{l} of the girder is the difference in $s$
...
\vn{match} element are called the ``entrance Twiss parameters''. The
parameters \vn{beta_a1}, \vn{alpha_a1}, etc. of the \vn{match} element
...
\vn{match} element are equal to the entrance Twiss parameters of the
\vn{match} element, then the computed Twiss parameters at the exit
end of the \vn{match} element will be the exit Twiss parameters of the
\vn{match} element and the phase advances (in radians) will be
\vn{dphi_a} and \vn{dphi_b}. Notice that there is no guarantee that
the actual Twiss parameters after a \vn{match} element be equal to 
the exit Twiss parameters of the \vn{match} element.
...
The \vn{match_end} attribute is used for appropriately setting the
entranch Twiss parameters of the \vn{match} element from within a
program. If the \vn{match_end} attribute is set to True, the entrance
...
of the \vn{match} element. The \vn{match_end} attribute may only be
used with \vn{open} lattices (\sref{s:param}) since, for a \vn{closed}
...
parameters are appropriately set as explained above. If \vn{match_end}
...
\vn{match} element's starting coordinate parameters, \vn{(x0, px0, y0,
...
A \vn{x_ray_init} element is used to simulate a general purpose x-ray source.
...
Attributes specific to an \vn{x_ray_init} element are:
\begin{example}
