...
\begin{tabular}{|l|l||l|l|} \hline
  ...
  Floor_Shift        & \ref{s:floor.ele}     &  Patch             & \ref{s:patch}      \HH
...
A \vn{floor_shift} element shifts the global floor coordinates without affecting particle tracking.xs.
...
The \vn{floor_shift} element shifts the global position
(\sref{s:global}) of the reference coordinates relative to the
proceeding element. Unlike the \vn{patch element} \sref{s:patch}, the
transfer map through a \vn{floor_shift} element will be the unit map. That
is, the phase space coordinates of a particle will not change when
tracking through a \vn{floor_shift} element. The reference position
transformation through a \vn{floor_shift} element is given in
