...
If the branch line is to transport particles different from
the originating line (such as in a photon line branching
from an electron line), the particle type and the beginning reference energy
must be set for that line (\sreffff{}). Additionally, the \etcetc...
...
\begin{example}
  ...
  ring: branch, to_line = ring_layout, 
          lattice_type = circular_lattice    ! Branch to the ring
  ring_layout: line = (..., x_ray, ...)      ! Define the ring
  x_ray: photon_branch, E_tot_start = 1e3,   ! 1 KeV reference photon.
                        to_line = x_layout 
...
(\sref{s:global}) of the reference coordinates. The transfer map
through the \vn{floor_position} will be the unit map. That is, the
phase space coordinates of a particle will not change when tracking
through a \vn{floor_position} element.

Setting the floor position at an element will affect the floor
position of all elements following the element but will not affect any
preceding elements. The \vn{floor_position} element can be used, for
