...
For calculating the sigmas of any given slice, \vn{sig_x} and \vn{sig_y} 
...
\begin{example}
  ...
  mosaic_angle_rms_in_plane  = <Real> ! In-plane mosaic angular half-width. Default = 0.
  mosaic_angle_rms_out_plane = <Real> ! Out-of-plane angular half-width. Default = -1
\end{example}
...
A mosaic crystal is an idealized model of an imperfect crystal imagined to consist of numberous
small perfect crystals that are to some extent randomly misoriented.  Mosaic crystals are
parameterized by four element parameters:
\begin{example}
  is_mosaic          = <Logical>    ! Is a mosaic crystal? Default = False
  mosaic_thickness   = <Real>       ! Mosaic element thickness
  mosaic_angle_rms_in_plane  = <Real> ! In-plane angular half-width. Default = 0
  mosaic_angle_rms_out_plane = <Real> ! Out-of-plane angular half-width. Default = -1
\end{example}
The \vn{is_mosaic} logical is used to toggle between mosaic and non-mosaic tracking. Tracking in a
mosaic is detailed in Section~\sref{s:mosaic.track}. The angular spread of the orientation of mosaic
crystals in the diffraction plane is given by \vn{mosaic_angle_rms_in_plane} and the angular spread
of the orientation of mosaic crystals in the plane perpendicular to the diffraction plane is given
by \vn{mosaic_angle_rms_out_plane}. If the value of \vn{mosaic_angle_rms_out_plane} is negative, the
angular distribution will be taken to the same in-plane and out-of-plane so the value used for the
out-of-plane angular spread will be the value of \vn{mosaic_angle_rms_in_plane}.
...
\Begineq
  H_{\text{crab}} = -\frac{q_{rel} \, V\, x}{P_0} \, \sin(k \, z + 2 \, \pi \, \phi_0)
\Endeq
where $x$ and $z$ are particle coordinates, $q_{rel}$ is the charge relative to the reference
particle, $V$ is the cavity voltage, $P_0$ is the reference energy, $\phi_0$ is a user settable
...
\begin{align} 
  \Delta p_x &= -\frac{\partial H_{\text{crab}}}{\partial x} = 
    \frac{q_{rel} \, V \, x}{P_0} \, \sin(k \, z + 2 \, \pi \, \phi_0) \CRNO 
  \Delta p_z &= -\frac{\partial H_{\text{crab}}}{\partial z} = 
    \frac{q_{rel} \, V \, \, k \, x}{P_0} \, \cos(k \, z + 2 \, \pi \, \phi_0) 
\end{align} 
Note: The sign of $H_{\text{crab}}$ used by Authors in the literature is not
standardized. \bmad uses the convention such that a particle with the charge of the reference 
particle and with $z$ and $V$ positive will have a positive $\Delta p_x$.
