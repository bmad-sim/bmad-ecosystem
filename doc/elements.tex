...
\theta_i$ must hold for all vertex pairs. Another restriction is that
the origin $(0,0)$ must be in the interier of all cross-sections.
...
\begin{example}
    cross = \{  ! Rectangular cross-section with x/y symmetry.
      s = 0.34
      v(1) = \{0.03, 0.01\}
    \}
\end{example}

Between cross-sections, the capillary wall is defined using a spline
...
\Begineq
  r_c(\theta, s) \equiv (1 - P(ds)) \, r_{c1}(\theta) + P(ds) \, r_{c2}(\theta)
\Endeq
where
\Begineq
  ds \equiv \frac{s - s_1}{s_2 - s_1}
\Endeq
and $P(s)$ is a cubic polynomial spline
\Begineq
  P(s) = c_1 \, s + c_2 \, s^2 + c_3 \, s^3
\Endeq
