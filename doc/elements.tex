...
\begin{tabular}{|l|l||l|l|} \hline
  {\it Element}      & {\it Section}         & {\it Element}       & {\it Section}      \HH
  Branch             & \ref{s:branch}        &  Instrument         & \ref{s:monitor}    \HH
  ...
  Diffraction_Plate  & \ref{s:diff.plate}    &  Mirror             & \ref{s:mirror}     \HH
...
the computed Darwin width, in radians, for sigma andd pi polarized
...
plate is divided into ``clear'' (white) and ``mask'' (black)
sections. In this example there are three clear sections labeled \#1,
\#2, and \#3. B) All sections must be star shaped with respect to the
section's origin. In this example, The section is {\em not} start
shapped since a line drawn from the origin point $o$ to the point $p$
on the boundary intersects the boundary twice in between. In this case
the section can be made star shapped by moving the origin to $o'$.
  }
  \label{f:diff.plate}
\end{figure}

A \vn{diffraction_plate} element is something like a Fresnel zone plate and is
used in places where diffraction effects must be taken into
...
NOTE: \vn{DIFFRACTION_PLATE} ELEMENT TRACKING IS IN DEVELOPMENT AND IS NOT USABLE AT THIS TIME...

An example diffraction plate geometry is shown in \fig{f:diff.plate}A.
The geometry of the plate is definded using the ``wall'' attribute of
the element. EXample:
\begin{example}
  fresnel: diffraction_plate, wall = {...}, ...
\end{example}
And the syntax for defining a wall is:
\begin{example}
  wall = \{
    thickness = <real>
    mask_material = <material_type>
    clear_material = <material_type>
    section = \{ 
      type = <section_type>   ! "clear" or "mask"
      x0 = <x_origin>, y0 = <y_origin>,
...
A wall is comprised a a set of ``clear'' ``sections''. Each clear
section can have associated with it zero or more ``mask'' sections.
An ``opening'' is a clear section along with its associated mask
sections. In the figure, there are three openings labeled \#1, \#2,
and \#3. 

The default behavior is that a photon will be transmitted if it
``hits'' any opening. Otherwise it will be considered to have hit a
mask section. A photon is considered to have hit a given opening if
its $(x,y)$ coordinates put in within the clear section of the opening
but not within any mask section of the opening.

The mask section surrounding all of the clear sections does not have
to be explicitly specified.  

For computational reasons, All sections must be ``star shapped'' with
...
\fig{f:diff.plate}B where the section is not star shapped since a
line drawn from the origin $o$ to the point $p$ on the boundary passes
through two boundary points in between. In this case the section can
trivially be made star shapped by moving the origin to point $o'$. If
it is not possible to make a section star shapped by moving the
origin, then the section must be divided into multiple sections.
...
Nominally, a diffraction plate is orientated transversly to the
beam. Like all other elements, the diffraction plate can be reoriented using the element's
...
space, use a \vn{pipe} element instead of a \vn{drift}.
...
respect to its entrance face. B) The length of a \vn{patch} element is
