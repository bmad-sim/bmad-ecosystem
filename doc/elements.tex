...
counter clockwise manner. There are two solutions for connstructing an
...
\begin{figure}[tb]
  \centering
  \includegraphics{crystal-ele.pdf}
  \caption[width=5in][Crystal element geometry.]
...
\begin{example}
  ...
  crystal_type     = <String>  ! Crystal material and reflaction plane.
...
The \vn{b_param} is the standard assymetry factor
\Begineq
...
\Endeq
and $\alpha_H$ (\vn{alpha_angle}) is the angle of the reciprical
lattice $\bH$ vector with respect to the surface normal as shown in
...
then there is Laure diffraction again with \vn{alpha_H} zero. With the
...
reference orbit, $\bfhat n$ and $\bH$ are all coplainer. A non-zero
\vn{psi_angle} Rotates the $\bH$ vector around the $+\bfhat x$ axis
...
of a crystal is fixed at zero. The reference trajectory is determined
by the values of the \vn{graze_angle} and \vn{tilt} parameters. A
positive \vn{graze_angle} bends the reference trajectory in the same
