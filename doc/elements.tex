...
\begin{example}
  ...
  t_offset        = <Real>  ! Exit face time offset.
...
\end{example}
When \vn{runge_kutta} tracking is used, \vn{field_calc}
(\sref{s:integ}) must be set to \vn{custom} since, otherwise, \bmad
has no way of calculating the fields. In this case, custom code must be
supplied for calculating the fields as a function of position
(\sref{s:custom.ele}).
