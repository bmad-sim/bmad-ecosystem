...
\vn{marker} (\sref{s:marker}) element. Also see \vn{patch}
...
alternative line for the beam. The only difference between \vn{fork}
and \vn{photon_fork} is that the default particle type for the
\vn{branch} that branches from a \vn{fork} is the same particle type
of the \vn{branch} which contains the \vn{fork} The default particle
type for a \vn{branch} that branches from a \vn{photon_branch} element
is a photon.

Collectively \vn{fork} and \vn{photon_fork} elements are called
forking elements. An example geometry is shown in \fig{f:x.fork}.
...
\begin{example}
  from_line: line = (... A, PB, B, ...)  ! Line containing a fork
...
\end{example}
The difference between a \vn{fork} element and a \vn{photon_fork}
element is that for a \vn{fork} element the default particle for the
branch is the same as the line that the branch branches off from. The
default particle of the branch from a \vn{photon_fork} element is a
\vn{photon}. The actual particle associated with a branch can be set
by setting the \vn{particle} attribute of the forking element.
