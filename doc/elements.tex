...
\begin{example}
  ...
  E_tot           = <Real>      ! Output reference energy. A settable dependent var (\sref{s:depend}).
\end{example}

The species of the outgoing particles is specified by the \vn{species_out} parameter
(\sref{s:species.name}).

The converter must be first element in a lattice branch (\sref{s:branch.def}). A \vn{fork}
(\sref{s:fork}) element can be used to connect the line containing the elements that come before the
converter to the line containing the converter. Example:
\begin{example}
  ...
  post_linac[p0c] = 3e6
  post_linac[beta_a] = 27; post_linac[beta_b] = 32
  use, pre_linac
\end{example}
...
\vn{electron}s are the reference particles. It is not mandatory for the reference particle of the
...
The Twiss and dispersion at the exit end of the converter
will be taken to be equal to the values of these parameters at the beginning of the beginning of the
converter. Since the converter is the first element in the branch, These parameters correspond to
the parameters set for the branch. Thus, in the above example, The beta functions at the exit end
of the converter will be $\beta_a = 27$~m and $\beta_b = 32$~m.

The \vn{p0c} and \vn{E_tot} attributes set the reference momentum or energy at the exit end of the
converter. At least one of these attributes must be set. If both are set, \vn{E_tot} is calculated
from \vn{p0c}.
...
value for the first column is always zero and the radius values are strickly increasing.  Each
...
coefficients for calculating $c_x$, $\alpha_x$, $\alpha_y$, and $\beta$ respectiely in
...
can be incorperated into a \bmad lattice, there is modeling code that is distributed with
...
If an element (like a solenoid) is superimposed on top of a converter, the effect of the element on
tracking within the converter is ignored, the superimposed element will have two slave elements to
either side of the converter, and the converter itself will remain untouched.
