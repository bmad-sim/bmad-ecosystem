...
\index{cross}\index{s (capillary attribute)}\index{v (capillary attribute)}
...
\begin{example}
  ele_name[cross(n).v(j).x]     ! x value of j^th vertex of n^th cross-section
  ele_name[n_slice_spline(n)]   ! n_slice_spline value set between n^th and 
                                !     n+1^th cross-sections.
  ele_name[s_spline(n).coef(k)] ! k^th coefficient (k = 1, 2, or 3) of s_spline 
