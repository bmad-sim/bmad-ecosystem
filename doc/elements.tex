...
\begin{tabular}{|l|l||l|l|} \hline
  ...
  E_Gun             & \ref{s:e.gun}       &  Photon_Fork       & \ref{s:fork}     \HH
  ...
  Fork              & \ref{s:fork}        &  Sad_Mult          & \ref{s:sad.mult} \HH
  HKicker           & \ref{s:hvkicker}    &  Sbend             & \ref{s:bend}     \HH
  Hybrid            & \ref{s:hybrid}      &  Sextupole         & \ref{s:sex}      \HH
  Beginning_Ele     & \ref{s:begin.ele}   &  Solenoid          & \ref{s:sol}      \HH
...
\begin{tabular}{|l|l||l|l|} \hline
  {\it Element}      & {\it Section}         & {\it Element}       & {\it Section}      \HH
  Capillary          & \ref{s:capillary}     &  Marker             & \ref{s:mark}       \HH
  ...
  Drift              & \ref{s:drift}         &  Patch              & \ref{s:patch}      \HH
  Fiducial           & \ref{s:fiducial}      &  Photon_Fork        & \ref{s:fork}       \HH
  Floor_Shift        & \ref{s:floor.ele}     &  Pipe               & \ref{s:monitor}    \HH
  Fork               & \ref{s:fork}          &  Sample             & \ref{s:sample}     \HH 
  Beginning_Ele      & \ref{s:begin.ele}     &  X_Ray_Init         & \ref{s:x.init}     \HH
...
extension of the incoming reference tranjectory. This trajectory is
that trajectory that photons whose energy is far from the Bragg
condition (that is, far from the referennce energy) will follow. The
\vn{forward_diffracted} reference orbit is parallel to the
\vn{undefracted} trajectory and is the trajectory of the forward
diffracted photons whose energy is the reference energy and whose
incoming orbit is on the incoming reference rajectory. Finally, the
...
\vn{bragg_angle_out}. A positive \vn{bragg_angle_in} +
\vn{bragg_angle_out} bends the reference trajectory in the same
...
\Begineq
  d\theta = \frac{2 \, \Gamma \, |P| \, F_H}{|b|^{1/2} \, \sin\theta_{tot}}
\Endeq
where
\begin{example}
...
\vn{detector} is modeled as a plate upon which particles and x-rays
can impinge.
...
Example:
\begin{example}
  fresnel: diffraction_plate, wall = {...}, x_limit = 0.1, y_limit = 0.08
\end{example}
...
\vn{Elseparator} is used to compute the electric field for a given
...
If \vn{origin_ele} is not specified, previous element is used. If the
...
Additionally, like \vn{marker} elements, The \vn{x_offset},
\vn{y_offset} and \vn{tilt} attributes are not used by any \bmad
routines. The \vn{is_on} attribute is also not used by \bmad proper.
...
it will connect to the downstream end of the \vn{x_line}. The default
setting of \vn{direction} is 1.
...
\vn{direction} set to -1), the last element in the target branch must
be a marker-like element. Note that, by default, a marker element is
placed at the end of all branches (\sref{s:branch.construct})
...
(\sref{s:beginning}). This is useful, for example, for a photon branch line
...
\begin{example}
  ...
  dphi_origin       = <Real>     ! orientation angle offset.
  dpsi_origin       = <Real>     ! orientation angle offset.
  ds_path_length                 ! ``Path length'' of girder. Dependent attrib (\sref{s:depend}).
\end{example}
...
lattices (\sref{s:param}) since, for a \vn{closed} lattice, it is
...
values, and hence the transfer matrix for the \vn{match} element, will
be frozen. Variation now of any parameter in the lattice that affects
the calculated Twiss parameters through the \vn{match} element will
not affect the \vn{match} element's transfer matrix.
...
of a \vn{patch} a dependent (\sref{s:depend}) parameter and is the
...
\index{rf_frequency}\index{harmon}\index{voltage}\index{phi0}\index{dphi0}\index{harmon_master}
...
\index{ks}\index{phi0}\index{rho}\index{voltage}\index{x_offset_sol}
\index{y_offset_sol}\index{x_pitch_sol}\index{y_pitch_sol}
\index{fringe_kind}\index{kill_fringe}
...
\begin{example}
  ...
  x_offset_sol    = <Real>    ! Solenoid offset. See SAD dx attribute.
  y_offset_sol    = <Real>    ! Solenoid offset. See SAD dy attribute.
  x_pitch_sol     = <Real>    ! Solenoid pitch. See SAD dpx or chi1 attribute.
  y_pitch_sol     = <Real>    ! Solenoid pitch. See SAD dpy or chi2 attribute.
  fringe_kind     = <Switch>  ! Type of fringe. See SAD disfrin attribute.
...
solenoid field at a SAD \vn{mult} element is not explicitly declaired
in the \vn{mult} element definition. \bmad, on the other hand,
requires a \vn{sad_mult} element to explicitly declare the solinoid
parameters.
...
aligned with the axis of any elements within the solenoid region).

The \vn{fringe_kind} and \vn{fringe_at} switches are discussed in
Sec.~\sref{s:fringe} The translation between these two switches and
...
\vn{linear} fringe kick is a kick that is linear in the transverse
...
\vn{nonlinear} fringe kick comes from the nonlinear part of the
quadrupolar field plus the fringes of the other multipoles.

The \vn{eps_step_scale} is equivalent to the SAD \vn{eps}
attribute. The default \vn{eps_step_scale} value is 1 which is
essentially the same default as SAD.  From \vn{eps_step_scale}, the
values of \vn{num_steps} and \vn{ds_step} (\sref{s:integ}) are
computed. Thus, for a \vn{sad_mult} element, \vn{num_steps} and
\vn{ds_step} are \vn{dependent} attributes (\sref{s:depend}).
...
of the element as opposed to \bmad's convention of referenceing to the element center.
\end{itemize}
...
\section{X_Ray_Init}
\label{s:x.init}
\index{x_ray_init|hyperbf}

A \vn{x_ray_init} element is used to simulate an x-ray x_ray_init.

General \vn{x_ray_init} attributes are:
\begin{center}
\tt
\begin{tabular}{|l|l||l|l|} \hline
  {\sl Attribute Class}      & Section           & {\sl Attribute Class}      & Section         \HH
