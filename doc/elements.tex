...
This formula holds for both positive and negative angles.
  \index{exact_edge}
  \item[exact_edge]
When \vn{e1} or \vn{e2} are non-zero, \vn{bmad_standard} tracking uses a second order
map to handle the edge focusing. In some cases, for instance in a chicane, this is
not good enough. With \vn{exact_edge} to True, the wedge shape of the edge is properly 
taken into account. The hard edge model is still used here. \vn{exact_edge} = True will
be somewhat slower.
  \index{angle}
  \item[angle] \Newline
