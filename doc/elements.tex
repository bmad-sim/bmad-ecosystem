...
\begin{tabular}{|l|l||l|l|} \hline
  ...
  Description strings    & \ref{s:string}   & Is_on                      & \ref{s:is.on}   \HH 
  Integration settings   & \ref{s:integ}    & Hkick \& Vkick             & \ref{s:kick}    \HH
  Instrumental variables & \ref{s:inst_var} &                            &                 \HH
\end{tabular}
\end{center}
\toffset
...
\vn{freq_spread} is used to randomly spread out the long range mode
frequencies among different cavities. The spread is Gaussian in shape
with an RMS of \vn{freq_spread} * $F$ where $F$ is the frequency of a
