...
\begin{tabular}{|l|l||l|l|} \hline
  ...
  Fork               & \ref{s:fork}          &  X_Ray_Source       & \ref{s:x.source}   \HH
...
%-----------------------------------------------------------------
\section{X_Ray_Source}
\label{s:x.source}
\index{x_ray_source|hyperbf}

A \vn{x_ray_source} element is used to simulate a general purpose x-ray source.

General \vn{x_ray_source} attributes are:
\begin{center}
\tt
\begin{tabular}{|l|l||l|l|} \hline
  {\sl Attribute Class}      & Section           & {\sl Attribute Class}      & Section         \HH
...
Attributes specific to an \vn{x_ray_source} element are:
\begin{example}
    energy_distribution = 'GAUSSIAN'     ! Or 'UNIFORM'. 
    spatial_distribution = 'GAUSSIAN'    ! Or 'UNIFORM'. 
    velocity_distribution = 'GAUSSIAN'   ! Or 'SHPERICAL', 'UNIFORM'. 
    sig_x = 0
    sig_y = 0
    sig_z = 0
    sig_vx = 0
    sig_vy = 0
    sig_E = 0                            ! Initial photon energy width (eV).
    transverse_sigma_cut = 3                      
    dE_center = 0                        ! Initial photon energy shift (eV).
    dE_relative_to_ref = True            ! dE_center relative to reference E?
    e_field_x = 0                        ! Polarization. x & y = 0 -> random
    e_field_y = 0
    ds_slice
\end{example}

  \item[\vn{dE_center}] \Newline
Average initial photon energy in eV. If
\vn{dE_relative_to_ref} is set to True (default)
then \vn{dE_center} will be relative to the reference energy
(\sref{ss:x.ray.init}).

  \item[\vn{dE_relative_to_ref}] \Newline
Is \vn{dE_center} with respect to the reference energy or
