...
To have the reference energy and reference particle for the elements downstream of a converter match
the output from a converter, the converter should be first element in a lattice branch
(\sref{s:branch.def}). A \vn{fork} (\sref{s:fork}) element can be used to connect the line
containing the elements that come before the converter to the line containing the
converter. Example:
\begin{example}
  ...
  pre_linac: line = (..., to_cvter) ! to_cvter fork last element in the line.
  post_linac: line = (cvter, ...)   ! cvter element first element in the line.
...
converter itself though typically this will be what is wanted.

The \vn{distribution} parameter of a \vn{converter} element specifies the distribtion of outgoing
...
when there is only one\vn{distribution} present. In this case, the calculation will just used the
distribution for the calculation independent of the element length and the \vn{thickness} as set in
the distribution.

If there are multiple \vn{distributition}s, specifying an element length that is outside the range
of thicknesses will generate an error.
...
particular momentum would generagte, on average, 0.42 particles, the outgoing particle in the
...
weight of the outgoing particles will be approriately adjusted such that the weighted distribution
...
A converter element does not have a well defined transfer matrix. Rather, the Twiss and dispersion
parameters are specified at the exit surface of the converter
\begin{example}
  beta_a_out     beta_b_out 
  alpha_a_out    alpha_b_out
  eta_x_out      eta_y_out  
