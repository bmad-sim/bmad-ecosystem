...
control large sets of elements. Also \vn{ramper} elements can be used to smoothly vary parameters
as particle are propagated through a lattice. \vn{Ramper} elements where implemented to solve the problem of
...
programs that that are designed to handle them.\footnote 
  {
...
Parameters that \vn{ramper} elements control must not be controlled by any \vn{overlay}.
...
The syntax for \vn{ramper} elements is exactly the same as for \vn{overlay} (\sref{s:overlay}) elements 
...
\vn{spline} based.  The general syntax for a \vn{expression} based \vn{ramper} element is
\begin{example}
...
(\sref{s:tao.intro}) called \vn{controller_function_plot}.
...
\begin{example}
  ...
  k1x                  ! Planar wiggler hoizontal defocusing strength. Dependent attribute (\sref{s:depend}).
  k1y                  ! Planar wiggler vertical focusing strength. Dependent attribute (\sref{s:depend}).
