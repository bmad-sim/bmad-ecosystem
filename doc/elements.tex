...
The attributes of the \vn{beginning} element in the root branch are are generally set using
\vn{beginning} (\sref{s:beginning}) statements or line parameter (\sref{s:beginning}) statements.
[The attributes of other \vn{beginning} elements are set solely with line parameter statements.]
...
\vn{g} and not \vn{dg} which resluts in downstream elements moving around. See \Sref{s:ex.chicane}
for an example.
  %
  \index{h1}\index{h2}
  \item[h1, h2] \Newline
...
target branch forked from a \vn{fork} element is the same particle type as the base branch. The
default particle type for the target branch from a \vn{photon_fork} element is a photon. The actual
...
set to the refernece energy and momentum at the fork element. In this case, neither the reference
...
element, a patch element (\sref{s:patch}) can be used at the beginning of the target branch. In all
...
Note: If all beginning and ending betas and alphas (\vn{beta_a0}, \vn{beta_a1}, etc.) are zero, and
$\Bf M$ will be the appropriate matrix such that the phase advance through the element for the $a$
and $b$ modes will be \vn{dphi_a} and \vn{dphi_b} respectively, and the ending Twiss parameters will
...
only be set to True with \vn{open} lattices (\sref{s:param}) since, for a \vn{closed} lattice, it is
not possible to calculate the Twiss parameters at the previous element independently of the design
end Twiss parameters at the \vn{match} element.
...
\vn{match_end}. This attribute does not affect any computations and is not settable by the user. It
is simply present to tell the user (and \bmad) what the setting of \vn{match_end} was in the lattice
file.
...
\vn{match_end_orbit} attribute may only be used with \vn{open} lattices (\sref{s:param}).
...
user. It is simply present to tell the user (and \bmad) what the setting of \vn{match_end_orbit} was
in the lattice file.
  \end{description}
