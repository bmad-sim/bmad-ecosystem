...
\begin{example}
  ...
  eps_step_scale  = <Real>    ! Used to scale step size. See SAD eps attribute.
  ...
  kill_fringe     = <Switch>  ! Where fringe is applied. See SAD fringe attribute.
\end{example}
...
The \vn{kill_fringe} switch can be set to one of the following:
\begin{example}
  no_end       
  entrance_end 
  exit_end
  both_ends       ! Default
\end{example}
The \vn{fringe_kind} switch can be set to one of the following:
\begin{example}
  none
  linear
  nonlin_only     ! Default
  full
\end{example}
The translation between these two switches and the \vn{fringe} and
\vn{disfrin} switches of SAD is:
\begin{center}
\tt
