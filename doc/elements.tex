...
\begin{figure}[tb]
  \centering
  %% \includegraphics{capillary.eps}
  \caption[cross-sectional slices of a capilary.]
{The inside wall of a capillary is defined by a number of 
cross-sectional slices.  Each cross-section is made up of a number of vertices.
The segment between vertices can be either a line segment, the arg of a circle, 
or a section an ellipse.}
  \label{f:capillary}
\end{figure}

\begin{figure}[tb]
  \centering
  %% \includegraphics{cap_arc.eps}
  \caption[Construction of the arc for a cross-section segment.]
{Given a radius there are four solutions to the problem of 
constructing an arc between two given points. The one choisen (a) for 
the construction of a cross-section segment is the one that is convex 
with respect to the origin and of shortest length. The same logic applies 
to the construction of an ellipse segment.}
  \label{f:cap.arc}
\end{figure}
...
The inside wall of a capillary is defined by a number of
cross-sectional slices as shown in Fig.~\ref{f:capillary}. Each
cross-section is defined by a number of vertices. The arc between each
verticy may be either a straight line, an arc of a circle, or a
section of an ellipse. 

The syntax to define a capillary is
\begin{example}
  cross = \{ 
...
\begin{example}
  this_cap: capillary, 
    cross = \{   ! Cross section with top/bottom symmetry
      s = 0, v(1) =  \{0.02, 0.00\}, 
      v(2) = \{0.00, 0.02, 0.02\}, v(3) = \{-0.01, 0.01\}, 
    \}, 
    s_splin = \{0.1, 0.7\}, 
    n_slice_spline = 4
    cross = \{  ! Cross-section that is a tilted ellipse.
      s = 0.34, 
      v(1) = \{0.003, -0.001, 0.015, 0.008, 0.2*pi\}, 
    \}
\end{example}

Each individual cross_section begins with the word \vn{cross}. The
\vn{s} parameter of a cross-section gives the longitudinal position of
the cross-section. \vn{s} must be zero for the first cross-section and
the length of the capillary is given by the value of \vn{s} of the
last cross-section.

The \vn{v(<j>)} parameters within a cross-section define the vertices
for each cross section. Each \vn{v(<j>)} in turn has five
parameters. It is manditory to specify the first two parameters
...
and \vn{<radius_y>} must not be negative. In general, for a given
radius, there are four circular arcs that can be constructed to
connect two points (as long as the points are not too far apart). The
arc used to construct a circular segment of a cross-section is the arc
that is convex with respect to the origin and of shortest distance as
shown in Fig.~\ref{f:cap.arc}. The same logic applies 
to the construction of an ellipse segment.
...
The vertex points must be arranged in a clockwise manner. That is, If
the (\vn{<x>}, \vn{<y>}) point of vertex \vn{<v(i)>} is expressed in
polar $(r, \theta)$ coordinates, the inequality $\theta_{i+1} >
\theta_i$ must hold for all vertex pairs. Additionally, the origin
$(0,0)$ must be in the interier of all cross-sections and all cross
sections must be convex.

Given two vertices \vn{v(i)} and \vn{v(i+1)}, if the segment between
them is specified to be a circle or ellipse, there will, in general,
be two solutions as to how the circle or ellipse can be
constructed. The solution choisen will be the shorter one. With this
choice, and with the restriction that the $\theta$ angle of the
vertices must be strictly increasing, A line drawn from the origin at
any given angle $\theta$ will intersect the cross-section at exactly
one point as shown in Fig.~\ref{f:capillary}. This is an important
...
points of a cross section are in the first quadrent, that is, all
...
\begin{example}
    cross = \{ s = 0.34, v(1) = \{0.03, 0.01\} \}
\end{example}

Between cross-sections, the capillary wall is defined using a linear
interpolation scheme.  let $r_{c1}(\theta)$ be the radius of the
capillary as a function of $\theta$ for a given cross-section defined
at $s = s_1$. Let $r_{c2}(\theta)$ be the radius function at the next
defined cross-section at $s = s_2$. The capillary wall $r_c(\theta,
s)$ at any point $s$ between $s_1$ and $s_2$ is defined by the
equation
\Begineq
...
\bmad assumes that the cross section $r_c(\theta, s)$ at any given $s$
is convex. Note that it is {\em not} sufficient for $r_c(\theta, s)$ to be
convex at the specified cross sections.
...
spline between two consecutive cross-sections and by specifying
a value for \vn{n_slice_spline}. The region between the two cross-sections will then be divided into \vn{n_slice_spline} 

. In this case, 


using a spline
interpolation: In $(r, \theta)$ cylindrical coordinates, let
...
\Begineq
  r_c(\theta, s) \equiv (1 - P(\stilde)) \, r_{c1}(\theta) + P(\stilde) \, r_{c2}(\theta)
\Endeq
where
\Begineq
  \stilde \equiv \frac{s - s_1}{s_2 - s_1}
\Endeq
and $P(s)$ is a cubic polynomial spline
\Begineq
...
\begin{example}
  graze_angle_in   = <Real>    ! Angle between incoming beam and mirror surface.
  graze_angle_out  = <Real>    ! Angle between incoming beam and mirror surface.
  ...
  b_param          = <Real>    ! 
  bragg_angle      = <Real>    ! 
  ref_wave_length  = <Real>    ! 
  diffraction_type = <Real>    ! 
  crystal_type     = <Real>    ! 
...
\vn{crystal} is that of a zero length bend (\sref{s:mirror.coords}) and
hence the length (\vn{l}) parameter of a crystal is fixed at zero. The
...
parameters affect the orientation of the \vn{crystal} in the global
...
\sref{s:wiggler.map.std} and \sref{s:wiggler.periodic.std} for more
details.  Tracking a particle through a wiggler is always done so that
...
explained in \sref{s:wiggler.map.std}. \vn{polarity} is used to scale the
...
explained in section~\sref{s:wiggler.periodic.std}.
