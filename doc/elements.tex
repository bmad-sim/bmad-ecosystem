...
must be a \vn{marker} or \vn{init_ele} element. If the \vn{origin_ele} is specified,
the alsolute reference position attributes \vn{x_origin}, etc., are
ignored.
...
a \vn{flexible} \vn{patch} in between the two elements.
  \item
Two \vn{fiducial} elements may not appear in the same lattice branch
unless separated by a \vn{flexible} \vn{patch}
  \item
If a \vn{fiducial} element affets the position of element 0 in the
lattice branch, any positioning of element 0 via \vn{beginning} or
\vn{line initialization} statements (\sref{s:beginning}) are
ignored.
  \end{description}
...
\index{flexible}\index{flexible patch}
...
lattice. In this case, as explained in \sreff, it must be possible to
compute the recerence coordinates of the next element before the
reference coordinates of the \vn{patch} are computed. In this case,
the offsets, pitches, and tilt of the \vn{patch} are dependent
parameters (\sref{s:depend}) and will be computed. Here the \vn{patch}
is called ``flexible'' since the geometry of the patch will depend
upon the geometry of the rest of the lattice and, therefore, if the
geometry of the rest of the lattice is modified (is ``flexed''), the
geometry of the \vn{patch} will vary as well.

\index{rigid patch}\index{infleible}
