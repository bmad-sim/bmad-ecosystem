...
\begin{tabular}{|l|l||l|l|} \hline
  {\it Element}      & {\it Section}         & {\it Element}       & {\it Section}      \HH
  Aperture           & \ref{s:aperture}      &  Instrument         & \ref{s:monitor}    \HH
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=5in]{aperture-ele.pdf}
  \caption[Example aperture element]{
Example \vn{aperture} element. A) The aperture wall is divided in
``clear'' (white) and ``mask'' (black) sections. In this example there
are three clear sections labeled \#1, \#2, and \#3. B) All sections
must be star shaped with respect to the section's origin. In this
example, The section is {\em not} start shapped since a line drawn
from the origin point $o$ to the point $p$ on the boundary intersects
the boundary twice in between. In this case the section can be made
star shapped by moving the origin to $o'$. 
  }
  \label{f:aperture.ele}
\end{figure}

An \vn{aperture} element is something like a Fresnel zone plate and is
...
General \vn{aperture} element attributes are:
\begin{center}
\tt 
\begin{tabular}{|l|l||l|l|} \hline
  {\sl Attribute Class}  & \s              & {\sl Attribute Class}      & \s              \HH
...
NOTE: \vn{APERTURE} ELEMENT TRACKING IS IN DEVELOPMENT AND IS NOT USABLE AT THIS TIME...

The aperture ``wall'' defines the two dimensional plate that the
x-rays hit. An example is shown in \fig{f:aperture.ele}A. 
The general syntax for defining a wall is:
\begin{example}
  wall = \{
    thickness = <real>
