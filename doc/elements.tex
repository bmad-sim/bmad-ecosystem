...
\vn{ptc_field_geometry} determines what coordinates PTC uses within a bend for calculating fields.
This only affects tracking if PTC is being used. Possible values for
...
field, tracking is essentially uneffected.  When there are quadrupole or higher order fields, the
fields are centered about the reference frame set by \vn{ptc_field_geometry}.

The above discussion is true for \vn{ptc_exact_model} set to True, for \vn{ptc_exact_model} set to
