...
\begin{tabular}{llll} \toprule
  ...
  Instrument        & \ref{s:monitor}     &  Stripper          & \ref{s:stripper} \\
...
\section{Stripper}
\label{s:stripper}
\index{stripper|hyperbf}

A \vn{stripper} element represents a stripper foil which strips all the electrons from a particle.
Additionally, there will be scattering of the trajectory of a particle going through a \vn{stripper}.

General \vn{stripper} attributes are:
\begin{center}
\tt
\begin{tabular}{llll} \toprule
  {\sl Attribute Class}      & Section           & {\sl Attribute Class}      & Section            \\ \midrule
...
Attributes specific to a \vn{stripper} element are:
\begin{example}
...
In terms of element placement, The length of a \vn{stripper} element (\Eq{l00l}) is considered to be
...
The 6x6 transfer matrix of a \vn{stripper} element is the unit matrix. That is, scattering does
not affect the transfer matrix.

Currently, particles going through the \vn{stripper} are considered to be fully stripped.

Example:
\begin{example}
  foil: stripper, material_type = "Cu", thickness = 0.127
\end{example}
