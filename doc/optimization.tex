...
the datum (\sref{s:data.anatomy}).  There are five \vn{merit_type} constraint types as given in
...
just the value of the variable.  A \vn{limit} \vn{merit_type} has the form
\Begineq
  \delta V = 
    \begin{cases}
...
unstable region. To avoid this, an \vn{unstable_ring} constraint (\sref{s:data.types}) must be set.
