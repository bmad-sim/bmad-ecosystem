...
\vn{data_types} are given in table~\ref{t:cons}. the method by which the data
types are calculated depends on the tracking type. For {\it single} tracking all
data is found from the lattice parameters. For {\it macroparticle} tracking the
twiss parameters, bunch sizes and orbit are found from the macroparticle
distribution.
\begin{table}[ht] 
\centering 
{\tt
...
\label{t:eval}
\end{table}
\vn{base} is determined by a datum's \vn{data_type}.
