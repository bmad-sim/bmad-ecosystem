...
by the setting of the datums's \vn{merit_type} component. The possible settings of this component
are:
\begin{example}
  "target"                       ! Default
...
With \vn{"average"} as the \vn{merit_type}, the value for a lattice is the average of the
\vn{data_type} over the evaluation region.
%
...
\end{tabular}
  ...
  \caption{
The expression
used to combine the values of \vn{model}, \vn{base}, \vn{design}, \vn{meas}, and \vn{ref} into one
``composit'' value is determined by the settings of \vn{opt_with_ref} and \vn{opt_with_base} global
...
component (\sref{s:dat.lat.ele}). \vn{ele_ref_name} is used to set a reference element which is used
when evaluating \vn{model}, \vn{base}, and \vn{design} values (\sref{s:lat.correct}). The value of
the \vn{ref} component is set by the user.
  }. 
The formula used to combine these values into one ``\vn{composit}'' value is determined by the
...
After the \vn{composit} value is calculated, $\delta D$ will be set equal to the \vn{composit} value
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
That is, the datum only contributes to the merit function if the \vn{composit} is value is negative.
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
That is, the datum only contributes to the merit function if the \vn{composit} is value is positive.
...
the settings of \vn{opt_with_ref} and \vn{opt_with_base} global parameters. These expressions are
...
There are several variations of how optimimization is done. To make the discussion concrete, the
case where a beta function measurement is used to calculate quadrupole \vn{K1} strength changes to correct
...
variables will have their \vn{merit_type} set to \vn{"target"}, the global \vn{opt_with_ref} and
\vn{opt_with_base} will be False, and the $\delta D$ and $\delta V$ in \Eq{m1} will be
\begin{align}
  \delta &\tv{D} = \tv{model} - \tv{meas} \CRNO
  \delta &\tv{V} = \tv{model} - \tv{meas}
  \label{dd1}
\end{align}
The data \vn{meas} values will is the measured beta function and the variable \vn{meas} values will
be the measured quadrupole \vn{k1} at the time the data was taken. The quadrupole \vn{k1} could be
...
degeneracies) in the problem which would allow \tao to find solutions where data \vn{model} matches
\vn{meas} with the variable \vn{model} strengths having ``unphysical'' values far from the measured
strengths. The weights $w_i$ and $w_j$ need to be set depending upon how accurate the measured data
is relative to how accurate the calibrations for measuring the variable \vn{meas} values are.
...
quadrupole strengths respectively. Typically, the \vn{design} quadrupole strengths are zero but that is
...
\vn{model} quadrupole strengths and the \vn{design} quadrupole strengths (typically the \vn{design}
...
With \vn{CRL}, the \vn{opt_with_base} and \vn{opt_with_ref} (\sref{s:generalized.design}) global
