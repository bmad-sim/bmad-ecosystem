...
region never exceeds a given value.  Section~\sref{c:optimizer} gives a tutorial on how to
setup the merit function and run the \vn{optimizer} in \tao.  In this chapter,
...
with the command \vn{show top10} (\srefl{s:show.top10}). And individual contributions
...
as shown with the \vn{show top10 -derivative} (\srefl{s:show.top10}) or the
