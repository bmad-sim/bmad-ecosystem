\chapter{Lattice Correction and Design Via Optimization of a Merit Function}
\label{c:opti}

This chapter covers using minimization (called \vn{optimization}) of a
\vn{Merit Function} to correct and and to design lattices. Examples
...
desireable properties. For example, requireing that the beta fuction
...
Section~\sref{s:lattice_correction} presents the merit function in the
context of lattice corrections while Section~\sref{s:lattice_design}
...
similar, \tao combines the two into one generalized merit function as 
discussed in Section~\sref{s:generalized_design}.

%------------------------------------------------------------------------
\section{Lattice Corrections}
\label{s:lattice_corrections}
\index{Modeling Data}
...
through a lattice to conform to some desired orbit. The process
...
discussed in Chapter~\sref{c:prog_customizing}. \tao does, however,
implement a generalized algorithim procedure for minimizing a
...
orbit correction) within the \vn{model} lattice (\sref{1.2.2}) with
the aim to make the measured \vn{data} (position data from BPMs for an
orbit correction) correspond to the values as calculated from the
\vn{model} lattice.  
...
(for example, by measureing the current through a steering and using a
