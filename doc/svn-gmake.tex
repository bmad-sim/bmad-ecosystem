...
\vn{<repos_dir>}. \vn{<repos_dir>} must be a
sub-directory of \vn{<repository_root_dir>}. Also
\vn{<repos_dir>} must be an absolute path name. If the \svn
repository is local then \vn{<repos_dir>} must be have
...
this could cause \svn to misbehave. Here the \vn{file://} prefix
tells svn to look for a repository on the local disk. For internet
access, there are the \vn{http://} and \vn{https://} prefixes.
...
\vn{svn ci}. Similarly, a file can be renamed 
\begin{example}
  svn mv <file>
\end{example}
...
\fig{f:devel.dir}. In \fig{f:devel.dir} names in italics
...
\fig{f:devel.dir} \vn{prog1} and \vn{prog2}
...
name. For example, in \fig{f:devel.dir}, the executable made
