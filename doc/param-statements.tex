...
tracked. Possible setting include all the settings of \vn{parameter[particle]}. In addition, this
...
coordinate sense. For example, when a machine has a number of repeating ``super-periods'', it may be
convenient to only use one period in a simulation. Since \bmad ignores closure in the global
coordinate sense, it is up to the user to ensure that a lattice is truly geometrically closed if
that is desired.
