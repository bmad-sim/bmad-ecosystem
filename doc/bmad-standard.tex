...
\sref{s:crystal.ref}.] The \vn{body coorinate system} will be
discussed below. The general procedure for tracking a praticle through
...
surface. The \vn{element exit coordinates} will genarally have it's
$y$-axis aligned with the $y$-axes of the other two coordinate
systems. The exceptional case occures for a \vn{crystal} element when
...
%-----------------------------------------------------------------
\subsection{Transformation from Element to Latboratory Coordinates}
...
{Reciprical space diagram showing crystal diffraction. The coordinates
shown are the element body coordinates. All points in the diagram are
in the plane of the paper except for the tip of $\bH$.  $\bK_{0}$ and
$\Bf K_{H}$ are the wave vectors inside the crystal and $\Bf k_{0}$ and
$\Bf k_{H}$ are the wave vectors outside the crystal. The reference
photon travelong along the reference trajectory has $\bK_0$ and
$\bK_H$ originating at the $Q$ point.}
...
\begin{align}
  |\bf k_0| &= |\bf k_H| = \frac{1}{\lambda} \CRNO
...
The vectors $\bK_0$ and $\bH$ must add up to the reciprical lattice vector $\bK_H$
\Begineq
  \bK_H + \bK_0 + \bH
\Endeq
or equivalently
\Begineq
  ...
  +
  \frac{1}{d}
  \begin{pmatrix} -\cos \alpha_H \\ \sin \alpha_H \, \sin \psi \\ \sin \alpha_H \, \cos \psi \end{pmatrix}
  \label{1l1dk}
\Endeq
where $\bfhat K_H$ has unit length and $\alpha_H$ (\vn{alpha_anlge})
and $\psi$ (\vn{psi_angle}) characterize the orientation of $H$.
Taking the length of both sides gives
\Begineq
  \frac{\lambda (1 + \delta)}{2 \, d} = \cos\alpha_H \, \sin\theta_0 = 
  \sin\alpha_H \, \cos\psi \, \cos\theta_0
\Endeq
Solving for $\theta_0$ gives
\Begineq
  \theta_0 = \sin^{-1} \frac{\lambda (1 + \delta)}{2 \, d \, \sqrt{1 - \sin^2\alpha_H \, \sin^2\psi}} +
  \tan^{-1} (\tan\alpha_H \, \cos\psi)
\Endeq
...
The outgoing graze angle $\theta_{g,out}$ is computed as follows. The
vector $\bK_H$ and $\Bf k_h$ share the same $y$ and $z$ coordinates so
from \Eq{1l1dk} 
\begin{align}
  k_{H,y} &= \frac{1}{d} \sin\alpha_H \, \sin\psi \CRNO
  k_{H,z} &= \frac{1}{d} \sin\alpha_H \, cos\psi + \frac{1}{\lambda \, (1 + \delta)} \, \cos\theta_0
\end{align}
And the $x$ component is obtained from \Eq{kk1l}
\Begineq
  k_{H,x} = -\sqrt{\frac{1}{\lambda^2} - k_{H,y}^2 - k_{H,z}^2}
\Endeq
The outgoing graze angle is then the total bending angle minus the incoming graze angle
\Begineq
  \theta_{g,out} = \tan^{-1}\frac{\wt k_{H,z}}{\sqrt{\wt k_{H,x}^2 + \wt k_{H,y}^2}}
\Endeq
where the tilde indicates that the components of $\Bf k_H$ are expressed in the is 

In order to track photons, transformation matricies between coordinate
systems are needed. To keep the discussion simple here, it is assumed that there is no misalignments.
When $\psi$ zero, 
the transformation matrix $T_{in}$ between laboratory entrance coordinates and
element entrance coordinates is, as it is for every other type of
element, just the unit matrix. That is, the two coordinate systems are
identical.
The transformation $M_{in}$ from element entrance coordinates to body coordinates is
...
\Begineq
  ...
  \label{mt0t010}
\Endeq
The transformation from element body coordinates to element exit coordinatates is
another rotation around the $y$ axis and the transformation from element exit
coordinates to laboratory exit coordinates is the unity matrix. Thus, the combind
transformation from laboratory entrance to laboratory exit coordinates is a rotation
around the $y$ axis as explained in section \sref{s:global}.

\index{tilt}
When $\psi$ is non-zero the situation is complicated since, if the above
transformations for the coordinate systems are used, the vector $\Bf k_H$
would be bent out of the $x$-$z$ plane. 
By {\em definition}, the reference trajectory has the form
given \Eq{lrca1} with the $\bT$ matrix depending only upon the \vn{tilt} parameter.
That is, the crystal must be reorientated to keep the
$\Bf k_H$ vector in the $x$-$z$ plane of the laboratory entrance coordinates. 
The reorientation is done by rotating the crystal
about the laboratory entrance $\Bf z$ axis by an amount $\theta_{corr}$ (\vn{tilt_corr})
that is determined as follows. With this ``tilt correction'' rotation, the transformation $\bT_{in}$
is 
\Begineq
  \bT_{in} = 
  \begin{pmatrix}
  ...
  \end{pmatrix}
\Endeq
The transformation from element entrance coordinates to element body coordinates
is not affected by a finite $\psi$ and so \Eq{mt0t010} is unmodified. The $\Bf k_H$
vector, expressed in laboratory entrance coordinates, must have zero $y$ component
\Begineq
...
\Endeq
The transformation $\bM_{out}$ from element body coordinates to element exit coordinates
is now obtaind by requiring that the total transformation from laboratory
entrance to laboratory exit coordinates be the $\wt\bS$ matrix given in
\Eq{lrca1}
\Begineq
  ...
  \end{pmatrix}
\Endeq
where $alpha_b = \theta_{g,in} + \theta_{g_out}$. In the above equation,
the transformation $T_{out}$ from element exit coordinates
to laboratory exit coordinates has been dropped since it is the
unit matrix independent of $\psi$. 

With misalignments, the transformation between laboratory and
element coordinates is more complicated as outlined in 
section \sref{s:photon-lab-ele}
...
coordinates to element entrance coordinates $\wt \Bf k_0$ is given
in \sref{s:photon-lab-ele}. The transformation to element body coordinates $k_{in}$
is
\begin{equation}
  \Bf k_0 =  \bM_{in} \, \wt\Bf k_0
\end{equation}
with $\bM_{in}$ given by \Eq{mt0t010}.

The equation to be solved is (\cite{b:batterman} Eq.~(21))
\Begineq
  \xi_0 \, \xi_H = \frac{1}{4} \, k^2 \, P^2 \, \Lambda^2 \, F_H \, F_{\bar H}
\Enedeq
where $\xi_0$ and $\xi_H$ are given by \cite{b:batterman} Eq.~(18)
and $P$ is the polaraization factor
\Begineq
  P = 
  \begin{cases}
...
approximated by $\theta_{g,in}$ and $\theta_{g_out}$.

The relationship between the outside and inside wave vectors is 
(\cite{b:batterman} Eq.~(26))
\begin{align}
  \bK_0 = \Bf k_0 + q \, k \, \bfhat n \CRNO
  \bK_H = \Bf k_0 + \bH + q \, k \, \bfhat n 
\end{align}
