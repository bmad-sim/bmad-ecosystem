...
\Begineq
  H_t = \wt\psi + \left[ \wt m^2 + (p_x - a_x)^2 + (p_y - a_y)^2 +  
  \left( \frac{p_s - a_s}{1 + g\, x} \right)^2 \right]^{1/2}
\Endeq
...
\Begineq
  H \equiv H_s = -(1 + g \, x) \sqrt{(1 + p_z)^2 - (p_x - a_x)^2 - (p_y - a_y)^2 - \wt m^2} - a_s
  \label{h1gx1}
\Endeq
Here $(p_z)$ is canonical coordinate (\sref{s:phase.space}) and the
equation for $H$ has been simplified by assuming that $\wt\psi$ is
zero. 
...
\Begineq
  H_{-s} = (1 + g \, x) \sqrt{(1 + p_z)^2 - (p_x - a_x)^2 - (p_y - a_y)^2 - \wt m^2} - a_s
\Endeq
...
\Begineq
  ...
  \left[ (p_x - a_x)^2 + (p_y - a_y)^2 + \wt m^2 \right] - \int \! ds \, g \, x
  \label{zz121p}
\Endeq
...
\Begineq
  x'_2 = -x_1 \, \frac{\left( 1 + 2 \cos^2 (\Delta \phi) \right) \, dE^2}{8 \, E_i^2 \, L}
\Endeq
