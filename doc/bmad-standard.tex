...
frames. The \Hyperref{r:offset.photon}{offset_photon} routine does an
...
\fig{f:photon.ele.coords}. In the \vn{element entrance
...
%-------------------------------------------------------------------------
\subsection{Calculation of Entrance and Exit Graze Angles}
...
\Begineq
  ...
  \label{k1lst}
\Endeq
Where, as shown in \fig{f:crystal.diffraction}, $\theta_{g,in}$,
...
components of $\bfk_H$ are given by \Eq{k1dapb} or Eq\eq{k1dapl}. To
...
ulta-relativistic limit with $beta = 1$ and do not include the
longitudinal motion. To fix this, consider first the entrance and exit
kicks given by R\&S Eq.~(12). Concentrating on the entrance horizontal kick, 

Starting with
the entrance horizontal kick, R\&S Eq.~(9) can be writen as
\Begineq
  dp_x = \frac{-|e| \, G \, \beta}{2 \, c \, P_0} \, x
  \label{dpxeg}
\Endeq
where $dp_x$ is the change in horizontal canonical momentum through
the entrance field, $e$ is the particle charge, $G$ is the
accelerating gradient, $\beta$ is the particle velocity, and $P_0$ is
the reference momentum. The underlying contact Hamiltonian
for the entrance kick can be deduced from \Eq{dpxeg} to be
\Begineq
  H_{e} = \frac{G \, \beta}{4 \, P_0} \, (x^2 + y^2)
\Endeq
In this and future equations, factors of $e$ and $c$ will be dropped
for convienience. The longitudinal transport is then
\Begineqs
  dz &= \dsfrac{\partial H_{e}}{\partial p_z} = 
    \dsfrac{G}{4 \, P_0} \, \dsfrac{d\beta}{dp_z} \, (x^2 + y^2) \\
  dp_z &= \dsfrac{-\partial H_{e}}{\partial z} = 
    \dsfrac{-\beta}{4 \, P_0} \, \dsfrac{dG}{dz} \, (x^2 + y^2)
\Endeqs
where
\Begineqs
  \dsfrac{d\beta}{dp_z} &= \dsfrac{m^2 \, P_0}{E^3}
  \dsfrac{dG}{dz} = G_{\mbox{max}} \, \dsfrac{d\cos(\Delta\phi)}{dz} \\
    &= \dsfrac{2 \, \pi \, G_{\mbox{max}} \, \sin(\Delta\phi) \, f_{rf}}{\beta}
\Endeqs
where $m$ is the particle mass, $E$ is the particle energy,
$G_{\mbox{max}}$ is the maximum accelerating gradiant at phase
$\Delta\phi = 0$, and $f_{rf}$ is the RF frequency.

The transport at the exit end of the cavity is handled in a similar manner to the
entrance end with the appropriate change in sign of the kick (cf.~R\&S Eq.~(12)).
...
\Begineq
  ...
  \begin{pmatrix}
    ...
    r_\beta \, m_{21} + r'_\beta \, m_{11} & r_\beta \, m_{22} + r_\beta \, m_{12} 
  \end{pmatrix}
  \,
  \begin{pmatrix}
    x \\ 
...
\Begineqs
  r_\beta &\equiv \sqrt{\dsfrac{\beta_1}{\beta_2}} \\
  r'_\beta &\equiv \dsfrac{dr_\beta}{ds}
\Endeqs
...
\begin{align}
  c \, ccc\Delta t &= \int_{s_1}^{s_2} \!\! ds \,\, \frac{1}{\beta(s)} \CRNO
...
{Tracking through a patch element. A particle's starting coordinate at
...
The transformation of the reference coordinates through a patch is given 
by \Eqs{vwlv} and \eq{wws} with $\Bf L$ and $\Bf S$ defined by \Eqs{xyz}
and \eq{swww}. Before tracking through the patch, a particle's coordinates 
in the entrance coordinate system will be
\begin{alignat}{1}
  \Bf r_{\text{ent}} &= (x, y, 0) \CRNO
  \Bf P_{\text{ent}} &= (P_x, P_y, P_s) = 
    \left( p_x, p_y, \pm \sqrt{(1+p_z)^2 - p_z^2 - p_y2} \right) \, P_0
\end{alignat}
where $p_x$, $p_y$ and $p_z$ are the phase space momenta, and $z$,
which is coordinate $z$ and not phase space $z$, is always zero by
construction as shown in \fig{f:patch} [Also see \fig{f:local.coords}
and the discussion in \sref{s:phase.space}.] The sign of the
longitudinal momentum $P_s$ is determined by whether the particle is
traveling in the positive $s$ or negative $s$ direction (which will
occur when an element is flipped longitudinally).

The transformation between entrance and exit coordinate systems is
\begin{alignat}{1}
  \Bf r_1 &= \Bf S^{-1} \, (\Bf r_{\text{ent}} - \Bf L) \CRNO
  \Bf P_1 &= \Bf S^{-1} \, \Bf P_{\text{ent}}
\end{alignat}
The $z_1$ coordinate will, in general, be non-zero. If this is so, the
particle must be propagated until it intersects the $z = 0$ plane in
the exit coordinate system.  It is assumed that the particle is in a
field free region. The final coordinates $r_{\text{exi}}$ and $P_{\text{exi}}$ are then
\begin{alignat}{1}
  \Bf r_{\text{exi}} &= (x_1 - z_1 * P_{x1} / P_{s1}, y_1 - z_1 * P_{y1} / P_{s1}, 0) \CRNO
  \Bf P_{\text{exi}} &= \Bf P_1
\end{alignat}

In addition, the phase space $z$ must be corrected due to the finite drift distance
and/or a finite \vn{t_offset}
\Begineq
  z_{\text{exi}}(\text{canon}) = 
    z_{\text{ent}}(\text{canon}) + z_1 \, \frac{|\Bf P|}{P_{s1}} + 
    \beta \, (\text{z_offset} + c \, \text{t_offset})
\Endeq
where $\beta$ is the particle velocity. The \vn{z_offset} factor here is
to make the \vn{patch} transformation the same as a drift transformation when
only \vn{z_offset} is nonzero.
