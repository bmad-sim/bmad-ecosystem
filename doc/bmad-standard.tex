...
The transformation of the reference coordinates through a patch is
given by \Eqs{vwlv} and \eq{wws}. Before tracking through the patch, a
particle's coordinates in the entrance coordinate system will be
\begin{alignat}{1}
...
\Begineq
  z_{\text{exi}}(\text{canon}) = 
    z_{\text{ent}}(\text{canon}) + z_1 \, \frac{P_0}{P_{s1}} + 
    \frac{\beta}{\beta_0} \, \text{z_offset} + \beta \, c \, \text{t_offset}
\Endeq
where $\beta$ is the particle velocity, and $\beta_0$ is the velocity
of the reference particle. The \vn{z_offset} factor here is to make
the \vn{patch} transformation the same as a \vn{drift} transformation
when only the \vn{patch}'s \vn{z_offset} attribute is nonzero.
