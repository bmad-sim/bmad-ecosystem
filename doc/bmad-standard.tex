...
(\sref{s:ref}). The \vn{element reference coordinates} stays fixed
relative to the element. Therefore, if the element is misaliged, the
\vn{element reference coordinates} will follow as the element
shifts in the laboratory frame.
...
chosen so that the $z$-axis points into the element as shown in
...
curve at the point (\cite{b:batterman} $\S$2.8B). This is illustrated
