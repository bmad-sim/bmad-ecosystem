...
reference trajectory, the bar diacritic is dropped.

Since $\bfhat n$ is in the $-\bfhat x$ direction, $\bfKbar_0$ is also in
the $x$-$z$ plane. Thus $\bfkbar_0$ and $bKbar_0$ can be written in the form
\Begineq
...
and $\theta_0$ are the angles of $Bf kbar_0$ and $\bfKbar_0$ with respect to
...
\Begineq
  \bfK_H = \bfK_0 + \bfH
\Endeq
Using this with \Eqs{k1lld}, \eq{kk1l1}, \eq{kk1l2}, and \eq{h1daa} gives
\Begineq
  ...
  \label{1l1dk}
\Endeq
Taking the length of both sides and using \Eq{kk1l2} gives
\Begineq
...
given in \label{ss:crystal.trans.el}. The transformation $\Bf\Sigma_1$
needs a ``tilt correction'' (\Eq{tttt}) as explained below when
$\psi_H$ is nonzero.  Since this tilt correction is independent of any
misalignments, the tilt correction calculation proceeds assumeing here
that there are no misalignments. The finite $\bfV$ due to the finite
crystal thickness in Laue diffraction will also be ignored for the moment.
...
tilt $theta_t$ is zero. But $\bfkbar_H$ points in the same direction
as the $z$ axis of the outgoing reference trajectory. Futhermore, by
...
With this ``tilt correction'', the
transformation $\Bf\Sigma_1$ is a rotation about the $z$ axis
\Begineq
  \Bf\Sigma_1 = 
  \begin{pmatrix}
...
For Laue diffraction, the $\bfV$ displacement vector
