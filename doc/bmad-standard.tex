...
\begin{align}
  x_1    &= x_0 - x_{off} + \frac{L}{2} x'_{pitch}
  ...
  z_1    &= z_0 + x'_{pitch} \, x_1 + y'_{pitch} \, y_1 - 
    \frac{L}{4} (x^{'2}_{pitch} + y^{'2}_{pitch})
\end{align}
Notice that $z_1$ is written in terms of $x_1$ and $y_1$
\item
Apply any tilt $\theta_t$
\begin{align}
  x_2    &=  \cos(\theta_t) \, x_1    + \sin(\theta_t) \, y_1
  p_{x2} &=  \cos(\theta_t) \, p_{x1} + \sin(\theta_t) \, p_{y1}
  y_2    &= -\sin(\theta_t) \, x_1    + \cos(\theta_t) \, y_1
  p_{y2} &= -\sin(\theta_t) \, p_{x1} + \cos(\theta_t) \, p_{y1}
\end{align}
\end{enumerate}
...
Apply any tilt $\theta_t$
\begin{align}
  x_1    &=  \cos(\theta_t) \, x_0    - \sin(\theta_t) \, y_0
  p_{x1} &=  \cos(\theta_t) \, p_{x0} - \sin(\theta_t) \, p_{y0}
  y_1    &=  \sin(\theta_t) \, x_0    + \cos(\theta_t) \, y_0
  p_{y1} &=  \sin(\theta_t) \, p_{x0} + \cos(\theta_t) \, p_{y0}
\end{align}
\item
Apply offsets and pitches
\begin{align}
  x_2    &= x_1 + x_{off} + \frac{L}{2} x'_{pitch}
  p_{x2} &= p_{x1} + (1 + p_{z1}) \, x'_{pitch}
  y_2    &= y_1 + y_{off} + \frac{L}{2} y'_{pitch}
  p_{y2} &= p_{x1} + (1 + p_{z1}) \, y'_{pitch}
...
Apply any tilt $\theta_t$
\begin{align}
  x_2    &=  \cos(\theta_{tot}) \, x_1    + \sin(\theta_{tot}) \, y_1
  p_{x2} &=  \cos(\theta_{tot}) \, p_{x1} + \sin(\theta_{tot}) \, p_{y1}
  y_2    &= -\sin(\theta_{tot}) \, x_1    + \cos(\theta_{tot}) \, y_1
  p_{y2} &= -\sin(\theta_{tot}) \, p_{x1} + \cos(\theta_{tot}) \, p_{y1}
  I_{x2}    &= 
  \phi_{x2} &=  
\end{align}
where $\theta_{tot}$ is the sum of \vn{tilt} and \vn{tilt_err}.
\item
Apply \vn{graze_angle_err}
\begin{align}
  x_3    &= x_2
  p_{x3} &= p_{x2} + \theta_{gerr}
  y_3    &= y_2
  p_{y3} &= p_{Y2}
  z_3    &= z_1 - x_2 \, \theta_{gerr}
  p_{z3} &= p_{z1}
\end{align}
