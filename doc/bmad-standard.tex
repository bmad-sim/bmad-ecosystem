...
\begin{align}
  ...
  y_1    &= y_0 - y_{off} \\
  p_{y1} &= p_{x0} - (1 + p_{z0}) \, y'_{pitch} \CRNO
  z_1    &= z_0 + x'_{pitch} \, x_1 + y'_{pitch} \, y_1 \CRNO
\end{align}
\item
Apply \vn{tilt} and \vn{tilt_err}
Apply any tilt $\theta_t$
\begin{align}
  x_2    &=  x_1    \, \cos\theta_{tot} + y_1    \, \sin\theta_{tot}  \CRNO
  y_2    &= -x_1    \, \sin\theta_{tot} + y_1    \, \cos\theta_{tot}  \\
  p_{x2} &=  p_{x1} \, \cos\theta_{tot} + p_{y1} \, \sin\theta_{tot}  \CRNO
  p_{y2} &= -p_{x1} \, \sin\theta_{tot} + p_{y1} \, \cos\theta_{tot}  \CRNO
  E_{x2} &=  E_{x1} \, \cos\theta_{tot} + E_{y1} \, \sin\theta_{tot}  \CRNO
  E_{y2} &= -E_{x1} \, \sin\theta_{tot} + E_{y1} \, \cos\theta_{tot}  \CRNO
\end{align}
where $\theta_{tot}$ is the sum of \vn{tilt} and \vn{tilt_err} and $E$ is shorthand notation for
\Begineq
  E \equiv \sqrt{I} \, e^{i \, \phi}
\Endeq
...
The back transformation from exit element to exit laboratory coordinates is
accomplished by the transformation
\begin{enumerate}
\item
