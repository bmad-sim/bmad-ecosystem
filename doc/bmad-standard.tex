...
\Begineq
  H_t = \wt\psi + \left[ (m \, c / P_0)^2 + (p_x - a_x)^2 + (p_y - a_y)^2 +  
...
and $\Bf a$ and $\wt\psi$ are the normalized vector and scalar
potentials:
\Begineq
  \left( a_x, a_y, \frac{a_s}{1+g \, x} \right) = \frac{q \, \Bf A}{P_0 \, c} \qquad 
  \wt\psi(x,y,z) = \frac{q \, \psi}{P_0 \, c^2}
\Endeq
...
zero. For backwards propagation, where $p_s$ is negative, solving for $p_s$ involves using
