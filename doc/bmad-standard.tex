...
clarity, all three coordinate systems are displaced from each
other. In actuality, all three share a common origin.  Independent of
any ``misalignments'', all three coordinate systems share a common $y$
axis (pointing out of the page in the figure) and for all three
reference frames the surface normal $\bfhat n_s$ is in the $x$-$z$
plane.}
...
tilt, (henceforth called ``misalignments''), the element reference
...
therefore shift as the element shifts in the laboratory frame.
...
complicates the geometry. For these elements, three element reference
coordinates are used as shown in Fig.~\ref{f:photon.ele.coords}. The
first set of coordinates are the \vn{element entrance coordinates}.
Without misalignment, these coordinates are the same as the
\vn{laboratory entrance coordinates} shown in
Fig.~\ref{f:local.coords}. The second set of coordinates are the
\vn{element body coordinates} which always have the $x$ coordinate
pointing into the element antiparallel to the outward surface normal
$\bfhat n_s$. The third set of coordinates are the \vn{element exit
coordinates}.  Without misalignment, these coordinates are the same as
the \vn{laboratory exit coordinates} shown in
Fig.~\ref{f:local.coords}.
...
in the plane of the paper except for the tip of $H$.  $K_{0}$ and
$K_{H}$ are the wave vectors inside the crystal and $k_{0}$ and
$k_{H}$ are the wave vectors outside the crystal.}
...
theory\cite{b:batterman}. Before any photon tracking is done, the
incoming grazing angle $\theta_{g,in}$ and outgoing grazing angle
$\theta_{g,out}$ must be computed to orient the reference trajectory
with respect to the crystal surface. Fig.~\ref{f:crystal.diffraction}
shows the geometry of the problem. The graze angles are calculated
such that a photon with the reference energy on the reference
trajectory will be in the center of the Darwin curve. That is The
internal wave vectors $K_0$ and $K_H$ originate from point $Q$ (See
\cite{b:batterman} Figs.~8 and 29). The $H$ vector in the element body
...
The starting photon coordinates are
specified in the laboratory entrance coordinates. The transformation
from laboratory entrance coordinates to element entrance coordinates $\tilde k_{in}$
is given in \sref{s:lab-ele-transform}. The diffraction algorithm is 
then
  \begin{enumerate}
  \item
Transform to \vn{element body coordinates} $k_{in}$
\begin{align}
  k_{in,x} &=  \tilde k_{in,x} \, \cos \, \theta_{g,in} + \tilde k_{in,z} \, \sin \, \theta_{g,in} \CRNO
  k_{in,y} &=  \tilde k_{in,y} \\
  k_{in,z} &= -\tilde k_{in,x} \, \sin \, \theta_{g,in} + \tilde k_{in,z} \, \cos \, \theta_{g,in} \CRNO
\end{align}
Construct $\bH_n$ which is the $H$ vector normalized by the photon wavelength $\lambda$
\Begineq
  H_n = \frac{\lambds}{d}
  \begin{pmatrix}
    -\cos \alpha_H \\
  ...
  \end{pmatrix}
\Endeq
where $d$ is the spacing between the reflection planes.
