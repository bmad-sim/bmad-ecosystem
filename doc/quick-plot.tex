...
Figure~\ref{f:plot.out} from the input file named \vn{plot.dat}. The
...
\hyperref[r:qp.draw.text]{routine!qp_draw_text} call on line 34 draws the title above the two graphs.

The \hyperref[r:qp.read.data]{routine!qp_read_data} call on line 28 will skip any ``header'' lines
(lines that do not begin with something that looks like a number) in
the data file. In this instance \hyperref[r:qp.read.data]{routine!qp_read_data} will read the
first, third forth and fifth data columns and put them into the
\vn{x}, \vn{y}, \vn{z}, and \vn{t} arrays.

\hyperref[r:qp.set.page.border]{routine!qp_set_page_border}, \hyperref{routine!qp_set_box}, and \hyperref{routine!qp_set_margin}
sets where the graph is going to be placed.  \hyperref[r:qp.set.box]{routine!qp_set_box}\vn{(1,
1, 2, 1)} on line 37 tells \quickplot to put the first graph in the
left box of a 2 box grid. The \hyperref[r:qp.set.margin]{routine!qp_set_margin} on line 33 sets the
margins between the box and the graph axes.

\hyperref[r:qp.calc.and.set.axis]{routine!qp_calc_and_set_axis} on lines 38 and 39 are used to scale the
axes. \vn{"ZERO_AT_END"} ensures that the $x$--axis starts (or stops)
at zero.  \hyperref[r:qp.calc.and.axis.axis]{routine!qp_calc_and_axis_axis} is told to restrict the number of
...
graph, the routines \hyperref[r:qp.set.symbol.attrib]{routine!qp_set_symbol_attrib} and
\hyperref[r:qp.set.line.attrib]{routine!qp_set_line_attrib} are called on lines 44 and 45 to plot the
...
the routines \hyperref[r:qp.save.state]{routine!qp_save_state} and \hyperref{routine!qp_restore_state} on lines
43 and 47 are used. \hyperref[r:qp.save.state]{routine!qp_save_state} saves the current attributes
in a attribute stack. \hyperref[r:qp.restore.state]{routine!qp_restore_state} restores the saved
attributes from the attribute stack. \hyperref[r:qp.draw.axes]{routine!qp_draw_axes} is called on
line 40 to draw the $x$ and $y$-axes along, and \vn{qp_draw_data} is called
...
\hyperref[r:qp.draw.axes]{routine!qp_draw_axes} in lines 40 and 57/58 comes from PGPLOT and allows
...
\quickplot uses the following concepts as shown in Figure~\ref{f:plot.coords}
\begin{example}
...
\hyperref[r:qp.open.page]{routine!qp_open_page} determines the size of the \vn{page} if it is
...
edges of the page are set by \hyperref[r:qp.set.page.border]{routine!qp_set_page_border}.  The box that
the graph falls into is set by \hyperref[r:qp.set.box]{routine!qp_set_box}. The default is to
have no margins with 1 box covering the entire page. The
\hyperref[r:qp.set.margin]{routine!qp_set_margin} routine sets the distance between the box edges
...
see that the arguments for \hyperref[r:qp.draw.rectangle]{routine!qp_draw_rectangle} are
\begin{example}
...
Everything must be in upper case.
 \item 
For some routines (\hyperref[r:qp.set.margin]{routine!qp_set_margin}, etc.) only a relative distance
...
default is \vn{INCH/PAGE/LB}. use \hyperref[r:qp.set.default]{routine!qp_set_default} to change this.
\end{itemize}
Examples:
\begin{example}
...
respectively as shown in Figure~\ref{f:plot.coords}. Normally the
...
respectively. \hyperref[r:qp.set.axis]{routine!qp_set_axis} can be used to disable mirroring. For
example:
\begin{example}
  call qp_set_axis ("Y2", mirror = .false.)  ! y2-axis now independent of y.
\end{example}
\hyperref[r:qp.set.axis]{routine!qp_set_axis} can also be used to set \vn{Y2} axis parameters
...
\hyperref[r:qp.set.axis]{routine!qp_set_axis}. For example:
\begin{example}
  call qp_set_axis ("Y2", draw_numbers = .true.)  ! draw y2 axis numbers
\end{example}

To plot data using the \vn{X2} or \vn{Y2} scale use the
\hyperref[r:qp.use.axis]{routine!qp_use_axis} routine. For example:
\begin{example}
  call qp_save_state (.true.)
