...
\vnr{qp_set_line_attrib} are called on lines 44 and 45 to plot the
...
the routines \vnr{qp_save_state} and \vnr{qp_restore_state} on lines
43 and 47 are used. \vnr{qp_save_state} saves the current attributes
in a attribute stack. \vnr{qp_restore_state} restores the saved
attributes from the attribute stack. \vnr{qp_draw_axes} is called on
...
\vnr{qp_draw_axes} in lines 40 and 57/58 comes from PGPLOT and allows
...
the graph falls into is set by \vnr{qp_set_box}. The default is to
have no margins with 1 box covering the entire page. The
\vnr{qp_set_margin} routine sets the distance between the box edges
