...
\vnr{qp_draw_text} call on line 34 draws the title above the two graphs.

The \vnr{qp_read_data} call on line 28 will skip any ``header'' lines
(lines that do not begin with something that looks like a number) in
the data file. In this instance \vnr{qp_read_data} will read the
first, third forth and fifth data columns and put them into the
\vn{x}, \vn{y}, \vn{z}, and \vn{t} arrays.

\vnr{qp_set_page_border}, \vnr{qp_set_box}, and \vnr{qp_set_margin}
sets where the graph is going to be placed.  \vnr{qp_set_box}\vn{(1,
1, 2, 1)} on line 37 tells \quickplot to put the first graph in the
left box of a 2 box grid. The \vnr{qp_set_margin} on line 33 sets the
margins between the box and the graph axes.

\vnr{qp_calc_and_set_axis} on lines 38 and 39 are used to scale the
axes. \vn{"ZERO_AT_END"} ensures that the $x$--axis starts (or stops)
at zero.  \vnr{qp_calc_and_axis_axis} is told to restrict the number of
...
graph, the routines \vnr{qp_set_symbol_attrib} and
\vnr{qp_set_line_attrib} are called on lines 43 and 44 to plot the next
...
\vnr{qp_save_state} and \vnr{qp_restore_state} on lines 42 and 46 are
used. \vnr{qp_save_state} saves the current attributes in a attribute
stack. \vnr{qp_restore_state} restores the saved attributes from the
attribute stack. \vnr{qp_draw_data} on line 45 is similar to
\vnr{qp_draw_graph} except only the data is drawn. Not the axes and
associated labels.

Lines 49 through 58 draw the third curve in the right hand graph.  The
\vn{qp_set_axis} call on lines 54/55 sets a log scale for the \vn{y2}
(right hand) axis. The syntax of the string arguments of
\vnr{qp_draw_graph} in lines 40 and 56/57 comes from PGPLOT and allows
...
\vnr{qp_open_page} determines the size of the \vn{page} if it is
...
edges of the page are set by \vnr{qp_set_page_border}.  The box that
the graph falls into is set by \vnr{qp_set_box}. The default is to have
no margins with 1 box covering the entire page. The \vnr{qp_set_margin}
...
see that the arguments for \vnr{qp_draw_rectangle} are
\begin{example}
...
Everything must be in upper case.
 \item 
For some routines (\vnr{qp_set_margin}, etc.) only a relative distance
...
default is \vn{INCH/PAGE/LB}. use \vnr{qp_set_default} to change this.
\end{itemize}
Examples:
\begin{example}
...
respectively. \vnr{qp_set_axis} can be used to disable mirroring. For
example:
\begin{example}
  call qp_set_axis ("Y2", mirror = .false.)  ! y2-axis now independent of y.
\end{example}
\vnr{qp_set_axis} can also be used to set \vn{Y2} axis parameters
...
defines a line is a \vni{qp_line_struct}
\begin{example}
  type qp_line_struct
...
The \vni{qp_symbol_struct} defines how symbols are drawn 
\begin{example}
...
The \vni{qp_axis_struct} defines how axes are drawn 
\begin{example}
  type qp_axis_struct
