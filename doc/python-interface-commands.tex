...
\begin{verbatim}
where
  {ix_universe} is a universe index. Defaults to \vn{s%global%default_universe.}
...
where
  {ix_universe} is a universe index. Defaults to \vn{s%global%default_universe.}
  {ix_branch} is a lattice branch index. Defaults to \vn{s%global%default_branch.}
...
Data constraints output is:
  ...
  measured value
  ref value (only relavent if \vn{global%opt_with_ref} = T)
  model value
  base value (only relavent if \vn{global%opt_with_base} = T)
  weight
  merit value
...
Note: When setting multiple data parameters, 
      temporarily toggle \vn{s%global%lattice_calc_on} to False
...
Note: The 3 values for \vn{spin_axis%n0,} as a group, are optional. 
      Also the 3 values for \vn{spin_axis%l} are, as a group, optional.
...
where {ele_id} is an element name or index and {which} is one of
  ...
  base
  design
{index} is the index number in the \vn{ele%cartesian_map(:)} array
{who} is one of:
  base
  terms
Example:
...
where {ele_id} is an element name or index and {which} is one of
  ...
  base
  design
{index} is the index number in the \vn{ele%cylindrical_map(:)} array
{who} is one of:
  base
  terms
Example:
...
where {ele_id} is an element name or index and {which} is one of
  model, base, design
{index} is the index number in the \vn{ele%grid_field(:)} array.
{who} is one of:
  base, points
Example:
...
%% python ele:taylor_field ------------------------------------
\subsection{python ele:taylor_field}
\index{python!ele:taylor_field}
\label{p:ele:taylor.field}
...
where {ele_id} is an element name or index and {which} is one of
  ...
  base
  design
{index} is the index number in the \vn{ele%taylor_field(:)} array
{who} is one of:
  base
  terms
Example:
...
where {ele_id} is an element name or index and {which} is one of
  ...
  base
  design
{index} is the index number in the \vn{ele%wall3d(:)} array (size obtained from "ele:head").
{who} is one of:
  base
  table
Example:
...
where:
 Optional {flags} are:
  ...
  -array_out : If present, the output will be available in the \vn{tao_c_interface_com%c_real} or
    \vn{tao_c_interface_com%c_integer} arrays. See the code below for when %c_real vs %c_integer is used.
...
%% python plot_plot_manage ------------------------------------
\subsection{python plot_plot_manage}
\index{python!plot_plot_manage}
\label{p:plot.plot.manage}
...
\begin{verbatim}
Output syntax:
  {v1_var \vn{name};{v1_var%v} lower \vn{bound};{v1_var%v} upper bound}
