...
disadvantage is that the conversion does not preserve any of the look and feel of the original
\mad-X file. The other disadvantage is that conversion via PTC does not work with \mad8 lattices.
...
Note: \mad-X does not correctly handle \vn{elseparator} and \vn{matrix} elements. These
...
Accelerator Parser} (\sref{s:aml}). As an alternative, consider converting via PTC
(\sref{s:mad.bmad.ptc}).
