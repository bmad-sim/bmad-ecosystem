...
the routines \vn{track1_preprocess} and \vn{track1_postprocess}. See
Section~\sref{s:custom.track} for more details.
...
For customizing tracking, there are three routines that are available:
\begin{example}
  \zzz{track1_preprocess}
  \zzz{track1_custom}
  \zzz{track1_postprocess}
\end{example}
All three routines are called by the \zzz{track1} routine.
\vn{track1_custom} is only called when the element being tracked
through has its tracking method set to \vn{custom}. See
Section~\sref{s:custom.ele} for more details. The other routines are
always called. The \vn{track1_preprocess} and \vn{track1_postprocess}
...
A potential problem with \vn{track1_custom} is that the calling
routine, that is \vn{track1}, does some work like checking aperture,
etc. (see the \vn{track1} code for more details). If this is not
desired, the \vn{track1_preprocess} routine can be used to do custom
tracking and to make sure that \vn{track1} does not do any extra
calculations. This is accomplished by putting the custom tracking code
in \vn{track1_preprocess} and by setting the \vn{finished} argument of
