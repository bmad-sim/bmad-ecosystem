...
routine. This routine is divided into three parts: The code in the
first part is at the start of \vn{make_mat6} and looks like:
\begin{example}
...
\end{example}
Call this the ``custom'' part. Another part is the call
to the routine that is specific to the \vn{ele%mat6_calc_method} setting.
...
\end{example} 
Let us call this the ``method specific'' part. Notice that here when
\vn{ele%mat6_calc_method} is set to \vn{custom2\$} (not \vn{custom\$},
\vn{make_mat6_custom2} is called. The last part, call it the
``common'' part, is everything else. The common part involves
...
calculation. That is, only the custom part of \vn{make_mat6} is
...
part of \vn{make_mat6}. In this case, \vn{ele%make_mat6_custom} needs
...
\Hyperref{r:track1.custom}{track1_custom} routins are called by the
...
\vn{%type_out} and print an error message irregarless.
