...
There are nine routines that implement custom calculations:
\begin{example}
...
the element is set to \vn{custom}.  \vn{track1_custom} is actually
called twice by \vn{track1}. The reason for this is as follows:
The code of \vn{track1} can be divided into three sections. The middle
section uses the value of \vn{ele%tracking_method} to call the appropriate routine for the
...
\begin{example}
  ...
  case (runge_kutta\$) 
    ...
    if (err) return
  case (custom\$) 
    call track1_custom (start2_orb, ele, param, end_orb, track, entry_pt2\$, finished)
  ... etc ...
  end select
\end{example} 
The first and last sections that are before and after the middle
section do a number of things that are common to all tracking
methods. For example, checking apertures, adding in radiation damping
and excitation kicks, etc. In some cases with custom tracking, the
code in the first and last sections needs to be skipped. In order to
do this, \vn{track1_custom} is called at the start of \vn{track1}:
\begin{example}
...
\end{example}
The definition of \vn{track1_custom} is:
\begin{example}
  track1_custom (start_orb, ele, param, end_orb, track, err_flag, entry_pt, finished)
\end{example}
The idea is that to skip the first and last sections,
\vn{track1_custom} simply sets the \vn{finished} argument to True. If
these sections are not to be skipped, \vn{track1_custom} should set
\vn{finished} to False and return without doing anything when
\vn{entry_pt} has a value of \vn{entry_pt1\$}. In the middle section,
\vn{track1_custom} will be called with \vn{entry_pt} set to
\vn{entry_pt2\$}.
...
was modified to meet the need. In order to ratain some of the speedups
that can be achieved with saved variables, a switch was introduced
in the \vn{global_common_struct} called \vn{%be_thread_safe}. The
default is False. This should be set True in a a multithreaded program:
\begin{example}
