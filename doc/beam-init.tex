...
\begin{example}
  type beam_init_struct
    ...
    real emit_jitter(2)   = 0.0        ! %RMS a and b-mode emittance jitter
...
The \vn{%center} parameter is used to offset the center position of a bunch when the
\vn{%use_particle_start_for_center} is set to False. See the description of
\vn{%use_particle_start_for_center} below for more details.
%
...
center. Note: The old, deprecated, name for \vn{%sig_pz_jitter} is \vn{%sig_e_jitter}. This deprecated
...
\vn{%dPz_dz} determine the longitudinal profile. Note: The old, deprecated, name for \vn{%sig_pz} is
\vn{%sig_e}. This deprecated name is currently accepted.
%
...
%
\item[\%use_particle_start_for_center] \Newline
If \vn{%use_particle_start_for_center} is set to \vn{True} (default is \vn{False}), the center of
the bunch is determined by (\vn{particle_start[x]}, \vn{particle_start[px]}, $\ldots$,
\vn{particle_start[pz]}) (\sref{s:beam.start}) setting in the lattice file rather than the setting
of \vn{%center} in the \vn{beam_init_struct}. In this case, \vn{%center} is a dependent parameter
and will be set to the value of \vn{particle_start}.
%
...
describing particles using ``time coordinates'' (\sref{s:time.phase.space}). This is useful, for
example, for low energy electrons generated at the cathode of an electron gun.

The \vn{use_z_as_t} parameter is only used if \vn{use_t_coords} is set to True. When
\vn{use_t_coords} is True, if \vn{use_z_as_t} is also True, the \vn{z}-coordinate of the
distribution (which is the phase space $z$-coordinate when \vn{use_t_coords} is False), will be
taken as describing the time coordinate. This is useful for describing the situation, for example,
where particles may originate at a cathode at the same $s$-position, but different times.  If
\vn{use_z_as_t} is False (the default), the $z$ coordinate is taken to describe the $s$-coordinate.
This is useful for modeling particles that have the same time but different $s$ positions. In this
case, particles are ``born'' inside the lattice element at the location of the particle's
$s$-coordinate. To properly track the particles, the bunch will need to be tracked through the
element where they are born with a tracking method that can handle inside particles. Currently the
only such tracking method is \vn{time_runge_kutta}.  When the particles get to the end of the
element, the particle positions are converted to standard $s$-coordinates.
\end{description}
...
\vn{beam_init%center} (unless \vn{beam_init%use_particle_start_for_center} is set True) and
