...
\vn{beam_init_struct} structure is explained in Section~\sref{s:beam.init.struct}. The other wayto
...
\vn{%file_name} sets the name of the file to be read in containing the particle coordinates. 
Input from a file is triggered if \vn{%distribution_type(1)} is set to "FILE". The format of the
file is discused in Section~\sref{s:beam.init.file}.
%
