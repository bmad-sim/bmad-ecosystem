...
uses PTC on a per element basis. That is, if a tracking method
(\sref{c:tracking}) of a given element is set to a tracking method
that uses PTC, then PTC will be used for tracking through that
element.  The advantage of single element mode is that it is quite
flexible in that different tracking methods can be assigned to
different element. The drawback of single element mode is that PTC's
powerful analysis tools cannot be used in this case.

normal form analysis Twiss parameters
