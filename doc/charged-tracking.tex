...
$zzz$ is the normalized beam--ion force.
$\zzz(y)$ is normalized such that $\zzz(y) = -y$ for $y$ small. For a
Gaussian beam $zzz$ can be calculated from the standard Bassetti--Erskine
complex error function formula\cite{bassetti}
\begin{equation}
  zzz = \sigma_y \, \sqrt{\pi \, (1 + r) \over 2 \, (1 - r)} \cdot
  \Re{w \left( {i \, r\, y/\sigma_y \over \sqrt{2 \, (1 - r^2)}} \right) -
  e^{-y^2/2\sigma_y^2} \cdot
  w \left( {i \, y/\sigma_y \over \sqrt{2 \, (1 - r^2)}} \right)}
  \comma \label{fsp1r}
\end{equation}
where $r \equiv \sigma_y/\sigma_x$ and $w$ is the complex error function. $zzz$ is
shown in.
