...
$\calO'$ is the point that the incoming particle would strike the top surface if it went straight
...
origin $\calO'$ of this coordinate system is constructed by placing $\calO'$ at the point where the
...
N_t$. If the actual converter thickness is outside the range of these thicknesses, this is an
error. The particle distribution is also calculated within a certain incoming particle momentum
range. If an incoming particle has a momentum outside of this range, this is an error.

The first step is to choose the value of the momentum $p_\txt{out}$. For each thickness $t_i$, the
precomputed particle distribution parameters includes a two-dimensional table of $P(p_\txt{out}, r)$ ---
the probability density of creating an outgoing particle versus $p_\txt{out}$ and $r$ where $r$ is the
distance from the origin $\calO'$ to where the outgoing particle leaves the surface. $P(p_\txt{out},r)$
is normalized so that the integrated problability is equal to the average number of outgoing particles
created for each incoming particle $N_\txt{out}/N_{in}$
\begin{equation}
  \frac{N_\txt{out}}{N_{in}} = \int \int dp_\txt{out} \, dr \, P(p_\txt{out}, r)
\end{equation}
If the momentum or angular range of the outgoing particles is restricted using any of the three
element parameters as discussed above, new ``normalized'' probability $P_{norm}(p_\txt{out},r)$ tables
are computed. to save time, this ``setup'' computation step is only done if one of the three element
parameters changes. Additionally, the setup computation includes creating a table of $I(p_\txt{out})$
which is the integrated probability for generating a particle with momentum less than $p_\txt{out}$
\begin{equation}
  I(p_\txt{out}) = \frac{\dstyle \int_{p_\txt{min}}^{p_\txt{out}} dp'_\txt{out} \int dr \, P_{norm}(p'_\txt{out}, r)}
...
$P(p_\txt{out}, r)$ table or the value of \vn{pc_out_max} which ever is smallest. $I(p_\txt{out})$ is
normalized such that $I(p_\txt{max}) = 1$. A value for $p_\txt{out}$ is generated using the
$I(p_\txt{out})$ tables for the two $t_i$ thicknesses that straddle the actual thickness. The value of
$p_\txt{out}$ assigned to the outgoing particle is obtained via linear interpolation between the two
computed values at the two thicknesses. Note that for both thicknesses the same random number used
in the computation.

The next step is to choose a value for $r$... 
...
Except for the placement of $\calO'$, the above algorithm for calculating the position and
