...
%---------------------------------------------------------------------------------
%---------------------------------------------------------------------------------
\section{Bend Element: Body Tracking with k1 = 0}
...
\end{align}
where $\rho_p$ is the radius of curvature of the particle trajectory in the $(u, v)$ plane.
...
\begin{align}
  ...
  g_p &= \frac{1}{\rho_p} = \frac{g_\tot}{\sqrt{(1 + p_z)^2 - p_y^2}}
\end{align}
and $g_\tot$ is the bending strength of the actual field. Both \Eq{xctlg1} and \Eq{xctlg2} are
needed since \Eq{xctlg1} is singular when $g_p$ is zero and \Eq{xctlg2} is singular when $\alpha
= 0$ and $\theta + \phi_1 = \pi$ which happens when the particle is bent by 180\Deg. A simple way to
implement the calculation for $x_2$ is to use \Eq{xctlg1} when $|\theta + \phi_1| < \pi/2$ and
...
\end{equation}
where $\bfL_c$ is vector (chord) from point 1 and point 2
\begin{equation}
  \bfL_c = (L_{cu}, L_{cv}) =
...
%---------------------------------------------------------------------------------
%---------------------------------------------------------------------------------
\section{Bend Element: Body Tracking with finite k1}
...
\begin{equation}
  c_x(p_\txt{out},r) = \left( \sum_{n=1}^4 a_{n} \, r^n \right) \, 
  \left( \sum_{n=0}^4 b_{n} \, p_\txt{out}^n \right)
\end{equation}
...
\begin{equation}
  \alpha_i(r) = \left( \sum_{n=0}^4 d_{n,i} \, r^n \right) \, e^{-k \, r}
\end{equation}
...
\begin{equation}
  ...
  \left( \sum_{n=0}^4 k_{n,i} \, r^n \right) \, \left( \sum_{n=0}^4 w_{n,i} \, p_\txt{out}^n \right)
\end{equation}
