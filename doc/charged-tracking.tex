...
The space charge (SC) component of the coherent synchrotron radiation simulaiton is
diveded into two parts: A longitudinal and a transverse part. The transverse part uses the
same Bassetti--Erskine complex error function formula\cite{b:talman} as with the beam-beam
interaction (\sref{s:beambeam.std}) except here, since all the particles are moving in the
same direction, the kicks due to the electric and magnetic fields generated by a given
particle tend to cancel.

In equation~\eq{yt2gy} $\fbar(y)$ is the normalized beam--ion force.
$\fbar(y)$ is normalized such that $\fbar(y) = -y$ for $y$ small. For a
Gaussian beam $\fbar(y)$ can be calculated from the standard Bassetti--Erskine
complex error function formula\cite{bassetti}
\begin{equation}
  \fbar(y) = \sy \, \sqrt{\pi \, (1 + r) \over 2 \, (1 - r)} \cdot
  \Re{w \left( {i \, r\, y/\sy \over \sqrt{2 \, (1 - r^2)}} \right) -
  e^{-y^2/2\sy^2} \cdot
  w \left( {i \, y/\sy \over \sqrt{2 \, (1 - r^2)}} \right)}
  \comma \label{fsp1r}
\end{equation}
where $r \equiv \sy/\sx$ and $w$ is the complex error function. $\fbar$ is
shown in figure~\ref{fig:fbar} for $r = 0.1$. Note that since $\rhobarom$ and
$\fbar$ are normalized, the quantity $\alpha$ in equation~\eq{yt2gy} is
directly proportional to the ion density.
