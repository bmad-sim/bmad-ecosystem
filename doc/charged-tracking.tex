...
\section{Sprint Spin Tracking} Bmad includes precalculated first-order spin transfer quaternions for
simple lattice elements. This method is much faster than PTC integration, and its runtime does not
increase proportionally to element length. Currently, the supported lattice elements are bends
(including bends with $k_1 \neq 0$), quadrupoles, and solenoids.
...
\begin{alignat}{4}
  \chi   &= 1+a\gamma           & \zeta  &= \gamma - 1   \qquad & \psi  &= \gamma^2 - 1     &        &                      \CRNO   
  {d}    &= g \, l              & {e}    &= a \, g \, l\gamma   & s     &= a \, k_s l       & {t}    &= (1+a) k_s l         \\
  c_{d}  &= \cos({d})    \qquad & s_{e2} &= \sin(\frac{{e}}{2}) & c_{s} &= \cos({s}) \qquad & c_{t}  &= \cos({t})           \CRNO
  c_{d2} &= \cos(\frac{{d}}{2}) & c_{e2} &= \cos(\frac{{e}}{2}) & s_{s} &= \sin({s})        & s_{t2} &= \sin(\frac{{t}}{2}) \CRNO
  s_{d}  &= \sin({d})           &        &                      &       &                   & c_{t2} &= \cos(\frac{{t}}{2}) \nonumber
\end{alignat}
