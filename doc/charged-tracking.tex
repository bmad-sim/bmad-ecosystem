...
with axes labeled by $(\xw,\yw)$. Each outgoing particle will be assigned its own $(\xw,\yw)$
...
particle would pass straight through the converter. The angular orientation of the $(\xw,\yw)$ axes
...
the converter will be generated at a point a distance $r$ along the $\xw$-axis.
...
\begin{equation}
  I_p(p_\txt{out}) = \frac{\dstyle \int_{p_\txt{min}}^{p_\txt{out}} dp'_\txt{out} 
  \int dr \, P_\txt{n}(p'_\txt{out}, r)}
  {\dstyle \int_{p_\txt{min}}^{p_\txt{max}} dp'_\txt{out} \int dr \, P_{n}(p'_\txt{out}, r)}
\end{equation}
...
\begin{equation}
  I_r(r) = \frac{\dstyle \int_0^r dr' \, P_\txt{n}(p_\txt{out}, r')}
  {\dstyle \int_0^{r_\txt{max}} dr' \, P_{n}(p_\txt{out}, r')}
\end{equation}
...
parameters using the derivatives $\xw' = d\xw/ds$ and $\yw' = d\yw/ds$ in the form of a skewed Lorenzian
probability distribution $P_d$
\begin{equation}
  P_d\left( \xw', \yw' ; p_\txt{out}, r \right) =
...
$\xw$-axis, the distribution is symmetric about $\yw'$-axis. The pre-computed distribution
...
value of $\yw'$ that the pre-computed distribution parameters $\beta$, $c_x$, $\alpha_x$, and
