...
\begin{align}
  {\pmb\Omega}_{BMT} (\mathbf{r,P},t) &= 
    ...
    \right] \\
  &= - \frac{q}{m} \left[ 
    \left( \frac{1}{\gamma} + G \right) \Bf B_{\perp} +
    \frac{1 + G}{\gamma} \, \Bf B_\parallel -
...
%---------------------------------------------------------------------------------
\subsection{Spin Hard Edge Multipole Fringe Tracking}
\label{s:spin.hard.fringe}

The effect on the spin when tracking through the fringe field of a multipole field tends
to be weak. As such, a hard edge, first order, treatment should be sufficient. In this
case, it is the longitudinal integrated fringe field that is important. The longitudinal
integrated field at constant $(x,y)$ for the $n$\Th order multipole is simply obtained by
requireing that the curl is zero. For electric fields this is:
\Begineq
...
Once the integrated field is computed, the spin is tracked using the T-BMT equation
(\Eq{tbmt}).
...
The vector potential for the \vn{hyper-y} form group with \vn{plane} set to
\vn{x} is (cf.~\Eq{cm1})
\begin{alignat}{4}
...
\end{alignat}
The vector potential for the \vn{hyper-xy} form group with \vn{plane} set to
\vn{x} is (cf.~\Eq{cm2})
\begin{alignat}{4}
...
\end{alignat}
The vector potential for the \vn{hyper-x} form group with \vn{plane} set to
\vn{x} is (cf.~\Eq{cm3})
\begin{alignat}{4}
