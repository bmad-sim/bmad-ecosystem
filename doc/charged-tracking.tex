...
\end{align}
and $g_\tot$ is the bending strength of the actual field. Both \Eq{xctlg1} and \Eq{xctlg2} are
needed since \Eq{xctlg1} is singular when $\alpha = 0$ and $\theta + \phi_1 = \pi$ (which happens
when the particle is bent by 180\Deg), and \Eq{xctlg2} is singular when $g_p$ is zero. A simple way
to implement the calculation for $x_2$ is to use \Eq{xctlg1} when $|\theta + \phi_1| < \pi/2$ and
otherwise use \Eq{xctlg2}.
...
integrating through the actual field [Cf.~Runge Kutta tracking (\sref{s:tkm})]}. 
...
With \vn{cavity_type} set to \vn{standing_wave}, and with \vn{longitudinal_mode} set to 0 (the
...
With \vn{cavity_type} set to \vn{standing_wave} and with \vn{longitudinal_mode} set to 1, the field is modeled as
\begin{align}
...
%---------------------------------------------------------------------------------
\subsection{LCavity Fringe Fields}
...
Bmad includes precalculated first-order spin transfer quaternions for simple lattice elements. This method is much faster than PTC integration, and its runtime does not increase proportionally to element length. Currently, the supported lattice elements are bends (including bends with $k_1 \neq 0$), quadrupoles, and solenoids.

Elements with fringe field contributions are split into three quaternions representing the entrance, body, and exit of the element. Before propagation, the exit fringe quaternion is always equivalent to the entrance fringe quaternion, with all field strengths multiplied by -1, and $e_1$ replaced with $-e_2$. The exit quaternion is then propagated to the end of the element via Bmad mapping tools. Appropriate quaternions are concatenated according to the values of spin\_fringe\_on and fringe\_at.


The precalculated quaternions, with empty components omitted, are as follows:

\subsection{Constants}
To improve readability, the following definitions will be used. Many of these definitions and symbol choices are taken from Weisbaecker (how to cite?).

\begin{table}[h]
    \centering
    \begin{tabular}{l l l l}
        \bf{d} = $gl$ & \bf{e} = $agl\gamma$ & \bf{s} = $a k_s l$ & \bf{t} = $(1+a) k_s l$\\
        \bf{cd} = $\cos(\textbf{d})$ & & \bf{cs} = $\cos(\textbf{s})$ &  \bf{ct} = $\cos(\textbf{t})$ \\
        \bf{cdh} = $\cos(\frac{\textbf{d}}{2})$ & \bf{ceh} = $\cos(\frac{\textbf{e}}{2})$& & \bf{cth} = $\cos(\frac{\textbf{t}}{2})$ \\
        \bf{sd} = $\sin(\textbf{d})$ & & \bf{ss} = $\sin(\textbf{s})$ & \\
        & \bf{seh} = $\sin(\frac{\textbf{e}}{2})$& &\bf{sth} = $\sin(\frac{\textbf{t}}{2})$ \\
    \end{tabular}
\end{table}
    \centering
     \begin{tabular}{l l l}
        $\chi = 1+a\gamma$ & $\zeta = \gamma - 1$ & $\psi = \gamma^2 - 1$
    \end{tabular}
\begin{table}[h]
\end{table}
...
\subsubsection{Element Body, $k_1 = 0$}
\begin{table}[h]
    \centering
    \begin{tabular}{|l|l|l|l|}
    \hline
    &
    \multicolumn{1}{|c|}{$q_0$} & \multicolumn{1}{|c|}{$q_y$} & \multicolumn{1}{|c|}{$q_z$} \\

    $1$ & \bf{ceh} & -\bf{seh} & \\
    
    $x$ & $-\frac{1}{2} g \chi \textbf{sd seh}$ & $-\frac{1}{2} g \chi \: \textbf{sd ceh}$& \\
    
    $p_x$ & $\frac{1}{2} \chi  \: (\textbf{cd} - 1)  \textbf{seh}$ & $\frac{1}{2} \chi \: (\textbf{cd} - 1) \textbf{ceh}$ & \\
    
    $p_y$ & & & $\frac{1}{\gamma} \zeta \: \textbf{seh}$ \\
    
     $p_z$ & $\frac{1}{2 \gamma} \left(\gamma  \: \chi \: \textbf{sd} -a \: \psi \: \textbf{d} \right) \textbf{seh}$ & & \\
    \hline
    \end{tabular}
\end{table}

\subsubsection{Element Body, $k_1 \neq 0$}

To increase readability, the following constants are used:
...
\begin{table}[h]
    ...
    \hline
    &
    \multicolumn{1}{|c|}{$q_0$} & \multicolumn{1}{|c|}{$q_x$} \\
    
    1 & \bf{ceh} & \\
    
    $x$ & $- \frac{1}{2 \omega_x} k_x \chi s_x \textbf{seh}$ & \\
    
    $p_x$ & $\frac{1}{2\omega_x^2}k_x \chi \tau_x (1-c_x) \textbf{seh}$& \\
    
    $y$ & &$-\frac{1}{\alpha} \left(\beta (1+c_y) \textbf{seh}+\tau_y \sigma s_y \textbf{ceh}\right)$\\
    
    $p_y$ & &$-\frac{1}{\omega_y \alpha}\left(\xi (-1 + c_y) \textbf{ceh} + \beta s_y \textbf{seh} \right)$ \\
    
    $p_z$ & $-\frac{1}{2}g\left(\frac{a l \psi}{\gamma} - \frac{\chi s_x}{\omega_x}\right) \textbf{seh}$ &\\
    \hline
    \end{tabular}
\end{table}
...
\begin{table}[h]
    ...
    \hline
    &
    \multicolumn{1}{|c|}{$q_y$} & \multicolumn{1}{|c|}{$q_z$}\\
    
    1 & -\bf{seh} &\\
    
    $x$ & $-\frac{1}{2 \omega_x} k_x \chi s_x \textbf{ceh}$ & \\
    
    $p_x$ & $\frac{1}{2 \omega_x^2} k_x \chi \tau_x \left(1-c_x\right) \textbf{ceh}$& \\
    
    $y$ & &$-\frac{1}{\alpha} (\beta (-1+c_y) \textbf{ceh} - \tau_y \sigma s_y \textbf{seh})$\\
    
    $p_y$ & &$\frac{1}{\omega_y \alpha} \left(\xi (1+c_y) \textbf{seh} - \beta s_y \textbf{ceh}\right)$ \\
    
    $p_z$ & $-\frac{1}{2}g\left(\frac{a l \psi}{\gamma} - \frac{\chi s_x}{\omega_x}\right) \textbf{ceh}$& \\
    \hline
    \end{tabular}
\end{table}


\subsubsection{Entrance Fringe}
...
\begin{table}[h]
    ...
    \hline
    &
    \multicolumn{1}{|c|}{$q_0$} & \multicolumn{1}{|c|}{$q_x$}& \multicolumn{1}{|c|}{$q_y$}& \multicolumn{1}{|c|}{$q_z$} \\
...
\begin{table}[h]
    ...
    \hline
    &
    \multicolumn{1}{|c|}{$q_0$} & \multicolumn{1}{|c|}{$q_x$} & \multicolumn{1}{|c|}{$q_y$}\\
...
\begin{table}[h]
    ...
    $x$ & &$\frac{1}{4} k_s \zeta ((1 - \textbf{cs}) \textbf{cth} - \textbf{ss sth})$&$\frac{1}{4} k_s \zeta ((-1 + \textbf{cs}) \textbf{sth} - \textbf{ss cth})$&\\
    
    $p_x$ & &$\frac{1}{2} \zeta ((1-\textbf{cs}) \textbf{sth} + \textbf{ss cth})$&$\frac{1}{2} \zeta ((1 - \textbf{cs}) \textbf{cth} - \textbf{ss sth})$&  \\
    
    $y$ & &$\frac{1}{4} k_s \zeta ((1 - \textbf{cs}) \textbf{sth} + \textbf{ss cth})$&$\frac{1}{4} k_s \zeta ((1 - \textbf{cs}) \textbf{cth} - \textbf{ss sth})$&\\
    
    $p_y$ & &$\frac{1}{2} \zeta ((-1 + \textbf{cs}) \textbf{cth} + \textbf{ss sth})$&$\frac{1}{2} \zeta ((1-\textbf{cs}) \textbf{sth} + \textbf{ss cth})$& \\
    
    $p_z$ & $\frac{1}{2} \textbf{t sth}$& & & $\frac{1}{2} \textbf{t cth}$ \\
    \hline
    \end{tabular}
\end{table}
