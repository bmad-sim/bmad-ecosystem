...
lattice is \vn{parameter[absolute_time_tracking]} (\sref{s:param}).
...
With \vn{absolute time tracking}, $t_\text{eff}$ is defined by
\begin{equation}
...
will always be zero at the entrance to the cavity.  With \vn{absolute time tracking}, the particle,
...
time tracking is more correct since RF cavities may have frequencies that are not comensurate with
...
that is exactly conmensurate with the RF frequency. And it is potentially confusing to see non-zero
closed orbits when one is not expecting it due to a mismatch between the ring circuference and the
...
\begin{figure}[tb]
  ...
  {
An incoming particle strickes the bottom of the converter. At some point within the interior, a new
...
through and the $(\xw,\yw)$ axes are randomly rotated with respect to the $(x,y)$ body coordinate axes.
By construction, the position of the outgoing particle will be along the $\xw$-axis.
  }
  \label{f:converter}.
\end{figure}
...
orientation of the outgoing particle. The outgoing particle distribution is precomputed by a program
\vn{converter_element_modeling} and the distribution parameters are included in the converter element
description in the \bmad lattice file (\sref{s:converter}). 
...
outgoing particles is correct. This weight will be the same for all outgoing particles. The weight will
depend upon whether the momentum or angular range of the outgoing particles is restricted using the
following element parameters (\sref{s:converter}):
\begin{example}
  pc_out_min
  pc_out_max
  angle_out_max
...
particle emerges, $(x,y)$ are the axes for the element body coordinate system (\sref{s:coords.3}).
To generate the position and orientation of the outgoing particle, another coordinate system is used
with axes labeled by $(\xw,\yw)$. Each outgoing particle will be assigned its own $(\xw,\yw)$ axes. The
origin $\wt\calO$ of this coordinate system is constructed by placing $\wt\calO$ at the point where the
incoming particle under consideration would strike the top surface if the incoming particle would
pass straight through the converter. The angular orientation of the $(\xw,\yw)$ axes with respect to
the $(x,y)$ axes is chosen by setting the orientation angle using a random number with a uniform
probability in the interval $[0, \pi]$. By construction, the outgoing particle, at the surface of
the converter will be generated at a point along the $\xw$-axis.
...
thicknesses. The particle distribution is also calculated within a certain incoming particle
momentum range. It is also an error if an incoming particle has a momentum outside of this range.

The first step is to choose the value of the outgoint particle's momentum $p_\txt{out}$. For each
thickness $t_i$, the precomputed particle distribution parameters includes a two-dimensional table
of $P(p_\txt{out}, r)$ --- the probability density of creating an outgoing particle versus
$p_\txt{out}$ and $r$ where $r$ is the distance from the origin $\wt\calO$ to where the outgoing
particle leaves the surface. $P(p_\txt{out},r)$ is normalized so that the integrated problability is
equal to the average number of outgoing particles created for each incoming particle
$N_\txt{out}/N_{in}$
\begin{equation}
  \frac{N_\txt{out}}{N_{in}} = \int \int dp_\txt{out} \, dr \, P(p_\txt{out}, r)
\end{equation}
The integrals are done using linear interpolation between grid points.
From a $P(p_\txt{out},r)$ probability table, a ``normalized'' probability
$P_\txt{norm}(p_\txt{out},r)$ table is computed where $P_\txt{norm}$ is the
probability of generating a particle at given $p_\txt{out}$ and $r$ with an angular range restricted
by \vn{angle_out_max}. If \vn{angle_out_max} is not set, $P_\txt{norm}$ will be equal to $P$.  This
calculation is part of a ``setup'' computation which, to save time, is only done if an one of the
three element parameters, \vn{pc_out_min}, \vn{pc_out_max}, or \vn{angle_out_max},
changes. Additionally, the setup includes creating a table of $I(p_\txt{out})$ which is
the integrated probability for generating a particle with momentum less than $p_\txt{out}$
\begin{equation}
  I(p_\txt{out}) = \frac{\dstyle \int_{p_\txt{min}}^{p_\txt{out}} dp'_\txt{out} 
  \int dr \, P_\txt{norm}(p'_\txt{out}, r)}
  {\dstyle \int_{p_\txt{min}}^{p_\txt{max}} dp'_\txt{out} \int dr \, P_{norm}(p'_\txt{out}, r)}
\end{equation}
...
is normalized such that $I(p_\txt{max}) = 1$. A value for $p_\txt{out}$ is generated using the
$I(p_\txt{out})$ tables for the two $t_i$ thicknesses that straddle the actual thickness. The value
of $p_\txt{out}$ assigned to the outgoing particle is obtained via linear interpolation between the
two computed values at the two thicknesses. Note that for both thicknesses the same random number
needs to be used in the computation.

The next step is to choose a value for $r$. This is simply done by using a weighted average of the 
$P(p_\txt{out}, r)$ columns at the 

Once $p_\txt{out}$ and $r$ have been choisen, The next steps are to choose values for the outgoing
angular orientation. The angular orientation is characterized by the distribution parameters using
the derivatives $\xw' = d\xw/ds$ and $\yw' = d\yw/ds$ in the form
\begin{equation}
  P_d\left( \xw', \yw' \right) =
  A_d \, \frac{1 + \beta \, \xw'}{1 + \alpha_x \, \left( \xw' - c_x \right)^2 +
  \alpha_y \, \left( \yw' \right)^2}
  \label{pxsxs}
\end{equation}
where the parameters $A_d$, $\beta$, $c_x$, $\alpha_x$, and $\alpha_y$ all depend upon $p_\txt{out}$ and
$r$. Notice that by the symmetry of the problem with the outgoing particle generated on the
$\xw$-axis, the distribution is symmetric about $\yw' = 0$. The precomputed distribution
characterizes each of these parameters by a set of one or more fit functions. 

For $\beta(p_\txt{out},r)$, there is a set of one-dimensional fit functions $\beta_i(r)$ at momentum
$p_i$, $i = 1, \ldots, N_\beta$ with
\begin{equation}
  \beta_i(r) = \sum_{n=0}^4 c_{n,i} \, r^n
\end{equation}
To evaluate $\beta(p_\txt{out},r)$ at momenta lower than $p_{N_\beta}$, the $\beta_i$ are used with
...
\begin{equation}
  ...
  \left( \sum_{n=0}^4 w_{n} \, p_\txt{out}^n \right)
\end{equation}

For $c_x(p_\txt{out},r)$ the fit function is
\begin{equation}
  c_x(p_\txt{out},r) = \left( \sum_{n=0}^3 a_{n} \, r^n \right) \, 
...
$r$, The calculation of $\xw'$ starts with integrating $P_d$ in \Eq{pxsxs} over $\yw'$
\begin{align} 
  I_{xd} (\xw') &\equiv \int_{-\yw'_\txt{lim}}^{\yw'_\txt{lim}} d\yw' \, P_d(\xw', \yw')
  \label{ixdyp} \\
  &= 2 \, A_d \, \frac{1 + \beta \, \xw'}{\sqrt{ \alpha_y \, (1 + \alpha_x \, (\xw' - c_x)^2) }} \,
  \tan^{-1} \left( \sqrt{ \frac{\alpha_y}{1 + \alpha_x \, (\xw' - c_x)^2} } \, \yw'_\txt{lim} \right)
  \nonumber
\end{align}
where $\yw'_\txt{lim}$ is either the lesser of $\tan^{-1}(\text{angle_out_max})$ or the maximum
value of $\yw'$ that the precomputed distribution parameters $\beta$, $c_x$, $\alpha_x$, and
$\alpha_y$ were fit out to (typically something greater than 80$^\txt{o}$ from the perpendicular).
A spline fit is used to integrate $I_{xd}$ and this is used to choose a value for $\xw'$. Once $xw'$
is known, The integral of $P_d(\xw', \yw')$ over $\yw'$ is used to choose a value for $\yw'$.
...
introduce inaccuraces. Typically, however, the incoming particles will be fairly close to being
