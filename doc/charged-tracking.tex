...
outlined in \sref{s:symp.track} is used for tracking. In this case,
the gauge where $a_x$ is zero is used. With this, the other components
of the vector potential are:
The first form (cf.~\Eq{f1})
\begin{alignat}{4}
  a_y &= -&C &\dfrac{k_z}{k_x \, k_y} & \sin(\kxx) \sinh(\kyy) \sin(\kzzz) \label{af1} \CRNO
  a_s &= -&C &\dfrac{1}{k_x}          & \sin(\kxx) \cosh(\kyy) \cos(\kzzz) 
\end{alignat}
The second form (cf.~\Eq{f2})
\begin{alignat}{4}
  a_y &= -&C &\dfrac{k_z}{k_x \, k_y} & \sinh(\kxx) \sinh(\kyy) \sin(\kzzz) \label{af2} \CRNO
  a_s &= -&C &\dfrac{1}{k_x}          & \sinh(\kxx) \cosh(\kyy) \cos(\kzzz) 
\end{alignat}
The third form (cf.~\Eq{f3})
\begin{alignat}{4}
  a_y &= -&C &\dfrac{k_z}{k_x \, k_y} & \sinh(\kxx) \sin(\kyy) \sin(\kzzz) \label{af3} \CRNO
  a_s &= -&C &\dfrac{1}{k_x}          & \sinh(\kxx) \cos(\kyy) \cos(\kzzz) 
\end{alignat}
