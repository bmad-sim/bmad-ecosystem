...
accept strings without quotes. Even thouse that do will generally not
...
\begin{example}
  &element_shapes_floor_plan
    ...
    ele_shape(5) = "dat::beta.x"            "x"       "orange"  10     name
...
Data can also be specified to be drawn. In the above example, an ``x'' will be drawn
at each location where \vn{beta.x} data is evaluated. For the \vn{orbit.x} data, an ``x''
will de drawn but only for datums whose \vn{useit_opt} parameter is True.
