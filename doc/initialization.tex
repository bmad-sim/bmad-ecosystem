...
\begin{example}
  tao \{-init <tao_input_file>\} \{-beam <beam_input_file>\} \{-lat <lattice_file>\}
\end{example}
...
lattice becomes long. The default is to not read in a beam data file.

The \vn{<lattice_file>} is used to override the lattice file specified
in the root initialization file. To specify an XSIF lattice format
file use the prefix \vn{xsif::} in the \vn{<lattice_file>} name. To
specify a digested file use the prefix \vn{digested::}. Example:
\begin{example}
...
\vn{global%n_opti_loops}, will automatically stop.
...
For \vn{Single Mode} (\sref{s:single} the bindings of variables to
...
\vn{plot_page%text_height} sets the baseline height of the text that
is drawn.  \vn{plot_page%main_title_text_scale} scales the height of
the main title text relative to this baseline. Similarly
\vn{plot_page%graph_title_text_scale} scales the height of the titles
of any graph. The axis numbers and axis label are scalled by
\vn{plot_page%axis_number_text_scale} and
\vn{plot_page%axis_label_text_scale} respectively.
\vn{plot_page%key_table_text_scale} scales the text in a \vn{key
table} and \vn{plot_page%legend_text_scale} scales the text in a
\vn{lat layout} or a \vn{floor plan}.
...
the element name is to be printed underneath the corresponding rectangle.

%-----------------------------------------------------------------
\subsection{Key Table}\index{Initialization!Key Table}
