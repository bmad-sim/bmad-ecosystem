...
the setting of the plot_page%border parameter in the tao_plot_page namelist.
Plots are placed in ``\vn{regions}'' whose location is determined
by the setting of the \vn{region(i)} parameters in the same namelist. 
  \label{f:plot.page}
\end{figure}
...
\begin{example}
  &tao_plot_page
    ...
    plot_page%border                   = <\(x1_b\)>, <\(x2_b\)>, <\(y1_b\)>, <\(y2_b\)>, "<units>"
    ...
    region(i) = "<region_name>" <\(x1_r\)>, <\(x2_r\)>, <\(y1_r\)>, <\(y2_r\)>  ! % plot area
...
window. As shown in Figure~\ref{f:plot.page} $x1_b$, $x2_b$ are
the right and left border widths and $y1_b$ and $y2_b$ are the
...
"CC" and \vn{units} = "\vn{%PAGE}". See the quickplot documentation for 
...
may not contain a dot ``.''.  The last four elements of the \vn{retion(i)} array, $x1_r$, $x2_r$,
$y1_r$ and $y2_r$ define the location of the region as illustrated in Figure~\ref{f:plot.page}.  $x1_r$ and $x2_r$
are normalized to the width of the plot area and $y1_r$ and $y2_r$ are normalized to the height of the plot
area. That is, these four number should be in the range $[0, 1]$.
Regions may overlap any one can define as many regions as one likes.
