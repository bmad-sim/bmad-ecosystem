...
\end{example}
Where \vn{<name_string>} is used in the generation of the data name
and \vn{restriction} is used to restrict counting of lord or slave
elements. The hashes in the \vn{data(0)%name} string are replaced by
the data number in the series starting with \vn{ix_min_data} (as an
integer whose length is equal to the number of
hashes). The possible choices for \vn{restriction} are:
\begin{example}
  "no_lords"        ! No lord (control type) elements included.
  "no_slaves"       ! No superposition or multipass slave elements. 
  " "               ! No restrictions (Default).
\end{example}
\vn{<search_string>} can contain the wildcards \vn{*} and
\vn{%}. For example:
\index{SEARCH}
\begin{example}
  &tao_d1_data
