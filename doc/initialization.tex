...
\index{lattice_calc_on}\index{command_file_print_on}\index{twiss_calc_on}
...
type tao_global_struct
  ...
  logical twiss_calc_on = T             ! Turn on/off Twiss calculations.
...
synchrotron radiation fluctuations (but not damping) is always turned off.
  ...
  \item{\vn{global%twiss_calc_on}} \Newline
\vn{global%twiss_calc_on} controls whether the calculation of the Twiss parameters is done when
there are changes in the lattice. The default is \vn{True}. Setting this parameter to \vn{False} is
useful in speeding up calculations if only recalculation of the orbit is needed. Note that if
\vn{global%lattice_calc_on} is set to \vn{False} then no Twiss (or other lattice) calculations are
done irregardless of the setting of \vn{global%twiss_calc_on}.
...
through part of the root lattice branch.  \vn{beam_track_start} gives the starting element name
...
element with name or index \vn{beam_track_end}. The default, if \vn{beam_track_start} and
\vn{beam_track_end} are not present, is to track through the entire root lattice branch.
\vn{beam_track_start} and \vn{beam_track_end} is ignored for lattice branches other than the root
branch (branch 0). Note: Deprecated names for \vn{beam_track_start} and \vn{beam_track_end} are
\vn{track_start} and \vn{track_end} respectively.
