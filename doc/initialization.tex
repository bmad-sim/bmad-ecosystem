...
\begin{example}
  \&tao_var
    ...
    var(i)%attribute   = "<attrib_name>"     ! Element to be controlled.
    var(i)%universe    = "<integer>"         ! Element to be controlled.
...
\end{example}
With both \vn{"gang"} and \vn{"clone"} individual \vn{var(i)%universe} may 
not be set in the namelist. \vn{"gang"} means that each variable will control
the given attribute in each universe simultaneously. \vn{"clone"} means that
an array of variables will be duplicated, one for each universe. To be able
to specify a given variable the \vn{v1_var%name} must have the character
\vn{"\#"} in it. For example, if \vn{v1_var%name} is \vn{quad\#} then the
variable block name for the second universe will be \vn{quad1}. The default 
if both \vn{default_universe} and all \vn{var(i)%universe} are not given is for
\vn{default_universe} to be \vn{"gang"}
...
\begin{example}
  \&tao_d1_data
    ...
    d1_data%name       = "<d1_name>"    ! d1_data Sub-class name.
...
defined. \vn{d2_data%name} gives the class name of the
structure. Certain \vn{d2_data} names are recognized by \tao. These are:
\begin{example}
  "orbit"       ! Orbit data
...
is set up in each universe. \vn{d1_data%name} gives the
name for a \vn{d1_data} structure within the \vn{d2_data}
...
\begin{example}
  data(0)%name(0) = "SAME: <d1_data_name>"
  data(0)%ele_name(0) = "SAME: <d1_data_name>"
\end{example}
...
the plot.  \vn{plot%type} is the type of plot. \tao knows about the
following types:
\begin{example}
  data       ! Data plots.
...
For \vn{plot%type = "data"} the \vn{plot%class} defines what class of
data is being plotting.  \tao knows about the following classes:
\begin{example}
...
\begin{example}
  \&tao_template_graph
  	graph%name            = "<name>"
  	graph_index           = <number>
    graph%this_box        = <ix>, <iy>
    graph%title           = "<label>''
    graph%margin          =  <ix1>, <ix2>, <iy1>, <iy2>, "<Units>"
    graph%y               = <qp_axis_struct>
    graph%y2              = <qp_axis_struct>
    graph%n_curve         = <number_of_curves>
    curve(1)%use_y2       = <logical>
    curve(1)%data_class   = "<data_class>"
    curve(1)%units_factor = <factor>
    curve(1)%line         = <qp_line_struct>
    curve(1)%symbol       = <qp_symbol_struct>
  /
\end{example}
For example:
\begin{example}
...
along a straight line with figures for the various elements elements.
The \vn{tao_template_plot} needed to define a lattice layout looks like:
\begin{example}
  \&tao_template_plot
    plot%name        = "<plot_name>"
    plot%type        = "lat_layout"
  /
\end{example}
...
\vn{key_bindings} namelist. The syntax is:
\begin{example}
  \&key_bindings
