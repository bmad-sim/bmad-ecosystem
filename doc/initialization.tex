...
Overrides the setting of \vn{design_lattice(i)%slice_lattice}. Note: A \vn{slice_lattice} command
may also be put directly in the lattice file. See the \bmad manual for more documentation.
%
\item[\vn{-start_branch_at <element>}]
...
To calculate the dynamic aperture for the $i$\Th universe, the \vn{design(i)%dynamic_aperture_calc}
...
Figure~\ref{f:plot.page} x1$_{\dstyle{b}}$, x2$_{\dstyle{b}}$ (corresponding to \vn{%border%x1} and
\vn{%border%x2}) are the right and left border widths and y1$_{\dstyle{b}}$ and y2$_{\dstyle{b}}$
(corresponding to \vn{%border%y1} and \vn{%border%y2}) are the bottom and top border widths
respectively.  The rectangle within this border is called the plot area.

\vn{plot_page%title} set the page title and \vn{page%subtitle} set the subtitle.
...
documentation (\sref{s:quick.plot}) on the \vn{qp_axis_struct}.  If \vn{min} and \vn{max} are absent,
...
\begin{example}
  &tao_template_graph
    ...
    curve(i)%data_type_z         = "<string>"     ! Used with phase space plots
    ...
    curve(i)%orbit   = <tao_graph_orbit_struct>   ! For E & B field plots
    curve(i)%hist    = <tao_histogram_struct>     ! For histograms
    curve(i)%z_color = <tao_curve_z_color_struct> ! For phase space plotting
  /
\end{example}
For example:
\begin{example}
...
\begin{description}
%
\item[\vn{graph_index}] \Newline
The graph index. The first \vn{tao_template_graph} namelist after the associated
\vn{tao_template_plot} must have the \vn{graph_index} component set to \vn{1}. The next must have
\vn{graph_index} set to \vn{2}, etc. \tao uses the \vn{graph_index} component to check for errors.
%
\item[\vn{graph\%allow_wrap_around}] \Newline
The \vn{graph%allow_wrap_around} parameter sets if, for a lattice with closed geometry, the curves
contained in the graph are ``wrapped'' around the end of the lattice if \vn{graph%x%min} is
negative. The default is \vn{True}. This only applies if \vn{plot%x_axis_type} is set to \vn{"s"}.
%
\item[\vn{graph\%box}] \Newline
The \vn{graph%box} parameter sets the layout of the box which the \vn{graph} is placed in. For a
definition of what a box is see the QuickPlot documentation (\sref{s:quick.plot}). In the above
example the graph divides the region into two vertically stacked boxes and places itself into the
bottom one.
%
\item[\vn{graph\%clip}] \Newline
Clip the graph curves at the graph top and bottom boundaries? Default is \vn{True}.
%
\item[\vn{graph\%component}] \Newline
The \vn{%component} parameter determines in part where data for drawing the graph's curves comes
from. See \sref{s:plot.data} for more details. The \vn{graph%component} is the default used for 
\vn{curve(k)%component} if the component parameter of curve $k$ is not set.
%
\item[\vn{graph\%curve_legend_origin}]\Newline
The curve legend displayes which curves are associated with which of the plotted lines and symbols. Two
...
\begin{example}
  ...
  graph%curve_legend_origin%x = "POINTS/GRAPH/LT"
\end{example}
The \vn{%curve_legend_origin} is of type \vn{qp_point_struct}. See \sref{s:quick.plot} for details on this
structure. Also see \vn{graph%draw_curve_legend}
%
\item[\vn{graph\%draw_axes}] \Newline
Draw the graph axes? Default is \vn{True}. 
%
\item[\vn{graph\%draw_curve_legend}] \Newline
Draw the curve legend? Default is \vn{True}. The curve legend displayes which curves are associated
with which of the plotted lines and symbols. Two examples are given \fig{f:plot.typ}. Also see
\vn{graph%curve_legend_origin}.
%
\item[\vn{graph\%draw_grid}] \Newline
Draw the graph grid? Default is \vn{True}.
%
\item[\vn{graph\%draw_only_good_user_data_or_vars}] \Newline
%
\item[\vn{graph\%draw_title}] \Newline
%
\item[\vn{graph\%floor_plan}] \Newline
%
\item[\vn{graph\%ix_universe}] \Newline
%
\item[\vn{graph\%margin}] \Newline
\vn{graph%margin} sets the margin between the \vn{graph} and the \vn{box}
it is drawn in.
%
\item[\vn{graph\%n_curve}] \Newline
%
\item[\vn{graph\%name}] \Newline
...
command line.
%
\item[\vn{graph\%scale_margin}] \Newline
...
%
\item[\vn{graph\%symbol_size_scale}] \Newline
%
\item[\vn{graph\%text_legend_origin}]\Newline
%
\item[\vn{graph\%title}] \Newline
...
%
\item[\vn{graph\%x}] \Newline
%
\item[\vn{graph\%x_axis_scale_factor}] \Newline
%
\item[\vn{graph\%x2}] \Newline
%
\item[\vn{graph\%y}] \Newline
%
\item[\vn{graph\%y2}] \Newline
%
\item[\vn{graph\%y2_mirrors_y}] \Newline
%
\end{description}
...
\begin{description}
%
\item[\vn{curve\%component}] \Newline
%
\item[\vn{curve\%data_source}] \Newline
%
\item[\vn{curve\%data_index}] \Newline
%
\item[\vn{curve\%data_type}] \Newline
%
\item[\vn{curve\%data_type_x}] \Newline
%
\item[\vn{curve\%data_type_z}] \Newline
%
\item[\vn{curve\%ele_ref_name}] \Newline
%
\item[\vn{curve\%hist}] \Newline
%
\item[\vn{curve\%ix_branch}] \Newline
%
\item[\vn{curve\%ix_bunch}] \Newline
%
\item[\vn{curve\%legend_text}] \Newline
%
\item[\vn{curve\%line}] \Newline
%
\item[\vn{curve\%name}] \Newline
%
\item[\vn{curve\%orbit}] \Newline
%
\item[\vn{curve\%scale}] \Newline
%
\item[\vn{curve\%smooth_line_calc}] \Newline
%
\item[\vn{curve\%symbol}] \Newline
%
\item[\vn{curve\%units}] \Newline
%
\item[\vn{curve\%y_axis_scale_factor}] \Newline
%
\item[\vn{curve\%z_color}] \Newline
%
\end{description}

See the \vn{Quick_Plot} documentation (\sref{s:quick.plot}) description of the \vn{qp_symbol_struct} and the
\vn{qp_line_struct}.
...
&tao_template_graph
  ...
  curve(1)%scale = 10              ! Draw beam ellipse at 10 sigma
...
\begin{example}
  "dynamic_aperture"          ! Curve points include the reference orbit offset.
  "dynamic_aperture_centered" ! Curve points are plotted relative to the reference orbit.
...
\vn{%data_type} can be set to \vn{"beam_ellipse_full"}. The beam ellipse is set by the curve's
\vn{%scale} value. In this example, the $10 \, \sigma$ ellipse is drawn. 
