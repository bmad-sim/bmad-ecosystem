...
Note: Some \tao string variables are case sensitive.
...
\index{print_command}\index{default_init_file}\index{no_symbols_without_layout}
...
\begin{example}
  &tao_plot_page
    ...
    plot_page%shape_height_max       = <num> ! maximum half height for drawing elements. Def = 40
    plot_page%n_curve_pts            = <num> ! Num points used to construct a smooth curve. Def = 401
    plot_page%box_plots              = <T/F> ! For debugging layout problems. Def = F.
    plot_page%no_symbols_without_layout = <T/F> ! Supresses drawing symbols? Def = T
...
\begin{example}
  &tao_template_graph
    ...
    curve(i)%legend_text         = "<string>" ! Text for curve legend. Default is the data_type.
...
If the \vn{plot_page%no_symbols_without_layout} switch is set to
\vn{T} (which is the default), the drawing of symbols for a given
curve is suppressed, irregardless of a \vn{curve%draw_symbols}
setting, if three conditions are met:
\begin{example}
  The graph%type associated with the curve is set to "data".
  The plot%x_axis_type associated with the curve is set to "s".
  There is no visible lat_layout being drawn on the plot page.
\end{example}
