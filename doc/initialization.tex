...
\begin{example}
  ...
  ""            ! Section is ignored in any constraint calculation.
\end{example}
...
\begin{example}
  "beam_chamber_wall"  ! Draw the beam chamber wall (\sref{s:beam.wall.draw}).
...
\end{example}
Note: \vn{"floor_plan"}, \vn{"key_table"} and \vn{"lat_layout"} graphs
do not have associated \vn{curve}s. The \vn{"beam_chamber_wall"} type
graph can have a single associated \vn{curve}.
...
\begin{example}
  ...
  /
\end{example}
To prevent the drawing of the axes set 
...
\vn{lat_layout} drawings, \vn{<v_size>} = 100 corresponds to full
scale if the default \vn{graph%y%min} and \vn{graph%y%max} are
used. For \vn{var_height_box}, the height is proportional to the
...
can be drawn in a \vn{graph} by setting \vn{graph%type} to
...
machine centerline. A beam chamber wall graph may be combined with a
\vn{lat_layout} graph by assigning both graphs to the same plot and 
setting \vn{graph%box} to be the same for both to physically place
both of them at the same spot.

To set the line color or width for the beam chamber wall, 
set \vn{curve(1)%line%color} or \vn{curve(1)%line%width}.
