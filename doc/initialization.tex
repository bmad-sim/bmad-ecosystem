...
Table~\ref{t:con.type}. Custom data types not specified in this table must have a corresponding
...
\begin{example}
  &tao_template_graph
    ...
    graph%floor_plan_orbit       = <floor_plan_orbit_struct>    
                                                      ! Structure for drawing oribts on a floor_plan.
...
The beam orbit can be drawn upon the floor plan. 
The \vn{graph%floor_plan_orbit} structure controls how the orbit is drawn.
This structure has components:
\begin{example}
  type tao_floor_plan_orbit_struct
    scale   = <real>              ! Scale for the orbit. Default = 0 => No orbit drawn.
    color   = "<color-name>"      ! Line color. Default = "red".
    pattern = "<pattern-name>"    ! Line pattern. Default = "solid_line".
    width   = <integer>           ! Line width. Default = 1.
  end type
\end{example}
...
of 100.0, a 1~cm orbit is drawn 1~meter from the centerline. Note: If \vn{scale} is not unity, the
plotted orbit when going through a \vn{patch} element with a finite transverse offset will show a
discontinuity due to the discontinuity of the reference orbit. A setting of zero (the default) means
that the orbit is now drawn. Example:
\begin{example}
  &tao_template_graph
    ...
    graph%floor_plan_orbit%scale = 10
    graph%floor_plan_orbit%color = 'green'
    graph%floor_plan_orbit%width = 3
  /
\end{example}
