...
file is \vn{tao.init} but this default may be overridden when \tao is started using the \vn{-init}
...
\begin{example}
  &tao_design_lattice
    ...
    design_lattice(i)%use_element_range = "<ele1>", "<ele2>"             
  /
\end{example}
...
The \vn{%use_element_range} parameter specifies two elements (labeled here \vn{<ele1>} and
\vn{<ele2>}) to be used to pare down the lattice so that the only elements in the pared down lattice
are the elements in the range from \vn{<ele1>} to \vn{<ele2} inclusive plus any lord elements that
control elements in this range. The names \vn{<ele1>} and \vn{<ele2>} can be either an element names
or an element index (\sref{s:ele.list.format}). Currently, only lattices with a single branch can
be used with \vn{%use_element_range}. \vn{%use_element_range} can be used to minimize calculational
overhead when only part of the lattice needs to be used.
...
\begin{example}
  &tao_design_lattice
    ...
    design_lattice(1)%use_element_range = "Q1", "Q2"  ! Lattice will be all 
                                                ! elements in the range [Q1, Q2]
...
floor coordinates to an XSIF lattice file. Note: If a \vn{%use_element_range} parameter is used with
a secondary lattice file then the paring specified by \vn{%use_element_range} is applied before
...
type tao_global_struct
  ...
  logical plot_calc_always = F          ! Always calc plot curve points even with %plot_on = F?
...
calculations (for example, calculating the Twiss parameters) that are not needed.
  ...
  \item{\vn{global%plot_calc_always}} \Newline
...
the data to be plotted is calculated by \tao. In this case by setting \vn{global%plot_calc_always} to
...
\begin{example}
  &tao_beam_init
    ...
    beam_position0_file         = <string>     ! Initial particle positions
...
The \vn{beam_init} parameter is an instance of a \vn{beam_init_struct} structure which holds parameters (for
example, the beam emittances) from which a distribution of particles can be constructed. Documentation on this
can be found in the \bmad manual in the \vn{Beam Initialization} chapter.

The \vn{beam_position0_file} component specifies a file (which can be created with the \vn{write
beam -at <ele_name>} command) which contains a beam's particle coordinates which are to be used at
the start of the lattice.  Note: The file name can be overridden by using the \vn{-beam_position0} argument
on the command line (\sref{s:command.line}). The file can either be in binary format (binary files
can be created by the \vn{write beam} command), or written in ASCII. If the \vn{beam_position0_file} string
is set, the \vn{beam_init} component is ignored and the initial particle distribution is read in
from the file. This is documented in the \bmad manual in the \vn{Beam Initialization} chapter.
Note: When the particle coordinates are read in from the \vn{beam_position0_file}, the centroid will be
shifted by the setting of \vn{beam_init%center}.  To vary the centroid of the beam on the \tao
command line, the \vn{set beam_init%center} command (\sref{s:set}) can be used.
...
tracking.  Note: A \vn{beam_all_file} will supersede a \vn{beam_position0_file}

When there is no \vn{beam_position0_file} the Twiss parameters at the beginning of the lattice are used in
...
\sref{s:ele.list.format}, is used to specify to liast of elements for \vn{beam_saved_at}. The
\vn{BEGINNING} element (with index 0 in the lattice list) and the last element are automatically
saved.
\begin{example}
  &tao_beam_init
...
\begin{example}
  ...
  /
\end{example}
See the Quick Plot chapter of the \bmad manual for description of the \vn{qp_symbol_struct} and the
\vn{qp_line_struct}.
...
the elements and the orbit. So a with a value of 100.0, a 1~cm orbit is drawn 1~meter from the centerline.
Note: If \vn{graph%floor_plan_orbit_scale} is not unity, the
...
If \vn{graph%ix_universe} is set to -2, all universes are plotted.
...
\vn{graph%y%min} = -1 and \vn{graph%y%max} = 1 are used. For
