...
match the name given by \vn{"pattern:<pattern_name>"}. The pattern is a series of
piecewise linear curves. Each curve is specified by a number of points. In the above
example, there is one curve with five points which represents an isosceles trapezoid.
When drawn, the \vn{s} coordinate is scaled so that $s = 0$ corresponds to
the entrance end of the element and $s = 1$ corresponds to the exit end. The \vn{x}
coordinate is scaled by the \vn{size} attribute of the \vn{ele_shape}. The default color
for a given curve is taken to be the \vn{color} attribute of the \vn{ele_shape} ("red" in
the example).
