...
\begin{example}
  ...
  <n_particle>     ! Number of particles per bunch to use. 0 => Use all.
  [bunch loop: ib = 1 to n_bunch]
    ...
    [particle loop: Stop when END_BUNCH marker found]
      <x>    <px>   <y>    <py>   <z>   <pz>   <particle_charge> <ix_lost> <spin>
    [end particle loop]
    END_BUNCH   ! Marker to signal the end of the particle loop
  [end bunch loop]
\end{example}
...
data, one for each bunch. Each one of these blocks consists of three
lines followed by a set of lines, one for each particle and ending
with a line starting with a \vn{END_BUNCH} marker string. Only the
phase space coordinates need to be specified for each particle. if
\vn{<particle_charge>} is not present, or is zero, it defaults to
\vn{bunch_charge/n_particle}. \vn{<ix_lost>} is the index at which the
particle has been lost at. A value of -1 for \vn{<ix_lost>} indicates
that the particle is still alive.

The number of particles specified may be more then
\vn{<n_particle>}. In this case, particles will be discarded so that
the the beam has \vn{<n_particle>} particles. If \vn{<n_particles>} is
zero, all the particles will be used. Additionally, if
...
%-----------------------------------------------------------------
\subsection{Graphing a Data Slice}\index{Lattice Layout}
\label{s:lat.layout}
...
must be set to \vn{'data_slice'}, the \vn{curve(:)%data_source} must be
set to \vn{'data_array'} and the \vn{curve(:)%data_type} is of the form:
\begin{example}
  curve(:) = '<x_data_slice> <y_data_slice>'
\end{example}
A space is used as the separator between \vn{<x_data_slice>} and
\vn{y_data_slice}. If either \vn{<x_data_slice>} or
\vn{<y_data_slice>} has a \vn{'\#'} character then the string given by
the corresponding \vn{curve(:)%ele_ref_name} is used. Example:
\begin{example}
  &tao_template_plot
    plot%name = 'at_bpm'
    plot%x%label = 'x'
  ...
  &tao_template_graph
    ...
    graph%n_curve = 1
    curve(1)%data_source = 'data_array'
    curve(1)%data_type   = '1:57@bpm_orbit.x[\#]  1:57@bpm_orbit.y[\#]'
  ...
  /
\end{example}
In this example, \vn{bpm_orbit.y[23]} from universes 1 through 57 is
plotted as a function of \vn{bpm_orbit.x[23]}. The \vn{set curve
ele_ref_name} command (\sref{s:curve}) can be used to change the index
from the initial value of \vn{23}. If the universe number is wanted
for either the x or y-axis, then use the string \vn{'ix_uni'} for
\vn{x_data_slice} or \vn{y_data_slice}. Example:
\begin{example}
  curve(1)%data_type   = 'ix_uni  *@bpm_orbit.x[23]'
\end{example}
This would plot \vn{bpm_orbit.x[23]} as a function of universe number for all universes.
...
\end{example}
In this example the $x$-axis will correspond to the $x$ phase space
coordinate and the $y$-axis will correspond to the $p_x$ coordinate.
A space is mandatory as the separator between the plane names.
