...
%
\item[-symbol_import]
Import symbolic constants defined in any lattice files? (the default is not to).
%
...
\vn{%symbol_import} to True (default is False). Note that if the value of these symbols is
subsequently modified in \tao, this will {\em not} affect the lattice. For example,
control functions used in lattice control elements (\vn{group} and \vn{overlay} elements), will not
be affected even if a control function expression used a symbolic constant.
%
