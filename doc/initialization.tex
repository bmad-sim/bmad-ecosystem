...
\begin{example}
  &tao_template_graph
    ...
    curve(N)%data_type_z         = "<string>"     ! Used with phase space plots
    curve(N)%component           = "<string>"     ! Eg: "model - design". Overrides graph%component.
...
%
\item[curve(N)\%data_type_z] \Newline
Used with \vn{phase space} plotting (\sref{s:phase.space}).
%
...
- reference data (when \vn{%component} is set to \vn{meas-ref}) is different from just plotting
...
appearence (\sref{s:curve.line}). Default is True.
%
...
Curve vertical axis scale factor. The plotted value will be:
\begin{example}
  plot_value = scale_factor * datum_value
\end{example}
The default value is \vn{1}. For example, a \vn{%y_axis_scale_factor} of 1000 will draw an orbit
with a milimeter scale.
%
...
\vn{curve(N)%component} if the component parameter of curve $N$ is not set.
%
