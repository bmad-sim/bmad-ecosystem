...
through part of a lattice branch. \vn{beam_track_start} gives the starting element name or
...
the beginning of the branch if the beam is not being injected into the branch via a \vn{fork} element. If
the beam is being injected from another branch, the default is to start at forked to element in the 
branch. The default for \vn{beam_track_end} is the end of the lattice.
...
%-----------------------------------------------------------------
\subsection{Drawing the Beam Aperture}
\index{beam aperture drawing}
...
To draw an aperture, the \vn{data_type} parameter of a \vn{curve} is set to one of
\begin{example}
  "aperture.+x"     ! Aperture in +X direction
  "aperture.-x"     ! Aperture in -X direction
  "aperture.+y"     ! Aperture in +Y direction
  "aperture.-y"     ! Aperture in -Y direction
\end{example}
The apertures in the $+x$ and $+y$ directions will have positive values and the apertures in the
$-x$ and $-y$ directions will have negative values.
...
\begin{example}
  ...
  &tao_template_graph
    ...
    curve(1)%data_type    = "aperture.+x"
    curve(1)%draw_symbols = T
    curve(1)%draw_line    = F
    curve(2)%data_type    = "aperture.-x"
...
%%-----------------------------------------------------------------
%\subsection{Drawing a Controller Element Curves}
