...
\begin{example}
  &tao_design_lattice
    ...
    design_lattice(i)%slice_lattice = "<ele1>", "<ele2>"             
  /
\end{example}
...
The \vn{%slice_lattice} parameter specifies two elements (labeled here \vn{<ele1>} and \vn{<ele2>})
to be used to pare down the lattice so that the only elements in the pared down lattice are the
elements in the range from \vn{<ele1>} to \vn{<ele2} inclusive plus any lord elements that control
elements in this range. This is identical to putting the following command in the lattice file
\begin{example}
  slice_lattice <ele1> : <ele2>
\end{example}
See the \bmad manual for more details about the \vn{slice_lattice} command.
