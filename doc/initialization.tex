...
\begin{example}
  &tao_template_graph
    ...
    curve(i)%data_source_x = "<string>"       ! Used with plot%x_axis_type = "data".
...
y-axis, then use the \vn{ix_uni} conponent. Example:
\begin{example}
  curve(1)%data_type   = "*@orbit.x[23]|ix_uni"
  curve(1)%data_type   = "*@orbit.x[23]"
\end{example}
This would plot \vn{orbit.x[23]} as a function of universe number
for all universes. Additionally, the component \vn{ix_d1} can be used
to specify the index in the \vn{d1_data} array and the component \vn{ix_ele}
can be used to specify the lattice element index.
