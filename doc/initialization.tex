...
overrides the \vn{beam0_file} variable (\sref{s:beam.init}). The
centroid of the particle distribution can be shifted using the
\vn{change beam_start} command (\sref{s:change}).

The \vn{-lat} switch is used to override the \vn{design_lattice}
...
particles in a beam and for misaligning elements in a lattice. As
explained below, there are four parameters that govern how random
numbers are generated.  These can be set in the
\vn{tao_global_struct}. For particle beam generation, these parameters
may be overridden by setting the corresponding parameters in the
\vn{beam_init} struct (\sref{s:beam.init}). That is, separate
parameters may be setup for beam particle generation verses everything
else.
...
\index{beam_init!sig_z}\index{beam_init!n_bunch}\index{beam_init!ds_bunch}
...
\begin{example}
  &tao_beam_init
    save_beam_at(1) = "marker:m*" ! Save beam at all markers starting with "m".
    save_beam_at(2) = "*34w*"     ! And all elements that have "34w" in their name. 
  /
\end{example}
...
\begin{example}
   search_for_lat_eles = "{-no_grouping} <element class>:<element name>"  
\end{example}
...
All elements with the same name are grouped under a single
variable. That is, a single variable can control multiple element.
If the \vn{-no_grouping} flag is present, however, each
...
say, two bends have the name "bend0" then a single variable will be
...
\begin{example}
  &tao_d1_data
    ...
    datum(j)%ele_name       = "<ele_name>"       ! Lattice element names.
    datum(j)%ele_start_name = "<ele_start_name>" ! Start element names.
    ...
    datum(j)%ix_bunch       = <integer>          ! Bunch index. Default = 0 (all bunches).
  /
\end{example}
For example:
\begin{example}
...
\begin{example}
  &tao_d1_data
    ix_d1_data        = 1 
    d1_data%name      = "t"  
    !           data_      ele_ref_  ele_start_   ele_     merit_   meas_    weight good_
...
bunch that the data is extracted from can be specified via \vn{datum(:)%ix_bunch}.
...
\begin{example}
  &tao_plot_page
    ...
    plot_page%box_plots              = <T/F> ! For debugging layout problems. Def = F.
...
\vn{default_plot} set defaults for the \vn{plot} structure that is
defined in the \vn{tao_template_plot} namelist (\sref{s:template}).
Similarly, \vn{default_graph} sets defaults for the \vn{graph}
structure defined in the \vn{tao_template_graph} namelist
...
is set, a value will be choisen for \vn{major_div_nominal} by \tao. If
you are unsure which to set, it is recommended that
\vn{major_div_nominal} be used.

The \vn{plot%autoscale_gang_x} and \vn{plot%autoscale_gang_y}
components set how the \vn{x-scale} (\sref{s:x.scale}) and \vn{scale}
(\sref{s:scale}) commands behave when autoscaling entire plots. See
these individual commands for more details.
...
\index{Curve!ix_bunch}\index{Curve!ix_ele_ref}
\begin{example}
  &tao_template_graph
    graph_index            = <number>
...
(\sref{s:set}) to toggle the drawing of symbols.
...
\vn{"data_array"} and the \vn{curve(:)%data_type_x} and
...
\vn{"[2:57]@orbit.x[50]|meas-ref"}. That is, the \vn{meas - ref}
...
\begin{example}
  &element_shapes_lat_layout
    ele_shape(i) = "<class:name>" "<shape>" "<color>" "<v_size>" "<Label_type>"
  /
...
\begin{example}
  &element_shapes_floor_plan
    !               Class:Name       Shape     Color     Size   Label_Type
...
last shape matched will be used. For a floor plan, for \vn{wiggler}s that have an
