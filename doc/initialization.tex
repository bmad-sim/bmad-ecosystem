...
\vn{-beam_all_file} argument on the command line
(\sref{s:command.line}). Note: \tao will set the variable
\vn{use_saved_beam_in_tracking} to \vn{True} to prevent actual
tracking.  Note: A \vn{beam_all_file} will supersede  \vn{beam_init%position_file}
...
%-----------------------------------------------------------------
\subsection{Quick Plot Plotting}
\label{s:line.symb}

\vn{Quick PLot} is an interface library developed for \bmad for graphics plotting. \vn{Quick Plot} defines
a number of structures to parameterize such things like line and symbol properties. The ones that \tao uses
are as follows

The \vn{qp_axis_struct} structure that defines the properties of a graph axis
\begin{example}
  type qp_axis_struct
    label             = <string>
    min               = <real>
    max               = <real>     ! min is actually left or bottom axis number.
    number_offset     = <real>    ! offset from axis line in inches.
    label_offset      = <real>     ! offset from numbers in inches.
    major_tick_len    = <real>   ! in inches.
    minor_tick_len    = <real>   ! in inches.
    label_color       = <string>
    major_div         = <integer>
    major_div_nominal = <integer>Nominal value.
    minor_div         = <integer>      ! 0 = auto choose.
    minor_div_max     = <integer>  ! max number for auto choose.
    places            = <integer>
    type              = <string>  'LINEAR'      ! or 'LOG', or 'CUSTOM'
    bounds            = <string>    'GENERAL'   ! or 'ZERO_AT_END' or 'ZERO_SYMMETRIC'
    tick_side         = <integer>1 = draw to the inside, 0 = both, -1 = outside.
    number_side       = <integer>= draw to the inside, -1 = outside.
    draw_label        = <logical>
    draw_numbers      = <logical>
  end type
\end{example}
...
\begin{example}
  type qp_symbol_struct
    type              = <string>dot'
    height            = <real>! in points (same as text height)
    color             = <string>
    fill_pattern      = <string>ill'
    line_width        = <integer>
  end type
\end{example}
...
\begin{example}
  &floor_plan_drawing
    ...
    ele_shape(8) = "building_wall::*"       "-"        "black"    0    "-"
    ele_shape(3)%multi = T
    ele_shape(5:6)%line_width = 5, 6
  /
\end{example}
...
the building wall section name. Use ``\vn{*}'' to for \vn{<name>} to match to all names. For the
building wall, the only \vn{ele_shape} attribute that is relevant is the \vn{color}.
...
calculation. Example plot setup:
\begin{example}
&tao_template_plot
  plot%name = 'da'
  plot%x%min = -20
...
/
\end{example}
This produces the plot on Fig.~\ref{f:dynamic-aperture}.  Each curve represents a single
...
styles of the previous curves. Note that apertures are calculated at element 0. 
