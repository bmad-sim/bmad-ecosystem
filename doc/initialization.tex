...
\begin{example}
  ...
  &tao_template_graph
    ...
    graph%n_curve = 1
    curve(1)%data_source = "data_array"
    curve(1)%data_type_x = "[2:57]@orbit.x[\#]"
    curve(1)%data_type   = "[2:57]@orbit.y[\#]"
    curve(1)%data_index  = "[2:57]@orbit.y[\#]|ix_uni"
...
universes 2 through 57 is used for the x-axis and \vn{orbit.x[23]} is
...
of \vn{23}. \vn{curve%data_index} sets the index for the symbol points
(\sref{s:template}). In this example, the \vn{ix_uni} component will
result in the symbol index being the universe number.  Additionally,
the component \vn{ix_d1} can be used to specify the index in the
\vn{d1_data} array and the component \vn{ix_ele} can be used to
specify the lattice element index.
...
\begin{example}
  ...
  curve(1)%data_type   = "#@orbit.x|meas-ref"
