...
\begin{example}
  &element_shapes_lat_layout
    ...
    ele_shape(i) = "<ele_name>" "<shape>" "<color>" "<v_size>" "<label>" <draw> "<shape_name>"
  /
...
\begin{example}
  &element_shapes_floor_plan
    ...
    !              ele_name                 Shape     Color   Size    Label Draw? Shape_name
    ele_shape(1) = "quadrupole::q*"         "box"     "red"     30     name  T      "qq"
...
The \vn{<shape_name>} field establishes the name for an element shape that
can be used with the \vn{set shape} command. Default names are ``\vn{l<n>}''
for \vn{element_shapes_lat_layout} shapes where \vn{<n>} is the shape index number,
and ``\vn{f<n>}'' for \vn{element_shapes_floor_plan} shapes.

\index{beam chamber wall}
The \vn{draw_beam_chamber_wall} logical determines whether the beam
chamber wall is drawn and \vn{beam_chamber_wall_scale} determines how
the wall is scaled when it is drawn. Note: Beam chamber walls, which
are different from building walls (\sref{s:building.wall}), are
defined in the \bmad lattice file and are associated with individual
elements. See the \bmad manual for more details.
