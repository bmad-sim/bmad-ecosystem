...
Supresses use of a \tao initialization file. In this case the use of
...
\begin{example}
  &tao_beam_init
    ...
    beam_init%emit_jitter   = <bumber>*2       ! Emittance rms jitter (\(d\epsilon/\epsilon\)) 
...
\begin{example}
  ...
  [bunch loop: ib = 1 to n_bunch]
    ...
    [particle loop: Stop when END_BUNCH marker found]
      <x> <px> <y> <py> <z> <pz> <particle_charge> <ix_lost> <spinor1> <spinor2>
    [end particle loop]
...
(\sref{s:init.plot})
\begin{example}
  plot_page%floor_plan_rotation
\end{example}
Alternatively, the global coordinates at the start of the lattice can
be defined in the lattice file and this can rotate the floor plan. See
...
Note: The \vn{show ele} command (\sref{s:show}) can be used to view an
element's global coordinates.

%-----------------------------------------------------------------
\subsection{Lat_layout and Floor_plan Drawings}
...
is determined via two namelists: \vn{lat_layout_drawing} for
the lattice layout and \vn{floor_plan_drawing} for floor plan
...
\begin{example}
  &lat_layout_drawing
    draw_beam_chamber_wall  = <logical>
    beam_chamber_wall_scale = <num> 
    draw_orbit              = <logical>
    orbit_scale             = <num> 
    ele_shape(i) = "<ele_name>" "<shape>" "<color>" "<v_size>" "<label>" <draw> 
  /
...
\begin{example}
  &floor_plan_drawing
    !              ele_name                 Shape     Color V_Size    Label Draw?
    ele_shape(1) = "quadrupole::q*"         "box"     "red"     30     name  T    
    ele_shape(2) = "quadrupole::*"          "xbox"    "red"     30     none 
    ele_shape(3) = "sbend::*"               "box"     "blue"    15     none  F
...
\vn{<ele_name>} specification (\sref{s:ele.list.format}) of any \vn{ele_shape(:)}.
...
\end{example}
The \vn{"none"} option is useful for only drawing the beam chamber wall
without an accompanying drawn shape.
...
\vn{<v_size>} is the vertical half height of the shape with
\vn{<v_size>} = 100 corresponding to full scale. For
\vn{var_height_box}, the height is proportional to the element
strength and here \vn{<v_size>} is the multiplyer used to scale from
element strength to vertical height. For example, for a quadrupole the
height is proportional to the \vn{K1} focusing strength. Not all
lattice elements can be used with a \vn{var_height_box}.
...
The \vn{draw_beam_chamber_wall} logical determines whether the beam
chamber wall is drawn and the setting of \vn{beam_chamber_wall_scale}
determines how the wall is scaled when it is drawn. Beam chamber walls
are only drawn for an element if the element has an associated
\vn{ele_shape}. To draw the wall without drawing a shape, set the
\vn{shape} parameter for the appropriate \vn{ele_shape} to
\vn{"none"}. 

The \vn{<v_size>} 
Note: Beam chamber walls, which are different from
building walls (\sref{s:building.wall}), are defined in the \bmad
lattice file and are associated with individual elements. See the
\bmad manual for more details.

The \vn{draw_orbit} logical determines whether the particle orbit is
drawn. This is independent of what elements are drawn. The
\vn{orbit_scale} determines how the orbit is scaled.
...
%-----------------------------------------------------------------
\subsection{Building Wall Plotting}
\index{building wall plotting}
