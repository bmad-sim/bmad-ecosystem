...
%-----------------------------------------------------------------
\section{Initializing Lattice Sigma Matrix Calculation}
...
calculated from the lattice can be done faster than tracking a beam). The \vn{show beam -lattice}
(\sref{s:show.beam}) will display the lattice derived sigma matrix. For optimizations, the
\vn{sigma} data type (pg.~\pref{sigma}) with \vn{data_source} is set to \vn{lat} can be used.

The calculation of the lattice based sigma matrix starts at the beginning of a lattice branch. If
\vn{global} parameter \vn{init_lat_sigma_from_beam} (\sref{s:tao.global.struct}) is set to True, the
...
Once the initial sigma matrix $\sigma_0$ is calculated, the sigma matrix $\sigma(s)$ at a point
downstream is calculated using the standard formula
\begin{equation}
  \sigma(s) = \bfM \, \sigma_0 \, \bfM^t
\end{equation}
...
differing parameter values, the \vn{write bmad_lattice} command (\sref{s:write}) will not produce a
valid lattice.
