...
\begin{tabular}{llll} \toprule
  {\it Namelist}                     & \parbox{2in}{\it Associated tao_start Namelist component}
                                                            & {\it Parameter Type Initialized}  & {\it Section} \\ \midrule
  \vn{lat_layout_drawing}            & \vn{plot_file}$^*$          & Plotting          & \sref{s:layout.and.floor}  \\ 
  \vn{floor_plan_drawing}            & \vn{plot_file}$^*$          & Plotting          & \sref{s:layout.and.floor}  \\ 
  \vn{tao_beam_init}                 & \vn{beam_file}              & Particle beam     & \sref{s:beam.init}         \\ 
  \vn{tao_building_wall}             & \vn{building_wall_file}     & Plotting          & \sref{s:building.wall}     \\ 
...
\vn{plot_file} array set in the \vn{tao_start} namelist (\sref{s:init_global}).
