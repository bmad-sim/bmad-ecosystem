...
\begin{example}
  ...
  * What to plot and how plots should be layed out in the plotting window (\sref{s:init.plot}).
...
default name is \vn{tao.statup} but this name can be changed by setting the \vn{startup_file}
...
\begin{tabular}{llll} \toprule
  ...
  \vn{lat_layout_drawing}            & Plotting           & \sref{s:layout.and.floor}  \\ 
  \vn{floor_plan_drawing}            & Plotting           & \sref{s:layout.and.floor}  \\ 
  ...
  \vn{tao_d1_data}                   & \vn{data_file}               & Data              & \sref{s:init.data}         \\ 
  \vn{tao_d2_data}                   & \vn{data_file}               & Data              & \sref{s:init.data}         \\ 
  \vn{tao_params}                    &                              & Global Variables  & \sref{s:globals}           \\ 
  \vn{tao_plot_page}                 & \vn{plot_file}$^*$           & Plotting          & \sref{s:init.plot}         \\ 
  \vn{tao_single_particle_tracking}  & \vn{particle_tracking_file}  & Particle Tracking & \sref{s:single.track}      \\
  \vn{tao_template_graph}            & \vn{plot_file}               & Plotting          & \sref{s:init.plot}         \\ 
  \vn{tao_template_plot}             & \vn{plot_file}               & Plotting          & \sref{s:init.plot}         \\ 
  \vn{tao_var}                       & \vn{var_file}                & Variables         & \sref{s:init.var}          \\ \bottomrule
\end{tabular}}
\break
...
\etcetc: Must finish up going to each section and putting in info for in what directory tao looks for the namelist
See the lat_tayout/floor plan section for an example...

%-----------------------------------------------------------------
\section{Lattice Initialization}\index{initialization!lattice}
\label{s:init.lat} 

In the \vn{tao_start} namelist, the \vn{lattice_file} variable gives the name of the file that
contains the \vn{tao_design_lattice} namelist. The default, if \vn{lattice_file} is not present is
to look in the \tao root initialization file. The \vn{tao_deisgin_lattice} namelist defines where
the lattice input files are. The variables that are set in the \vn{tao_design_lattice} namelist are:
...
are the elements in the range from \vn{<ele1>} to \vn{<ele2} inclusive plus any lord elments that
...
the \vn{set universe} command (\sref{s:set.universe}) for ways to supress certain types of
...
any plotting (that is, \global%plot_on} = F). For example, when \tao is run as a server by a client
...
True will force \tao to always calcuate curve data points even when \global%plot_on} = F.
...
(\sref{s:init_global}). The default, if \vn{beam_file} is not set, is the root initialization file.
...
When there is no \vn{beam0_file} the Twiss parameters at the beginning of the lattice are used in
...
(\sref{s:init.global}). In general, this file will contain a number of
...
\vn{plot_file} array set in the \vn{tao_start} namelist (\sref{s:init_global}). The default, if
