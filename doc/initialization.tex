...
\begin{example}
  &tao_plot_page
    ...
    plot_page%floor_plan_size_is_absolute = <logical>  ! Floor plan shape sizes
                                                       ! scalled to absolute dimensions?
...
\begin{example}
  &lat_layout_drawing
    ele_shape(i) = "<name>" "<shape>" "<color>" "<size>" "<label>" <draw> 
  /
...
\begin{example}
  &floor_plan_drawing
    !               ele_id                   Shape      Color     Size     Label Draw?
...
\vn{<ele_id>} specification (\sref{s:ele.list.format}) of any \vn{ele_shape(:)}.
...
is combinded with the \vn{plot_page} (\sref{s:init.plot}) parameter
\begin{example}
