...
\begin{tabular}{|l|l|l|l|} \hline
  ...
  \vn{tao_design_lattice}     & \vn{lattice_file} & lattice files    & \sref{s:init_lat}      \\ \hline
  \vn{tao_params}             & 'tao.init' & Global Variables        & \sref{s:globals}       \\ \hline
  \vn{tao_connected_uni_init} & 'tao.init' & Connected Universes     & \sref{s:connected_uni} \\ \hline
  \vn{tao_beam_init}          & 'tao.init' & Particle beam           & \sref{s:beam_init}     \\ \hline
  \vn{tao_macro_init}         & 'tao.init' & Macroparticle beam      & \sref{s:macro_init}    \\ \hline
  \vn{tao_var}                & \vn{var_file}     & Variables        & \sref{s:init_var}      \\ \hline
  \vn{tao_d2_data}            & \vn{data_file}    & Data             & \sref{s:init_data}     \\ \hline
  \vn{tao_d1_data}            & \vn{data_file}    & Data             & \sref{s:init_data}     \\ \hline
  \vn{tao_plot_page}          & \vn{plot_file}    & Plotting         & \sref{s:init_plot}     \\ \hline
  \vn{tao_template_plot}      & \vn{plot_file}    & Plotting         & \sref{s:init_plot}     \\ \hline
  \vn{tao_template_graph}     & \vn{plot_file}    & Plotting         & \sref{s:init_plot}     \\ \hline
  \vn{element_shapes_lat_layout}         
                              & \vn{plot_file}    & Plotting         & \sref{s:ele_shape}     \\ \hline
  \vn{element_shapes_floor_plan}         
                              & \vn{plot_file}    & Plotting         & \sref{s:ele_shape}     \\ \hline
  \vn{key_bindings}           & \vn{single_mode_file} & Single Mode  & \sref{s:init_key_bind} \\ \hline
\end{tabular}}
...
parameters at the center of an element is desired, a lattice where the
...
\begin{example}
  &tao_connected_uni_init
    ...
    connect%from_universe = <universe_number> ! connect from this universe
    ...
    connect%at_s          = <number>          ! connect at poisiton s 
    connect%match_to_design = <logical>       ! match optics to design parameters
  /
\end{example}
\vn{ix_universe} refers to the universe to be connected. Any of
...
lattice will be used as the connection point. If \vn{universe_number =
...
See~ \sref{s:init_key_bind} and \sref{s:key_bind} for more details.
...
A set of data (\sref{s:data}) is initialized using a \vn{tao_d2_data}
...
The assocaited \vn{tao_d1_data} namelists must come directly after
...
keys is defined in one of two ways.  Keys can be bound directly within
the \vn{v1_var} namelist used to set up a variable list (\sref{s:init_var})
...
element of this array is the name of the reason. This name may not
...
Section~\sref{s:lat_layout} for more details.
...
using an \vn{element_shapes_lat_layout} namelist. See Section~\sref{s:ele_shape}
for more details.
...
Section~\sref{s:key_bind}.  An example is shown in
Figure~\ref{f:layout_table}. A template to create a key table looks like:
\begin{example}
  &tao_template_plot
    plot%name = 'table' 
    plot%n_graph = 1
...
Figure~\ref{f:floor_plan}. Like a \vn{Lattice Layout}
(\sref{s:lat_layout}), Elements are represented by colored rectangles
and which elements are drawn is determined by an
\vn{element_shapes_floor_plan} namelist (see~\sref{s:ele_shape}).
...
\vn{Floor plan} (\sref{s:floor_plan}) and \vn{lattice layout} drawings
...
\vn{plot_page%shape_height_max} (\sref{s:plot_page}).
