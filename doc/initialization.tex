...
\end{tabular}}
\caption{Table of \vn{tao} Initialization Namelists. $^*$This namelist taken from the first file in the \vn{plot\_file} array.}
\label{t:init.files}
\end{table}
...
\begin{example}
  &tao_plot_page
    ...
    plot_page%n_curve_pts      = 401   ! Num points used to construct a smooth curve
...
axis.  If it is desired to have differeng scales for different graphs,
...
component. If it is desired to have differeng scales for different
...
\vn{curve%data_index} sets the
index for the symbol points (\sref{s:template}). In this example, the
\vn{ix_uni} component will result in the symbol index being the
universe number.  Additionally, the component \vn{ix_d1} can be used
to specify the index in the \vn{d1_data} array, and the component
\vn{ix_ele} can be used to specify the lattice element index.
