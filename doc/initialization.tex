...
\begin{example}
  type tao_global_struct
    ...
    character(16) :: default_key_merit_type = 'limit'
    ...
    logical :: single_mode = .false.       ! Startup in single mode?
...
\begin{example}
  &tao_beam_init
    ...
    beam_init(i)%a_norm_emit    = <number> ! A-mode emittance
    beam_init(i)%b_norm_emit    = <number> ! B-mode emittance
    beam_init(i)%dPz_dZ     = <number>      ! Energy-Z correlation
    beam_init(i)%center    = <number>*6 ! Bunch center offset relative to
                                         ! reference particle (BMAD coords)
    beam_init(i)%sig_e     = <number>   ! e_sigma in eV
    beam_init(i)%sig_z     = <number>   ! Z sigma in m
    beam_init(i)%sig_e_cut = <number>   ! Energy cut in sigmas
    beam_init(i)%sig_z_cut = <number>   ! Z cut in sigmas
    beam_init(i)%sig_trans_cut = <number> ! transverse dimension cut
    beam_init(i)%n_bunch   = <integer>  ! Number of bunches
    beam_init(i)%ds_bunch  = <number>   ! distance between bunches (meters)
    beam_init(i)%n_particle = <number>  ! Number of particles per bunch
    beam_init(i)%bunch_charge = <number> ! charge per bunch (Coulombs)
  /
\end{example}
where \vn{i} refers to the universe index. See \bmad documentation on what the
\vn{beam_init} parameters refer to. Wakefields can only be turned on or off for all
...
The twiss parameters at the beginning of the lattice are used in intitiating the
beam distribution.
...
\vn{radiation_integrals} unless \vn{calc_emittance = F} in which case the emittance
...
\begin{example}
  &tao_macro_init
    ...
    macro_init(i)%x%norm_emit    = <number> 
    macro_init(i)%y%norm_emit    = <number> 
    beam_init(i)%dPz_dZ     = <number>   ! Energy-Z correlation
...
universes and not for each universe individually.
...
nare combined for each datum to form the datum's \vn{type}. Certain
...
\textit{tao\_hook\_load\_data\_array.f90}. See
...
\vn{graph%who(i) = model - design} then by default the graph will show the \vn{model}
value. Possibilities are:
\begin{example}
  model     ! model values.
...
%-----------------------------------------------------------------
\section{Initializing Key Bindings for Single Mode}
\label{s:init_single} 
