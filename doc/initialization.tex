...
\begin{example}
  tao \{-init <tao_input_file>\} \{-beam <beam_file>\} \{-beam0 <beam_file>\} \{-lat <lattice_file>\}
\end{example}
...
The \vn{-beam} optional argument reads in a beam data file for the
...
overrides the \vn{global%beam0_file} variable (\sref{s:globals}.
...
type tao_global_struct
  ...
  character(80) :: beam0_file = ''     ! Input beam data file at start of lattice.
  ...
  logical :: save_beam_everywhere = T   ! Save the beam info at all elements?
  logical :: use_save_beam_in_tracking = F
end type
\end{example}

\vn{global%beam_file} specifies a beam data file (which can be created
...
becomes long.  The file name can be overridden by using the \vn{-beam}
argument on the command line (\sref{s:command_line}. Note: \tao will
set the variable \vn{global%use_saved_beam_in_tracking} to \vn{True}
to prevent actual tracking.

\vn{global%beam0_file} specifies a beam data file (which can be
...
Note: \vn{global%beam_file} and \vn{global%beam0_file} cannot both be set.
