...
\begin{example}
  \&tao_var
    ...
    default_merit_type = <merit_type_name>   ! Small step value.
    ...
    var(i)%merit_type  = <merit_type_name>   ! Small step size.
  /
\end{example}
Example:
\begin{example}
...
commands. \vn{var(i)%ele_name} is the lattice element associated
...
the weight coefficient for a variable in the merit function. If not
...
\begin{example}
  \&tao_data
    ...
    data(i)%weight(j)   = "<weight>"    ! Merit function weight.
  /
\end{example}
For example:
\begin{example}
