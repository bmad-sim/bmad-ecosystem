...
not present is to look in the \tao root initialization file. The \vn{tao_deisgin_lattice} namelist
...
\begin{example}
  &tao_params
    ...
    opti_de_param = <opti_de_param_struct>  ! de optimizer parameters. \sref{opti.de.param.struct}
  /
\end{example}
Example:
\begin{example}
...
True will force \tao to always calcuate curve data points even when \vn{global%plot_on} = F.
...
the \vn{tao_start} namelist (\sref{s:init_global}). The default, if \vn{data_file} is not set, is
...
the \vn{tao_start} namelist (\sref{s:init_global}). The default, if \vn{plot_file} is not set, is
the root initialization file.
