...
Do not put any spaces inbetween. Example:
\begin{example}
...
\begin{example}
  &tao_design_lattice
    ...
    unified_lattices = <logical>        ! Default = False
...
\vn{design_lattice(i)} gives the lattice file name for universe \vn{i}.
Example:
\begin{example}
  &tao_design_lattice
    n_universe = 4
...
The \vn{unified_lattices} switch can be used when there is a baseline
lattice that is common to all universes. This case can occur, for
example, with an \vn{orbit response matrix} analysis where each
universe is associated with a measured orbit when a particular
steering magnet is varied. When \vn{unified_lattices} is set to True,
\tao will set up \vn{common} \vn{model}, \vn{design} and \vn{base}
lattices in memory for all universes to share. This is unlike the
standard setup where a \vn{model}, a \vn{design} and a \vn{base}
lattice is instantiated in memory for each universe. For problems
using a large number of universes, this can result is a substatial
savings in memory. With \vn{unified_lattices}, the difference between
a lattice and the common lattice is defined soley by the variables
(\sref{s:init.var}) that vary attributes in that lattice. That is,
changing a lattice attribute using the \vn{change} (\sref{s:change})
command will only affect the common \vn{model} lattice. By convention,
this common lattice has universe index 0 and the total number of
universes, including the common one, will be \vn{n_universes + 1}.
...
a \vn{var_box} or \vn{asym_var_box} can get arbitrarily large, there
is a maximum size which is set by \vn{plot_page%shape_height_max}
(\sref{s:plot.page}).
