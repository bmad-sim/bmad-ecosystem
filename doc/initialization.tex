...
\tao initialization file.
  \item[\vn{-startup <starup_command_file>}]
Overrides the \vn{starup_file} (\sref{s:init.global}) specified in the
\tao initialization file.
...
\vn{%a_norm_emit} and \vn{%b_norm_emit} are the normalized emittances used to
...
\begin{example}
  &tao_template_graph
    ...
    graph%symbol_size_scale      = <real>     ! Symbol size scale factor for phase_space plots.
