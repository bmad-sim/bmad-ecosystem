...
parameters may be setup for beam particle generation verses everthing
else.
...
The random number generator parameters (\vn{%random_engine},
...
for beam particle generation verses everthing else.  These parameters
...
\begin{example}
  &tao_template_graph
    ...
    curve(i)%draw_interpolated_curve = <Logical> ! Calc and draw smooth curve? 
  /
\end{example}
For example:
\begin{example}
...
color of the line can be controlled by setting \vn{curve%line}.
Sometimes \tao will attempt to calculate ``intermediate'' values in
order to draw a smooth, accurate curve. Occasionally, this process is
too slow or not desired for other reasons so setting
\vn{curve%draw_interpolated_curve} to False will prevent this
calculation and the curve will be drawn as a series of lines
connecting the symbols.  Setting \vn{curve%draw_interpolated_curve} to
True does not affect anything.
