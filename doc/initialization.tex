...
\begin{example}
  \&tao_d1_data
    ...
    data(j)%type       = "<type_name>"  ! Eg: "orbit:x", etc.
...
\begin{example}
  data(0)%name(0) = "SAME: <d2_name:d1_name>"
  data(0)%ele_name(0) = "SAME: <d2_name:d1_name>"
\end{example}
For example:
\begin{example}
  \&tao_d1_data
...
If \vn{data(j)%type} is not given then the \vn{d2_data} name and the
\vn{d1_data} name are combined for each datum to form the datum's
\vn{type}. Certain types are recognized by \tao. These are:
\begin{example}
  "orbit:x", "orbit:y"             ! Orbit 
  "phase:x", "phase:y"             ! Betatron phase (relative) 
  "beta:x",  "beta:y"              ! Beta function
  "alpha:x", "alpha:y"             ! Alpha function
  "eta:x",   "eta:y"               ! Dispersion 
  "etap:x",  "etap:y"              ! Dispersion derivative
  "cbar:11", "cbar:12",            ! Cbar coupling 
     "cbar:21", "cbar:22"       
  "coupling:11", "coupling:12a",   ! Raw coupling 
     "coupling:12b", "coupling:22" 
  "floor:x", "floor:y", "floor:z"  ! Floor position (relative)
  "floor:theta"                    ! Floor angle (relative)
  "s_length"                       ! longitudinal length (relative)
  "i5"                             ! Radiation integral
  "r56"                            ! Transfer matrix element (relative)
  "t566"                           ! 2nd order Xfer element (relative)
\end{example}
