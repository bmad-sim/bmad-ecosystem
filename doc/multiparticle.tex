...
Sagan\cite{b:csr} with the modification to be able to handle off-axis beams as
discussed in Sagan and Mayes\cite{b:csr2}.

The total kick received by a particle is divided into two parts: The
...
\bmad has a code module for simulating the effect of space charge at high energies.  If a a \bmad
based program has been constructed to use this module (the documentation for the problem should
indicate if this is true), the ultra-relativistic space charge force can be turned on or off
by setting the \vn{bmad_com[space_charge_on]} parameter (\sref{s:bmad.com}) (default is \vn{False}). 

It is important to note that the The high energy space charge module is separate from the module for
the coherent synchrotron radiation calculation (\sref{s:csr}). Thus turning on of both the CSR space
chage component and the high energy space charge will result in double counting of the space charge
effect.
...
Note: Unlike the CSR calculation, The high energy space charge kick is computed using fixed
emittances supplied by the user. That is the computation is not self-consistant. The advantage is
that to calculate the high energy space charge kick, it is not necessary to track a bunch of
particles in order to determine the bunch size, This being the case, the high energy space charge
effect can be included when tracking single particles.
