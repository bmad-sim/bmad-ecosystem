...
\begin{example}
  ...
  ptc call <file>   ! Run a ptc script
  ptc write \{-old\} \{<filename>\}
  ptc read \{-old\} <filename>
\end{example}
...
\begin{example}
  ptc init
  ptc call doit.ptc  ! Execute doit.ptc file.
\end{example}
...
The \vn{read} command is used to modify the \vn{model} lattice
contained in the default universe (\sref{s:universe}) using a ``secondary''
lattice file. Format:
\begin{example}
