...
damping and excitation. See the \vn{Synchrotron Radiation} in the \bmad manual for how the matrices
displayed are used for tracking. Typically, radiation damping should be turned on and RF cavities
powered since these affect the reference orbit and transfer matrix. This can be done using the
commands:
\begin{example}
...
By default, the \vn{rfcavities} in the output lattice will be set back \vn{on} if the cavities were
initially turned off when \tao was started (turning RF off at startup is the default procedure). If
this is not wanted, the \vn{-no_rf} switch can be used to prevent this.
