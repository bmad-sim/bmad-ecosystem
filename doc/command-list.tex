...
The \vn{quiet} command can only be used in command files (\sref{s:call}).  When placed in a command
file, output to the terminal is suppressed but only from the \vn{quiet} command until all nested command files
have run and input is passed back to the terminal level.

Possible <action> settings are:
\begin{example}
  all       -- 
  output    -- Suppress output but not command echoing.
\end{example}
...
\begin{example}
  ...
  show string ...            ! Print a string \vn{s:show.string}.
...
Shows the value of an expression. Also see \vn{show string}. Examples:
\begin{example}
