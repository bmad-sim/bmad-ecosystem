...
The \vn{show -write}  and \vn{show -append} commands write the results of the
\vn{show} command to a file as well as printing the information at the terminal.
The \vn{show -append} command will appended to the output file when \vn{show
-append} is used multiple times. The \vn{show -write} command 
will first erase the conents of
an output file. If separate files are desired then if
...
\begin{example}
  ...
  sigma_x, sigma_y, sigma_z   ! Beam sigmas.
...
\begin{example}
  ...
  show -write abc.dat data    ! Same as previous except results are also written
                              !   to the file abc.dat.
