...
single line of input. Conversely, \vn{Single Mode}, which is described in Chapter~\sref{c:single},
...
\vn{single mode} use the \vn{single_mode} command (\sref{s:sing}).
...
semicolon used as as part of an \vn{alias} (\sref{s:alias}) definition is part of that definition.]
...
The \vn{help} command works by parsing the file \vn{\$TAO_DIR/doc/command-list.tex} which is
the LaTeX file for the \vn{Tao Commands} chapter of the \tao manual. Thus, for the \vn{help} command
to work properly, the environmental variable \vn{TAO_DIR} must be appropriately defined. Generally,
\vn{TAO_DIR} will be defined if the appropriate setup script has been run. For ``Distributions'',
this is the same setup script used to setup a distribution. See your local \bmad guru for details.

When the \vn{help} command parses the \vn{\$TAO_DIR/doc/command-list.tex} file, LaTeX syntax will
...
\vn{<ele_type>} is the type of element to misalign.  Only elements of type \vn{<ele_type>} will be
misaligned within the range.  If \vn{<ele_type>} begins with "*@" then choose all universes. If
\vn{<ele_type>} begin with "n@" then choose universe n. Otherwise the default universe
(\sref{s:universe})) is used.

A lattice element will only be misaligned if its lattice index falls within a range given by
\vn{<range>}. \vn{<range>} is of the form \vn{nnn:mmm} or the word \vn{ALL}.

The element attribute \vn{<ele_attrib>} is ``misaligned'' by the rms value \vn{<misalign_value>}
with respect to the setting of \vn{<wrt>}. Any element attribute can be misaligned provided the
attribute is free to vary.

If \vn{<misalign_value>} is prepended by 'x' then the misalignment value will be a relative
misalignment with respect to the \vn{<wrt>} value. Otherwise, it's an absolute rms value about the
\vn{<wrt>} value.

In the special case where sbend strengths are misaligned then use \vn{<ele_attrib> = dg}. However,
if a relative error is specified it will be relative to 'g'.
...
graphs associated with the plot are assigned the \vn{<component>}.
...
For further documentation on the python command and interfacing to python is in Chapter~\sref{c:python}.

Documentation on interfacing Python scripts to \tao's python command is given in
section~\sref{s:python.python}.
...
axis. Normally, the \vn{scale} command will scale both axes.  Scaling of just one of these axes can
be achieved by using the \vn{-y} or \vn{-y2} switches.
...
various parameters that can be set. \vn{<branch-id>} may be the branch index or branch name.
\vn{<branch-id>} may also contain an optional \vn{n@} prefix to specify a particular universe to
...
calculation at startup, set the \vn{design_lattice(i)%one_turn_map_calc} component
(\sref{s:init.lat}).
...
Note: When running \tao as a subprocess, consider using the \vn{python} command (\sref{s:python})
...
the \vn{-derivative} switch is present, the field derivatives as well as curl and divergence are
also printed.

\vn{<x>}, and \vn{<y>} are the transverse coordinates and \vn{<s>} coordinate is the longitudinal
position with respect to the beginning of the element specified by \vn{<ele>}. The syntax for
\vn{<ele>} is explained in section \sref{s:ele.list.format}. Wild card characters are permitted. If
...
If there are visible plots with the same name as the plot component of \vn{<graph>}, a template plot
...
command. This index is printed with the command unless the \vn{-no_num} switch is present.
...
equilibrium polarization, polarization rates, and any G matrices needed to evaluate datums that have
been defined of type \vn{spin_g_matrix.$ij$} (\sref{s:data.source}).

If \vn{-element} is present, the output will be the 8x8 G-matrix from the downstream end of the
element given by \vn{<ref_ele_name>} to the downstream end of the element given by \vn{<ele_name>}.
If \vn{<ref_ele_name>} is not given, the default is to use the element before the element given by
\vn{<ele_name>}. This gives the G-matrix for transport through the \vn{<ele_name>} element.  The
G-matrix is dependent upon the $(\Bf l, \Bf n, \Bf m)$ axes used to define the spin vector (see the
\vn{SLIM Formalism} section of the Spin chapter in the Bmad manual). The $\Bf n$ and $\Bf l$ axes
...
The \vn{-universe} switch is used to select only variables what control components in a given
...
Note: The command `\cmd{veto data *.*[10:20]}' will veto all d1\_data elements within the range
...
\begin{example}
  ...
  write bmad \{-format <format_type>\} \{<file_name>\}                        ! \sref{s:write.bmad}
  ...
  write variable \{-good_var_only\} \{-tao_format\} \{<file_name>\}               ! \sref{s:write.variable}
...
(\sref{s:universe}). The suffix must by '.py' and if this suffix is not present it will be added. To
run the script in \vn{blender}, use the following on the operating system command line:
\begin{example}
...
\end{example}
where \vn{slave_param} is a paramter of \bmad element \vn{slave_ele} that is controlled by a \tao
...
\end{example}
With this format, the output of \vn{write variable} can be read back into \tao useing a \vn{call}
command.
