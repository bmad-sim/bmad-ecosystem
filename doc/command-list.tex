...
\begin{tabular}{|l|l||l|l|} \hline
  ...
  end_file      & \sref{s:end.file} & single_mode   & \sref{s:sing}     \\ \hline 
  ...
  misalign      & \sref{s:misalign} & value         & \sref{s:value}    \\ \hline
  pause         & \sref{s:pause}    & view          & \sref{s:view}     \\ \hline 
...
The \vn{read} command is used to modify the currently viewed
\vn{model} lattice using a ``secondary'' lattice file. Format:
\begin{example}
...
create a 3D display of the currently viewed (\sref{s:view})
lattice. The suffix '.jou' will be appended if the given file name
does not have it. To read in the script to \vn{cubit}:
\begin{example}
  0) Run cubit
