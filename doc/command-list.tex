...
Syntax:
\begin{example}
  show emittance \{-element <ele_id>\} \{-universe <uni_index>\}
\end{example}
...
(also non-PTC). With radiation damping, the emittance is not an exact invariant of the motion. Thus
the calculated emittance will vary from element to element.

If the \vn{-element} switch is not present, the beginning element of the default branch (set by
\vn{set default branch}) is used.

Note: The 6D calculation is new and undergoing testing. Currently the radiation integrals emittances
are what is shown with the \vn{show universe} command but this is likely to change in the future
since the 6D calculation is more accurate.
