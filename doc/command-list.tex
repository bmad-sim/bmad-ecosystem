...
The \vn{call} command opens a command file (\sref{s:command.file}) and
...
command file (\sref{s:command.file}) after a \vn{pause} command
(\vn{s:pause}). Format:
\begin{example}
  continue
\end{example}
...
(\sref{s:command.file}) to signal the end of the file. Everything
...
file (\sref{s:command.file}). Format:
\begin{example}
