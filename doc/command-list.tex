...
lattices that have an \vn{e_gun} (which necesarily implies that the
...
The \vn{-orbit} switch, if presen,t will show the particle reference
...
\vn{geodesic_lm} oiptimizer will be shown. These parameters are shown in any case
if the opeimizer has been set to use \vn{geodesic_lm}.
...
For on-line help when running \tao goto \vn{single mode} and type ``?''.
