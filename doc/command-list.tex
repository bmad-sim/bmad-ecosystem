...
For \vn{change element}, the \vn{<name_or_number>} may be a name, an element number or
...
For \vn{change element}, and \vn{change beam_start}, The optional \vn{n@}
...
\begin{example}
  ...
  set universe <what_universe> mat6_recalc
...
\item[\protect\parbox{6in}{
        ...
        set universe <what\_universe> mat6\_recalc}] \Newline

The \vn{set <what_universe> on/off} command will turn the specified universe(s) on or
...
\vn{-1} as an index.  To specify all universes, use \vn{*}.
...
The \vn{set <what_universe> recalculate} command will recalcuate the
lattice parameters for that universe.

The \vn{set <what_universe> mat6_recalc} command will toggle whether
...
are needed to computer the Twiss parameters). Use the \vn{show
...
\begin{example}
  ...
  show hom
  show key_bindings
  show lattice \{-no_header\} \{-middle\} \{-0undef\} \{-custom <file_name>\} 
                 \{-branch <branch>\} \{-s\} \{-elements <name>\} \{<location_range>\}
  show optimizer
...
\item[\protect\parbox{6in}{
    show lattice \{-no\_header\} \{-middle\} \{-0undef\} \{-custom <file\_name>\} \{-branch <branch>\} \\ 
...
``----'', will be placed in the approprite spot in the table.
