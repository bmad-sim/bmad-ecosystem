...
(\sref{s:ele.list.format}), without any embeded blanks, is used for
...
The \vn{-noplot} optional argument surpresses the opening of the plot window.
...
\item[set elements <ele_names> <on/off>] \Newline
An element that is turned off acts like a drift. The syntax
of \vn{<ele_names>} is the element list format (\sref{s:ele.list.format}).

Example:
\begin{example}
  set ele *@rfcav::* off  ! Turns off all rfcavity elements in all universes.
\end{example}
...
Example:
\begin{example}
  set opti_de_param binomial_cross = T  ! Use binomial corssovers 
\end{example}
...
The \vn{set wave} command sets the boundries of the $A$ and $B$ regions 
...
\begin{example}
  ...
  show global \{-optimization\} \{-bmad_com\} \{-csr_param\}
...
\item[show element \{-taylor\} \{-wig\_terms\} \{-data\} 
                     \{-all\_attributes\} <ele\_name>] \Newline
...
\item[show global \{-optimization\} \{-bmad_com\} \{-csr_param\}] \Newline
...
Displays \vn{bmad_com} compoents (\sref{s:globals}).
  \item[-csr_param] \Newline
Displays \vn{csr_param} compoents (\sref{s:globals}).
  \end{description}
...
in by another program. The \vn{-no_tail_lines} just supress the header
