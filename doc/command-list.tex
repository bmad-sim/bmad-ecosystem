...
\vskip 1pt The \vn{call} command without \vn{-ptc} is for running a set of \tao commands.  Up to 9
...
the command \vn{ptc init} must be issued.
...
The \vn{-gang} switch is apply a clip to corresponding data in a \vn{d2_data} structure. For
example
\begin{example}
...
The \vn{plot} command is used to determine what ``components'' (\sref{s:plot.data}) are plotted in
the specified graphs or plots. Format:
\begin{example}
...
For \vn{set bmad_com}: The \vn{show global} command will give a list of \vn{<parameter>}s.
...
\vn{plot%x%major_divisions} (\sref{s:template})) to try to give a nice looking axis. The result can be
