...
\begin{tabular}{ll|ll} \toprule
  ...
  change        & \sref{s:change}   & restore       & \sref{s:restore}  \\ 
  clip          & \sref{s:clip}     & reinitialize  & \sref{s:reinit}   \\ 
  continue      & \sref{s:continue} & run_optimizer & \sref{s:run}      \\ 
  derivative    & \sref{s:deriv}    & scale         & \sref{s:scale}    \\ 
  do, enddo     & \sref{s:do}       & set           & \sref{s:set}      \\  
  end_file      & \sref{s:end.file} & show          & \sref{s:show}     \\ 
  exit          & \sref{s:exit}     & single_mode   & \sref{s:sing}     \\ 
  flatten       & \sref{s:flatten}  & spawn         & \sref{s:spawn}    \\ 
  help          & \sref{s:help}     & timer         & \sref{s:timer}    \\
  misalign      & \sref{s:misalign} & use           & \sref{s:use}      \\ 
  pause         & \sref{s:pause}    & veto          & \sref{s:veto}     \\ 
  place         & \sref{s:place}    & wave          & \sref{s:wave}     \\
  plot          & \sref{s:plot}     & write         & \sref{s:write}    \\ 
  ptc           & \sref{s:ptc}      & x_axis        & \sref{s:x.axis}   \\ 
  python        & \sref{s:python}   & x_scale       & \sref{s:x.scale}  \\ 
  quit          & \sref{s:quit}     & xy_scale      & \sref{s:xy.scale} \\ \bottomrule
\end{tabular}}
...
\vn{Command Files and Aliases} section (\sref{s:command.files}) for more details.
...
terminal level. See Section~\sref{s:tao.global.struct} for more details.
...
parameters. Also consider using the \vn{set} command (\sref{s:set}) which is more general.
...
For \vn{change particle_start}, The optional \vn{n@} universe specification
...
\vn{use_particle_start_for_center} of the \vn{beam_init} structure (\sref{s:beam.init}) is set to
True, \vn{particle_start} will also vary the centroid coordinates for beam tracking. Here
...
which is the LaTeX file for the \vn{Tao Commands} chapter of the \tao manual. Thus, for
...
viewed using the \vn{show history} command. The \vn{show history} command will also display the
...
\bmad manual for the definition of ``secondary lattice''.]
...
\begin{example}
  ...
  set dynamic_aperture \{n@\}<component = <value>             ! \sref{s:set.da}
...
The \vn{set beam \{n@\}add_saved_at} command adds to the list of elements where the beam distribution is 
saved at. 

The \vn{set beam \{n@\}subtract_saved_at} command subtracts from the list of elements where the beam distribution is 
saved at. 
...
Sets parameters associated with a lattice branch. Use the \vn{show branch} command to see the
various parameters that can be set. \vn{<branch>} may be the branch index or branch
name. \vn{<branch>} may also contain an optional \vn{n@} prefix to specify a particular universe
...
If there are visible plots with the same name as the plot component of \vn{<curve>}, a template plot
...
(\sref{s:dynamicaperture}) Also see the \vn{set universe dynamic_aperture} (\sref{s:set.universe})
and \vn{show dynamic_aperture} (\sref{s:show.da}).

To set the particle energy for the <n>\Th scan use \vn{pz(<n>)}. Use a value less than -1 to remove the scan.
...
\end{example}
Where \vn{<ele_shape_name>} is of the form ``\vn{ele_shape(<n>)}'' where \vn{<n>} is the index
...
The \vn{set graph} command is used to set components of a graph structure (\sref{s:template}).

If the \vn{<graph>} name corresponds to a plot, the set is applied to all the graphs associated with
the plot. If there are visible plots with the same name as the plot component of \vn{<graph>}, a
...
lattices with an open geometry. If the \vn{use_particle_start_for_center} of the \vn{beam_init}
structure (\sref{s:beam.init}) is set to True, \vn{particle_start} will also vary the centroid
...
\begin{example}
  ...
  visible            = <logical>
  component          = <string>    ! \sref{s:plot.data}
  x%<axis_component> = <value>
  n_curve_pts        = <integer>
\end{example}
Use the \vn{show plot <plot_name>} to see the settings of various parameters. See the
section~\sref{s:template} for information on the plotting parameters.
...
The \vn{n_curve_pts} parameters sets the number of points to use for drawing ``smooth''
...
If \vn{<plot_or_region>} is a plot name, and there are visible plots of that name, any template plot of the same
name is ignored. To set a template plot in this case, add a ``\vn{T::}'' prefix.
...
-ran_state} to show the random number generator state. Manipulating the state for generating
...
calculation for a ring. See the \vn{Initializing Dynamic Aperture} section
(\sref{s:dynamicaperture}) for more details. To enable the dynamic aperture calculation at startup,
set the \vn{design_lattice(i)%dynamic_aperture} component (\sref{s:init.lat}).
...
\begin{example}
  ...
  show normal_form ...       ! Show transport map normal form \sref{s:show.normal}.
  ...
  show variable ...          ! Show optimization variable info \sref{s:show.variable}.
...
<subcommand>} command. For example, \vn{help show element} will show information on the
\vn{show element} subcommand.
...
If \vn{<element_name_or_index>} is absent, \vn{show beam} shows parameters used with beam tracking
including the number of particles in a bunch, etc.

If \vn{<element_name_or_index>} is present, \vn{show beam} will show beam parameters at the selected
element. Also see \vn{show particle}.  Use the \vn{set beam_init} command to set values of the
\vn{beam_init} structure.
...
Default values for \vn{<data_name(s)>} and \vn{<var_name(s)>} is "*" (all data or variables).
...
is, derivatives are only calculated for data and variables whose \vn{useit_opt} component (see
...
Shows parameters and results of the dynamic aperture calculation (\sref{s:dynamicaperture}).
...
Syntax:
\begin{example}
  show field <ele> <x> <y> <z> \{<t/z>\}
\end{example}

The \vn{show field} command shows the electric and magnetic field at a point in space-time. 
\vn{<x>}, and \vn{<y>} are the transverse coordinates and 
\vn{<z>} coordinate is the longitudinal position with respect to the beginning of the element
specified by \vn{<ele>} (not to be confused with phase space $z$). The syntax for \vn{<ele>} is
explained in section \sref{s:ele.list.format}. If multiple elements are matched, the fields in each
will be printed.

The \vn{<t/z>} argument is optional and specifies the time if absolute time tracking is on or the
phase space $z$ value if absolute time tracking is off (use the \vn{show universe} command to see if
absolute time tracking is on or off). The \vn{<t/z>} argument is only useful for elements with RF fields.
If not set, \vn{<t/z>} will default to zero.

\begin{example}
  show field q10w 0.1  0.2, 1.2    ! Show field in q10w element.
\end{example}
...
If there are visible plots with the same name as the plot component of \vn{<graph>}, a
...
The \vn{-beginning} switch is ignored when displaying ``Intrinsic'' element
parameters such as the element's length or an element's field strength (which can be displayed using
the \vn{-attribute} switch as discussed below). Also the \vn{-beginning} switch is ignored when
displaying beam based parameters. Also see \vn{-middle}.

%
\item[-blank_replacement <string>] \Newline
The \vn{-blank_replacement} switch specifies that whenever a blank string is
encountered (for example, the \vn{type} attribute for an element can be blank), \vn{<string>} should
be substituted in its place. \vn{<string>} may not contain any blank characters. Example:
\begin{example}
...
\vn{<name_or_index>} can be the name or index of the branch.  The default is the main
branch (\# 0).
%
\item[-custom <file_name>] \Newline
A table with custimized columns may be constructed either by using the \vn{-custom} switch which
specifies a file containing a description of the custom columns or by using one or more \vn{-attribute} switches.
...
\begin{example}
  ...
  /
\end{example}
each \vn{column(n)} line has four components. The first component is what is to be displayed in that
...
%
\item[-design] \Newline
  Show values from the \vn{design} lattice instead of the \vn{model} lattice. Also see the \vn{-base} switch.
%
...
is given in section~\sref{s:ele.list.format}.  In this case there should be no blank characters in
the list.
%
...
%
\item[-python] \Newline
The \vn{-python} switch gives a comma deliminated table as output. This switch is used with the
...
Shows the transfer matrix (Jacobian) for the \vn{model} lattice of the default universe (set by
\vn{set default universe}). The matrix is calculated with respect to the particle orbit, not the
zero orbit. This command is equivalent to \vn{show taylor_map -order 1}. See the \vn{show
taylor_map} documentation for more details.
...
The default for the optional \vn{-bunch} index is set by the global variable
\vn{global%bunch_to_plot}. The default \vn{-element} is \vn{init} which is the initial beam
distribution. The default \vn{-particle} to show is the particle with index 1.
...
Syntax:
\begin{example}
  show plot \{-floor_plan\} \{-plot_page\} \{-lat_layout\} \{-regions\} 
...
The \vn{show plot -plot_page} command shows some global plotting parameters like the size of the plot
window.
...
use: \vn{set bmad_com spin_tracking_on = T}), and then the \vn{show element} command will display $n_0$.

If \vn{-element} is not present, \vn{show spin} will show various quantities including the
equilibrium polarization, polarization rates, and any G matrices needed to evaluate datums that
have been defined of type \vn{spin_g_matrix.}$ij$ (\sref{s:data.source}).

If \vn{-element} is present, the output will be the 8x8 G-matrix from the downstream end of the
element given by \vn{-ref_element} to the downstream end of the element given by \vn{-element}.
If \vn{-ref_element} is not given, the default is to use the element before the element given
by \vn{-element}. This gives the G-matrix for transport through the \vn{-element} element.
The G-matrix is dependent upon the $(\Bf l, \Bf n, \Bf m)$ axes used to define the spin vector (see
the \vn{SLIM Formalism} secton of the Spin chapter in the Bmad manual). The $\Bf n$ and $\Bf l$ axes
can be specified using the \vn{-n_axis} and \vn{-l_axis} switches. The $\Bf m$ axis is calculated
...
is enclosed in brackets ``[...]''. Also if the evaluated quanty ends in \vn{``@@N''} where \vn{N} is
...
\begin{example}
  ...
  show `1e3*lat::orbit.x[3:5]@@2`      ! Prints something like "[3.4, 3.2, 2.7]"
\end{example}
...
Syntax:
\begin{example}
  show taylor_map \{-eigen_modes\} \{-order <n_order>\} \{-ptc\} \{-s\} \{loc1 \{loc2\}\}
\end{example}
...
If the \vn{-eigen_modes} switch is present, the first order part of the map will be treated
...
The \vn{-ptc} switch is used with \vn{-order 1}. By default, order 1 maps (matricies) are calculated
using native Bmad code. If the \vn{-ptc} switch is present, the matrix is calculated using the
\vn{PTC} code (see the \bmad manual for details on PTC). Since PTC is always used to calcuate maps
...
Format:
\begin{example}
  write beam \{-ascii\} -at <element_list> \{<file_name>\}                    ! \sref{s:write.beam}
  write blender \{<file_name>\}                                             ! \sref{s:write.blender}
  write bmad \{-binary\} \{<file_name>\}                                      ! \sref{s:write.bmad}
  write covariance_matrix \{file_name\}                                     ! \sref{s:write.covar.matrix}
  write curve <curve_name> \{<file_name>\}                                  ! \sref{s:write.curve}
  write derivative_matrix \{file_name\}                                     ! \sref{s:write.deriv.matrix}
  write digested \{<file_name>\}                                            ! \sref{s:write.digested}
  write gif \{<file_name>\}                                                 ! \sref{s:write.gif}
  write gif-l \{<file_name>\}                                               ! \sref{s:write.gif}
  write hard                                                              ! \sref{s:write.hard}
  write hard-l                                                            ! \sref{s:write.hard}
  write mad8 \{<file_name>\}                                                ! \sref{s:write.mad8}
  write madx \{<file_name>\}                                                ! \sref{s:write.madx}
  write namelist \{-append\} \{-data\} \{-plot\} \{-variable\} \{file_name\}        ! \sref{s:write.namelist}
  write opal \{file_name\}                                                  ! \sref{s:write.opal}
  write ps \{-scale <scale>\} \{<file_name>\}                                 ! \sref{s:write.ps}  
  write ps-l \{-scale <scale>\} \{<file_name>\}                               ! \sref{s:write.ps}
  write ptc \{-all\} \{-old\} \{-branch <name_or_index\} \{<file_name>\}          ! \sref{s:write.ptc}
  write sad \{<file_name>\}                                                 ! \sref{s:write.sad}
  write variable \{-good_var_only\} \{<file_name>\}                           ! \sref{s:write.variable}
  write xsif \{<file_name>\}                                                ! \sref{s:write.xsif}
\end{example}
...
The \vn{write beam} command writes beam information to a file.
Syntax:
\begin{example}
    write beam \{-ascii\} -at <element_list> \{<file_name>\} 
\end{example}

The default file name is \vn{beam_\#.dat} if an ASCII file is to be created and \vn{beam_\#.hdf5} when
an HDF5 binary file is to be created where \vn{\#} is replaced by the universe number.

\vn{write beam} will create a file of the particle positions when beam tracking is being used. The
...
argument to the \vn{-at} switch. The \vn{-ascii} switch is for writing ASCII text files instead of
the default HDF5 files. See the \vn{Beam Initialization} chapter in the \bmad manual for a
discussion of the syntax. The default is to write with a compressed binary format. Note: Beam files
can be used to initialize \tao (\sref{s:command.line}). Example
\begin{example}
...
\vn{Blender} is a free, open source, program
...
\vn{binary}. The difference between \vn{ascii} and \vn{binary} is that for \vn{grid_field}s, which may
have a huge amount of associated data, the \vn{binary} format stores \vn{grid_field}s using HDF5. The other
three field description types are always stored using ASCII files.
...
The default file name is \vn{tao.gif}.
...
The \vn{write ps} and \vn{write ps-l} commands produce postscript output. The difference is that
\vn{write ps} uses portrait orientation while \vn{write ps-l} uses landscape orientation. Syntax:
\begin{example}
...
The optional \vn{-scale} switch sets the scale for the postscript file. The default is 0 which
...
The \vn{write variable} command writes a number of files, one for each universe, of the \tao
variables.
Syntax:
\begin{example}
...
The optional \vn{-good_var_only} switch.  If present, only the information on variables that are
...
The default file name is \vn{lat_\#.xsif} where \vn{\#} is replaced by the universe number. 
XSIF is a version of MAD-8 customized by SLAC to handle \vn{lcavity} elements.
