...
\begin{tabular}{|l|l||l|l|} \hline
  ...
  quiet         & \sref{s:quiet}    & xy_scale      & \sref{s:xy.scale} \\ \hline
  quit          & \sref{s:quit}     &               &                   \\ \hline
\end{tabular}}
...
\begin{example}
  show lattice \{-0undef\} \{-all\} \{-attribute <attrib>\} \{-base\}
      ...
      \{-sum_radiation_integrals\} \{-tracking_elements\} \{-undef0\} \{-universe\} 
      \{<element_list>\} 
\end{example}
...
lines. The \vn{-all} switch overrides this default and shows all tracking and lord elements. 
\end{description}
%
\item[-orbit] \Newline
...
definitions of the radiation integrals. Also see \vn{-radiation_integrals}.
...
read in by another program. The \vn{-no_tail_lines} just suppress the header lines at the bottom of
the table.
...
or by specifying a list \vn{<element_list>} of elements.  The syntax used for specifying this list
...
lattice. If neither \vn{-tracking_elements}, nor a range is given, the first 200 elements are shown.
...
\begin{example}
  ...
  /
\end{example}
each \vn{column(1)} has four components. The first component is what is to be displayed in that
...
Additionally, The \vn{-blank_replacement <string>} switch specifies that whenever a blank string is
...
Note: Data can be used in custom output but data is always evaluated at the exit end of an element
even when the \vn{-middle} switch is used.
...
If present, the \vn{-remove_line_if_zero <column \#>} switch will suppress any lines where the value
