...
\item[show element \{-taylor\} \{-wig\_terms\} \{-data\} 
                     \{-all\_attributes\} <ele_name>] \Newline
This shows information on lattice elements. The syntax for \vn{<ele_name>} 
is explained in section \sref{s:ele.name}. If
...
\item[\protect\parbox{6in}{
  show lattice \{-all_tracking} \{-0undef\} \{-branch <branch>\} \{-custom <file\_name>\} \\
  \hspace*{0.35in} \{-lords\} \{-middle\} \{-no\_label_lines\} \\
  \hspace*{0.35in} \{-no_tail_lines\} \{-s <s1>:<s2>\} \{<elements>\}  }] \Newline
...
\vn{-elements} switch is used is given in section~\sref{s:ele.name.syntax}. 
For example:
\begin{example}
