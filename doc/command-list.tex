...
\vn{-filed} switch is present, the numbering is shifter so that the current \vn{show history} command
has index zero.
...
command will not affect magnet strengths. The reason why this is done is so that sucessive
...
elemnts with the same same). However, creating a lattice file in a non-\bmad (MAD, etc.) format will
be problematical.
