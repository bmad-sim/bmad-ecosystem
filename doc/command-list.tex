...
\begin{example}
  ...
  lattice \{-0undef\} \{-all_tracking\} \{-base\} \{-blank_replacement <string>\} 
        \{-branch <name_or_index>\} \{-custom <file_name>\} \{-design\} \{-floor_coords\} 
        \{-lords\} \{-middle\} \{-no_label_lines\} \{-no_tail_lines\} \{-orbit\} 
        \{-radiation_integrals\} \{-remove_line_if_zero <column \#>\} \{-s <s1>:<s2>\} \{<elements>\} 
  matrix \{-s\} \{loc1 \{loc2\}\}
  ...
  top10 \{-derivative\}
  twiss_and_orbit \{-base\} \{-branch <name_or_index>\} \{-design\} \{-universe <ix_uni>\} <s_position>
  ...
  wall \{-branch <name_or_index>\} \{-section <index>\} \{-angle <angle>\} \{-s <s1>:<s2>\} \{<n1>:<n2>\}
  wave
\end{example}
...
\item[\protect\parbox{6in}{
  ...
  \hspace*{0.35in} \{-branch <name_or_index>\} \{-custom <file\_name>\} \{-design\} \{-floor_coords\} \{-lords\} \\
  \hspace*{0.35in} \{-middle\}  \{-no\_label\_lines\} \{-no\_tail\_lines\} \{-orbit\} \{-radiation_integrals\} \\
...
The locations to show are specified either by specifying a longitudinal 
position range with \vn{-s},
or by specifying a list \vn{<element_list>} of elements.
The syntax
used for specifying this list is given in section~\sref{s:ele.list.format}.
In this case there should be no blank characters in the list. 
For example:
\begin{example}
