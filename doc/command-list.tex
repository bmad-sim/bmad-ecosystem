...
Here components are a property of a graph (or curve) so when <plot_or_graph> specifies a plot, all the
...
\begin{example}
  ...
  set csr_param <parameter> = <value>                       ! \sref{s:set.csr.param}
...
command to see a list of \vn{<parameter>}s.
...
\begin{example}
  ...
  autoscale_y        = <logical>
  visible            = <logical>
  parameter          = <string>    ! \sref{s:plot.data}
...
For \vn{set plot_page}, the \vn{<parameter>}s that can be set are:
\begin{example}
...
\end{example}
The \vn{subtitle_loc} parameter can be used to place the subtitle anywhere on the plot
...
set the \vn{design_lattice(i)%dynamic_aperture} parameter (\sref{s:init.lat}).
...
analysis (\sref{c:wave}). The parameters are
\begin{example}
...
\begin{example}
  ...
  show global -csr_param    ! Displays \vn{csr_param} parameters (\sref{s:globals}).
