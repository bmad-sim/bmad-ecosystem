...
\begin{example}
  show beam \{-comb_index\} \{-universe <uni_index>\} \{-lattice\} \{<element_id>\}
\end{example}
...
If the \vn{-comb_index} is present, \vn{<element_id>} should be an integer which is the index
of the comb of longitudinally equally spaced points where the beam parameters are evaluated at.
Note: \vn{comb_ds_step} (\sref{s:tao.global.struct}) is used to set the spacing between points.
...
\begin{example}
  show spin \{-element \{<ref_ele_name>\} <ele_name>\} \{-g_map\}
                  ...
                  \{-x_nullify <ele_list>\} \{-y_nullify <ele_list>\} \{-z_nullify <ele_list>\}
\end{example}
...
The \vn{-x_nullify}, \vn{-y_nullify}, and \vn{-z_nullify} options are for testing if supressing
certain terms in the linear part of the spin transport map for a set of elements selected by the
user will significantly affect the polarization. This is discussed in the section \vn{Linear dn/dpz
Calculation} in the \bmad manual. In particular, \vn{-x_nullify} will zero the $\bfq_1$ and $\bfq_2$
terms in the $\arrowbfq$ vector (equivalent to zeroing $\bfG_x$ in the SLIM fromalism) for the spin
transport map of the chosen lattice elements. Similarly, \vn{-y_nullify} and \vn{-z_nullify} will zero
...
\begin{example}
  ...
  show spin -x_nul s1:s2 ! Zero $\bfq_1$ and $\bfq_2$ components for all lattice elements
