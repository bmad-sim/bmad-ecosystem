...
The \vn{call} command opens a command file and executes the commands
in it.  Format:
\begin{example}
...
Note: To reduce unnecessary calculations, \vn{global%lattice_calc_on}
and \vn{global%plot_on} can be toggled. Example
\begin{example}
...
The \vn{end-file} command is used in command files (\sref{s:call}) to
signal the end of the file. Everything after an \vn{end-file} command
is ignored. An \vn{end-file} command entered at the command line will
simply generate an error message.  Format:
\begin{example}
  end-file
\end{example}
