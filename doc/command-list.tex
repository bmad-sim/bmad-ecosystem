...
\begin{example}
  ...
  change \{-branch <branch_list>\} \{-mask <veto_list>\} tune Qa Qb
  change \{-branch <branch_list>\} z_tune Qz
\end{example}
...
The \vn{change tune} and \vn{change z_tune} commands will vary the transverse and longitudinal tunes
respectively. The \vn{<branch_list>} is a list of lattice branches with optional universe prefix, to
vary the tunes. The \vn{<veto_list>} of the \vn{-mask} option gives a list of quadrupoles {\em not}
to use for varying the tune. See the \vn{set tune} (\sref{s:set.tune}) command for more details.
