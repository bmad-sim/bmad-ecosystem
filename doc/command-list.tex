...
Syntax:
\begin{example}
  show beam \{-universe <universe>\} \{-lattice\} \{<element_name_or_index>\}
\end{example}

If both \vn{<element_name_or_index>} and \vn{-lattice} are absent, \vn{show beam} shows parameters
...
If \vn{<element_name_or_index>} is present, and \vn{-lattice} is not, \vn{show beam} will show beam
...
lattice at the position given by \vn{<element_name_or_index>} (\sref{s:lat.sig.init}). If an
...
\vn{<ref_ele_name>} beging blank or the same as \vn{<ele_name>}. The spin-orbit resonance strength
...
element is a \vn{beambeam} element, the track will show the before and after particle positions at each
strong beam slice. See the documentation on the \vn{beambeam} element for more details on how tracking
through the element is done.
%
...
Write a lattice fiel in \vn{Elegant} format. The default file name is \vn{lat_\#.lte} where \vn{\#}
