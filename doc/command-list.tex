...
\begin{tabular}{ll|ll} \toprule
  ...
  call          & \sref{s:call}     & reinitialize  & \sref{s:reinit}   \\ 
  change        & \sref{s:change}   & restore       & \sref{s:restore}  \\ 
  clear         & \sref{s:clear}    & run_optimizer & \sref{s:run}      \\ 
  clip          & \sref{s:clip}     & scale         & \sref{s:scale}    \\ 
  cut_ring      & \sref{s:cut.ring} & set           & \sref{s:set}      \\  
  continue      & \sref{s:continue} & show          & \sref{s:show}     \\ 
...
malformed datum. This is useful when creating datums from scratch (via \vn{python data_d2_create})
...
\begin{example}
  ...
  show wake_elements           ! Show wake info \sref{s:show.wake_elements}.
...
\vn{write bunch_comb} (\sref{s:write.bunch.comb}) and \vn{python bunch_comb} (\sref{p:bunch.comb})
commands.
...
the \vn{show beam -comb} (\sref{s:show.beam} and \vn{python bunch_comb} (\sref{p:bunch.comb})
commands. Syntax:
\begin{example}
