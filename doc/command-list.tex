...
Also see the commands: \vn{set beam} (\sref{s:set.beam}) , \vn{set beam_init}
(\sref{s:set.beam.init}), and \vn{change} particle_start (\sref{s:change}).
...
\begin{example}
  ...
  show internal ...          ! Used for code debugging {s:show.internal}.
  ...
  show particle ...          ! Show tracked particle info \sref{s:show.particle}.
...
column. Algebraic expressions are permitted (\sref{s:arithmetic.exp}). Note: Use of \vn{ele::} and
\vn{beam::}, etc sources is accepted but these constructs cannot be evaluated at the center of an
element. That is the \vn{-middle} switch will have no effect on such constructs.
...
1}. See the \vn{show taylor_map} documentation for more details.
...
Syntax:
\begin{example}
    show particle \{-bunch <bunch_index>\} \{dtime\} \{-particle <particle_index>
                  \{-element <element_name_or_index>\} \{-lost\} \{-all\}
\end{example}


Shows individual particle information except if the the \vn{-lost} or \vn{-all} options are used. 
...
The \vn{-dtime} option shows the time relative to the reference time ($t - t_{ref}$) instead of
the $z$ phase space coordinate. The conversion equation is $z = -\beta \, c \, (t - t_{ref})$ where
