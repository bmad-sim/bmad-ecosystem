...
\begin{tabular}{ll|ll} \toprule
  ...
  alias         & \sref{s:alias}    & read          & \sref{s:read}     \\
  call          & \sref{s:call}     & restore       & \sref{s:restore}  \\ 
  ...
  derivative    & \sref{s:deriv}    & set           & \sref{s:set}      \\  
  do, enddo     & \sref{s:do}       & show          & \sref{s:show}     \\ 
  end_file      & \sref{s:end.file} & single_mode   & \sref{s:sing}     \\ 
  exit          & \sref{s:exit}     & spawn         & \sref{s:spawn}    \\ 
  flatten       & \sref{s:flatten}  & taper         & \sref{s:taper}    \\
...
\begin{example}
  taper \{-universe <ix_uni>\} \{-include_solenoids\}
\end{example}
...
\begin{example}
  dg, k1, k2, k3, hkick, vkick, kick, a1, a2, ..., b1, b2, ...
\end{example}
Not included is \vn{ks} unless the \vn{-include_solenoids} switch is present. Notice that something
like wiggler strengths are not included.

The scaling of the magnet strengths is done with respect to the \vn{base} lattice (which is the same
as the \vn{design} lattice if the \vn{set lattice base = ...} command has not been issued). That is,
if two \vn{taper} commands are issued one after another, the second \vn{taper} command will not affect
magnet strengths.
...
\begin{example}
  ...
  taper -inc        ! Taper default uni and include solenoid ks strength.
\end{example}
