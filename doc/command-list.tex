...
Exception: If the \vn{-clear} switch is present, all initalization parameters are set to their
...
there are command line arguments before the \vn{-clear} switch, these argumnets will be cleared.
...
given by \vn{<ele_name>}.  If \vn{<ref_ele_name>} is not given, the default is to use the element
before the element given by \vn{<ele_name>}. This gives the spin transport through the
\vn{<ele_name>} element. The spin map can be displayed in two different forms. If the \vn{-l_axis}
...
between axis components are optional. If \vn{q_map} is present, or if the G-matrix is not being
