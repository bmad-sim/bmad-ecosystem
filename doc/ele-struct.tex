...
variable must either be defined with the \vn{save} attribute or
the pointers within the variable must be deallocated  with a call to
\hyperref[r:lat.low]{deallocate_ele_pointers}
\begin{example}
  type (ele_struct), save :: ele     ! Either do this at the beginning ...
  ...
  call deallocate_ele_pointers (ele) ! ... Or do this at the end of a routine.
\end{example}
...
an input lattice file (\sref{s:string}). On input, from a lattice file,  
...
when a lattice file is parsed, all \vn{rbend} elements are converted into
...
lattice files from a \vn{lat_struct} (\sref{r:read}).
...
\index{ele_struct!twiss_struct substructure}
\begin{example}
  type twiss_struct
...
The routine to propagate the Twiss parameters from one element to the
next is \vn{twiss_propagate1}. To propagate the
twiss_parameters from the start of the lattice to the end, the routine
\vn{twiss_propagate_all} can be used. In propagating the Twiss
...
sure everything is consistent, the changed_attribute_bookkeeper
routine can be used.
...
can control the attributes of other elements. The \vn{%lord_status}
and \vn{%slave_status} components of an element identifies whether the
element is a slave element (it is being controlled or is a lord
element (it controls other elements) or both. See \sref{s:lat.control}
for more details.
