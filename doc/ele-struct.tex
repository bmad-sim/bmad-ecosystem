...
next in the lattice is \vn{twiss_propagate1}. To propagate the
...
can control the attributes of other elements. The \vn{%lord_status} and
\vn{%slave_status}
components of an element identifies whether the element is a slave
element (it is being controlled or is a lord element (it controls
other elements) or both. Possible values for \vn{%lord_status} are:
\index{free}\index{super_lord}
\index{group_lord}\index{girder_lord}
\index{multipass_lord}
\begin{example}
  free\$                No slaves.
  super_lord\$          Has super_slaves.
  multipass_lord\$      Has multipass_slaves.
  group_lord\$          Is a group element.
  girder_lord\$         Is a girder element.
  overlay_lord\$        Is a overlay element.
\end{example}
...
controlled by any other element. The exception is that attributes of
this element may be controlled by a \vn{group_lord}.
...
and \vn{overlay_slave} element (\sref{s:overlay}).
