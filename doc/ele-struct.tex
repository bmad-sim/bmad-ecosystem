...
\hyperref[type.ele]{type_ele} or \hyperref[type2.ele]{type2_ele} routines
...
if a particle has hit the vacuum chamber wall. See Section~\sref{s:tracking.aperture}
for more details.
...
Taylor maps (\sref{c:methods}) are stored in
...
\end{example}
Each Taylor series has an array of \vn{taylor_term_struct} term defined as
\begin{example}
