...
\begin{verbatim}
  type ele_struct
    ...
    logical on_an_girder         ! Have an Girder lord?
...
If the Twiss parameters need to be transfored from 
...
like:
\begin{example}
  iteger, parameter :: time_stamp\$ = general1\$
  ...
...
\begin{description}
\item[free element] 
A \vn{free} element is just that, An element that is neither
...
\item[girder_lord]
\index{on_an_girder}
An \vn{girder_lord} is an \vn{girder} element. In this case, the slaves
are marked by setting the \vn{%on_an_girder} component in the
\vn{ele_struct}.
...
calculations.  Wake fields are stored in the \vnes{%wake} struct:
\begin{example}
  type wake_struct
