...
instance. Such ``external'' elements always have \vn{%ix_ele} = -1.
...
%--------------------------------------------------------------------------
\section {EM Fields and RF Wakes}
\index{wake fields!in ele_struct}

\index{ele_struct!\%rf_wake}\index{ele_struct!\%em_field}
\index{lcavity}\index{rfcavity}
For any type of element, the \vn{%em_field} component holds information on 
the electric and magnetic fields of an element. 
These fields are used, for example, with Runge-Kutta tracking

\NeedsToBeWorkedOn...

For \vn{lcavity} and \vn{rfcavity} elements, the \vn{ele_struct%rf}
structure holds information on ``rf fields'' and ``rf wakes.''
Actually the two are not totally disjoint since the field substructure
can hold information about the HOM modes. The \vn{%rf} component
is of type \vn{rf_struct}:
\begin{example}
  type rf_struct
...
\end{example}
Since \vn{%rf%wake} and \vn{%rf%field} are pointers their association
...
\begin{example}
  ...
  if (associated(ele%rf%wake)) then
    ...
\end{example}
\bmad observes the following rule: If \vn{rf%wake} or \vn{rf%field} are associated then it
is assumed that all the sub--components (\vn{rf%wake%sr_table}, etc.) are
...
\index{ele_struct!\%rf\%wake}
See \sref{s:wake.fields} for the equations used in wake field
calculations.  Wake fields are stored in the \vn{%rf%wake} struct:
\begin{example}
  type rf_wake_struct
...
\end{example}
All \vn{%rf%wake%sr_table(0:)%z} must be negative except \vn{%rf%wake%sr_table(0)%z} 
...
This parameterization utilizes the \vn{%rf%wake%sr_mode_long}, and \vn{%rf%wake%sr_mode_trans}
...
\vn{%rf%wake%z_sr_mode_max} is the maximum $z$ value beyond which the pseudo
mode representation is not valid. This is set in the input lattice file.

\index{rf_wake_lr_struct}
The \vn{%rf%wake%lr} array stores the long--range wake modes. The
