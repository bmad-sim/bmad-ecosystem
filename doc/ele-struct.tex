...
The exception here is when the \zzz{transfer_ele} routine is used with
a local \vn{ele_struct} variable. Example:
\begin{example}
  call transfer_ele (lat%ele(i), loc_ele)
\end{example}
In this case, the local \vn{loc_ele} element will have its pointers
pointing to the same memory locations as \vn{lat%ele(i)} and calling
\vn{deallocate_ele_pointers} on \vn{loc_ele} will mess up the
\vn{lat%ele(i)} element.  To guard against accidental pointer
deallocation, \vn{transfer_ele} has an optional third argument which
will prevent future accidental deallocation
\begin{example}
  call transfer_ele (lat%ele(i), loc_ele, prevent_accidental_deallocation = .true.)
  ...
  call deallocate_ele_pointers (loc_ele)  ! This does nothing.
\end{example}
...
The assignment \vn{ele1 = ele2} will not modify \vn{ele1%ix_ele} or
\vn{ele1%ix_branch}. If \vn{ele1} is associated with a lattice then
\vn{ele1%lat} will also be unaffected.

The reason to use \vn{transfer_ele(ele_in, ele_out)}, as opposed to
\vn{ele_out = ele_in}, is the speedup in time since new memory for
components in \vn{ele_out} does not have to be created.  The drawback
in using \vn{transfer_ele(ele_in, ele_out)} is that one has to be
careful not to simply modify any component of \vn{ele_out} that is a
pointer.
...
\zzz{deallocate_ele_pointers} routine from deallocating the pointers
...
The ``entrance'' and ``exit'' ends of an element are, by definition,
where the physical ends of the element would be if there were no
offsets. In particular, if an element has a finite \vn{s_offset}, the
physical ends will be displaced from entrance and exit ends. See
\sref{s:ele.track} for more details.
...
\index{ele_struct!\%rf_wake}
\index{lcavity}\index{rfcavity}
The \vn{ele%rf_wake} component holds information on the RF wakes associated
with an \vn{lcavity} or \vn{rfcavity}.
Since \vn{ele%rf_wake} is a pointer, its association
status must be tested before any of its sub--components are accessed.
\begin{example}
  type (ele_struct) ele
  ...
  if (associated(ele%rf_wake)) then
    ...
\end{example}
\bmad observes the following rule: If \vn{%rf_wake} is associated, it
is assumed that all the sub--components (\vn{%rf_wake%sr_table}, etc.) are
...
calculations.  Wake fields are stored in the \vn{%rf_wake} struct:
\begin{example}
  type rf_wake_struct
    character(200) :: sr_file = ' '
    character(200) :: lr_file = ' '
    type (rf_wake_sr_table_struct), pointer :: sr_table(0:) => null()
...
\end{example}
All \vn{%rf_wake%sr_table(0:)%z} must be negative except \vn{%rf_wake%sr_table(0)%z} 
= 0. Wake field kicks are applied using \Eqs{delvp} and \eq{pelqxw}.

\index{rf_wake_sr_mode_struct}
The alternative short--range wake parameterization uses pseudo--modes (\Eq{wadzk}).
This parameterization utilizes the \vn{%rf_wake%sr_mode_long}, and \vn{%rf_wake%sr_mode_trans}
arrays for the longitudinal and transverse modes respectively.
The structure used for the elements of these arrays are:
\begin{example}
  type rf_wake_sr_mode_struct  ! Pseudo-mode short-range wake struct 
...
\vn{%rf_wake%z_sr_mode_max} is the maximum $z$ value beyond which the pseudo
mode representation is not valid. This is set in the input lattice file.

\index{rf_wake_lr_struct}
The \vn{%rf_wake%lr} array stores the long--range wake modes. The
