...
= 0. Wakefield kicks are applied using \Eqs{delvp} and \eq{pelqxw}.

The alternitive short--range wake parameterization uses pseudo--modes (\Eq{wadzk}).
This parameterization utilizes the \vn{%wake%sr_mode_long}, and \vn{%wake%sr_mode_trans}
arrays for the longitudinal and transvere modes respectively.
...
(\sref{s:wiggler_phys}) are stored in the \vn{%wig_term(:)} array of
