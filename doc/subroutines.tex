...
\index[routine]{apply_bend_edge_kick}
\label{r:apply.bend.edge.kick}
\item[apply_bend_edge_kick (orb, ele, param, stream_end, in_to_out, kx, ky)] \Newline 
...
\index[routine]{rf_wake_to_c}
\label{r:rf.wake.to.c}
\item[rf_wake_to_c (f_rf_wake, c_rf_wake)] \Newline 
...
\item[em_field_calc (ele, param, s_rel, time, orbit, local_ref_frame, field, calc_dfield, err_flag)] \Newline 
...
\item[mat_symplectify (mat_in, mat_symp, f_scale)] \Newline
...
\item[ele_geometry (floor0, ele, floor, reverse)] \Newline 
...
\index[routine]{shift_reference_frame}
\label{r:shift.reference.frame}
\item[shift_reference_frame (floor0, dr, theta, phi, psi, floor1)] \Newline 
Starting from a given reference frame, and given a shift in position,
return the resulting reference frame.
...
\item[valid_tracking_method (ele, tracking_method, num_valid) result (is_valid)] \Newline 
...
\item[valid_mat6_calc_method (ele, mat6_calc_method, num_valid) result (is_valid)] \Newline 
...
super_lord due to changes in the lord's z_offset. 
...
\item[control_bookkeeper (lat, ele, super_and_multipass_only, do_free_eles)] \Newline
...
\item[ele_compute_ref_energy_and_time (ele, param, e_tot_start, p0c_start, ref_time_start, err_flag)] \Newline 
...
\item[mat6_from_s_to_s (lat, mat6, vec0, s1, s2, orbit, ix_branch, one_turn, unit_start, err_flag, ele_save)] \Newline 
...
\item[qp_eliminate_xy_distortion(axis_to_scale, mirror_axes)] \Newline 
...
\item[check_aperture_limit (orb, ele, at, param, check_momentum)] \Newline
...
\item[check_aperture_limit_custom (orb, ele, at, param, err_flag)] \Newline
Routine to check if an orbit is outside an element's aperture.
...
\item[closed_orbit_from_tracking (lat, closed_orb, i_dim, eps_rel, eps_abs, init_guess, err_flag)] \Newline
...
\item[multi_turn_tracking_analysis (track, i_dim, track0, ele, stable, growth_rate, chi, err_flag)] \Newline
...
\item[offset_photon (ele, coord, set, offset_position_only)] \Newline 
Routine to effectively offset an element by instead offsetting
...
\item[track_from_s_to_s (lat, s_start, s_end, orbit_start, orbit_end, all_orb, ix_branch, track_state)] \Newline
...
\item[track1_boris (orb_start, ele, param, orb_end, err_flag, track, s_start, s_orb)] \Newline
...
\label{r:chrom.calc}
\item[chrom_calc (lat, delta_e, chrom_x, chrom_y, err_flag)] \Newline
