...
{\em Routine Type} & {\em Section} \\ \hline
  ...
  Transfer Maps: Complex Taylor Maps          & \ref{r:ctaylor}         \\ \hline
  Transfer Maps: Taylor Maps                  & \ref{r:taylor}         \\ \hline
  Tracking: Tracking and Closed Orbit         & \ref{r:track}          \\ \hline
...
\item[bend_edge_kick (orb, ele, param, particle_at, mat6)] \Newline 
...
\item[\protect\parbox{6in}{
    em_field_calc (ele, param, s_rel, time, orbit, local_ref_frame, field, \\
...
\item[add_lattice_control_structs (lat, ele, add_at_end)] \Newline 
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} save_null_drift, create_jumbo_slave)} ] \Newline
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} offset, param, include_upstream_end, \\
    \hspace*{1in} include_downstream_end, err_flag, old_slice)} ] \Newline 
...
\item[ele_geometry (floor0, ele, floor, len_scale, treat_as_patch)] \Newline 
...
\item[floor_w_mat_to_angles (w_mat, theta0, theta, phi, psi, floor0)] \Newline 
...
\index[routine]{local_to_floor}
\label{r:local.to.floor}
\item[local_to_floor (floor0, dr, theta, phi, psi) result (floor1)] \Newline 
...
\index[routine]{position_in_global_frame}
\label{r:position.in.global.frame}
\item[position_in_global_frame (local_position, ele, w_mat) result (global_position)] \Newline 
Given a position local to ele, return global floor coordinates

\index[routine]{position_in_local_frame}
\label{r:position.in.local.frame}
\item[position_in_local_frame  (global_position, ele, status, w_mat) result(local_position)] \Newline 
...
\index[routine]{switch_local_positions}
\label{r:switch.local.positions}
\item[switch_local_positions (position0, ele0, ele_try, position1, ele1, ww_mat)] \Newline 
...
\item[switch_attrib_value_name (attrib_name, attrib_value, ele, 
...
\item[lat_ele_locator (loc_str, lat, eles, n_loc, err, above_ubound_is_err)] \Newline 
...
\hspace*{1in} type_control, type_wake, type_floor_coords, \\
...
\hspace*{1in} type_taylor, twiss_out, type_control, type_wake, \\
...
\item[control_bookkeeper (lat, ele, mark_eles_as_stale)] \Newline
...
\label{r:init.ele}
\item[init_ele (ele, key, sub_key, ix_ele, ix_branch, branch)] \Newline
...
\item[set_ele_defaults (ele)] \Newline 
Subroutine to set the defaults for an element of a given type.

\index[routine]{set_on_off}
\label{r:set.on.off}
\item[set_on_off (key, lat, switch, orb, use_ref_orb, ix_branch)] \Newline
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} e_tot_start, p0c_start, ref_time_start, err_flag)} ] \Newline 
...
\label{r:make.mat6}
\item[make_mat6 (ele, param, start_orb, end_orb, end_in, err_flag)] \Newline
...
\item[transfer_matrix_calc (lat, rf_on, xfer_mat, xfer_vec, ix1, ix2, ix_branch, one_turn)] \Newline
...
\item[drift_mat6_calc (mat6, length, ele, param, start, end)] \Newline
...
\item[sol_quad_mat6_calc (ks_in, k1_in, s_len, orb, m, dz_coef)] \Newline
...
\item[multipole_ele_to_ab (ele, param, use_ele_tilt, has_nonzero_pole, a, b)] \Newline
...
\item[multipole_ele_to_kt (ele, param, use_ele_tilt, has_nonzero_pole, knl, tilt)] \Newline
...
\item[\protect\parbox{6in}{super_mrqmin (y, weight, a, \\
  \hspace*{1in} covar, alpha, chisq, funcs, alamda, status, maska)}] \Newline 
...
\item[ele_to_fibre (ele, ptc_fibre, param, use_offsets, integ_order, steps, for_layout)] \Newline
...
\item[real_8_to_taylor (y8, v0, beta0, beta1, bmad_taylor)] \Newline
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} exact_modeling, exact_misalign) }] \Newline
...
\item[taylor_to_real_8 (bmad_taylor, v0, beta0, beta1, y8)] \Newline
...
\index[routine]{make_mat6_custom2}
...
\item[complex_taylor_coef (bmad_complex_taylor, exp)] \Newline 
...
\item[type_complex_taylors (bmad_complex_taylor, max_order)] \Newline 
Subroutine to print in a nice format a Bmad complex_taylor Map at the terminal.

\index[routine]{type2_complex_taylors}
\label{r:type2.complex.taylors}
\item[type2_complex_taylors (bmad_complex_taylor, lines, n_lines, max_order)] \Newline 
...
\item[ele_to_taylor (ele, param, orb0, taylor_map_includes_offsets)] \Newline
...
\item[track_taylor (start_orb, bmad_taylor, end_orb)] \Newline
...
\item[check_aperture_limit (orb, ele, particle_at, param, check_momentum)] \Newline
...
\item[closed_orbit_calc (lat, closed_orb, i_dim, direction, ix_branch, err_flag)] \Newline 
...
\item[dynamic_aperture (lat, orb0, theta_xy, aperture_param, aperture)] \Newline
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} set_multipoles, set_hvkicks, set_z_offset, ds_pos)} ] \Newline
...
\index[routine]{particle_is_moving_backward}
\label{r:particle.is.moving.backward}
\item[particle_is_moving_backward (orbit) result (is_moving_backward)] \Newline 
...
\label{r:track1}
\item[track1 (start_orb, ele, param, end_orb, track, err_flag, ignore_radiation)] \Newline
...
\item[track1_bunch_csr (bunch_start, lat, ele, bunch_end, err, s_start, s_end)] \Newline 
...
\label{r:track.all}
\item[track_all (lat, orbit, ix_branch, track_state, err_flag)] \Newline
...
\label{r:odeint.bmad}
\item[\protect\parbox{6in}{
    odeint_bmad (orb_start, ele, param, orb_end, s1, s2, local_ref_frame, err_flag, track) }] \Newline
...
\item[track1_boris_partial (start, ele, param, s, t, ds, end)] \Newline
Routine to track 1 step using boris tracking. 

\index[routine]{track_a_drift}
\label{r:track.a.drift}
\item[track_a_drift (orb, ele, length)] \Newline
...
\label{r:track1.bmad}
\item[track1_bmad (start_orb, ele, param, end_orb, err_flag)] \Newline
Particle tracking through a single element BMAD_standard style. 

\index[routine]{track1_custom}
\label{r:track1.custom}
\item[track1_custom (start_orb, ele, param, end_orb, track, err_flag)] \Newline
Dummy routine for custom tracking.

\index[routine]{track1_custom2}
\label{r:track1.custom2}
\item[track1_custom2 (start_orb, ele, param, end_orb, track, err_flag)] \Newline
Dummy routine for custom tracking.
...
\item[transfer_map_calc (lat, t_map, ix1, ix2, ix_branch, integrate, one_turn, unit_start) ] \Newline 
...
\item[normal_mode3_calc (mat, tune, B, HV, synchrotron_motion)] \Newline 
...
\label{r:init.wake}
\item[init_wake (wake, n_sr_long, n_sr_trans, n_lr)] \Newline 
Routine to initialize a wake struct.

\index[routine]{lr_wake_apply_kick}
\label{r:lr.wake.apply.kick}
\item[lr_wake_apply_kick (ele, t0_bunch, orbit)] \Newline 
