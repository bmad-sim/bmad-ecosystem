...
{\em Routine Type} & {\em Section} \\ \hline
  ...
  Custom and Hook Routines                    & \ref{r:custom}         \\ \hline
  ...
  Helper Routines: Switch to Name             & \ref{r:switch}         \\ \hline
  ...
  C/C++ Interface                             & \ref{r:c.interface}    \\ \hline
...
\item[bend_edge_kick (ele, param, particle_at, orb, mat6, make_matrix)] \Newline 
...
\item[init_bunch_distribution (ele, param, beam_init, bunch, err_flag)] \Newline 
...
\item[apply_element_edge_kick_hook (orb, s_edge, t_rel, hard_ele, track_ele, param, particle_at, finished)] \Newline 
...
\item[\protect\parbox{6in}{
    em_field_calc (ele, param, s_pos, time, orbit, local_ref_frame, field, \\
    \hspace*{1in} calc_dfield, err_flag, potential) }] \Newline 
...
\item[autoscale_phase_and_amp(ele, param, err_flag, scale_phase, scale_amp, call_bookkeeper)] \Newline 
...
\label{r:lat.reverse}
\item[lat_reverse (lat_in, lat_rev, track_antiparticle, make_mats6)] \Newline 
...
\item[coords_local_curvilinear_to_floor (local_position, ele, in_ele_frame, w_mat) result (global_position)] \Newline 
...
\item[coords_floor_to_local_curvilinear  (global_position, ele, status, w_mat) result(local_position)] \Newline 
...
\item[match_ele_to_mat6 (ele, vec0, mat6, err_flag, twiss_ele)] \Newline 
...
\item[ab_multipole_kick (a, b, n, coord, kx, ky, dk, pole_type)] \Newline 
Routine to put in the kick due to an ab_multipole.

\index[routine]{multipole_kicks}
\label{r:multipole.kicks}
\item[multipole_kicks (knl, tilt, coord, pole_type, ref_orb_offset)] \Newline 
...
\item[multipole_ele_to_ab (ele, use_ele_tilt, has_nonzero_pole, a, b, pole_type)] \Newline
...
\item[multipole_ele_to_kt (ele, use_ele_tilt, has_nonzero_pole, knl, tilt, pole_type)] \Newline
...
\item[multipole_kick (knl, tilt, n, coord, pole_type, ref_orb_offset)] \Newline
Routine to put in the kick due to a multipole. 
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} for_layout, tracking_species, use_hard_edge_drifts) }] \Newline
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} exact_modeling, exact_misalign, init_complex) }] \Newline
...
\item[spinor_to_polar (coord) result (polar)] \Newline 
...
\item[vec_to_polar (vec, polar) result (phase)] \Newline
...
\label{r:spin.omega}
\item[spin_omega (field, coord, ele), result (omega)] \Newline 
...
\item[track1_taylor (start_orb, ele, param, end_orb, taylor_in)] \Newline
...
\item[transfer_map_calc (lat, t_map, err_flag, ix1, ix2, ix_branch, integrate, one_turn, unit_start) ] \Newline 
...
\item[normal_mode3_calc (t6, tunes, B, HV, counter_rotating_z)] \Newline 
...
\item[twiss3_at_start (lat, err_flag)] \Newline 
Routine to propagate the twiss parameters using all three normal modes.
