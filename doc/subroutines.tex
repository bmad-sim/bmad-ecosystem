...
\item[pointer_to_element_at_s (branch, s, choose_max, err_flag, s_eff, position, print_err) result (ele)] \Newline 
...
\item[\protect\parbox{6in}{
  coords_local_curvilinear_to_floor (local_position, ele, in_body_frame, \\
  \hspace*{1in} w_mat, calculate_angles, relative_to_upstream) result (global_position) }] \Newline 
...
\item[\protect\parbox{6in}{
      coords_floor_to_local_curvilinear  (global_position, ele, status, w_mat, relative_to_upstream) \\
...
\item[\protect\parbox{6in}{
    mat6_from_s_to_s (lat, mat6, vec0, s1, s2, ref_orb_in, ref_orb_out, ix_branch, one_turn, \\
...
\item[\protect\parbox{6in}{srdt_lsq_solution(lat, var_indexes, var_names, ls_soln, n_slices_gen_opt, n_slices_sxt_opt,\\
            chrom_set_x_opt, chrom_set_y_opt, weight_in)}] \Newline
