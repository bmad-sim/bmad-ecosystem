...
\item[\protect\parbox{6in}{
  em_field_custom(ele, param, s_rel, time, orb, local_ref_frame, field, calc_dfield, \\
  \hspace*{1in} err_flag) }] \Newline
...
\item[mat_symp_decouple (t0, tol, stat, u, v, ubar, vbar, g, twiss1, twiss2, gamma, type_out)] \Newline
...
\item[\protect\parbox{6in}{
    add_superimpose (lat, super_ele_in, ix_branch, err_flag, super_ele_out, save_null_drift, create_jumbo_slave)} ] \Newline
...
\item[\protect\parbox{6in}{
    ...
    \hspace*{1in} at_upstream_end, at_downstream_end, err_flag, old_slice)} ] \Newline 
...
\item[valid_mat6_calc_method (ele, species, mat6_calc_method, num_valid) result (is_valid)] \Newline 
...
\item[\protect\parbox{6in}{
    ele_compute_ref_energy_and_time (ele, param, e_tot_start, p0c_start, \\
    \hspace*{1in} ref_time_start, err_flag)} ] \Newline 
...
\label{r:photon.init}
\item[photon_init (g_bend_x, g_bend_y, gamma, orbit, E_min, E_max, E_integ_prob)] \Newline 
...
\hspace*{1in} draw_numbers, minor_div, minor_div_max, mirror, \\
\hspace*{1in} number_offset, label_offset, major_tick_len, minor_tick_len, ax_type)}] \Newline
...
\item[\protect\parbox{6in}{
    offset_particle (ele, param, set, coord, set_tilt, set_multipoles, set_hvkicks, set_z_offset, ds_pos)}] \Newline
...
\item[twiss_and_track_at_s (lat, s, ele_at_s, orb, orb_at_s, ix_branch, err, use_last)] \Newline
...
\item[\protect\parbox{6in}{
  ...
  \hspace*{1in} track_downstream_end, orbit_start, orbit_end, ele_start, ele_end, err) }] \Newline 
...
\item[create_uniform_element_slice (ele, param, i_slice, n_slice_tot, sliced_ele, s_start, s_end)] \Newline 
...
\label{r:chrom.calc}
\item[chrom_calc (lat, delta_e, chrom_x, chrom_y, err_flag, low_E_lat, high_E_lat, low_E_orb, high_E_orb, ix_branch)] \Newline
