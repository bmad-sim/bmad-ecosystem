...
\begin{example}
  &params
    ...
    photon_init%scale_initial_field_to_1 = False
...
an absolute value (\sref{ss:x.ray.init})?
...
(\sref{s:photon.descrip}).  If both are set to 0 then a random field is
chosen with unit intensity $E_x^2 + E_y^2 = 1$.

  \item[\vn{photon_init\%energy_distribution}] \Newline
...
(\sref{s:photon.descrip}). Possible settings are:
\begin{example}
  GAUSSIAN
  UNIFORM
\end{example}
...
photons. The longitudinal velocity is always computed to make $v_x^2 +
...
\end{example}
If the parameter \vn{photon_init%scale_initial_field_to_1} is set to
...
\begin{example}
  ...
  undulator
  wiggler
  x_ray_init        ! Example in \sref{ss:x.ray.init}
\end{example}
...
src: x_ray_init, l = 0, x_offset = 0, y_offset = 0, z_offset = 0, 
        tilt = 0, x_pitch = 0, y_pitch = 0
...
Lines 1 through 22 define the charged particle branch which is
...
elements of \vn{c_line} are given in lines 40 and 41: Two slits, two
...
for ix in range(-n_max, n_max+1):
  ...
  subprocess.call('rm *digested*', shell=True)
  subprocess.call('/home/dcs16/linux_lib/production/bin/lux', shell=True)
...
The heart of the script is the loop that begins on line 20 and ends on
line 63. Each iteration of this loop changes one variable in the
lattice file (lines 27 to 33), runs \lux (line 39), and then extracts
the pertanent data (lines 43 to 47). Finally at the end of the loop
the data is appended to the output file in lines 51 through 63.
...
first readinging in a lattice file called \vn{template.bmad} (lines (7
to 9). In lines 29 to 31 the script transfers, line-by-line, the lines
...
\begin{example}
  var_to_change = 0
\end{example} 
and modify this (in line 30) to read:
\begin{example}
  var_to_change = <var>
\end{example}
where \vn{<var>} is the value of the \vn{var} variable in the
script. The value of \vn{var} is set in line 24 and ranges from
...
\end{example}
When the script (see line 45) finds a line in \vn{det.pix} that has
...
\vn{intensity_y_norm} and \vn{intensity_norm} in lines 46 and 47.
