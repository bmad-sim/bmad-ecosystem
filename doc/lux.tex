...
\begin{example}
  &params
    ...
    lux_param%histogram_out_file = ''              ! Histogram of photon x', y', or angle
    ...
    lux_param%histogram_variable = 'x_angle'       ! "x_angle", "y_angle", or "energy"
...
evaluated at the detector element.  
...
\end{example}
All averages (\vn{E_ave}, \vn{x_ang_ave}, and \vn{y_ang_ave}) are
...
\vn{det_pix_out_file} with ``.x'' and ``.y'' suffix indicating
...
\end{example}
Photons that fall on the detector are binned according to the photon's
...
%------------------------------------------------------------------
\section{Histogram Output}
\label{s:histo}

A histogram of the photon velocity angle or energy distribution can
created by setting \vn{lux_param%histogram_out_file} to a non-blank
string. 
...
\end{example}
All photons hitting a detector pixel are used for the histogram. 
...
first reading in a lattice file called \vn{template.bmad} (lines (8
