...
parallel compute version using MPI whose executable is called \vn{lux_mpi}.

%------------------------------------------------------------------
\section{Building \lux} 
\label{s:build}

Chances are that a \lux executable is already built for you to use. \lux is a standard program that
comes with Bmad Distributions (see the online documentation on the Bmad build system for more
details). If the mpi version of \lux has not been built, or if you want to recompile from scratch,
use the \vn{mk_mpi} command in the \vn{lux} directory to build both the multi and non-multi
processor versions. [Ask your local Bmad guru where the \vn{lux} directory resides. You may need to
make a local copy of the \vn{lux} directory.]
...
the random number generator. If set to 0, the system clock will be used so that the
output results will vary from run to run. Not used if the \vn{random_engine} is set to \vn{quasi}.
...
The first data file has the \vn{det_pix_out_file} parameter as the
file name. This file contains pixel-by-pixel information.  The
intensity of a pixel is the accumulated intensity of the photons
...
\end{example}
Here \vn{lux} is the root directory for the \lux program (\sref{s:build}).

The script needs Python3 to run. You can invoke the script using the command
\begin{example}
  <path-to-lux-root-dir>/plot/det_pix_plot.py <det_pix_out_file>
\end{example}
where \vn{<det_pix_out_file>} is the actual name of the \vn{det_pix_out_file}.
If the default version of python is not python3 then the following may work
\begin{example}
  python3 <path-to-lux-root-dir>/plot/det_pix_plot.py <det_pix_out_file>
\end{example}
