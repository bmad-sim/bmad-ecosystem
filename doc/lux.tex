...
\begin{example}
  &params
    ...
    lux_param%ix_ele_photon1_file = -1
...
Coefficient used to normalize the photon intensity with (\sref{s:det}).
  ...
  \item[\vn{lux_param\%ix_ele_photon1}] \Newline
...
Used with \vn{lux_param%simulation_type} = 'NORMAL'.  
...
(\sref{s:photon.start}). Possible settings are:
\begin{example}
  GAUSSIAN
  UNIFORM
\end{example}
...
Used with \vn{lux_param%simulation_type} = 'NORMAL'.  Sets the
transverse $(v_x/c, v_y/c)$ velocity spectrum of emitted photons. The
...
bending magnets, wigglers, or undulators. There must be a
\vn{photon_fork} element in the lattice that branches to the x-ray
line. 
...
beginning[beta_a]  =  2.2611   ! Twiss at "exit" end with alpha reversed.
...
bend_line: line = (b05w)
frk: photon_fork, to_line = c_line, superimpose, 
        ref = b05w, ref_origin = beginning, offset =  23.6980 - 23.3814

! The origin element is used to set the origin of the global coordinate system at the
! fork element.

origin: fiducial, superimpose, ref = frk   
...
det: detector, x_limit = 1, y_limit = 1, surface = {grid = {x_bounds = (-1, 1), y_bounds = (-1, 1), dr = (1, 1)}}

c_line: line = (drift1, white_beam_slit, drift2, cryst1, drift3, cryst2, drift4, mono_slit, det)
...
! drift3 length is set so that crystal2 is positioned 75 mm vertically from the beam plane
...
\end{example}
If the parameter \vn{photon_init%normalize_field} is set to True, \lux
will scale the field so that the \vn{unnormalized} intensity
(\sref{s:photon.descrip}) of each photon is 1 at the start of
tracking. If both field components are set to zero, the
polarization will be random. As photons
