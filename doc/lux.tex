...
\begin{example}
  &params
    ...
    photon_def%source_type = 'ROCKING_CURVE'        ! Or 'INSERTION_DEVICE', 'X_RAY_INIT'
    photon_def%tracking_mode = 'INCOHERENT'         ! Or 'COHERENT'
    photon_def%energy_distribution = 'GAUSSIAN'     ! Or 'UNIFORM'. 
    photon_def%spatial_distribution = 'GAUSSIAN'    ! Or 'UNIFORM'. 
    photon_def%velocity_distribution = 'GAUSSIAN'   ! Or 'SHPERICAL', 'UNIFORM'. 
    photon_def%sig_x = 0
    photon_def%sig_y = 0
    photon_def%sig_z = 0
    photon_def%sig_vx = 0
    photon_def%sig_vy = 0
    photon_def%sig_E = 0                            ! Initial photon energy width (eV).
    photon_def%dE_center = 0                        ! Initial photon energy shift (eV).
    photon_def%dE_relative_to_ref = True            ! dE_center relative to reference E?
    photon_def%emit_x = 0                          
    photon_def%emit_Y = 0 
    photon_def%e_field_x = 0                        ! Polarization. x & y = 0 -> random
    photon_def%e_field_y = 0
    photon_def%normalize_field = False
...
Section~\sref{s:lat.file} and the Bmad manual for more details.
  ...
  \item[\vn{photon_def\%source_type}] \Newline
Type of source used for photon generation (\sref{s:photon.start}).
Possible settings are:
\begin{example}
  INSERTION_DEVICE       ! Bend or wiggler source
  ROCKING_CURVE         ! 
  X_RAY_INIT            ! 
\end{example}
The \vn{INSERTION_DEVICE} setting indicates that the source is a
\vn{bend} or or \vn{wiggler} element. 

The \vn{X_RAY_INIT} setting indicates that the source is an
\vn{X_RAY_INIT} element. 
...
will be done.
  ...
  \item[\vn{photon_def\%tracking_mode}] \Newline
...
\vn{diffraction_plate} element.
  ...
  \item[\vn{photon_def\%energy_spectrum}] \Newline
Sets the type of energy spectrum (\sref{s:photon.start}). 
Used with \vn{phton_def%source_type} = 'X_RAY_INIT'.
Possible settings are:
\begin{example}
  GAUSSIAN
  UNIFORM
...
set by \vn{photon_def%sig_E}. The \vn{'UNIFORM'} setting gives a flat
distribution in the range
\begin{example}
  [-photon_def%sig_E, photon_def%sig_E]
\end{example}

  \item[\vn{photon_def\%spatial_distribution}] \Newline
Sets spacial $(x, y, z)$ spectrum (\sref{s:photon.start}).
Used with \vn{phton_def%source_type} = 'X_RAY_INIT'.
Possible settings are:
\begin{example}
  GAUSSIAN
...
\begin{example}
  photon_def%sig_x     for x distribution
  photon_def%sig_y     for y distribution
  photon_def%sig_z     for z distribution
\end{example}
The \vn{'UNIFORM'} setting gives a flat
distribution in the range $[-\sigma, \sigma]$.


  \item[\vn{photon_def\%velocity_distribution}] \Newline
Sets the transverse $(v_x/c, v_y/c)$ velocity spectrum.
Used with \vn{phton_def%source_type} = 'X_RAY_INIT'.
The longitudinal velocity is always computed
to make $v_x^2 + v_y^2 + v_z^2 = c^2$
Possible settings are:
\begin{example}
  GAUSSIAN
  SPHERICAL
...
$\sigma$ where $sigma$ is 
\begin{example}
  photon_def%sig_vx     for vx/c distribution
  photon_def%sig_vy     for vy/c distribution
\end{example}
...
The \vn{SPHERICAL} setting gives flat distribution in all directions.
  ...
  \item[\vn{photon_def\%dE_center}] \Newline
Average initial photon energy in eV. If
\vn{photon_def%dE_relative_to_ref} is set to True (default)
then \vn{photon_def%dE_center} will be relative to the reference energy
(\sref{s:photon.start}).

  \item[\vn{photon_def\%sig_x, photon_def\%sig_y, photon_def\%sig_z}] \Newline
Width in $x$, $y$ and $z$ directions. See
\vn{photon_def%spatial_distribution} for more details.

  \item[\vn{photon_def\%sig_vx, photon_def\%sig_vy}] \Newline
Width in $v_x/c$ and $v_y/c$ directions. See
\vn{photon_def%velocity_distribution} for more details.

  \item[\vn{photon_def\%sig_E}] \Newline
Energy width in eV. See \vn{photon_def%energy_distribution} for more
details.

  \item[\vn{photon_def\%dE_relative_to_ref}] \Newline
Is \vn{photon_def%dE_center} with respect to the reference energy or
an absolute value (\sref{s:photon.start})?

  \item[\vn{photon_def\%emit_x, photon_def\%emit_y}] \Newline
$x$ and $y$ emittances. 
Used with \vn{phton_def%source_type} = 'INSERTION_DEVICE'.

  \item[\vn{photon_def\%e_field_x}] \Newline
Electric field component of initial photons in the x-plane (\sref{s:photon.start}).
  \item[\vn{photon_def\%e_field_y}] \Newline
...
blank file name will prevent a file being generated.
  ...
  \item[\vn{photon_def\%stop_total_intensity}] \Newline
Cumulative unnormalized intensity at the detector to stop at. (\sref{s:det}).
  \item[\vn{photon_def\%stop_num_photons}] \Newline
Maximum number of photons to track (\sref{s:det}).
  \item[\vn{photon_def\%dx_det_pixel}] \Newline
Detector pixel width (\sref{s:det}).
  \item[\vn{photon_def\%dy_det_pixel}] \Newline
Detector pixel height (\sref{s:det}).
  \item[\vn{photon_def\%intensity_min_photon1_cutoff}] \Newline
...
parameter \vn{photon_def%source_type}. This parameter may have the following
values
\begin{example}
  'SPHERICAL'
  'PLANAR'
...
\begin{example}
  photon_def%sig_E
  photon_def%dE_center
  photon_def%dE_relative_to_ref
  photon_def%energy_distribution
  photon_def%spatial_distribution     ! (x, y) spacial distribution
  photon_def%velocity_distribution     ! (x, y) spacial distribution
\end{example}
If \vn{photon_def%dE_relative_to_ref} is set to True, The average
starting photon energy is
\begin{example}
  E_average = photon_def%dE_center + beginning[e_tot]
\end{example}
where the reference energy \vn{beginning[e_tot]} is set in the lattice file. 
If \vn{photon_def%dE_relative_to_ref} is set to False, The average
starting photon energy is simply \vn{photon_def%dE_center}.
...
\begin{example}
  photon_def%e_field_x     ! Polarization along the x-axis
  photon_def%e_field_y     ! polarization along the y-axis
\end{example}
