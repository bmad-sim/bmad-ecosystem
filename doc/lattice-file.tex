...
name to differing values since this can lead problems later on. For example,
consider this in a lattice file named, say, \vn{lat.bmad}:
\begin{example}
  q1: quadrupole, ...
  a: line = (..., q1, ...)
  b: line = (..., q1, ...)
...
be used in element names (\sref{s:ele.names}). Examples:
\begin{example}
...
syntax as discussed in Section \sref{s:ele.names}. Example:
\begin{example}
  expand_lattice              ! Expand the lattice.
...
Inline calls can be used to call binary files (useful for speeding up lattice parsing).
...
speeding up lattice parsing. These binary files must have a \vn{.bin} suffix. The attributes
...
statement must come after lattice expansion (\sref{s:expand}).  The syntax of the \vn{slice_lattice}
statement is
\begin{example}
  slice_lattice <element-list>
\end{example}
where \vn{<element-list>} is a list of elements (\sref{s:ele.names}). For example
\begin{example}
...
The \vn{slice_lattice} is useful when analysis of only part of the lattice is desired and the
analysis of the entire lattice can take a significant amount of time. For a lattice with an open
geometry, if elements are discarded from the beginning of the lattice, the Twiss parameters and
reference energy may have to be adjusted. Example:
\begin{example}
  slice_lattice q1##2:357
  beginning[beta_a] = 10.3256  ! Reset the beginning Twiss
  beginning[p0c] = 1e7         ! Reset the beginning ref energy
  beginning[geometry] = open   ! Change the geometry.
  ... etc ...
\end{example}
