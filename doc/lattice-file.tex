...
\end{example}
The \vn{species} function turns a string (``He++'' in the above example) into a particle
ID (internally to \bmad, this is an integer). The use of \vn{species} is only needed when
setting a variable to a species ID (as the set of \vn{my_particle} in the above example).
...
\begin{itemize}
\item 
If there is an \vn{expand_lattice} statement (\sref{s:expand.lat}), all \vn{line}s
...
defined. This is an error. As an extension of this rule notice that the set of element parameters
using wild card characters will only set those parameters that have been already defined. For
example:
\begin{example}
  crystal::*[b_param] = 0.2
  c5: crystal
...
the lattice file but an explicit \vn{expand_lattice} statement in a lattice file will cause
