...
The \vn{ran_gauss(cut)} function with an argument truncates the distribution so that no values are returnd
...
If an element is used multiple times in a lattice, and if \vn{ran} or \vn{gauss_ran} is used to set
...
The \vn{calc_reference_orbit} statement is used before a \vn{combine_elements} statement
(\sref{s:combine.eles}) to signal that the maps of the Taylor elements produced by the
\vn{combine_elements} statement are computed from the standard reference orbit and not the zero
orbit.
...
The \vn{combine_elements} statement is used to merge groups of consecutive elements into single
...
The \vn{combine_elements} statement must come after an \vn{expand_lattice} command
(\sref{s:expand}).

If there is a \vn{calc_reference_orbit} statement before the \vn{combine_elements} statement, the
...
Also see the \vn{start_branch_at} statement (\sref{s:start.branch}).

%--------------------------------------------------------------------------
\section{Start_Branch_At}
...
tracking part of the lattice (\sref{s:lord.slave}). The \vn{BEGGINING} element of the branch
(\sref{s:use}) and any lord elements are unaffected. The \vn{start_branch_at} statement must come
after lattice expansion (\sref{s:expand}). The syntax of the \vn{start_branch_at} statement is
\begin{example}
  start_branch_at <lattice-element>
\end{example}
where \vn{<lattice-element>} is the name or index of a lattice element to be moved to the start
...
\vn{Q3} element is the first element in the branch. Thus, if the lattice branch orignally consisted
of the elements
\begin{example}
  Q1,  Q2,  Q3,  Q4,  Q5,  Q6
\end{example}
...
\begin{example}
  Q3,  Q4,  Q5,  Q6, Q1,  Q2
\end{example}
Elements that orignally come before the new starting point are always wrapped around to the end of
the branch. 
