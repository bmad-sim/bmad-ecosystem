...
\label{s:mad.xsif}
\index{MAD lattice files}
\index{XSIF lattice files}
...
Section~(\sref{s:xsif}).  Throughout this manual, unless explicitly
...
name, \vn{branch_id} is a branch name or index (\sref{s:branch}),
...
branching (\sref{s:branch}) is used, to differentiate elements that
...
For branch lines (\sref{s:branch}), the full ``branch
...
All \vn{line}s (\sref{s:line.wo.arg}), \vn{list}s
