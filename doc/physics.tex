...
\Begineq
  \sigmab(i,j) \equiv \sigma(i,j) - \frac{\sigma(i,6) \, \sigma(j,6)}{\sigma(6,6)}
\Endeq
and in terms of the betatron part the emittance is
\Begineq
  \epsilon_x^2 = \sigmab(1,1) \, \sigmab(2,2) - \sigmab^2(1,2)
\Endeq
...
\Begineq
  ...
  \end{pmatrix} = 
  \begin{pmatrix}
    \sigmab(1,1) & \sigmab(1,2) \\
    \sigmab(1,2) & \sigmab(2,2) 
  \end{pmatrix}
\Endeq
...
equation
\Begineq
  \nabla^2 \bfE_j + k_j^2 \, \bfE_j = 0
  \label{bke}
\Endeq
where $k_j = \omega_j/c$ with $\omega_j$ being the mode frequency.
...
\begin{align}
  E_\rho(\bfr) &= \sum_{n=-N/2+1}^{N/2} -e^{i \, n \, z / N \, dz} \, 
    e_0(n) \, \frac{i \, k_{zn} \, I_1(\kappa_n \, \rho)}{\kappa_n} \CRNO
  E_\phi(\bfr) &= 0 \\
  E_z(\bfr) &= \sum_{n=-N/2+1}^{N/2}e^{i \, n \, z / N \, dz} \, 
    e_0(n) \, I_0(\kappa_n \, \rho) \nonumber
\end{align}
The $e_0$ coefficients are obtained from the equation
\begin{align}
  e_0(n) &= \frac{1}{I_0(\kappa_n \, R)} \, \frac{1}{N} \, \sum_{p=0}^{N-1}
    e^{-i \, n \, p / N} \, E_{zc}(R, p \, dz) \CRNO
  &\equiv \frac{\wt E_{zc}(R, n)}{I_0(\kappa_n \, R)} 
\end{align}
where $I_m$ is a modified Bessel function and $\kappa_n$ is given by
\Begineq
  \kappa_n = \sqrt{k_{zn}^2 - k_j^2} = 
  \begin{cases}
    \sqrt{k_{zn}^2 - k_j^2} & |k_{zn}| > k_j \\
    -i \, \sqrt{k_j^2 - k_{zn}^2} & k_j > |k_{zn}|
  \end{cases}
\Endeq
with
\Begineq
...
the distance between points. When

The non-accelerating $m = 0$ mode with $\phi_0 = -\pi/2$ has an
electric field in the form
\begin{align}
  E_\rho(\bfr) &= E_z(\bfr) = 0 \CRNO
  E_\phi(\bfr) &= \sum_{n=-N/2+1}^{N/2}e^{i \, n \, z / N \, dz} \, 
    b_0(n) \, I_1(\kappa_n \, \rho)
\end{align}
where
\Begineq
  b_0(n) = \frac{\wt E_{\phi s}(R, n)}{I_1(\kappa_n \, R)} 
\Endeq
...
\begin{align}
  E_\rho(\bfr) &= \sum_{n=-N/2+1}^{N/2}e^{i \, n \, z / N \, dz} \, 
    \left[ 
    -e_m(n) \, \frac{i \, k_{zn} \, I_{m+1}(\kappa_n \, \rho)}{\kappa_n} +
    b_m(n) \, \frac{I_m(\kappa_n \, \rho)}{\kappa_n \, \rho}
    \right]
    \cos(m \, (\phi - \phi_0)) \CRNO
  E_\phi(\bfr) &= \sum_{n=-N/2+1}^{N/2}e^{i \, n \, z / N \, dz} \, 
    \left[
    -e_m(n) \, \frac{i \, k_{zn} \, I_{m+1}(\kappa_n \, \rho)}{\kappa_n} \, + \right. \\
  & \left. \qquad \qquad \qquad \qquad \qquad \qquad
    b_m(n) \, \left( \frac{I_m(\kappa_n \, \rho)}{\kappa_n \, \rho} - 
    \frac{1}{m} \, I_{m-1}(\kappa_n \, \rho) \right)
    \right] 
    \sin(m \, (\phi - \phi_0)) \CRNO
  E_z(\bfr) &= \sum_{n=-N/2+1}^{N/2}e^{i \, n \, z / N \, dz} \, 
    e_m(n) \, I_m(\kappa_n \, \rho) \cos(m \, (\phi - \phi_0)) \nonumber
\end{align}
with
\begin{align}
  e_m(n) &= \frac{\wt E_{zc}(R, n)}{I_m(\kappa_n \, R)} \CRNO
  b_m(n) &= \frac{1}{I_m(\kappa_n \, R)} \left[
    \kappa_n \, R \, \wt E_{\rho c} + i \, k \, R \, e_m(n) \, I_{m+1}(\kappa_n \, R)
    \right]
\end{align}
