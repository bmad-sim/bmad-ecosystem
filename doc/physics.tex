...
\textit{\large [Bunch initialization developed by Michael Saelim]}
...
\Begineq
  W_m(z) = c \, \left( \frac{R}{Q} \right)_m \,\,
  e^{k z/ 2 Q} \, \sin (k \, z)
  \label{wcrq}
\Endeq
where the mode wave number $k$ is related to the mode frequency $\omega$ by
\Begineq
  k = \omega / c
\Endeq
Here $z$ is the distance behind the particle generating the wake. $z$
is negative for all particles affected by the wake so $W_m(z)$ is a
decaying exponential as expected. The mode strength $(R/Q)_m$ has
units of Volts/meter$^{2m}$.
...
\begin{align}
  -\Delta p_x + i\Delta p_y &= C \, I_m \, W_m(z) \, 
    m \, r^{m-1} \, e^{-i m \theta_w} \, e^{i (m-1) \theta} 
    \label{ppcimr} \\
  \Delta p_z &= C \, I_m \, W'_m(z) \, r^m \, \cos [m(\theta - \theta_w)]
    \label{pciwr}
\end{align}
