...
the distance between points. When $\kappa_n$ is imaginary, $I_n(\kappa_n \, \rho)$
...
\begin{align}
  ...
  b_m(n) &= \frac{1}{I_m(\kappa_n \, R)} \left[
    \kappa_n \, R \, \wt E_{\rho c} + i \, k_{nz} \, R \, e_m(n) \, I_{m+1}(\kappa_n \, R)
    \right]
\end{align}
Notice that the definitions of $e_m$ and $b_m$ here differ by a factor
of $i^m$ from the definition used by Abell when $\kappa_n$ is imaginary.
