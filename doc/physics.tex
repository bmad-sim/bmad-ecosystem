...
\begin{figure}[!b]
\centering
\includegraphics[height=8.4cm]{csr.eps}
...
to be differentiated from the indirect \vn{wakefield} effects which are
...
some retarted time $t' \ne t$. A SC calculation, on the other hand, assumes
that the field produced by a prticle at time $t$ can be can be computed
...
\bmad simulates coherent synchrotron radiation (CSR) using the formalism
developed by Sagan\cite{b:csr}. 
This formalism divides the total kick
received by a particle due to another particle into two parts: One part is
called the \vn{Coulomb} kick and the other component is the \vn{coherent
synchrotron radiation} (CSR) kick. By definition, the Coulomb component is
the kick that would result if both particles were travelling in a straight
line. The CSR component is what is left when the Coulomb kick is subtracted
off from the total kick. Generally, the Coulomb kick is negligible compared
to the CSR kick at large enough particle energies.
...
kick, the density is actually calculated for a set of bins that have
been shifted by 1/2 the bin width. This gives the density at the edges
of the original set of bins. The density is cosidered to vary linearly
between the computed density points. For a description of the
parameters that affect the CSR calculation see
Section~\sref{s:csr_params}.
