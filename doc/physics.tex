...
number of modes. This parameterization essentially follows
Abell\cite{b:rf.abell}. The electric field is written in the form
\Begineq
  \bfE(\bfr) = \sum_{j=1}^M \, \bfE_j(\bfr)
\Endeq
where $M$ is the number of modes. Each mode satisfies the vector Helmohltz
equation
\Begineq
  \nabla^2 \bfE_j + k_t^2 \, \bfE_j = 0
\Endeq
where $k_t = \omega/c$ with $\omega$ being the mode frequency.

In cylindrical coordinates, it is
assumed that for each mode $j$, the field can be written in the form
\Begineq
  E_\rho(R, \phi, z) = \wt E_\rho(R, z) \, \cos(m \, \phi - \phi_0)
  E_\phi(R, \phi, z) = \wt E_\phi(R, z) \, \sin(m \, \phi - \phi_0)
  E_z(R, \phi, z)    = \wt E_z(R, z)    \, \cos(m \, \phi - \phi_0)
\Endeq
where in this, and subsequent equations, the subscript $j$ has been
dropped.  $\wt E_\rho$, $\wt E_\phi$, $\wt E_z$, $m$, and $\phi_0$ are
obtained from data of $\bfE_j$ on the surface of a cylinder of radius
$R$.
...
\begin{align}
  E_\rho(\bfr) &= -\sum_{n=-N/2+1}^{N/2}\exp{i \, n \, z / dz} \, 
    e_0(n) \, \frac{i \, k_z(n) \, I_1(\kappa(n) \, \rho)}{\kappa(n)} \CRNO
  E_z(\bfr) &= \sum_{n=-N/2+1}^{N/2}\exp{i \, n \, z / dz} \, 
    e_0(n) \, I_0(\kappa(n) \, \rho) 
\end{align}
The $e_0$ coefficients are obtained from the equation
\Begineq
  e_0(n) = \frac{1}{N \, I_0(\kappa(n)) \, R} \sum_{p=0}^{N-1}
    \exp{i \, n \, p / N} \, \wt E_z(R, p \, dz)
\Endeq
where $I_n$ is a modified Bessel function and $\kappa(n)$ is given by
\Begineq
  \kappa(n) = \begin{cases}
    \sqrt{k_z^2(n) - k_t^2} & |k_z(n)| > k_t \\
    -i \, \sqrt{k_t^2 - k_z^2(n)} & k_t > |k_z(n)|
  \end{cases}
\Endeq
with
\Begineq
  k_z(n) = \frac{2 \pi \, n}{N \, dz}
\End
$N$ is the number of points where $\wt E_z$ is evaluated and $dz$ is
the distance between points.

The non-accelerating $m = 0$ mode with $\phi_0 = -pi/2$ has an
electric field in the form
\begin{align}
  E_\phi(\bfr) &= \sum_{n=-N/2+1}^{N/2}\exp{i \, n \, z / dz} \, 
    b_0(n) \, I_1(\kappa(n) \, \rho)
\end{align}
where
\Begineq
  b_0(n) = \frac{1}{N \, I_1(\kappa(n)) \, R} \sum_{p=0}^{N-1}
    \exp{i \, n \, p / N} \, \wt E_\phi(R, p \, dz)
\Endeq
...
\begin{align}
  E_\rho(\bfr) &= \sum_{n=-N/2+1}^{N/2}\exp{i \, n \, z / dz} \, 
    \left[ 
    -e_m(n) \, \frac{i \, k_z(n) \, I_1(\kappa(n) \, \rho)}{\kappa(n)} +
    b_m(n) \, \frac{I_m(\kappa(n))}{\kappa(n) \, \rho}
    \right]
    \cos(m \, \phi - \phi_0) \CRNO
  E_\phi(\bfr) &= \sum_{n=-N/2+1}^{N/2}\exp{i \, n \, z / dz} \, 
    \left[
    -e_m(n) \, \frac{i \, k_z(n) \, I_{m+1}(\kappa(n) \, rho)}{\kappa(n)} + 
    b_m(n) \, \left( \frac{I_m(\kappa(n) \, \rho}{\kappa(n) \, \rho} - 
    \frac{1}{m} \, I_{m-1}(\kappa(n) \, \rho)
    \right] 
    b_0(n) \, I_1(\kappa(n) \, \rho) \sin(m \, \phi - \phi_0) \\
  E_z(\bfr) &= \sum_{n=-N/2+1}^{N/2}\exp{i \, n \, z / dz} \, 
    e_m(n) \, I_m(\kappa(n) \, \rho) \cos(m \, \phi - \phi_0) \nonumber
\end{align}
with
\Begineq
  e_m(n) = \frac{1}{N \, I_m(\kappa(n)) \, R} \sum_{p=0}^{N-1}
    \exp{i \, n \, p / N} \, \wt E_z(R, p \, dz)
\Endeq
