...
\index{dispersion}
In the general case the curvature vector $\Bf G = (G_x, G_y)$, which
...
plane. $\Bf G$ has a magnitude $|\Bg G| = 1/\rho$ and a positive $G_y$
...
local referece frame (\sref{s:ref}) by\cite{b:spin}
\Begineq
  \frac{\mathrm{d}}{\mathrm{d}s} \Bf S = \left\{ \frac{(1+\Bf r_t \cdot \Bf G) \, p}{v \, p_s} \, 
  \pmb\Omega}_{BMT} (\mathbf{r,p},s) - \Bf G \cross \zhat  \right\} \times \mathbf{S}
\Endeq
where $\Bf G$ is the curvature function which
points away from the center of curvature of the particle's orbit (see
Figure~\ref{f:local.coords}), $\Bf r_t = (x, y)$ are the transverse coordinates, $p$ is the momentum,
$p_s$ is the longitudinal momentum, $\zhat$ is the unit vector in the $z$ direction, and 
\Begineq
...
frame by the Thomas-Bargmann-Michel-Telegdi (T-BMT) equation,   
