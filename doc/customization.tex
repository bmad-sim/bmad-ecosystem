...
Creating a custom version of \tao involves the following steps:
  \begin{enumerate}
  \item
Create a "root" directory for the custom code you develop that
will be linked with \tao. For the purposes of this discussion, 
this directory will be called \vn{ROOT}. If you have a "local" copy of
the \vn{tao} directory which you are linking to, the \vn{ROOT} directory
should be in the same directory that the \vn{tao} directory is in.
Otherwise, the \vn{ROOT} directory can be located anywhere.
  \item
Put your custom code in the \vn{ROOT} directory. [Actually the
code can be put in any subdirectory of \vn{ROOT} but, to simplify
the discussion, it is assumed here that the code is in the
\vn{ROOT} directory.]
  \item
Copy the program file \vn{tao/program/tao_program.f90} 
to the \vn{ROOT} directory.
  \item
Copy the file \vn{tao/cmake.tao} file to the \vn{ROOT} directory.
To avoid confusion, rename this file to \vn{cmake.XXX} where \vn{XXX}
...
to
  set (EXENAME ZZZ)
\end{example}
where \vn{ZZZ} is the name of the executable file to be created.
Additionally, change
\begin{example}
...
\end{example}
  \item
Create a file in the \vn{ROOT} directory called \vn{CMakeLists.txt}
which contain the following two lines:
\begin{example}
  cmake_minimum_required(VERSION \$ENV\{ACC_CMAKE_VERSION\})
  set (EXE_SPECS cmake.XXX)
\end{example}
Here \vn{cmake.XXX} is is the name of the file copied from 
\vn{tao/cmake.tao} above.
  \end{enumerate}

The custom version of \tao can now be compiled using 
the \vn{mk} and \vn{mkd} commands.
