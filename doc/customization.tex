...
hook routine and \vn{found} should be set to \vn{False} for all other datums.
...
following changes the plot initialization file:
\begin{example}
  s%com%plot_file = 'HOOK::/nfs/acc/user/dcs16/my_plot_init.tao'
\end{example}
Normally something like \vn{s%com%plot_file} stores a file name that is set
on the command line when \tao is invoked. And normally a file name specified on the command line
will supersede a file name specified in the \tao root startup file. If it is desired for a file
specified in the \tao root startup file to supersede the \vn{s%com%...} file, a prefix of
\vn{"HOOK::"} for the \vn{s%com%...} name will tell \tao to switch the precedence order.

Note: The \tao startup command line will be parsed after \vn{tao_hook_parse_command_args} is parsed
so a file specified on the \tao startup command line will always supersede a file name set in this
routine.
...
\cmd{case select} construct. Also add the necessary type declarations. See the 
routine \vn{tao_evaluate_a_datum} as an example.
\begin{example}
