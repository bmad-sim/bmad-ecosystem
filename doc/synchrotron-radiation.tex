...
\Eq{sxx} does not have a unique solution, what matters here is that $\Cal{S}_{21} \, \Bf\Xi$ gives
...
The synchrotron radiation integrals were the original method used in \bmad to compute emittances,
the energy spread, etc. Currently, the use of radiation integrals is being retired in favor of
calculating emittances, etc. via the 6D damped and stochastic transport matrices
(\sref{s:map.rad}). Using these matrices has a number of advantages.
\begin{itemize}
%
\item
