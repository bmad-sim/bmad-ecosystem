...
The 1-turn damping decrement $\alpha$ for each mode $a$, $b$, and $c$ of oscillation can be
calculated from the eigen values $\lambda_i$ (cf. \Eq{lleit}) of the one-turn matrix with damping via
\begin{equation}
...
matrix will scale with the damping decrement (\Eq{allall}).  For most practical cases this is not an
