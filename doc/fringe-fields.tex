...
The dipole soft edge fringe model is adapted from SAD\cite{b:sad}. This model is only used for
\vn{sbend}, \vn{rbend}, and \vn{sad_mult} elements. For \vn{sbend} and \vn{rbend}
...
The SAD dipole soft edge map is ``incomplete'' and for a realistic fringe map this map must be
combined with a ``hard edge'' map (\sref{s:fringe}).
