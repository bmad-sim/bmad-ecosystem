...
\end{example}
The \vn{<beam_input_file>} is for reading in data generated from beam
tracking (\sref{s:command_line}). The \vn{<tao_input_file>} is the
root initialization file that \tao reads to start \tao's
initialization process. If not present, \vn{<tao_input_file>} defaults
to \vn{tao.init}. The \vn{<lattice_file>} is used to override the
lattice file specified in the root initialization file. See
section~\sref{s:command_line} for more details. Example:
\begin{example}
