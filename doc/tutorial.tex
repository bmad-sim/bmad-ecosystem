...
called \vn{Subversion}. Every so often (typically about once a week) a copy
...
set the environmental variables to point to the devel release, Follow the
directions on the web documentation at:
\begin{example}
  http://www.lepp.cornell.edu/~dcs/bmad
\end{example}
And click on the \vn{``Local Setup''} link.

After setting up the environmental variables \tao is run with the command
\begin{example}
  \$CESR_EXE/tao
\end{example}
This will run the \vn{devel} version of \tao.  If there has been a bug
fix that you need which is in the Subversion repository but not yet in
the \vn{devel} release, you need to create and use a local copy of
\tao. See the web documentation to checkout a copy of \tao from the
Subversion repository. The drawback of having a local copy of \tao is
that you will then be responsible for updating your local copy as
needed when updates are made in the repository.

Let \cs{Root} be the name of the root directory for your local copy of Tao.
The checked out copy of \tao will be put in a directory
\cs{ROOT/tao}. Basically the only time a local copy of \tao is needed
is when there have been changes made to Subversion that are needed that
...
\tao. To do this use the command \cmd{'svn update'} in the
