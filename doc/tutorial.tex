...
\begin{example}
  ...
  OBJ\_SRC\_DIRS := ./
\end{example}
This tells gmake to use the tao library (\cmd{../tao/code}) and then to compile
all of the hook files, including the main program file (\cmd{my_tao_cl.f90}) in
to object files. Code in object files always overide similarly named code in
...
\cmd{my_tao} directory with object files.
...
We are now ready to compile and then run the program.
\begin{example}
  cd ROOT/my_tao
  gmake
  ../bin/my_tao_cl
