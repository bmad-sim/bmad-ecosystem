...
There are 8 ``hook'' files located in the \cmd{ROOT/tao/hook} directory. These are
...
custom \tao programs using the same vanilla \tao library.
...
\begin{verbatim}
  ...
  case ('emittance:y')
    ...
    datum_value = (( ele%y%gamma * orb(ix1)%vec(3)**2 + &
		     2 * ele%y%alpha * orb(:)%vec(3) * orb(ix1)%vec(4) + &
