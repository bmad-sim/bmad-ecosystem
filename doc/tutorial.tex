...
\index{CVS}\tao is available in the \cesr CVS area. If at Wilson Laboratory, checkout a
copy with `\cmd{cvs co tao'} in the directory from where you want to run \tao,
hereto referred to as \vn{ROOT}. If you don't have \cesr CVS write permission
then type `\cmd{cesrcvs co tao}'. This will check out a copy but will not allow
...
Once \tao has compiled go to the subdirectory \cs{ROOT/tao/program} and type
...
\end{example}
The \vn{**Limited**} displayed in red on the bottom plots tells us that there are data
...
\vn{x} and \vn{y} but they are actually the \vn{a} and \vn{b} normal modes. So
...
re-label your twiss data planes as \vn{a} and \vn{b}.  Section~\ref{c:init}
