...
\end{example}
The \vn{<tao_input_file>} is the root initialization file that \tao
reads to start \tao's initialization process. If not present,
\vn{<tao_input_file>} defaults to \vn{tao.init}. The
\vn{-beam_all} switch is for reading in data generated from beam
tracking (\sref{s:command_line}). The \vn{-beam0} switch is for
specifying the initial beam distribution. 
The \vn{<lattice_file>} is used to
override the lattice file specified in the root initialization
file. See section~\sref{s:command_line} for more details. Example:
\begin{example}
  \$CESR_EXE/tao -init my.init -lat xsif::slac.xsif
\end{example}

To get an example set of input files to run, check out a copy of the
\tao code. Let \cs{Root} be the name of the root directory for the
...
in one file or it can be spread out and put into up to six different files.
  \vspace*{-2ex}
\begin{enumerate}
