...
\end{example}
The \vn{<tao_input_file>} is the file \tao reads to start \tao's
initialization process. If not present, \vn{<tao_input_file>} defaults
to \vn{tao.init}. The \vn{<beam_input_file>} is for reading in data
generated from beam tracking (\sref{s:command_line}). Example:
\begin{example}
  \$CESR_EXE/tao -init my.init
\end{example}
