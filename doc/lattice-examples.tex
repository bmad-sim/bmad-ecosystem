...
At the end of \vn{BND2} the branch element \vn{BR} connects \vn{INJ_L}
with the marker \vn{M} in \vn{RING_L}.

As a side note, notice that the partial bend \vn{BND2} may be
eliminated from the lattice by eliminating \vn{P} and \vn{BND2} from
\vn{INJ_L} and modifying \vn{BR} to have \vn{P}'s patch parameters:
\begin{example}
  INJ_L: line = (..., X, BR)
  RING_L: line = (..., BND, ...)
...
\end{example}
The problem here is that \vn{BND} will need to use \vn{runge_kutta}
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=5in]{erl.pdf}
  \caption[
Example Energy Recovery Linac. The ERL consists of an injection line,
accelerating linac, return arc, decellerating linac, and finally a
beam dump.}
  \label{f:erl}
\end{figure}

An Energy Recovery Linac (ERL) is illustrated in \fig{f:erl}. The ERL
starts with an injection line that feeds a linac which accelerates the
beam to some energy. The beam then transverses a \vn{return arc} which
...
steered through a dump line where, at the end, an absorber stops the beam.
