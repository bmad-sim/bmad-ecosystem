...
line, accelerating linac, return arc, deceleration linac, and finally
...
\vn{BEND_L1} has been determined before the orientation of
\vn{A_PATCH} is determined. The orientation of elements is determined
...
(\sref{s:global}) by setting \vn{beginning[theta]}. This does not affect photon tracking since
changing the initial orientation of the reference trajectory just shifts all the lattice element as
one rigid body. The purpose of setting \vn{beginning[theta]} is to keep the orientation of the
global coordinates as shown in \fig{f:rowland}A which mimics the actual experimental setup that this
