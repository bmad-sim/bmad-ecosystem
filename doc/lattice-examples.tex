...
\begin{example}
  D: drift, l = 2
  g_design = pi/12
  B: sbend, l = 2, g = g_design, dg = -2*g_design
  P: patch, x_pitch = pi
...
direction from the normal direction. If \vn{dg} were zero in this instance, a downstream moving
particle would feel a force that will rotate the particle in a clockwise manner opposite from the
counterclockwise direction of the bend. To counter this, \vn{dg} is set so the total bending
field \vn{g_tot = g + dg} is opposite the design field. That is, \vn{dg} is set so that
\vn{g_tot = -g}.
