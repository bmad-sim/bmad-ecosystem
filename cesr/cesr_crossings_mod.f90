...
!   ptrain(:)       --  Integer: Train associated with this crossing.
...
subroutine cesr_crossings(i_train, j_car, species, n_trains_tot, n_cars, &
  ...
  integer :: ib,it
  real(rp) :: length
  integer, dimension(:), allocatable :: oppos_buckets
  integer, dimension(:), allocatable :: trtimes, btimes
...
!       print *,' ==== trtimes array before subracting ',trlength
!       write(6,'(5i7)')trtimes
    ...
    j = 1
    do i = 1, tnumber
! Fixed bug in this line 5/9/05. Wrong for n_trains_tot=6,7,8.  JAC
!        do j = 1, n_trains_tot/tnumber - 1
        do j = 1, (n_trains_tot - 1)/tnumber
            trtimes(tnumber * j + i) = trtimes(i)
        enddo
    enddo


!       print *,' ==== trtimes array after loading ===='
!       write(6,'(5i7)')trtimes
           k = 1
    do i = 1, bunch_tot
        if (btimes(i) == 0) then
...
!
! Input:
!   n_bucket         -- integer: The bucket (position) of thebunch to find
!                               crossing points for.
...
subroutine LRBBI_crossings(n_bucket, oppos_buckets, cross_positions)
  ...
  integer, dimension(:), intent(in) :: oppos_buckets
...
!     print *,' LRBBI_CROSSINGS: n_bucket= ',n_bucket
  ...
  do i = 1, bunch_tot
    cross_positions(i) = mod(real(oppos_buckets(i) + n_bucket) / 2, bucket_tot)
...
!  print *
!  print *,'Rtn LRBBI_crossings calculates cross_positions array for   ',bunch_tot,' opposing bunches:'
!  write(6,'(5f7.1)')cross_positions*bucket_tot*2
!  print *
!  print *,'Rtn LRBBI_crossings uses array oppos_buckets:'
!  write(6,'(5i7)')oppos_buckets
