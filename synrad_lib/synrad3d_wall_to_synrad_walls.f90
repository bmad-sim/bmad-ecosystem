...
n_pt = ubound(branch%wall3d(1)%section, 1)
...
do i = 1, n_pt
  ...
  minus_side%pt(i-1)%s = branch%wall3d(1)%section(i)%s
  minus_side%pt(i-1)%name = branch%wall3d(1)%section(i)%name
  minus_side%pt(i-1)%phantom = .false.

  plus_side%pt(i-1) = minus_side%pt(i-1)
  sec => branch%wall3d(1)%section(i)
