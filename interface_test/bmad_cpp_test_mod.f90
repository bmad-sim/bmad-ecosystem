...
subroutine test1_f_wake_lr_position_array (ok)

implicit none

type(wake_lr_position_array_struct), target :: f_wake_lr_position_array, f2_wake_lr_position_array
logical(c_bool) c_ok
logical ok

interface
  subroutine test_c_wake_lr_position_array (c_wake_lr_position_array, c_ok) bind(c)
    import c_ptr, c_bool
    type(c_ptr), value :: c_wake_lr_position_array
    logical(c_bool) c_ok
  end subroutine
end interface
...
ok = .true.
call set_wake_lr_position_array_test_pattern (f2_wake_lr_position_array, 1)

call test_c_wake_lr_position_array(c_loc(f2_wake_lr_position_array), c_ok)
if (.not. f_logic(c_ok)) ok = .false.

call set_wake_lr_position_array_test_pattern (f_wake_lr_position_array, 4)
if (f_wake_lr_position_array == f2_wake_lr_position_array) then
  print *, 'wake_lr_position_array: C side convert C->F: Good'
else
  print *, 'wake_lr_position_array: C SIDE CONVERT C->F: FAILED!'
  ok = .false.
endif

end subroutine test1_f_wake_lr_position_array

!---------------------------------------------------------------------------------
!---------------------------------------------------------------------------------

subroutine test2_f_wake_lr_position_array (c_wake_lr_position_array, c_ok) bind(c)
...
c_ok = c_logic(.true.)
call wake_lr_position_array_to_f (c_wake_lr_position_array, c_loc(f_wake_lr_position_array))

call set_wake_lr_position_array_test_pattern (f2_wake_lr_position_array, 2)
if (f_wake_lr_position_array == f2_wake_lr_position_array) then
  print *, 'wake_lr_position_array: F side convert C->F: Good'
else
  print *, 'wake_lr_position_array: F SIDE CONVERT C->F: FAILED!'
  c_ok = c_logic(.false.)
endif

call set_wake_lr_position_array_test_pattern (f2_wake_lr_position_array, 3)
call wake_lr_position_array_to_c (c_loc(f2_wake_lr_position_array), c_wake_lr_position_array)

end subroutine test2_f_wake_lr_position_array

!---------------------------------------------------------------------------------
!---------------------------------------------------------------------------------

subroutine set_wake_lr_position_array_test_pattern (F, ix_patt)

implicit none

type(wake_lr_position_array_struct) F
integer ix_patt, offset, jd, jd1, jd2, jd3, lb1, lb2, lb3, rhs
...
!! f_side.test_pat[character, 0, NOT]
do jd1 = 1, len(F%formula)
  F%formula(jd1:jd1) = char(ichar("a") + modulo(100+1+offset+jd1, 26))
enddo
!! f_side.test_pat[type, 1, ALLOC]
...
!! f_side.test_pat[real, 0, NOT]
rhs = 6 + offset; F%t_max = rhs
!! f_side.test_pat[real, 0, NOT]
rhs = 7 + offset; F%polarization_angle = rhs

end subroutine set_wake_lr_position_array_test_pattern
...
if (ix_patt < 3) then
  ...
  if (.not. allocated(F%lr_position_array)) allocate (F%lr_position_array(-1:1))
  do jd1 = 1, size(F%lr_position_array,1); lb1 = lbound(F%lr_position_array,1) - 1
    call set_wake_lr_position_array_test_pattern (F%lr_position_array(jd1+lb1), ix_patt+jd1)
  enddo
endif
!! f_side.test_pat[real, 0, NOT]
...
!! f_side.test_pat[integer, 0, NOT]
rhs = 20 + offset; F%max_num_runge_kutta_step = rhs
!! f_side.test_pat[logical, 0, NOT]
rhs = 21 + offset; F%use_hard_edge_drifts = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 22 + offset; F%sr_wakes_on = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 23 + offset; F%lr_wakes_on = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 24 + offset; F%mat6_track_symmetric = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 25 + offset; F%auto_bookkeeper = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 26 + offset; F%space_charge_on = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 27 + offset; F%coherent_synch_rad_on = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 28 + offset; F%spin_tracking_on = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 29 + offset; F%radiation_damping_on = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 30 + offset; F%radiation_fluctuations_on = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 31 + offset; F%conserve_taylor_maps = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 32 + offset; F%absolute_time_tracking_default = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 33 + offset; F%convert_to_kinetic_momentum = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 34 + offset; F%aperture_limit_on = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 35 + offset; F%debug = (modulo(rhs, 2) == 0)
...
enddo; enddo
!! f_side.test_pat[real, 0, NOT]
rhs = 48 + offset; F%gamma_c = rhs
!! f_side.test_pat[real, 0, NOT]
rhs = 49 + offset; F%s = rhs
!! f_side.test_pat[real, 0, NOT]
rhs = 50 + offset; F%ref_time = rhs
...
if (ix_patt < 3) then
  ...
  do jd3 = 1, size(F%r,3); lb3 = lbound(F%r,3) - 1
    rhs = 100 + jd1 + 10*jd2 + 100*jd3 + 51 + offset
...
!! f_side.test_pat[integer, 0, NOT]
rhs = 63 + offset; F%key = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 64 + offset; F%sub_key = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 65 + offset; F%ix_ele = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 66 + offset; F%ix_branch = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 67 + offset; F%slave_status = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 68 + offset; F%n_slave = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 69 + offset; F%n_slave_field = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 70 + offset; F%ix1_slave = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 71 + offset; F%lord_status = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 72 + offset; F%n_lord = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 73 + offset; F%n_lord_field = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 74 + offset; F%ic1_lord = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 75 + offset; F%ix_pointer = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 76 + offset; F%ixx = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 77 + offset; F%iyy = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 78 + offset; F%mat6_calc_method = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 79 + offset; F%tracking_method = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 80 + offset; F%spin_tracking_method = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 81 + offset; F%ptc_integration_type = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 82 + offset; F%field_calc = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 83 + offset; F%aperture_at = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 84 + offset; F%aperture_type = rhs
!! f_side.test_pat[integer, 0, NOT]
rhs = 85 + offset; F%orientation = rhs
!! f_side.test_pat[logical, 0, NOT]
rhs = 86 + offset; F%symplectify = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 87 + offset; F%mode_flip = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 88 + offset; F%multipoles_on = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 89 + offset; F%scale_multipoles = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 90 + offset; F%taylor_map_includes_offsets = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 91 + offset; F%field_master = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 92 + offset; F%is_on = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 93 + offset; F%logic = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 94 + offset; F%bmad_logic = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 95 + offset; F%select = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 96 + offset; F%csr_calc_on = (modulo(rhs, 2) == 0)
!! f_side.test_pat[logical, 0, NOT]
rhs = 97 + offset; F%offset_moves_aperture = (modulo(rhs, 2) == 0)
