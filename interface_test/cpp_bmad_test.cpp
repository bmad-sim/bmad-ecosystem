...
extern "C" void test2_f_rf_wake_sr_table (CPP_rf_wake_sr_table&, bool&);

void set_CPP_rf_wake_sr_table_test_pattern (CPP_rf_wake_sr_table& C, int ix_patt) {

  int rhs, offset = 100 * ix_patt;

  // c_side.test_pat[real, 0, NOT]
  rhs = 1 + offset; C.z = rhs;

  // c_side.test_pat[real, 0, NOT]
  rhs = 2 + offset; C.long_wake = rhs;

  // c_side.test_pat[real, 0, NOT]
  rhs = 3 + offset; C.trans_wake = rhs;
...
extern "C" void test_c_rf_wake_sr_table (Bmad_rf_wake_sr_table_class* F, bool& c_ok) {
  ...
  rf_wake_sr_table_to_c (F, C);
  set_CPP_rf_wake_sr_table_test_pattern (C2, 1);

  if (C == C2) {
    cout << " rf_wake_sr_table: C side convert F->C: Good" << endl;
  } else {
    cout << " rf_wake_sr_table: C SIDE CONVERT F->C: FAILED!" << endl;
    c_ok = false;
  }

  set_CPP_rf_wake_sr_table_test_pattern (C2, 2);
  bool c_ok2;
  test2_f_rf_wake_sr_table (C2, c_ok2);
  if (!c_ok2) c_ok = false;

  set_CPP_rf_wake_sr_table_test_pattern (C, 3);
  if (C == C2) {
    cout << " rf_wake_sr_table: F side convert F->C: Good" << endl;
  } else {
    cout << " rf_wake_sr_table: F SIDE CONVERT F->C: FAILED!" << endl;
    c_ok = false;
  }

  set_CPP_rf_wake_sr_table_test_pattern (C2, 4);
  rf_wake_sr_table_to_f (C2, F);
...
extern "C" void test2_f_rf_wake_sr_mode (CPP_rf_wake_sr_mode&, bool&);

void set_CPP_rf_wake_sr_mode_test_pattern (CPP_rf_wake_sr_mode& C, int ix_patt) {
...
extern "C" void test_c_rf_wake_sr_mode (Bmad_rf_wake_sr_mode_class* F, bool& c_ok) {
  ...
  rf_wake_sr_mode_to_c (F, C);
  set_CPP_rf_wake_sr_mode_test_pattern (C2, 1);

  if (C == C2) {
    cout << " rf_wake_sr_mode: C side convert F->C: Good" << endl;
  } else {
    cout << " rf_wake_sr_mode: C SIDE CONVERT F->C: FAILED!" << endl;
    c_ok = false;
  }

  set_CPP_rf_wake_sr_mode_test_pattern (C2, 2);
  bool c_ok2;
  test2_f_rf_wake_sr_mode (C2, c_ok2);
  if (!c_ok2) c_ok = false;

  set_CPP_rf_wake_sr_mode_test_pattern (C, 3);
  if (C == C2) {
    cout << " rf_wake_sr_mode: F side convert F->C: Good" << endl;
  } else {
    cout << " rf_wake_sr_mode: F SIDE CONVERT F->C: FAILED!" << endl;
    c_ok = false;
  }

  set_CPP_rf_wake_sr_mode_test_pattern (C2, 4);
  rf_wake_sr_mode_to_f (C2, F);
...
extern "C" void test2_f_rf_wake_lr (CPP_rf_wake_lr&, bool&);

void set_CPP_rf_wake_lr_test_pattern (CPP_rf_wake_lr& C, int ix_patt) {
...
extern "C" void test_c_rf_wake_lr (Bmad_rf_wake_lr_class* F, bool& c_ok) {
  ...
  rf_wake_lr_to_c (F, C);
  set_CPP_rf_wake_lr_test_pattern (C2, 1);

  if (C == C2) {
    cout << " rf_wake_lr: C side convert F->C: Good" << endl;
  } else {
    cout << " rf_wake_lr: C SIDE CONVERT F->C: FAILED!" << endl;
    c_ok = false;
  }

  set_CPP_rf_wake_lr_test_pattern (C2, 2);
  bool c_ok2;
  test2_f_rf_wake_lr (C2, c_ok2);
  if (!c_ok2) c_ok = false;

  set_CPP_rf_wake_lr_test_pattern (C, 3);
  if (C == C2) {
    cout << " rf_wake_lr: F side convert F->C: Good" << endl;
  } else {
    cout << " rf_wake_lr: F SIDE CONVERT F->C: FAILED!" << endl;
    c_ok = false;
  }

  set_CPP_rf_wake_lr_test_pattern (C2, 4);
  rf_wake_lr_to_f (C2, F);
...
extern "C" void test2_f_rf_wake (CPP_rf_wake&, bool&);

void set_CPP_rf_wake_test_pattern (CPP_rf_wake& C, int ix_patt) {
  ...
  if (ix_patt < 3) 
    ...
    C.sr_table.resize(3);
    for (unsigned int i = 0; i < C.sr_table.size(); i++)  {set_CPP_rf_wake_sr_table_test_pattern(C.sr_table[i], ix_patt+i+1);}
  }

  // c_side.test_pat[type, 1, PTR]
  if (ix_patt < 3) 
    C.sr_mode_long.resize(0);
  else {
    C.sr_mode_long.resize(3);
    for (unsigned int i = 0; i < C.sr_mode_long.size(); i++)  {set_CPP_rf_wake_sr_mode_test_pattern(C.sr_mode_long[i], ix_patt+i+1);}
  }

  // c_side.test_pat[type, 1, PTR]
  if (ix_patt < 3) 
    C.sr_mode_trans.resize(0);
  else {
    C.sr_mode_trans.resize(3);
    for (unsigned int i = 0; i < C.sr_mode_trans.size(); i++)  {set_CPP_rf_wake_sr_mode_test_pattern(C.sr_mode_trans[i], ix_patt+i+1);}
  }
  ...
  if (ix_patt < 3) 
    ...
    C.lr.resize(3);
    for (unsigned int i = 0; i < C.lr.size(); i++)  {set_CPP_rf_wake_lr_test_pattern(C.lr[i], ix_patt+i+1);}
  }

  // c_side.test_pat[real, 0, NOT]
  rhs = 11 + offset; C.z_sr_mode_max = rhs;
...
extern "C" void test_c_rf_wake (Bmad_rf_wake_class* F, bool& c_ok) {
  ...
  rf_wake_to_c (F, C);
  set_CPP_rf_wake_test_pattern (C2, 1);

  if (C == C2) {
    cout << " rf_wake: C side convert F->C: Good" << endl;
  } else {
    cout << " rf_wake: C SIDE CONVERT F->C: FAILED!" << endl;
    c_ok = false;
  }

  set_CPP_rf_wake_test_pattern (C2, 2);
  bool c_ok2;
  test2_f_rf_wake (C2, c_ok2);
  if (!c_ok2) c_ok = false;

  set_CPP_rf_wake_test_pattern (C, 3);
  if (C == C2) {
    cout << " rf_wake: F side convert F->C: Good" << endl;
  } else {
    cout << " rf_wake: F SIDE CONVERT F->C: FAILED!" << endl;
    c_ok = false;
  }

  set_CPP_rf_wake_test_pattern (C2, 4);
  rf_wake_to_f (C2, F);
...
extern "C" void test2_f_em_field_map_term (CPP_em_field_map_term&, bool&);

void set_CPP_em_field_map_term_test_pattern (CPP_em_field_map_term& C, int ix_patt) {
...
extern "C" void test_c_em_field_map_term (Bmad_em_field_map_term_class* F, bool& c_ok) {
  ...
  em_field_map_term_to_c (F, C);
  set_CPP_em_field_map_term_test_pattern (C2, 1);

  if (C == C2) {
    cout << " em_field_map_term: C side convert F->C: Good" << endl;
  } else {
    cout << " em_field_map_term: C SIDE CONVERT F->C: FAILED!" << endl;
    c_ok = false;
  }

  set_CPP_em_field_map_term_test_pattern (C2, 2);
  bool c_ok2;
  test2_f_em_field_map_term (C2, c_ok2);
  if (!c_ok2) c_ok = false;

  set_CPP_em_field_map_term_test_pattern (C, 3);
  if (C == C2) {
    cout << " em_field_map_term: F side convert F->C: Good" << endl;
  } else {
    cout << " em_field_map_term: F SIDE CONVERT F->C: FAILED!" << endl;
    c_ok = false;
  }

  set_CPP_em_field_map_term_test_pattern (C2, 4);
  em_field_map_term_to_f (C2, F);
...
extern "C" void test2_f_em_field_map (CPP_em_field_map&, bool&);

void set_CPP_em_field_map_test_pattern (CPP_em_field_map& C, int ix_patt) {
  ...
  if (ix_patt < 3) 
    ...
    C.term.resize(3);
    for (unsigned int i = 0; i < C.term.size(); i++)  {set_CPP_em_field_map_term_test_pattern(C.term[i], ix_patt+i+1);}
  }
...
extern "C" void test_c_em_field_map (Bmad_em_field_map_class* F, bool& c_ok) {
  ...
  em_field_map_to_c (F, C);
  set_CPP_em_field_map_test_pattern (C2, 1);

  if (C == C2) {
    cout << " em_field_map: C side convert F->C: Good" << endl;
  } else {
    cout << " em_field_map: C SIDE CONVERT F->C: FAILED!" << endl;
    c_ok = false;
  }

  set_CPP_em_field_map_test_pattern (C2, 2);
  bool c_ok2;
  test2_f_em_field_map (C2, c_ok2);
  if (!c_ok2) c_ok = false;

  set_CPP_em_field_map_test_pattern (C, 3);
  if (C == C2) {
    cout << " em_field_map: F side convert F->C: Good" << endl;
  } else {
    cout << " em_field_map: F SIDE CONVERT F->C: FAILED!" << endl;
    c_ok = false;
  }

  set_CPP_em_field_map_test_pattern (C2, 4);
  em_field_map_to_f (C2, F);
...
void set_CPP_em_field_mode_test_pattern (CPP_em_field_mode& C, int ix_patt) {
  ...
  if (ix_patt < 3) 
    ...
    set_CPP_em_field_map_test_pattern((*C.map), ix_patt);
  }
...
void set_CPP_control_test_pattern (CPP_control& C, int ix_patt) {
  ...
  // c_side.test_pat[integer, 0, NOT]
  rhs = 2 + offset; C.ix_lord = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 3 + offset; C.ix_slave = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 4 + offset; C.ix_branch = rhs;
...
void set_CPP_ele_test_pattern (CPP_ele& C, int ix_patt) {
  ...
  if (ix_patt < 3) 
    ...
    set_CPP_rf_wake_test_pattern((*C.rf_wake), ix_patt);
  }
  ...
  for (unsigned int i = 0; i < C.taylor.size(); i++)
    {int rhs = 101 + i + 25 + offset; set_CPP_taylor_test_pattern(C.taylor[i], ix_patt+i+1);}
  // c_side.test_pat[type, 0, PTR]
  if (ix_patt < 3) 
    C.wall3d = NULL;
  else {
  ...
  for (unsigned int i = 1; i < Bmad::NUM_ELE_ATTRIB+1; i++)
    {int rhs = 100 + i + 30 + offset; C.value[i] = rhs;}

  // c_side.test_pat[real, 1, NOT]
  C.old_value[0] = 0;
  for (unsigned int i = 1; i < Bmad::NUM_ELE_ATTRIB+1; i++)
    {int rhs = 100 + i + 31 + offset; C.old_value[i] = rhs;}

  // c_side.test_pat[real, 1, NOT]
  for (unsigned int i = 0; i < C.gen0.size(); i++)
    {int rhs = 101 + i + 32 + offset; C.gen0[i] = rhs;}
  // c_side.test_pat[real, 1, NOT]
  for (unsigned int i = 0; i < C.vec0.size(); i++)
    {int rhs = 101 + i + 33 + offset; C.vec0[i] = rhs;}
  // c_side.test_pat[real, 2, NOT]
  for (unsigned int i = 0; i < C.mat6.size(); i++)  for (unsigned int j = 0; j < C.mat6[0].size(); j++) 
    {int rhs = 101 + i + 10*(j+1) + 34 + offset; C.mat6[i][j] = rhs;}
  // c_side.test_pat[real, 2, NOT]
  for (unsigned int i = 0; i < C.c_mat.size(); i++)  for (unsigned int j = 0; j < C.c_mat[0].size(); j++) 
    {int rhs = 101 + i + 10*(j+1) + 35 + offset; C.c_mat[i][j] = rhs;}
  // c_side.test_pat[real, 0, NOT]
  rhs = 36 + offset; C.gamma_c = rhs;

  // c_side.test_pat[real, 0, NOT]
  rhs = 37 + offset; C.s = rhs;

  // c_side.test_pat[real, 0, NOT]
  rhs = 38 + offset; C.ref_time = rhs;
    ...
    for (unsigned int i = 0; i < C.r.size(); i++) {
      ...
      for (unsigned int j = 0; j < C.r[0].size(); j++) {
        ...
        for (unsigned int k = 0; k < C.r[0][0].size(); k++) {
          int rhs = 101 + i + 10*(j+1) + 100*(k+1) + 39 + offset; C.r[i][j][k] = rhs;
        }
      }
    }
  }
  ...
  if (ix_patt < 3) 
    ...
    for (unsigned int i = 0; i < C.a_pole.size(); i++)
      {int rhs = 101 + i + 43 + offset; C.a_pole[i] = rhs;}  }
  ...
  if (ix_patt < 3) 
    ...
    for (unsigned int i = 0; i < C.b_pole.size(); i++)
      {int rhs = 101 + i + 45 + offset; C.b_pole[i] = rhs;}  }

  // c_side.test_pat[real, 1, NOT]
  for (unsigned int i = 0; i < C.map_ref_orb_in.size(); i++)
    {int rhs = 101 + i + 47 + offset; C.map_ref_orb_in[i] = rhs;}
  // c_side.test_pat[real, 1, NOT]
  for (unsigned int i = 0; i < C.map_ref_orb_out.size(); i++)
    {int rhs = 101 + i + 48 + offset; C.map_ref_orb_out[i] = rhs;}
  // c_side.test_pat[real, 1, NOT]
  for (unsigned int i = 0; i < C.time_ref_orb_in.size(); i++)
    {int rhs = 101 + i + 49 + offset; C.time_ref_orb_in[i] = rhs;}
  // c_side.test_pat[real, 1, NOT]
  for (unsigned int i = 0; i < C.time_ref_orb_out.size(); i++)
    {int rhs = 101 + i + 50 + offset; C.time_ref_orb_out[i] = rhs;}
  // c_side.test_pat[integer, 0, NOT]
  rhs = 51 + offset; C.key = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 52 + offset; C.sub_key = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 53 + offset; C.ix_ele = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 54 + offset; C.ix_branch = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 55 + offset; C.ix_value = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 56 + offset; C.slave_status = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 57 + offset; C.n_slave = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 58 + offset; C.ix1_slave = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 59 + offset; C.ix2_slave = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 60 + offset; C.lord_status = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 61 + offset; C.n_lord = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 62 + offset; C.ic1_lord = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 63 + offset; C.ic2_lord = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 64 + offset; C.ix_pointer = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 65 + offset; C.ixx = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 66 + offset; C.iyy = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 67 + offset; C.mat6_calc_method = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 68 + offset; C.tracking_method = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 69 + offset; C.spin_tracking_method = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 70 + offset; C.ptc_integration_type = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 71 + offset; C.field_calc = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 72 + offset; C.aperture_at = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 73 + offset; C.aperture_type = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 74 + offset; C.orientation = rhs;

  // c_side.test_pat[logical, 0, NOT]
  rhs = 75 + offset; C.symplectify = (rhs % 2 == 0);

  // c_side.test_pat[logical, 0, NOT]
  rhs = 76 + offset; C.mode_flip = (rhs % 2 == 0);

  // c_side.test_pat[logical, 0, NOT]
  rhs = 77 + offset; C.multipoles_on = (rhs % 2 == 0);

  // c_side.test_pat[logical, 0, NOT]
  rhs = 78 + offset; C.scale_multipoles = (rhs % 2 == 0);

  // c_side.test_pat[logical, 0, NOT]
  rhs = 79 + offset; C.map_with_offsets = (rhs % 2 == 0);

  // c_side.test_pat[logical, 0, NOT]
  rhs = 80 + offset; C.field_master = (rhs % 2 == 0);

  // c_side.test_pat[logical, 0, NOT]
  rhs = 81 + offset; C.is_on = (rhs % 2 == 0);

  // c_side.test_pat[logical, 0, NOT]
  rhs = 82 + offset; C.old_is_on = (rhs % 2 == 0);

  // c_side.test_pat[logical, 0, NOT]
  rhs = 83 + offset; C.logic = (rhs % 2 == 0);

  // c_side.test_pat[logical, 0, NOT]
  rhs = 84 + offset; C.bmad_logic = (rhs % 2 == 0);

  // c_side.test_pat[logical, 0, NOT]
  rhs = 85 + offset; C.on_a_girder = (rhs % 2 == 0);

  // c_side.test_pat[logical, 0, NOT]
  rhs = 86 + offset; C.csr_calc_on = (rhs % 2 == 0);

  // c_side.test_pat[logical, 0, NOT]
  rhs = 87 + offset; C.offset_moves_aperture = (rhs % 2 == 0);
...
void set_CPP_branch_test_pattern (CPP_branch& C, int ix_patt) {
  ...
  // c_side.test_pat[integer, 0, NOT]
  rhs = 3 + offset; C.ix_root_branch = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 4 + offset; C.ix_from_branch = rhs;

  // c_side.test_pat[integer, 0, NOT]
  rhs = 5 + offset; C.ix_from_ele = rhs;
