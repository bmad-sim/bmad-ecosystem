program anaylzer
 ...
 interface
  ...
  real(rp) sum1, sum2
  real(rp) sigx,sigy,sigz,sigx_0/0.01/,sigy_0/0.002/,sigz_0/0.01/, density
...
!  lat_file = line
   ...
   if(line(1:5) == 'SPACE')then  !space charge
    ...
    if(line(1:6) == 'ECLOUD')then  !electron cloud element
     call string_trim(line(6+1:),line,ix)
     if (ix /= 0)then
!        call string_trim(line(ix+1:), line, ix)
      read(line(1:ix),*)density
      call string_trim(line(ix+1:), line, ix)
      if(ix == 0)sigx = sigx_0
      read(line(1:ix),*)sigx
      call string_trim(line(ix+1:), line, ix)
      ...
      if(ix == 0)sigz = sigz_0
      read(line(1:ix),*)sigz
      call implement_beambeam(ring, sigx,sigy,sigz, density)
      print '(a,es12.4,a7,es12.4,a7,es12.4,a7,es12.4)', 'ECLOUD implemented with density = ',density, ' sigx =',sigx,' sigy=',sigy,' sigz=',sigz
     else  
      print '(a)',' Type <ECLOUD> <Density> <sigx> <sigy> <sigz> ' 
      cycle
     endif
      exit
   endif
...
!                        ring%ele(i)%floor%x,ring%ele(i)%floor%y,ring%ele(i)%floor%z, &
 ...
 real(rp) de, rms_x, rms_y, avg_x, avg_y,sum_x,sum_y
  ...
  subroutine list_commands
    ...
    print *,' "SPACE_CHARGE" : include ur space charge', ' - Type <SPACE_CHARGE> <n_part> <bmode_emit> ' 
