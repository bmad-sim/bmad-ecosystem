...
private this_merit_calc, solution_struct
...
!-------------------------------------------------------------------------
!+
! Function opti_de_mpi (v_best, generations, population, merit_func, 
...
!     integer iter_count  ! Output: Initially set to zero by opti_de and increased by 1 each call.
...
type (solution_struct), allocatable :: solution(:) 
type (solution_struct), allocatable :: try(:)

real(rp) :: v1(size(v_best)), r_vec(size(v_best))
real(rp), allocatable :: r_pop(:)
real(rp) :: F, r_ran

integer status, iter_count
integer :: i, ii, ng, nd, i_best, k, kk, n_vec
integer, allocatable :: i1(:), i2(:), i3(:), i4(:), i5(:)
integer p1, p2, p3, p4, p5
integer n_thread, mpi_rank, n_pop, n_pop_per_thread
integer, parameter :: master$ = 0

logical this_better_merit, this_best_merit

character(*), parameter :: r_name = 'opti_de_mpi'

! Initialize

call mpi_comm_size(MPI_COMM_WORLD, n_thread)
call mpi_comm_rank(MPI_COMM_WORLD, mpi_rank, ierr)
n_pop_per_thread = max(1, nint((population) / n_thread))
n_pop = n_thread * n_pop_per_thread

allocate(solution(n_pop), try(n_pop), r_pop(n_pop), i1(n_pop), i2(n_pop), i3(n_pop), i4(n_pop), i5(n_pop))
...
if (n_pop < 6 .and. opti_de_param%use_2nd_diff) then
...
call this_merit_calc(solution, mpi_rank, n_pop, n_thread, n_pop_per_thread)
...
if (mpi_rank == master$) then
  ...
  call opti_de_merit_report (v_best, best_merit, iter_count)
endif
...
do ng = 1, generations
  ...
  call this_merit_calc (try, mpi_rank, n_pop, n_thread, n_pop_per_thread)
...
end function opti_de_mpi

!--------------------------------------------------------------

subroutine this_merit_calc (try, mpi_rank, n_pop, n_thread, n_pop_per_thread)
...
type (solution_struct) :: try(:)

integer mpi_rank, n_pop, n_thread, n_pop_per_thread
integer i, j, k, nv, slave, stat(MPI_STATUS_SIZE)
integer, parameter :: vec_tag$ = 1001, merit_tag$ = 1002, master$ = 0
...
if (mpi_rank == master$) then
  do i = 1, n_thread-1
    !call print_mpi_info (lttp, ltt_com, 'Master: Init try to slave.')
    do j = 1, n_pop_per_thread
      k = i * n_pop_per_thread + j
      call mpi_send (try(k)%vec, nv, MPI_DOUBLE_PRECISION, i, vec_tag$, MPI_COMM_WORLD, ierr)
    enddo
  enddo
else
  do j = 1, n_pop_per_thread
    call mpi_recv (try(j)%vec, nv, MPI_DOUBLE_PRECISION, master$, vec_tag$, MPI_COMM_WORLD, stat, ierr)
  enddo
endif
...
if (mpi_rank == master$) then
  do i = 1, n_thread-1
    !call print_mpi_info (lttp, ltt_com, 'Master: Init try to slave.')
    slave = MPI_ANY_SOURCE
    do j = 1, n_pop_per_thread
      k = i * n_pop_per_thread + j
      call mpi_recv (try(k)%merit, 1, MPI_DOUBLE_PRECISION, slave, merit_tag$, MPI_COMM_WORLD, stat, ierr)
      slave = stat(MPI_SOURCE)
    enddo
  enddo
else
  do j = 1, n_pop_per_thread
    call mpi_send (try(j)%merit, 1, MPI_DOUBLE_PRECISION, master$, merit_tag$, MPI_COMM_WORLD, ierr)
  enddo
endif
