...
\begin{example}
  cmake -DGEANT4_INSTALL_DATA=ON \B
        -DCMAKE_INSTALL_PREFIX=$GEANT_DIR/geant4-build \B
        ../geant4.10.06
\end{example}
Change the version number as appropriate. The first \texttt{-D} flag will cause the necessary data
sets to be downloaded when we build Geant, and the second \texttt{-D} flag sets the install
directory.
...
Geant can be compiled to support either single threaded execution or multithreaded execution.
Due to the substantial associated performance improvement,
it is highly recommended that Geant be compiled for multithreaded use,
as the converter simulation takes full advantage of parallel execution.
To do so, add the flag
\begin{example}
  -DGEANT4_BUILD_MULTITHREADED=ON
\end{example}
to the \texttt{cmake} command used in step 4, then proceed with steps 5-6.
...
\begin{example}
  cd $GEANT_DIR/geant-build && source geant4make.sh
\end{example}
