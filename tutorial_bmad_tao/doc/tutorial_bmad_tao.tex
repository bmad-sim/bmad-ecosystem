...
%            Note: It is assumed that the bmad, tao, and examples directories are all on the same level (as they are in a Distribution).
%-----------------------------------------
...
(\sref{s:dist}). Instructions for both are given on the web page at the above URL.

Even if you do use \vn{Conda}, you should also download (but you do not need to compile) a
Distribution (\sref{s:dist}) since a Distribution contains many useful lattice examples.

If you compile a \vn{Distribution}, (or are using a Release) a useful check that everything is setup
...
variables pointing to a Distribution other than the one you want to use. The \vn{accinfo} command
...
(\sref{s:orientation})
\begin{lstlisting}[mathescape]
  \url{github.com/bmad-sim/bmad-examples}
\end{lstlisting}
...
The \vn{production} directory is created when the code in a \vn{Distribution} or \vn{Release} is
...
files in the \vn{tao/examples} directory.
%
\vspace{-0.4 ex}
\item[util] \Newline
...
programs are included in any \vn{Distribution} or \vn{Release}. The program executable will be in
...
optimization. The source code is in the \vn{util_programs/moga} directory.
%
  \item[synrad] \Newline
...
be found in a \vn{Distribution} or \vn{Release} in the directory:
\begin{code}
tao/examples
\end{code}
...
(See \extref{T-s:init.global} ``Beginning Initialization'' in the \tao manual) that sets the \vn{startup_file}
...
# Values shown are for the Exit End of each Element:
\end{code}
The \vn{s} column shows the longitudinal position from the beginning of the lattice
(\sref{s:coord.sys}) and the \vn{l} column shows the length of the elements.
...
\vn{s}-position of the first slave element. This does not affect any calculations and is done since it
...
/
\end{code}
Just like \vn{data}, \vn{variables} are grouped into a tree but in this case there are only two
levels.  The top level nodes of the tree are called \vn{v1_var} structures. In this example a
\vn{v1_var} structure is defined called \vn{quad} which controls the \vn{k1} attribute of all
element whose name matches \vn{quad::*}. This will match to all quadrupoles. In this case, the
...
\item
\label{s:opt.ex1}
Start with the files in \vn{examples/tutorial_bmad_tao/lattice_files/lattice_optimization}. Change
...
\vn{PyTao} is separate from the \bmad \vn{Distribution} (\sref{s:dist}) which contains the \bmad and
...
\item
The ``\vn{measured}'' orbit used for the analysis should be
\begin{code}
