...
def rms(rms_old, ave_old, intens_old, rms_now, ave_now, intens_now)
  ...
  return sqrt(max(0.0, (rms2_old * intens_old + rms2_now * intens_now) / (intens_old + intens_now))
...
for arg in sys.argv[1:]
  ...
  for n_header in range(1, 1000):
    ...
    if arg == sys.argv[1]:
      ...
      exec (param + '_tot = param')  # This initalizes photons_tracked_tot, etc.
  ...
  while line:
    ...
    pix.x_pix          = col[2]
    pix.y_pix          = col[3]
    pix.intens_x       = pix.intens_x + col[4]
    pix.phase_x        = pix.phase_x
    pix.intens_y       = pix.intens_y + col[6]
    pix.phase_y        = pix.phase_y
    intensity_old = pix_intensity
    pix.intensity      = pix.intensity + col[8]
    pix.n_photon       = pix.n_photon + col[9]
    ...
    pix.ang_x_rms      = rms(pix.ang_x_rms, pix.ang_x_ave, intensity_old, col[15], col[14], col[8])
    pix.ang_x_ave      = ave(pix.ang_x_ave, intensity_old, col[14], col[8])
    ...
    pix.y_rms_init     = rms(pix.y_rms_init, pix.y_ave_init, intensity_old, col[20], col[19], col[8])
    pix.y_ave_init     = ave(pix.y_ave_init, intensity_old, col[19], col[8])
    pix.ang_y_rms_init = rms(pix.ang_y_rms_init, pix.ang_y_ave_init, intensity_old, col[22], col[21], col[8])
    pix.ang_y_ave_init = ave(pix.ang_y_ave_init, intensity_old, col[21], col[8])
