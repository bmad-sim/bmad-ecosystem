...
program f77_to_f90
  ...
  character*132 line1, line2, line3, line4
  character*60 in_str, in_file, f90_file
  character*1 tab

  integer :: f_type, f77$ = 1, f90$ = 2, flx$ = 3
  integer lib$get_foreign, istat, nret
...
! get the input file and open input and output files
  ...
  istat = lib$get_foreign (in_str,,nret)

  if (nret == 0) then
    type '(a, $)', ' Input F77 (or flx or f90) file name: '
    accept '(a)', in_str
  endif

  call str$upcase (in_str, in_str)
  call string_trim (in_str, in_file, ix)

! if no suffix supplied then look for existing file with appropriate suffix

  inquire (file = in_file, exist = is_there)

  if (.not. is_there) then
    call file_suffixer (in_file, in_file, '.FLX', .true.)
    inquire (file = in_file, exist = is_there)
  endif

  if (.not. is_there) then
    call file_suffixer (in_file, in_file, '.F77', .true.)
    inquire (file = in_file, exist = is_there)
  endif

  if (.not. is_there) then
    call file_suffixer (in_file, in_file, '.F90', .true.)
    inquire (file = in_file, exist = is_there)
  endif

  if (.not. is_there) then
    type *, 'ERROR: CANNOT FIND: ', trim(in_str), ' .F77, .FLX, or .F90'
    call exit
  endif
...
! find input file type 
  ...
  ix = len_trim(in_file)
  if (in_file(ix-2:ix) == 'FLX') then
    f_type = flx$
  elseif (in_file(ix-2:ix) == 'F77') then
    f_type = f77$
  elseif (in_file(ix-2:ix) == 'F90') then
    f_type = f90$
  else
    type *, 'Assuming: F77 to F90 conversion.'
    f_type = f90$
  endif
...
! if f90 input file then make sure we are doing the right thing
  ...
  if (f_type == f90$) then
    ...
    if (.not. doit) then
      type *, 'OK I do not know what to do now. Quiting.'
      call exit
    endif
  endif

  call file_suffixer (in_file, f90_file, '.F90', .true.)

  type *, 'Input file: ', trim(in_file)
  type '(3a, $)', ' Ouput F90 file <CR = ', trim(f90_file), '>: '
  accept '(a)', line1
  call string_trim (line1, line1, ix)
  if (ix /= 0) f90_file = line1

  open (unit = 1, file = in_file, status = 'old', readonly)
  open (unit = 2, file = f90_file, status = 'new', carriagecontrol = 'list')
...
! LINE2 = input line with 4 leading blanks trimmed off
! LINE3 = f90 output line
...
! if a normal line then just trim off the leading blanks
    ...
    elseif (line1(1:6) == '      ') then     
      line2 = trim(line1(5:))              ! line2 = trimed input line
...
! flex "D" lines converted to "if (debug)"
    ...
    elseif (line1(1:1) == 'D' .and. f_type == flx$) then
      ...
      if (.not. debug_out) then
        type *, 'WARNING! THIS FILE USES THE FLEX "D ..." DEBUG FEATURE.'
        type *, '         THIS HAS BEEN CONVERTED TO "IF (DEBUG) ...".'
        type *, '         YOU NEED TO ADD A "LOGICAL DEBUG" TO THE CODE.'
        debug_out = .true.
      endif
...
! but do not output OPTIONS line
    ...
    call string_trim (line3, line4, ix)
    call str$upcase (line4, line4)
    if (line4(1:7) /= 'OPTIONS') then  
      if (ii .ge. 2) call write_line (line3, f_type, c3)
    endif
...
! Finish. Write final pending f90 line.
  ...
  call write_line (line3, f_type, c3)
...
! change "." to "%" for structures
  ...
  i_loop: do i = 1, len_trim(line)
    ...
    elseif (line(i:i) == ".") then
      ...
      call str$upcase (str, line(i:i+4))
      if (str(1:5) == '.AND.') cycle 
      if (str(1:4) == '.OR.') cycle 
      if (str(1:5) == '.XOR.') cycle 
      if (str(1:5) == '.NOT.') cycle 
      call str$upcase (str, line(i-4:i))
      if (str(1:5) == '.AND.') cycle 
      if (str(2:5) == '.OR.') cycle 
      if (str(1:5) == '.XOR.') cycle 
      if (str(1:5) == '.NOT.') cycle 
...
! TO Internal Procedure
  ...
  if (is_there .and. ix2 == 0 .and. line(ix_end:ix_end) /= '=') then
    ...
    if (.not. is_there) then
      type *, 'ERROR: NO PROCEDURE NAME AFTER "TO" IN:'
      type *, '    ', line
      call err_exit
    endif
...
! look for ending ")"
  ...
  if (ix2 == 0) then
    type *, 'ERROR: CANNOT FIND ENDING ")" IN:'
    type *, trim(line)
    call err_exit
  endif
